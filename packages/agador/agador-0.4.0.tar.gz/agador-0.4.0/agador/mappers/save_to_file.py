...
class SaveConfig(SaveResult):
    ...
    def post_processing(self):
        ...
        roles = [r for r in os.listdir(self._base_dir) if not re.match(r'(\.|obsolete)', r)]
        ...
        for role in roles:
            ...
            nb_devices = [d.name for d in nb.dcim.devices.filter(role=role, status="active")]

            for cfg_file in os.listdir(role_dir):
                if cfg_file not in nb_devices:
                    logger.debug(f"Moving {cfg_file} to obsolete directory")
                    os.rename(f'{role_dir}/{cfg_file}', f'{self._base_dir}/obsolete/{cfg_file}')
