...
class Domain(Component):
    ...
    def __call__(self, document: AnalysisDocument, diagnostics: Diagnostics, **kwargs) -> AnalysisDocument:
        ...
        if isinstance(analysis._cpp, cpp.results):
            ...
            if "concepts" not in document._metadata:
                document.metadata["concepts"] = set()
            elif isinstance(document.metadata["concepts"], list):
                document.metadata["concepts"] = set(document.metadata["concepts"])
            if isinstance(document.metadata["concepts"], set):
                document.metadata["concepts"].update(self.concepts)
            return document
        else:
            diagnostics.add(
                Diagnostic(
