...
import requests
from typing import Optional, Dict, List  # Добавляем аннотации для старых версий Python
...
_last_response: Optional[Dict] = None  # Заменяем dict | None на Optional[Dict]


def models() -> List[str]:  # Заменяем list[str] на List[str]
    """
...
def q(
    prompt: str,
    *,
    model: Optional[str] = None,  # Заменяем str | None на Optional[str]
    max_tokens: int = 2048,
) -> str:
    """
...
def get_last_response() -> Optional[Dict]:  # Заменяем dict | None на Optional[Dict]
    """
