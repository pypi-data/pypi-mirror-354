...
def run_static_samplers_helper(args):
    sampler_class, imageShape, sparsity, interpolMethod, numberOfFrames = args
    psnrs, ssims = [], []

    # Adjust constructor arguments based on sampler type
    if sampler_class.__name__ == "AdaptiveSampler":
        initialSampling = "stratified"
        sampler = sampler_class(imageShape, initialSampling, interpolMethod, sparsity, numberOfFrames)
    else:
        sampler = sampler_class(imageShape, interpolMethod, sparsity, numberOfFrames)

    for frame_idx in range(numberOfFrames):
        y, x = sampler.get_coordinates()
        pixel_values = sample_image_from_video_sequence(y, x, imageShape, frame_idx)
        ...
        psnr, ssim = evaluate_frame(REFERENCE_VIDEO_SEQUENCE[frame_idx], image)
        psnrs.append(psnr)
        ssims.append(ssim)
