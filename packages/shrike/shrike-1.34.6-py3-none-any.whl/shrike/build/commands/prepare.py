...
class Prepare(Command):
    ...
    def process_all_components(self, files: List[str], action_type) -> List[str]:  # type: ignore
        ...
        if files:
            ...
            if nb_cores == 1 and not self.config.force_parallelism:
                log.info("Batch component processing with 1 thread.")
                ...
                log.info(
                    f"Batch component processing with multiple threads. {nb_cores} threads. Action: '{action_type}'"
                )
                pool = multiprocessing.Pool(processes=nb_cores)
                if action_type == ActionType.BUILD:
                    rv = pool.map(self.build_each_components, files)
                    rv = [result for result in rv if result is not None] 
                elif action_type == ActionType.VALIDATE:
    ...
    def build_each_components(self, component) -> List[str]:
        ...
        build_component_success = self.execute_azure_cli_command(
            f"ml component build --file {component}", ignore_blowfish=True, ignore_pkg_resources=True
        )
...
"""
    ...
    def identify_repo_and_branches(self):
        ...
        try:
            ...
            log.info(f"Found a valid repository in {repo.git_dir}")
    ...
    def validate_each_components(self, component) -> None:
        ...
        validate_component_success = self.execute_azure_cli_command(
            f"ml component validate --file {component}", ignore_blowfish=True, ignore_pkg_resources=True
        )
