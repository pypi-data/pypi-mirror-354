...
def validate_inputs(region_start, region_end, bam_path, target_pattern):
    ...
    if not re.match(r'^[ATCGXN.*]+$', target_pattern, re.IGNORECASE):
        raise ValueError("Invalid characters in target_pattern")
...
def run_search(chromosome, region_start, region_end, bam_path,awk_script):
    ...
    validate_inputs(region_start, region_end, bam_path, awk_script)
