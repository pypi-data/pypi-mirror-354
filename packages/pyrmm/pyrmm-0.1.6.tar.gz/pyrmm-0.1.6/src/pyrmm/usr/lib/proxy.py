...
class ProxyManager:
    ...
    @classmethod
    def generate_unified_proxy_links(cls, project_path: Path, file_download_pairs: list[tuple[str, str]], max_proxies: int = 10) -> str:
        """
        生成统一的代理下载链接markdown文本，支持多个文件
        
        Args:
            project_path: 项目根目录路径
            file_download_pairs: 文件名和下载链接的列表 [(filename, download_url), ...]
            max_proxies: 最多显示的代理数量，默认10个
            
        Returns:
            str: 包含所有文件代理下载链接的markdown文本
        """
        proxies = cls.load_project_proxies(project_path)
        if not proxies or not file_download_pairs:
            return ""
        ...
        for i, proxy in enumerate(top_proxies, 1):
            ...
            proxy_links.append(f"**{i}. {domain_main} ({speed_text})**")
            
            # 为当前代理生成所有文件的下载链接
            for filename, download_url in file_download_pairs:
                proxy_download_url = f"{proxy.url}/{download_url}"
                proxy_links.append(f"   - [{filename}]({proxy_download_url})")
            
            proxy_links.append("")  # 代理节点之间的空行
