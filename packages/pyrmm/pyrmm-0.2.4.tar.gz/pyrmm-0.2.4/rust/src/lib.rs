...
fn run_cli(args: Vec<String>) -> Result<()> {
    ...
    match matches.subcommand() {        Some(("init", sub_matches)) => commands::init::handle_init(&config, sub_matches),
        ...
        Some(("clean", sub_matches)) => commands::clean::handle_clean(&config, sub_matches),        Some(("test", sub_matches)) => commands::test::handle_test(&config, sub_matches),
        Some(("completion", sub_matches)) => commands::completion::handle_completion(&config, sub_matches),
        Some(("mcp", sub_matches)) => commands::mcp::handle_mcp(&config, sub_matches),
        _ => {
            // 如果没有子命令，显示帮助
