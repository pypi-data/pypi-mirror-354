...
class RmmProject(RmmBase, metaclass=RmmProjectMeta):
    ...
    def init(cls, project_path: Path):
        ...
        if authors and isinstance(authors[0], dict):
          ...
          # 创建必要的文件
        cls._create_project_files(project_path, project_name, author_name , project_info )
    ...
    @classmethod
    def _create_project_files(cls, project_path: Path, project_name: str, author_name: str, project_info: dict[str, Any]) -> None:
        ...
        #region 创建 CHANGELOG.MD
        changelog_content = CHANGELOG

        from .basic import LICENSE
        #region 创建 LICENSE 文件内容
        ...
        with open(module_prop, 'w', encoding='utf-8') as f:
            # 使用 toml.dump 写入内容
            import toml
            toml.dump(module_prop_content, f)
        ...
        # 创建 dist 目录
        dist_dir = rmmp_dir / "dist"
        dist_dir.mkdir(exist_ok=True)

        #region Rmake.py
        from .basic import RMAKE
        rmake_script = rmmp_dir / "Rmake.py"
        with open(rmake_script, 'w', encoding='utf-8') as f:
            f.write(RMAKE)
        #endregion