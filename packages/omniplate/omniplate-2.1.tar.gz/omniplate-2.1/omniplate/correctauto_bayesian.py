...
def process_strain(strain, params):
    ...
    flperod_raw = np.full((t.size, params["noboots"]), np.nan)
    # correct autofluorescence for each time point
    for i_boot in range(params["noboots"]):
...
def process_strains_parallel(params, allstrains, max_workers=None):
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        futures_dict = {
            executor.submit(process_strain, s, params): s for s in allstrains
        }
...
def correctauto_bayesian(
    ...
    for e in sunder.getset(
        ...
        for c in sunder.getset(
            ...
            try:
                t, (ywt, odwt) = sunder.extractwells(
                    self.r, self.s, e, c, refstrain, [f[0], "OD"]
                )
                ywt, keep = de_nan(ywt)
            except TypeError:
            ...
            # get data for Null
            try:
                _, yn = sunder.extractwells(self.r, self.s, e, c, "Null", f)
            except TypeError:
            ...
            if len(allstrains):
                ...
                print(f"{c}: Processing {len(allstrains)} strains.")
                # correct autofluorescence
                noprocessors = np.min([noprocessors, os.cpu_count() - 1])
                if noprocessors > 1:
                    # parallel
                    print(
                        f"Using {noprocessors} of {os.cpu_count()} processors."
                    )
                    results = process_strains_parallel(
                        params, allstrains, noprocessors
                    )
                else:
                    # sequential
                    results = [
...
def finish_store_plot(
    ...
    for s, flperod_raw in results:
        flperod_samples, f_fitderiv = smooth_with_GPs(
            ...
            flperod_raw,
            bd=params["bdn"],
            flcvfn=params["flcvfn"],
            nosamples=params["nosamples"],
            experiment=e,
            condition=c,
            strain=s,
            maxdatapts=params["maxdatapts"],
        )
        store_results(
            self,
            f,
        ...
        if params["fitderiv_figs"]:
            f_fitderiv.plotfit(e, c, s, "flperOD", "d/dtflperOD", logs=False)


def plot_noise(p, f, experiment, condition, strain, refstrain):
    """Plot noise in y-channel from omniplate instance."""
    t, ywt = sunder.extractwells(p.r, p.s, experiment, condition, refstrain, f)
    ywt, od_keep = de_nan(ywt)
    t = t[od_keep]
    # get data for Null
    _, yn = sunder.extractwells(p.r, p.s, experiment, condition, "Null", f)
    yn, _ = de_nan(yn)
    # get data for tagged strain
...
def smooth_with_GPs(
    ...
    if np.any(y[~np.isnan(y)]):
        ...
        y_samples = f_fitderiv.fitderivsample(nosamples)[0]
    else:
        print("No positive data.")
        # return all NaN
        y_samples = np.nan * np.ones((t.size, nosamples))
