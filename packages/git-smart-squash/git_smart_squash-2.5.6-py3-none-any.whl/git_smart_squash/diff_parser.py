        ...
        if line.startswith('@@'):
            ...
            if current_hunk_header and current_hunk_lines:
                hunk = _create_hunk_from_lines(
                    ...
                    hunk_start_line, hunk_count, is_new_file
                )
                if hunk:
                    hunks.append(hunk)
            ...
            hunk_start_line, is_new_file = _extract_line_number(line)
            hunk_count += 1
...
def _extract_line_number(hunk_header: str) -> Tuple[int, bool]:
    ...
    Returns:
        Tuple of (starting line number, is_new_file)
        For new files, returns the new file's starting line number
    """
    ...
    if match:
        ...
        # Detect new files: old_start is 0
        if old_start == 0:
            # For new files, use the new file's starting line number
            return new_start, True
        else:
            # For existing files, use the original file line number (before changes)
            return old_start, False
    return 1, False


def _create_hunk_from_lines(file_path: str, hunk_header: str, hunk_lines: List[str], 
                           start_line: int, hunk_count: int, is_new_file: bool = False) -> Optional[Hunk]:
    ...
    Args:
        ...
        is_new_file: Whether this hunk represents a new file
    ...
    # Calculate end line based on file type
    if is_new_file:
        # For new files, count added lines to determine the range
        end_line = start_line - 1  # Start one before, will be incremented
        for line in hunk_lines[1:]:  # Skip header
            if line.startswith('+'):
                end_line += 1
            # Context lines also count for new files (though rare)
            elif line.startswith(' '):
                end_line += 1
        # Ensure we have at least the start line
        if end_line < start_line:
            end_line = start_line
    else:
        # For existing files, count context and removed lines (original behavior)
        end_line = start_line
        for line in hunk_lines[1:]:  # Skip header
    ...
    # Get context for this hunk (skip context for new files since they don't exist yet)
    if is_new_file:
        context = f"New file: {file_path} (lines {start_line}-{end_line})"
    else:
