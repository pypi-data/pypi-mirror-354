...
def _apply_patch_with_git(patch_content: str) -> bool:
    ...
    try:
        ...
        # CRITICAL FIX: Save working directory state for affected files
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # CRITICAL FIX: Restore working directory immediately after --index failure
                print("Restoring working directory state after --index failure...")
...
def _save_working_dir_state(affected_files: Set[str]) -> Dict[str, str]:
    """
    Save current working directory state for affected files.
...
def _restore_working_dir_state(saved_states: Dict[str, str], affected_files: Set[str]) -> bool:
    """
    Restore working directory state for affected files.
    ...
    try:
        ...
        print(f"Working directory restoration: {restored_count}/{len(affected_files)} files processed")
