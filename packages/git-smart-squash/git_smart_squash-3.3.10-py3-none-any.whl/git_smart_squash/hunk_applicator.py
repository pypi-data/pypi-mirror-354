...
def _sync_files_from_staging(file_paths: Set[str]) -> bool:
    ...
    # Sync each file individually using multiple methods for better reliability
    all_success = True
    for file_path in file_paths:
        try:
            # Method 1: Try git checkout-index first
            ...
            if result.returncode == 0:
                print(f"✓ Synced {file_path} via checkout-index")
                continue
                
            # Method 2: If checkout-index fails, try git show and write file directly
            print(f"checkout-index failed for {file_path}, trying direct write...")
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                print(f"✓ Synced {file_path} via direct write")
                continue
                
            print(f"Failed to sync {file_path}: both methods failed")
            all_success = False
