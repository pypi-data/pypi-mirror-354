...
class UnifiedAIProvider:
    ...
    def _generate_gemini(self, prompt: str) -> str:
        ...
        try:
            ...
            # Use dynamic max_tokens, ensuring total doesn't exceed context limit
            # Gemini 2.5 models have much higher output limits
            gemini_output_limits = {
                'gemini-2.5-pro-preview-06-05': 65536,
                'gemini-2.5-flash-preview-05-20': 65536,
                'gemini-1.5-flash': 8192,  # Older model
                'gemini-1.5-pro': 8192     # Older model
            }
            
            max_output_limit = gemini_output_limits.get(self.config.ai.model, 8192)
            max_response_tokens = min(
                params["response_tokens"], 
                max_output_limit,
                model_context_limit - params["prompt_tokens"] - 1000  # Safety buffer
            ...
            # Ensure we have a reasonable minimum for response
            if max_response_tokens < 500:
                raise Exception(f"Prompt too large for Gemini ({params['prompt_tokens']} tokens). Consider reducing diff size.")
