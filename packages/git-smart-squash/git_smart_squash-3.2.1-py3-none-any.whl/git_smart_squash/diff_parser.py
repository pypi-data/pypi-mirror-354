        ...
        for i, hunk in enumerate(sorted_hunks):
            ...
            if hunk_lines and hunk_lines[0].startswith('@@'):
                ...
                if not _validate_hunk_header(hunk_header):
                    ...
                    hunk_header = _reconstruct_hunk_header(hunk, hunk_lines)
                    if hunk_header:
                        hunk_lines[0] = hunk_header
                    else:
                        print(f"Error: Could not reconstruct valid header for hunk {hunk.id}")
                        continue
...
def _validate_hunk_header(header: str) -> bool:
    ...
    # Check for unreasonable jumps or counts
    if abs(old_start - new_start) > 100 and old_count < 10:
        # Large jump with small hunk is suspicious
        return False
