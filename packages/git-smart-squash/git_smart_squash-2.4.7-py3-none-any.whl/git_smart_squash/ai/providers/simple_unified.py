...
class UnifiedAIProvider:
    ...
    def _estimate_tokens(self, text: str) -> int:
        """Estimate token count using ultra-conservative heuristics."""
        # Ultra-conservative estimation for code/diffs: 1 token ≈ 1 character
        # This significantly overestimates to ensure we never truncate prompts
        # Better to be safe than sorry with context allocation
    ...
    def _calculate_ollama_params(self, prompt: str) -> dict:
        ...
        if params["max_tokens"] > self.MAX_CONTEXT_TOKENS * 0.8:
            ...
            # Use 50% safety margin since token estimation may be imperfect
            min_context_needed = int(estimated_prompt_tokens * 1.50) + 1000  # 50% safety margin + response space
    ...
    def _generate_local(self, prompt: str) -> str:
        ...
        try:
            ...
            # DEBUG: Print what we're actually sending to Ollama
            print(f"DEBUG: Calculated num_ctx = {ollama_params['num_ctx']}")
            print(f"DEBUG: Estimated prompt tokens = {self._estimate_tokens(prompt)}")
            print(f"DEBUG: Actual prompt length = {len(prompt)} characters")
            ...
            # DEBUG: Print the final payload being sent
            print(f"DEBUG: Final payload num_ctx = {payload['options']['num_ctx']}")
            print(f"DEBUG: Final payload num_predict = {payload['options']['num_predict']}")
