...
def main():
    ...
    # Remote client parser
    remote_parser = subparsers.add_parser("remote", help="Run FluidMCP remote client bridge")
    remote_parser.add_argument("--server-url", default="http://localhost:8099", 
                              help="FluidMCP server URL")
...
def run_remote_client(args):
    ...
    if args.server_url:
        os.environ["FMCP_SERVER_URL"] = args.server_url
    ...
    print(f"📡 Server: {os.environ.get('FMCP_SERVER_URL', 'http://localhost:8099')}", file=sys.stderr)
