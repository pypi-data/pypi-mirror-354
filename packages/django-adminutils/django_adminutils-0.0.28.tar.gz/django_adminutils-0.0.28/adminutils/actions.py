    ...
    *,
    ...
    def processor(func):
        ...
        def view(self, request, instance_or_queryset):
            ...
            kwargs = {}
            if takes_object:
                kwargs["instance"] = instance_or_queryset
            elif takes_queryset:
                kwargs["queryset"] = instance_or_queryset
            ...
            if isinstance(form_class, dict):
                ...
                single = False
                kw = True
            elif isinstance(form_class, (tuple, list)):
                form_classes = {f"f{i}": f for i, f in enumerate(form_class)}
                single = False
                kw = False
            else:
                form_classes = {None: form_class}
                single = True
                kw = False
            ...
            if request.method == "POST" and "_submit" in request.POST:
                forms = {
                    prefix: form_class(
                        request.POST, request.FILES, prefix=prefix, **kwargs
                    )
                ...
                forms = {
                    prefix: form_class(prefix=prefix, **kwargs)
                    for prefix, form_class in form_classes.items()
                }
            ...
            return render(
                ...
                template_name,
                {
                    ...
                    **({"form": list(forms.values())[0]} if single else {}),
                    "forms": forms,
                    "opts": opts,
