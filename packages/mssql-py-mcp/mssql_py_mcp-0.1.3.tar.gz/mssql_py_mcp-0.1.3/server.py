...
from mssql_py_mcp.app_config import config
from mssql_py_mcp.core import (
    execute_query,
    get_table_info,
    get_db_connection,
    list_show_tables,
...
@mcp.tool()
def get_table_structure(table_name: str, schema: Optional[str] = None) -> Dict[str, Any]:
    """获取指定表的结构信息
...
def sql_describe() -> str:
    ...
    5. 使用参数化查询，避免SQL注入风险
    6. 表名和列名应使用方括号([])包裹，避免与SQL Server关键字冲突
...
def introduction_prompt(
    user_name: Optional[str] = Field(default=None, description="用户姓名，非必填")
) -> str:
    """当用户问好时，需要生成的用户消息."""
    if user_name:
        return f"用户名叫 '{user_name}' ，你需要友好的回复对方的问好，需要有Emoji表情，且要使用中文 ."
    else:
        return "你需要友好的回复对方的问好，需要有Emoji表情，且要使用中文 ."

def main():
    """主函数，用于启动MCP服务器"""
    print(f"启动 {config.SERVER_NAME}...")
    mcp.run()
