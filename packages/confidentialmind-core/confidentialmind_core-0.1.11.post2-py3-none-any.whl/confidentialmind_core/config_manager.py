...
class ConfigManager(SingletonClass):
            ...
            try:
                ...
                async with websockets.connect(self.__realtime_url) as websocket:
                    ...
                    async for message in websocket:
                        ...
                        print(f"{datetime.datetime.now()} - Received message from websocket: {configs}")
    ...
    def __set_configs(self, json_configs, initial_request=False):
        print(f"{datetime.datetime.now()} - Setting configs (initial request: {initial_request}): {json_configs}")
        try:
            # print(json_configs)
            if initial_request:
            ...
            print(f"{datetime.datetime.now()} - Validated configs: {json_configs}")
            print(f"Connectors: {connectors_config}")
