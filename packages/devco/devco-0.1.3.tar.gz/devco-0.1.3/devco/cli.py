...
def create_parser():
    ...
    # Allow 'devco section name' as shorthand for 'devco section show name'
    section_parser.add_argument('name', nargs='?', help='Section name to show (shorthand for show command)')
...
def main():
    ...
    if args.command == 'init':
        ...
        if args.section_action is None:
            if args.name:
                # Show specific section (devco section foo)
                sections_manager.show_section(args.name)
            else:
                print("Section command requires an action or section name")
                sys.exit(1)
        ...
        if not results:
            if hasattr(args, 'json') and args.json:
                ...
                print("No similar content found. Try running 'devco embed' first to generate embeddings.")
            return
