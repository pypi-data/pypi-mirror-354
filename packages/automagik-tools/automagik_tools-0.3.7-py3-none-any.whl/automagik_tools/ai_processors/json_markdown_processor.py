...
class ProcessingResult(BaseModel):
    ...
    raw_json: Union[Dict[str, Any], List[Any], str, int, float, bool] = Field(description="Original JSON data")
...
class JsonMarkdownProcessor:
    ...
    def __init__(self, config: Optional[JsonMarkdownConfig] = None):
        ...
        if self.config.openai_api_key:
            ...
            logger.warning("No OpenAI API key provided. JSON processing will be disabled.")
    
    def _detect_error_response(self, json_data: Union[Dict[str, Any], List[Any], str, int, float, bool]) -> bool:
    ...
    def _has_complex_structure(self, json_data: Union[Dict[str, Any], List[Any], str, int, float, bool]) -> bool:
        """Determine if the JSON has complex nested structures"""
        if isinstance(json_data, dict):
            # Check for nested objects, arrays, or large number of keys
            return (
                len(json_data) > 10 or  # Many top-level keys
                any(isinstance(v, (dict, list)) for v in json_data.values()) or  # Nested structures
                any(isinstance(v, list) and len(v) > 5 for v in json_data.values())  # Large arrays
            )
        elif isinstance(json_data, list):
            return len(json_data) > 5 or any(isinstance(item, (dict, list)) for item in json_data)
...
Use simple headings and bullet points. Keep total output under 200 tokens when possible."""
        ...
        if tool_name == "list_agents":
            context = "For agent lists: show agent names, IDs, and brief descriptions only."
    ...
    async def process_json(
        ...
        try:
            ...
            is_error = (
                ...
                ("error" in json_data or "detail" in json_data or 
                 (isinstance(json_data.get("success"), bool) and not json_data["success"]))
            )
...
"""
    ...
    async def process_multiple(
        ...
        tool_name: str = "batch_operation"
    ) -> List[ProcessingResult]:
        """Process multiple JSON responses concurrently"""
        
        tasks = [
            self.process_json(data, f"{tool_name}_{i}", f"Item {i+1} of {len(data_list)}")
...
if __name__ == "__main__":
    async def test_processor():
        ...
        test_data = {
            "agents": [
                {"id": 8, "name": "claude-code", "description": "Advanced code analysis"},
