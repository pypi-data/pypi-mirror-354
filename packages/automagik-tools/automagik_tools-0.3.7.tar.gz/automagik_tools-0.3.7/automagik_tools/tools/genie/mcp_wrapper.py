...
class FilteredMCPServer:
    ...
    async def start(self):
        ...
        if create_new_process_group and hasattr(asyncio.subprocess, 'CREATE_NEW_PROCESS_GROUP'):
    ...
    async def _filter_stdout(self):
        ...
        while self.process and self.process.stdout:
            try:
                ...
                should_skip = any(pattern in line_str for pattern in self.filter_patterns)
                ...
                if should_skip:
                    ...
                    if self.transport_mode == "sse" and line_count > max_filtered_lines:
                        ...
                        print(f"Warning: Filtered {line_count} lines from MCP server", 
