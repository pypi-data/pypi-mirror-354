...
def _extract_language_service_credentials(conn: CustomConnection) -> Tuple[str, str]:
    ...
    # conn.secrets is a dict of secrets, conn is a dict of non-secrets
    secrets = conn.secrets or {}
    configs = dict(conn)

    endpoint = configs.get("endpoint") or configs.get("api_base")
    api_key = secrets.get("api_key") or secrets.get("key")

    if not endpoint:
        raise ValueError(
            "CustomConnection for Language Service must include the endpoint "
            "(e.g., 'endpoint', 'api_base')."
        )
    if not api_key:
        raise ValueError(
            "CustomConnection for Language Service must include the API key in its secrets "
            "(e.g., 'api_key', 'key')."
        )

    # Ensure endpoint is a base URL
