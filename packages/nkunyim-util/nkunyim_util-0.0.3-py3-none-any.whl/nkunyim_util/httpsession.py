...
class HttpSession:
    ...
    def set_account(self, data: any) -> None:
        self.req.session[settings.SESSION_KEY] = data
        self.req.session.modified = True


    def get_account(self) -> Union[dict, None]:
        if not bool(settings.SESSION_KEY in self.req.session):
            return None

        account = self.req.session[settings.SESSION_KEY]
        return account
    ...
    def get_user(self) -> Union[dict, None]:
        ...
        if self.req.user.is_authenticated:
            user_data = {
                "id": str(self.req.user.id),
                "username": self.req.user.username,
                "first_name": self.req.user.first_name,
                "last_name": self.req.user.last_name,
                "phone_number": self.req.user.phone_number,
                "email_address": self.req.user.email_address,
                "photo_url": self.req.user.photo_url,
                "is_active": self.req.user.is_active,
                "is_admin": self.req.user.is_admin
            }
            if self.req.user.is_superuser:
                user_data['is_superuser'] = self.req.user.is_superuser
    ...
    def get_subdomain(self) -> str:
        ...
        return host_parts[-3] if len(host_parts) > 2 else ""
    ...
    def get_app_data(self) -> dict:
        ...
        apps = settings.NKUNYIM_APPS
        return apps[domain] if domain in apps else {
            "name": "Nkunyim",
            "title": "ERP Books",
            "caption": "NKUNYIM Kasa, Amandze, Abakɔsɛm Na Kodzisɛm",
            "description": "Building The Future of Earth's Most Interesting Language",
            "keywords":"",
        }
