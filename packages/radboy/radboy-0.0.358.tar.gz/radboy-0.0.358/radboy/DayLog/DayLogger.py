...
"PalletCount",]
        ...
        for num,key in enumerate(totals):
            ...
            if dl:
                counter=0
                for f in in fields_for_total:
                    if getattr(dl,l) == 0:
                        counter+=1
                if counter == len(fields_for_total)-1:
                    continue
                ...
                msg+=f"""{Fore.orange_red_1}(Estimated/Inverted Shelf Qty) Shelf=ShelfCount - Qty {Fore.light_yellow}[{Fore.cyan}{dl.ShelfCount}{Fore.light_yellow} -{Fore.cyan}{dl.Shelf}{Fore.light_yellow}]={Fore.pale_green_1b}{dl.ShelfCount-dl.Shelf}{Style.reset}\n"""
                if export:
                    logInput(msg,user=False,filter_colors=True,maxed_hfl=False,ofile=text_cr_export)
                print(msg)
