...
class Cursor:
    ...
    async def execute(
        ...
        if not many and not itemize:
            ...
            return await self._query_str(utils.format_sql(sql, args))
        ...
        if m is None:
            # . execute row by row
            for arg in args:
                rows += await self._query_str(utils.format_sql(sql, arg))
    ...
    def mogrify(
        ...
        if list_len(_args) == 0:
            return sql
        return utils.format_sql(sql, _args[0])
...
class BaseConnection:
    ...
    def transaction(
        ...
        >>> async with conn.transaction() as cur:
                await cur.execute("INSERT INTO tb (id, name) VALUES (1, 'name')")
            # Equivalent to:
            BEGIN;
