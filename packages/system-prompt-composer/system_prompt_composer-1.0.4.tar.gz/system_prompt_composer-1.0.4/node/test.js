...
async function runTests() {
  ...
  try {
    ...
    // Test 3: Test listAvailableDomains
    log(colors.bold, '\n3. Testing listAvailableDomains()...');
    const domains = await listAvailableDomains();
    assert(Array.isArray(domains), 'listAvailableDomains() returns array');
    assert(domains.length > 0, 'At least one domain is available', `Found ${domains.length} domains: ${domains.join(', ')}`);

    // Test 4: Test listAvailableBehaviors
    log(colors.bold, '\n4. Testing listAvailableBehaviors()...');
    const behaviors = await listAvailableBehaviors();
    assert(Array.isArray(behaviors), 'listAvailableBehaviors() returns array');
    assert(behaviors.length > 0, 'At least one behavior is available', `Found ${behaviors.length} behaviors: ${behaviors.join(', ')}`);

    // Test 5: Test composeSystemPrompt with simple object
    ...
    log(colors.bold, '\n5. Testing composeSystemPrompt() with object input...');
    const simpleRequest = {
    ...
    // Test 6: Test composeSystemPrompt with JSON string input
    log(colors.bold, '\n6. Testing composeSystemPrompt() with JSON string input...');
    ...
    // Test 7: Test with MCP configuration
    log(colors.bold, '\n7. Testing composeSystemPrompt() with MCP configuration...');
    const mcpRequest = {
    ...
    // Test 9: Test prompts directory access
    log(colors.bold, '\n9. Testing prompts directory integration...');
    const fs = require('fs');
