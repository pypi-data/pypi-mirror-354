...
from cubicweb_web.views import baseviews, treeview
from cubicweb.predicates import match_kwargs, adaptable
...
class TVTreeViewItemView(treeview.TreeViewItemView):
    ...
    def cell_call(
        ...
        **morekwargs,
        ...
        liclasses = [f"cw-{entity.__regid__.lower()}"]
        # XXX
        ...
        if is_leaf:
            ...
            if is_open:
                ...
                w(f"<li cubicweb:loadurl=\"{url}\" class=\"{' '.join(liclasses)}\">")
...
class TVDefaultTreeViewItemView(treeview.DefaultTreeViewItemView):
    def cell_call(self, row, col, vid="oneline", treeid=None, **morekwargs):
        ...
        if morekwargs["is_last"]:
            liclasses.append("last")
        self.w(f"<li class=\"{' '.join(liclasses)}\">{itemview}</li>")
...
class TVTreeViewEditItemView(TVTreeViewItemView):
    ...
    __select__ = treeview.TreeView.__select__ & (
        match_kwargs(
            "related", "required", "multiple", "leavesonly", "tree_uid", "level"
        )
    )
...
class TVTreeEditView(treeview.TreeView):
    ...
    def _init_params(self, subvid, treeid, initial_load, morekwargs):
        ...
        if initial_fname == "reledit_form":
            form.pop("fname")
        return super()._init_params(subvid, treeid, initial_load, morekwargs)
...
class TVNotTreeviewOneLineEditView(baseviews.InContextView):
    ...
    def cell_call(
        self, row, col, tree_uid, multiple, leavesonly, related, required, level
    ):
        return ""
    ...
    def cell_call(
        ...
        if multiple:
            if level and entity_level != level:
                self.w(f"<span>{entity_name}</span>")
                ...
                self.w(
                    ...
                    % {
                        "eid": entity.eid,
                        "sel": entity.eid in related and 'checked="checked"' or "",
                    }
                )
                self.w(f'<span class="hidden">{entity_name}</span>')
                self.w(
                    '<a href="javascript:$.noop()" onclick="$(\'#treeitem_%s\').click()"'
                    'class="selectable">'
                    "%s</a>" % (entity.eid, entity_name)
                )
            else:
                self.w(f"<span>{entity_name}</span>")
        else:
            cssclass = "selected " if entity.eid in related else ""
            if level and entity_level.level != level:
                self.w(f'<span class="{cssclass}">{entity_name}</span>')
            elif not leavesonly or is_leaf:
                cssclass += ' selectable"'
                onclick = "cw.treewidget.validateRelated(this, {}, '{}', {})".format(
                    required,
                    tree_uid,
                    [[str(entity.eid)], []],
                )
                self.w(
                    '<a href="javascript:$.noop()" onclick="%s" class="%s" '
                    % (onclick, cssclass)
                )
                description = entity.dc_description()
                if description:
                    description = cut(description, 50)
                    self.w(f' title="{xml_escape(description)}"')
                self.w(f">{entity_name}</a>")
            else:
                self.w(f'<span class="{cssclass}">{entity_name}</span>')
...
def registration_callback(vreg):
    ...
    vreg.register_and_replace(
        TVDefaultTreeViewItemView, treeview.DefaultTreeViewItemView
    )
