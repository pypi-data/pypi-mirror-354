...
class TreeViewWidget(fwdgs.FieldWidget):
    ...
    html = (
        ...
        """onclick="$('#%(tree_uid)s').treewidget(%(required)s, %(multiple)s, false, {'cancelButton': '%(cancel)s', 'okButton': '%(ok)s'}) ;return false;">"""  # noqa
        """%(open_link)s&nbsp;%(remove_link)s</a>"""
        """<div id="sel_%(tree_uid)s">%(selected)s</div>"""
        """<div id="%(tree_uid)s" style="display: none;">"""
        """<div class="error"></div><div id="holder_%(tree_uid)s">%(form)s</div></div>"""
    )  # '

    remove_link_html = """<a href="javascript:$.noop()" cubicweb:link="%s" class="remove_link"
    onclick="$('#sel_%s').empty()">[%s]</a>"""

    def __init__(self, leavesonly=False, multiple=None, level=None, **kwargs):
        """
    ...
    def _render(self, form, field, renderer):
        ...
        selected = "\n".join(
            self.render_selected(field.input_name(form, self.suffix), related)
        )
        return self.html % {
    ...
    def _filter_rset(self, field, form):
        ...
        if vocab:
            ...
            return form._cw.execute(f"Any X WHERE X eid IN ({eids})")
        return None

    def render_treeform(self, form, treerset, tree_uid, related):
        return form._cw.view(
            "treeview_edit",
            treerset,
            subvid="oneline_edit",
            tree_uid=tree_uid,
    ...
    def compute_related(self, form, field):
        ...
        if initial_values:
            related = form._cw.execute(
                f"Any K WHERE K eid IN ({','.join([str(v) for v in initial_values])})"
            )
        else:
    ...
    def render_selected(self, name, related):
        ...
        for keyword in related:
            ...
            yield tags.div(
                '<a href="javascript:$.noop()" onclick="$(this).parent().remove()">[x]'
                "</a>"
                "%(input)s %(title)s"
                % {"input": input, "title": xml_escape(keyword.dc_title())}
            )
