...
class QuotesInfo:
    ...
    STANDARD_QUOTES_PER_LANGUAGE = {
        ...
        "en-us": {  # Not verified by linguist
            MetaQuote.OPEN1: "‘",
            MetaQuote.CLOSE1: "’",
            MetaQuote.OPEN2: '“',
            MetaQuote.CLOSE2: '”',
...
class MclehmQuoteDecNorm(NormalizerBase):
    ...
    def process_tgt_decoding(self, seg: Seg) -> None:
        ...
        if QuotesInfo.has_standard_quotes_for_language(self.get_tgt_lang()) and \
            QuotesInfo.has_quote_pairs_for_language(self.get_tgt_lang()):
            ...
            if self.tgt_lang == 'en' or self.tgt_lang == 'es':
                seg.tgt = seg.tgt.replace("‘ ", "‘").replace(' ’', '’').replace(
                    '“ ', '“').replace(' ”', '”') \
                    .replace('< ', '<').replace(' >', '>')
            if self.tgt_lang == "uk" or self.tgt_lang == "ru":
