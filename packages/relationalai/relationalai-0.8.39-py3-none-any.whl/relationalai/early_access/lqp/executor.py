...
from relationalai.early_access.lqp.ir import convert_transaction, lqp_program_to_transaction
    ...
    def execute(self, model: ir.Model, task:ir.Task, export_to:Optional[str]=None) -> DataFrame:
        ...
        if self._last_model != model:
            with debugging.span("compile", metamodel=model) as install_span:
                lqp_program = self.compiler.compile(model)
                model_txn = lqp_program_to_transaction(lqp_program, b"model")
                install_span["compile_type"] = "model"
                install_span["lqp"] = lqp_print.to_string(model_txn, lqp_print.ugly_config)
                self._last_model = model

        with debugging.span("compile", metamodel=task) as compile_span:
            query = f.compute_model(f.logical([task]))
            lqp_program = self.compiler.compile(query, {"wide_outputs": self.wide_outputs})
            query_txn = lqp_program_to_transaction(lqp_program, b"query")
            compile_span["compile_type"] = "query"
            compile_span["lqp"] = lqp_print.to_string(query_txn, lqp_print.ugly_config)
        ...
        return self.execute_transaction(txn_proto)
