...
class Flatten(Pass):
    ...
    def handle_logical(self, task: ir.Logical, ctx: Context):
        ...
        if groups["aggregates"]:
            ...
            connection = metamodel_utils.extract(agg, body, exposed_vars.get_list(), ctx.rewrite_ctx)

            # return a reference to the connection relation
            reference = f.logical([f.lookup(connection, exposed_vars.get_list())], merge_var_list(exposed_vars.get_list(), task.hoisted))
        ...
        if groups["ranks"]:
            ...
            connection = metamodel_utils.extract(rank, body, exposed_vars.get_list(), ctx.rewrite_ctx)
    ...
    def handle_match(self, match: ir.Match, ctx: Context):
        ...
        exposed_vars = intersection(ctx.info.task_inputs(match), outputs)
        ...
        for branch in match.tasks:
            ...
            if reference:
                branch_body.add(negate(reference, negation_len))
                branch_body = OrderedSet.from_iterable([f.union([f.logical(branch_body.get_list(), match.hoisted), reference], match.hoisted)])
            connection = metamodel_utils.extract(branch, branch_body, exposed_vars, ctx.rewrite_ctx, "match")
    ...
    def handle_union(self, union: ir.Union, ctx: Context):
        ...
        outputs = ctx.info.task_outputs(union)
        exposed_vars = intersection(ctx.info.task_inputs(union), outputs)
        ...
        for branch in union.tasks:
            ...
            if connection is None:
                ...
                connection = metamodel_utils.extract(branch, branch_body, exposed_vars, ctx.rewrite_ctx, "union")
            else:
    ...
    def handle_require(self, req: ir.Require, ctx: Context):
        ...
        if len(req.checks) > 1 and isinstance(domain, ir.Logical) and len(domain.body) > 1:
            ...
            connection = metamodel_utils.extract(req, body, vars, ctx.rewrite_ctx, "domain")
...
T = TypeVar('T')
def intersection(s1: Optional[OrderedSet[T]], s2: Optional[OrderedSet[T]]) -> list:
    """ Return the intersection of s1 | s2, accounting for when any is None. """
    if s1 and s2:
