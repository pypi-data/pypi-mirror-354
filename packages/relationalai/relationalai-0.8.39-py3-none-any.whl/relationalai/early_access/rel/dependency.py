...
from relationalai.early_access.metamodel import builtins, ir, helpers, visitor
...
class Cluster():
    ...
    def depends_on(self, other: Cluster):
        ...
        if self in other.dependencies:
            ...
            # relationship7 test
            print("Warning: there is a cycle in the dependency graph. This is a bug.")
        else:
        ...
        def has_dependency(c1: Cluster, c2: Cluster):
            ...
            if (c1.inputs and c2.outputs):
                ...
                for v in c1.inputs:
                    if v in c2.outputs:
                        return True
            # finally, if c1 has an effect and c2 is mergeable (basically it contains only
            # binders) then c2 behaves like a filter. If c1 has as input any variable
            # involved in c2, then it depends on c2.
            if c1.effectful and c2.mergeable and c1.inputs and (c2.inputs or c2.outputs):
                for v in c1.inputs:
                    if v in c2.inputs or c2.outputs:
                        return True
            return False
...
class BindingAnalysis(visitor.Visitor):
    ...
    def visit_logical(self, node: ir.Logical, parent: Optional[ir.Node]):
        ...
        if grounded_by_ancestors:
            ...
            self.input(node, grounded_by_ancestors & vars)
