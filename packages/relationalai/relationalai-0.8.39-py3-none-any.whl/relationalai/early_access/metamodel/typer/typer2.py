...
LATTICE:dict[tuple[ir.Type|None, ir.Type|None], float] = {
    ...
    (types.Int, types.Decimal64): 1.0,
    (types.Decimal64, types.Decimal128): 2.0,
    (types.Decimal128, types.Float): 3.0,
    # transitive closure of the above
    (types.Int, types.Decimal128): 3.0,
    (types.Decimal64, types.Float): 5.0,
    (types.Int, types.Float): 6.0,
}
...
def conversion_allowed(actual:ir.Type, expected:ir.Type, arg_types:set[ir.Type]=set()) -> bool:
    return promotion_cost(actual, expected, arg_types) < float("inf")
        ...
        for node, fields in self.type_requirements.items():
            ...
            for field in fields:
                ...
                if not type_matches(node_type, field_type):
                    ...
                    if (node_base, field_base) in LATTICE:
                        pass
                    else:
    ...
    def visit_rel_op(self, node: ir.Lookup|ir.Aggregate, parent: Optional[ir.Node]):
        ...
        for arg, field in zip(node.args, rel.fields):
            ...
            if not type_matches(arg_type, field_type, allow_any_parents):
                ...
                if not conversion_allowed(arg_type, field_type, arg_types):
            ...
            if not type_matches(left_type, right_type):
                ...
                if conversion_allowed(left_type, right_type):
                    ...
                    tasks.append(f.lookup(
                        conversions[(left_base, right_base)],
                        [left, new_left]
                    ))
                    ...
                    tasks.append(f.lookup(
                        conversions[(right_base, left_base)],
                        [right, new_right]
                    ))
                ...
                for arg, field in zip(lookup_args, overload.fields):
                    ...
                    if field_type != types.EntityTypeVar and not type_matches(arg_type, field_type):
                        ...
                        if (arg_base, field_base) in conversions:
                            ...
                            tasks.append(f.lookup(
                                conversions[(arg_base, field_base)],
                                [arg, new_arg]
                            ))
