...
class ScalarExpr(Expr):
    ...
    def to_z3(self, vars) -> Expr:
        raise Exception(f"Cannot convert \"{self.display()}\" to a Z3 expression")

    # This ScalarExpr is checkable in static verification contexts because
    # it can be trivially translated into a Z3 expression that is usable by
    # a Z3 constraint.
    #
    def checkable(self) -> bool: return False


class ScalarVariable(ScalarExpr):

    def checkable(self):
        return self.physical_typeof().basic_numeric()
...
class Literal(ScalarExpr):
    ...
    def to_z3(self, vars): return self.val
...
class Number(Literal):
    ...
    def checkable(self): return True


class String(Literal):

    def checkable(self): return False
...
class Negate(ScalarExpr):
    ...
    def to_z3(self, vars):
        result = self._part.to_z3(vars).__rsub__(0)
        return result
...
class BinaryScalarExpr(ScalarExpr):
    ...
    def checkable(self):
        return self._left.checkable() and self._right.checkable()
...
class Plus(BinaryScalarExpr):
    ...
    def to_z3(self, vars):
        result = self._left.to_z3(vars).__add__(self._right.to_z3(vars))
        return result
...
class Minus(BinaryScalarExpr):
    ...
    def to_z3(self, vars):
        left = self._left.to_z3(vars)
        right = self._right.to_z3(vars)
        #
        # [REKS] It is pretty awkward to convert constraints into a format
        #        that is accepted by Z3. Haven't yet figured out a good
        #        systematic way to do it.
        #
        if self._left.grounded():
            neg = right.__rsub__(0)
            result = neg.__add__(left)
        else:
...
class Modulus(BinaryScalarExpr):
    ...
    def to_z3(self, vars):
        return self._left.to_z3(vars).__mod__(self._right.to_z3(vars))
...
class Times(BinaryScalarExpr):
    ...
    def to_z3(self, vars):
        if self._left.grounded():
            right = self._left.to_z3(vars)
            left = self._right.to_z3(vars)
        else:
            left = self._left.to_z3(vars)
            right = self._right.to_z3(vars)
...
class Divide(BinaryScalarExpr):
    ...
    def to_z3(self, vars):
        return self._left.to_z3(vars).__truediv__(self._right.to_z3(vars))
