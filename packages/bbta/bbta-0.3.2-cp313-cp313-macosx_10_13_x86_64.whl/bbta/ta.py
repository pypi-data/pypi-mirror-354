...
class Array_2_banta_Series(go.GoClass):
	""""""
	...
	def __str__(self):
		s = 'ta.Array_2_banta_Series len: ' + str(len(self)) + ' handle: ' + str(self.handle) + ' ['
		if len(self) < 120:
			s += ', '.join(map(str, self)) + ']'
		return s
	def __repr__(self):
		return 'ta.Array_2_banta_Series([' + ', '.join(map(str, self)) + '])'
	def __len__(self):
		return _bbta.Array_2_banta_Series_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			...
			return go.banta_Series(handle=_bbta.Array_2_banta_Series_elem(self.handle, key))
		else:
	...
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_bbta.Array_2_banta_Series_set(self.handle, idx, value.handle)
			return
		raise IndexError('slice index out of range')
	def __iter__(self):
		self.index = 0
	...
	def __next__(self):
		if self.index < len(self):
			rv = go.banta_Series(handle=_bbta.Array_2_banta_Series_elem(self.handle, self.index))
			self.index = self.index + 1
			return rv
		raise StopIteration

# Python type for slice [3]banta.Series
class Array_3_banta_Series(go.GoClass):
	""""""
	def __init__(self, *args, **kwargs):
		"""
		handle=A Go-side object is always initialized with an explicit handle=arg
	...
	def __del__(self):
		_bbta.DecRef(self.handle)
	def __str__(self):
		s = 'ta.Array_3_banta_Series len: ' + str(len(self)) + ' handle: ' + str(self.handle) + ' ['
		if len(self) < 120:
			s += ', '.join(map(str, self)) + ']'
		return s
	def __repr__(self):
		return 'ta.Array_3_banta_Series([' + ', '.join(map(str, self)) + '])'
	def __len__(self):
		return _bbta.Array_3_banta_Series_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return go.banta_Series(handle=_bbta.Array_3_banta_Series_elem(self.handle, key))
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_bbta.Array_3_banta_Series_set(self.handle, idx, value.handle)
			return
		raise IndexError('slice index out of range')
	def __iter__(self):
		self.index = 0
	...
	def __next__(self):
		if self.index < len(self):
			rv = go.banta_Series(handle=_bbta.Array_3_banta_Series_elem(self.handle, self.index))
			self.index = self.index + 1
			return rv
		raise StopIteration

# Python type for slice [4]banta.Series
class Array_4_banta_Series(go.GoClass):
	""""""
	def __init__(self, *args, **kwargs):
		"""
		handle=A Go-side object is always initialized with an explicit handle=arg
	...
	def __del__(self):
		_bbta.DecRef(self.handle)
	def __str__(self):
		s = 'ta.Array_4_banta_Series len: ' + str(len(self)) + ' handle: ' + str(self.handle) + ' ['
		if len(self) < 120:
			s += ', '.join(map(str, self)) + ']'
		return s
	def __repr__(self):
		return 'ta.Array_4_banta_Series([' + ', '.join(map(str, self)) + '])'
	def __len__(self):
		return _bbta.Array_4_banta_Series_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return go.banta_Series(handle=_bbta.Array_4_banta_Series_elem(self.handle, key))
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_bbta.Array_4_banta_Series_set(self.handle, idx, value.handle)
			return
		raise IndexError('slice index out of range')
	def __iter__(self):
		self.index = 0
	...
	def __next__(self):
		if self.index < len(self):
			rv = go.banta_Series(handle=_bbta.Array_4_banta_Series_elem(self.handle, self.index))
			self.index = self.index + 1
			return rv
		raise StopIteration
...
# Python type for struct banta.CrossLog
class CrossLog(go.GoClass):
	""""""
	def __init__(self, *args, **kwargs):
		"""
		handle=A Go-side object is always initialized with an explicit handle=arg
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _bbta.banta_CrossLog_CTor()
			_bbta.IncRef(self.handle)
			if  0 < len(args):
				self.Time = args[0]
			if "Time" in kwargs:
			...
			if  1 < len(args):
				self.PrevVal = args[1]
			if "PrevVal" in kwargs:
				self.PrevVal = kwargs["PrevVal"]
			if  2 < len(args):
				self.Hist = args[2]
			if "Hist" in kwargs:
	...
	@property
	def Time(self):
		return _bbta.banta_CrossLog_Time_Get(self.handle)
	@Time.setter
	def Time(self, value):
		if isinstance(value, go.GoClass):
			_bbta.banta_CrossLog_Time_Set(self.handle, value.handle)
		else:
			_bbta.banta_CrossLog_Time_Set(self.handle, value)
	@property
	def PrevVal(self):
		return _bbta.banta_CrossLog_PrevVal_Get(self.handle)
	@PrevVal.setter
	def PrevVal(self, value):
		if isinstance(value, go.GoClass):
			_bbta.banta_CrossLog_PrevVal_Set(self.handle, value.handle)
		else:
			_bbta.banta_CrossLog_PrevVal_Set(self.handle, value)
	@property
	def Hist(self):
		return Slice_Ptr_banta_XState(handle=_bbta.banta_CrossLog_Hist_Get(self.handle))
	@Hist.setter
	def Hist(self, value):
		if isinstance(value, go.GoClass):
			_bbta.banta_CrossLog_Hist_Set(self.handle, value.handle)
		else:
			raise TypeError("supplied argument type {t} is not a go.GoClass".format(t=type(value)))
...
class Series(go.GoClass):
	...
	def Cross(self, obj2):
		"""Cross(str obj2) int"""
		return _bbta.banta_Series_Cross(self.handle, obj2)
...
def CCI(obj, period):
	...
	return go.banta_Series(handle=_bbta.ta_CCI(obj.handle, period))
def CRSIBy(obj, period, upDn, roc, vtype):
	"""CRSIBy(object obj, int period, int upDn, int roc, int vtype) object"""
	return go.banta_Series(handle=_bbta.ta_CRSIBy(obj.handle, period, upDn, roc, vtype))
def HL2(h, l):
	"""HL2(object h, object l) object"""
	return go.banta_Series(handle=_bbta.ta_HL2(h.handle, l.handle))
def ROC(obj, period):
	"""ROC(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_ROC(obj.handle, period))
def EMABy(obj, period, initType):
	"""EMABy(object obj, int period, int initType) object"""
	return go.banta_Series(handle=_bbta.ta_EMABy(obj.handle, period, initType))
def HLC3(h, l, c):
	"""HLC3(object h, object l, object c) object"""
	return go.banta_Series(handle=_bbta.ta_HLC3(h.handle, l.handle, c.handle))
def CHOP(e, period):
	"""CHOP(object e, int period) object"""
	return go.banta_Series(handle=_bbta.ta_CHOP(e.handle, period))
def CRSI(obj, period, upDn, roc):
	"""CRSI(object obj, int period, int upDn, int roc) object"""
	return go.banta_Series(handle=_bbta.ta_CRSI(obj.handle, period, upDn, roc))
def ChaikinOsc(env, sml, big):
	"""ChaikinOsc(object env, int sml, int big) object"""
	return go.banta_Series(handle=_bbta.ta_ChaikinOsc(env.handle, sml, big))
def KAMABy(obj, period, fast, slow):
	"""KAMABy(object obj, int period, int fast, int slow) object"""
	return go.banta_Series(handle=_bbta.ta_KAMABy(obj.handle, period, fast, slow))
def LowestBar(obj, period):
	"""LowestBar(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_LowestBar(obj.handle, period))
def RSI(obj, period):
	"""RSI(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_RSI(obj.handle, period))
def UpDown(obj, vtype):
	"""UpDown(object obj, int vtype) object"""
	return go.banta_Series(handle=_bbta.ta_UpDown(obj.handle, vtype))
def ADXBy(high, low, close, period, smoothing, method):
	...
	return go.banta_Series(handle=_bbta.ta_ADXBy(high.handle, low.handle, close.handle, period, smoothing, method))
def CMF(env, period):
	"""CMF(object env, int period) object"""
	return go.banta_Series(handle=_bbta.ta_CMF(env.handle, period))
def SMA(obj, period):
	"""SMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_SMA(obj.handle, period))
def Sum(obj, period):
	"""Sum(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_Sum(obj.handle, period))
def AvgDev(obj, period):
	"""AvgDev(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_AvgDev(obj.handle, period))
def HMA(obj, period):
	"""HMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_HMA(obj.handle, period))
def VWMA(price, vol, period):
	"""VWMA(object price, object vol, int period) object"""
	return go.banta_Series(handle=_bbta.ta_VWMA(price.handle, vol.handle, period))
def StdDev(obj, period):
	"""StdDev(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_StdDev(obj.handle, period))
def ADX(high, low, close, period):
	"""ADX(object high, object low, object close, int period) object"""
	return go.banta_Series(handle=_bbta.ta_ADX(high.handle, low.handle, close.handle, period))
def ATR(high, low, close, period):
	"""ATR(object high, object low, object close, int period) object"""
	return go.banta_Series(handle=_bbta.ta_ATR(high.handle, low.handle, close.handle, period))
def DV2(h, l, c, period, maLen):
	"""DV2(object h, object l, object c, int period, int maLen) object"""
	return go.banta_Series(handle=_bbta.ta_DV2(h.handle, l.handle, c.handle, period, maLen))
def PercentRank(obj, period):
	"""PercentRank(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_PercentRank(obj.handle, period))
def UTBot(c, atr, rate):
	"""UTBot(object c, object atr, float rate) object"""
	return go.banta_Series(handle=_bbta.ta_UTBot(c.handle, atr.handle, rate))
def LinRegAdv(obj, period, angle, intercept, degrees, r, slope, tsf):
	...
	return go.banta_Series(handle=_bbta.ta_LinRegAdv(obj.handle, period, angle, intercept, degrees, r, slope, tsf))
def WillR(e, period):
	"""WillR(object e, int period) object"""
	return go.banta_Series(handle=_bbta.ta_WillR(e.handle, period))
def ADL(env):
	"""ADL(object env) object"""
	return go.banta_Series(handle=_bbta.ta_ADL(env.handle))
def CMO(obj, period):
	"""CMO(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_CMO(obj.handle, period))
def RMA(obj, period):
	"""RMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_RMA(obj.handle, period))
def TR(high, low, close):
	"""TR(object high, object low, object close) object"""
	return go.banta_Series(handle=_bbta.ta_TR(high.handle, low.handle, close.handle))
def KAMA(obj, period):
	"""KAMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_KAMA(obj.handle, period))
def LinReg(obj, period):
	"""LinReg(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_LinReg(obj.handle, period))
def RMI(obj, period, montLen):
	"""RMI(object obj, int period, int montLen) object"""
	return go.banta_Series(handle=_bbta.ta_RMI(obj.handle, period, montLen))
def AvgPrice(e):
	"""AvgPrice(object e) object"""
	return go.banta_Series(handle=_bbta.ta_AvgPrice(e.handle))
def CTI(obj, period):
	"""CTI(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_CTI(obj.handle, period))
def MFI(e, period):
	"""MFI(object e, int period) object"""
	return go.banta_Series(handle=_bbta.ta_MFI(e.handle, period))
def RMABy(obj, period, initType, initVal):
	"""RMABy(object obj, int period, int initType, float initVal) object"""
	return go.banta_Series(handle=_bbta.ta_RMABy(obj.handle, period, initType, initVal))
def STC(obj, period, fast, slow, alpha):
	"""STC(object obj, int period, int fast, int slow, float alpha) object"""
	return go.banta_Series(handle=_bbta.ta_STC(obj.handle, period, fast, slow, alpha))
def WMA(obj, period):
	"""WMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_WMA(obj.handle, period))
def Lowest(obj, period):
	"""Lowest(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_Lowest(obj.handle, period))
def TD(obj):
	"""TD(object obj) object"""
	return go.banta_Series(handle=_bbta.ta_TD(obj.handle))
def CMOBy(obj, period, maType):
	"""CMOBy(object obj, int period, int maType) object"""
	return go.banta_Series(handle=_bbta.ta_CMOBy(obj.handle, period, maType))
def EMA(obj, period):
	"""EMA(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_EMA(obj.handle, period))
def Highest(obj, period):
	"""Highest(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_Highest(obj.handle, period))
def Stiffness(obj, maLen, stiffLen, stiffMa):
	"""Stiffness(object obj, int maLen, int stiffLen, int stiffMa) object"""
	return go.banta_Series(handle=_bbta.ta_Stiffness(obj.handle, maLen, stiffLen, stiffMa))
def RSI50(obj, period):
	"""RSI50(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_RSI50(obj.handle, period))
def Stoch(high, low, close, period):
	"""Stoch(object high, object low, object close, int period) object"""
	return go.banta_Series(handle=_bbta.ta_Stoch(high.handle, low.handle, close.handle, period))
def ALMA(obj, period, sigma, distOff):
	"""ALMA(object obj, int period, float sigma, float distOff) object"""
	return go.banta_Series(handle=_bbta.ta_ALMA(obj.handle, period, sigma, distOff))
def ER(obj, period):
	"""ER(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_ER(obj.handle, period))
def HighestBar(obj, period):
	"""HighestBar(object obj, int period) object"""
	return go.banta_Series(handle=_bbta.ta_HighestBar(obj.handle, period))


# ---- Functions ---
def KDJ(high, low, close, period, sm1, sm2):
	"""KDJ(object high, object low, object close, int period, int sm1, int sm2) []object"""
	return Array_3_banta_Series(handle=_bbta.ta_KDJ(high.handle, low.handle, close.handle, period, sm1, sm2))
def Aroon(high, low, period):
	"""Aroon(object high, object low, int period) []object"""
	return Array_3_banta_Series(handle=_bbta.ta_Aroon(high.handle, low.handle, period))
def HeikinAshi(e):
	"""HeikinAshi(object e) []object"""
	return Array_4_banta_Series(handle=_bbta.ta_HeikinAshi(e.handle))
def PluMinDI(high, low, close, period):
	"""PluMinDI(object high, object low, object close, int period) []object"""
	return Array_2_banta_Series(handle=_bbta.ta_PluMinDI(high.handle, low.handle, close.handle, period))
def StochRSI(obj, rsiLen, stochLen, maK, maD):
	"""StochRSI(object obj, int rsiLen, int stochLen, int maK, int maD) []object"""
	return Array_2_banta_Series(handle=_bbta.ta_StochRSI(obj.handle, rsiLen, stochLen, maK, maD))
...
def MACDBy(obj, fast, slow, smooth, initType):
	...
	return Array_2_banta_Series(handle=_bbta.ta_MACDBy(obj.handle, fast, slow, smooth, initType))
def BBANDS(obj, period, stdUp, stdDn):
	"""BBANDS(object obj, int period, float stdUp, float stdDn) []object"""
	return Array_3_banta_Series(handle=_bbta.ta_BBANDS(obj.handle, period, stdUp, stdDn))
def StdDevBy(obj, period, ddof):
	"""StdDevBy(object obj, int period, int ddof) []object"""
	return Array_2_banta_Series(handle=_bbta.ta_StdDevBy(obj.handle, period, ddof))
def MACD(obj, fast, slow, smooth):
	"""MACD(object obj, int fast, int slow, int smooth) []object"""
	return Array_2_banta_Series(handle=_bbta.ta_MACD(obj.handle, fast, slow, smooth))
def PluMinDM(high, low, close, period):
	"""PluMinDM(object high, object low, object close, int period) []object"""
	return Array_2_banta_Series(handle=_bbta.ta_PluMinDM(high.handle, low.handle, close.handle, period))
def KDJBy(high, low, close, period, sm1, sm2, maBy):
	...
	return Array_3_banta_Series(handle=_bbta.ta_KDJBy(high.handle, low.handle, close.handle, period, sm1, sm2, maBy))
