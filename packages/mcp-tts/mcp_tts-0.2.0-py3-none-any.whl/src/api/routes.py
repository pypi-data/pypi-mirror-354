...
def create_api_routes() -> APIRouter:
    ...
    async def get_status():
        ...
        return ServerStatus(
            ...
            audio_devices=[
                AudioDevice(**device) for device in status["audio_devices"]
            ]
        )
    ...
    async def test_audio_device(request: DeviceTestRequest):
        ...
        if success:
            device_info = "default device" if request.device_index is None else f"device {request.device_index}"
            return {"success": True, "message": f"Audio test successful for {device_info}"}
        else:
            raise HTTPException(
                status_code=400,
    ...
    async def set_volume(request: VolumeRequest):
        ...
        return {
            "success": True,
            "message": f"Volume set to {request.volume:.1%}"
        }
    ...
    async def set_provider(provider_name: str):
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Provider not available: {provider_name}"
            )
