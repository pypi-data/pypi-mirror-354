...
#             mag_unit=self.mag_unit, dir=dir, dir_unit=self.dir_unit)
    ...
    def x(self) -> Qty:
        ...
        return self.magnitude * np.cos(self.direction)
    
    @property
    def y(self) -> Qty:
        """Returns the y-component of the force."""
        return self.magnitude * np.sin(self.direction)
...
#         return Q_(self.dir, self.dir_unit)
    ...
    def tail(self) -> List[float]:
        ...
        x = Qty(value=0, unit=self.magnitude.unit)
        y = Qty(value=0, unit=self.magnitude.unit)
