...
class Scraper:
    ...
    def download_flashcard_fields(self,
        ...
        TODO: print available fields

        missing_fields: dict, field_name -> list of lemmas
        ...
        """
        logger.debug(f'download_flashcard_fields: {missing_fields.keys()}')
        downloaded_fields = {}
        if has_sentences:= any([field_name for field_name in missing_fields.keys() if field_name.startswith('sentences_')]):
            task_id = self.trigger_sentences(
                {'_'.join(field_name.split('_')[1:]): words for field_name, words in missing_fields.items()
                    if field_name.startswith("sentences_")}, known_sources_by_subdomain = known_sources_by_subdomain)

        tasks = []
        for field_name, words in {field_name: words for field_name, words in missing_fields.items()
                                    if not field_name.startswith("sentences_")}.items():
            try:
                func = getattr(self, f'fetch_{field_name}')
            except AttributeError as e:
                raise RuntimeError(f'Section name "{field_name}" not specified in scraper.py module.')

            for word in words:
                tasks.append((field_name, word, func))
            ...
            for i, ((field_name, word), future) in enumerate(word_to_future.items()):
                for attempt in range(5):
                    try:
                        ...
                        time.sleep(150)
                if field_name not in downloaded_fields:
                    downloaded_fields[field_name] = {}
                downloaded_fields[field_name][word] = result
                print("finished", field_name, word)
        ...
        if has_sentences:
            ...
            for subdomain, sentences in sentences_by_subdomains.items():
                downloaded_fields[f'sentences_{subdomain}'] = sentences
    ...
    def download_sentences(self, task_ids: List[int]):
        def generate_sentences_overview(sentence_overview_by_lemmas):
            results = {}
            for lemma, sentence_overview_fields in sentence_overview_by_lemmas.items():
                fields = []
                for field, overview in sentence_overview_fields.items():
                    fields.append(Collapsible(field, RawHtml(markdown.markdown(overview))))
                results[lemma] =  CardSection(f'Overview', 'Crimson', SpanEnumeration(fields), is_open = True)
            return results
        i = 0
                ...
                if response['status'] == 'SUCCESS':
                    ...
                    if (sentence_overview_by_lemmas:=response['result'].get('_overview')):
                        ...
                        for lemma, sentence_overview_fields in sentence_overview_by_lemmas.items():
                            ...
                            for field, overview in sentence_overview_fields.items():
                                fields.append(Collapsible(field, RawHtml(markdown.markdown(overview))))
                            sentences_overview[lemma] =  CardSection(f'Overview', 'Crimson', SpanEnumeration(fields), is_open = True)
                            ...
                            for i, source_entry_sentence in enumerate(source_entry_sentences):
                                ...
                                if subdomain == 'youtube':
                                    try:
                                        ...
                                        entries_field.append(YoutubeEmbed(youtube_hash, start_s, end_s))
                                    except:
                                        pass
                                else:
                                ...
                                for j, entry in enumerate(entries):
                                    ...
                                    entries_field.append(Collapsible(
                                        Sentence(entry, word), translation, summary_size='14px', light=True))
                                    logger.debug(f'entries_field: {entries_field}')

                                sentence_components.append(Enumeration(entries_field))
