...
class Card(OrderedDict):
    def __init__(self,
        ...
        Exposes content by inheriting from OrderedDict.
            Key is name of the flashcard field. (e.g. wikitionary or sentences_anime)
    ...
    def ensure_initialized(method):
        ...
        def wrapper(self, *args, **kwargs):
            if self.json_path and not self.is_initialized:
                ...
                for name, field in card_dict.get('fields', {}).items():
                    super(Card, self).__setitem__(name, CardSection.from_dict(field))
    ...
    @ensure_initialized
    def generate_back(self, field_names):
        """
        ...
        back = template.render(
            ...
            css_content=css,
            js_content=js,
            flashcard_content='<br>'.join([str(self[field_name]) for field_name in field_names])
        )
        return back
    ...
    def as_dict(self):
        ...
        return {
            ...
            'fields': {field_name: field.as_dict() for field_name, field in self.items()},
    ...
    def __repr__(self):
        return f'word: {self.word}\n' + \
               'occurrence frequencies in decks:\n' + \
               '\n'.join([f'    {source_name}: {freq}' for source_name, freq in self.source_references.items()]) + '\n' + \
               'fields with content: \n' + \
               '\n'.join([f'    {field_name}' for field_name in self.keys()]) + '\n' + \
