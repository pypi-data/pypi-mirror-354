...
def get_solidworks_install_path() -> str:
    try:
        key = winreg.OpenKey(winreg.HKEY_CLASSES_ROOT, "SOLIDWORKS Installation Manager")
        version_key = winreg.OpenKey(key, winreg.EnumKey(key, 0))
        install_path, _ = winreg.QueryValueEx(version_key, "InstallDir")
        
        return install_path
    except:
        raise Exception("solidworks not found")    
