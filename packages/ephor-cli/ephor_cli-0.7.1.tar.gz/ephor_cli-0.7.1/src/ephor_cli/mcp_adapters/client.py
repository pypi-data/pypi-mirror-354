...
from mcp_adapters.sessions import (
    Connection,
...
from mcp_adapters.tools import load_mcp_tools
...
class MultiServerMCPClient:
    ...
    async def get_tools(self, *, server_name: str | None = None, progress_callback=None) -> list[BaseTool]:
        ...
        Args:
            ...
            progress_callback: Optional callback function for tool progress updates.
        ...
        if server_name is not None:
            ...
            return await load_mcp_tools(None, connection=self.connections[server_name], progress_callback=progress_callback)

        all_tools: list[BaseTool] = []
        load_mcp_tool_tasks = []
        for connection in self.connections.values():
            load_mcp_tool_task = asyncio.create_task(load_mcp_tools(None, connection=connection, progress_callback=progress_callback))
