...
from accumulate.models.transactions import CreateKeyPage, Transaction
...
logger = logging.getLogger("AccumulateCreateKeyPageDebug")

#  Constants
ACCUMULATE_RPC_URL = "https://testnet.accumulatenetwork.io"
# ACCUMULATE_RPC_URL = get_accumulate_rpc_url()
...
async def process_create_key_page():
    ...
    custom_timestamp = 1739950965269927  # Replace with your own dynamic value

    tx_header = await TransactionHeader.create(
        principal=IDENTITY_KEYBOOK_URL,
...
######################### Create Transaction Body #########################
    ...
    key1 = KeySpecParams(key_hash=hashlib.sha256(bytes.fromhex("03419c374c5f2fd41716734ad5a36f25dfc9ec5176dadd075264c6f49ca7fef9")).digest())
#    key2 = KeySpecParams(key_hash=hashlib.sha256(public_key_32).digest())
    logger.info(f" New Page: Public Key Hash: {key1}")

    #  Create CreateKeyPage transaction body
    tx_body = CreateKeyPage(keys=[key1])

    marshaled_body = tx_body.marshal()
    logger.info(f" Marshaled CreateKeyPage Body (HEX): {marshaled_body.hex()}")
...
if __name__ == "__main__":
    asyncio.run(process_create_key_page())