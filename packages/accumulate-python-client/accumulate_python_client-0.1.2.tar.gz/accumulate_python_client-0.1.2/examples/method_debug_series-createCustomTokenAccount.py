# examples\method_debug_series-createCustomTokenAccount.py
...
from accumulate.models.transactions import CreateTokenAccount, Transaction
from accumulate.utils.hash_functions import LiteAuthorityForKey

#  Enable detailed logging
logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger("AccumulateCreateCustomTokenAccountDebug")
...
PUBLIC_KEY_HEX = "9e6797738d73a7cba1d9c02fabf834f9cfcc873a53776285be96f10f780a0046" # replace public key hex
...
NEW_TOKEN_ACCOUNT_URL = "acc://custom-adi-name-1741948502948.acme/CTACUST"
TOKEN_ISSUER_URL = "acc://custom-adi-name-1741948502948.acme/CUST"

async def process_create_cust_token_account():
    ...
    custom_timestamp = 1739950965269921  # Replace with your own dynamic value
...
######################### Create Transaction Body #########################
    ...
    #  Parse URLs
    token_account_url = URL.parse(NEW_TOKEN_ACCOUNT_URL)
    token_issuer_url = URL.parse(TOKEN_ISSUER_URL)

    #  Create CreateTokenAccount transaction body
    tx_body = CreateTokenAccount(url=token_account_url, token_url=token_issuer_url)

    marshaled_body = tx_body.marshal()
    logger.info(f" Marshaled CreateTokenAccount Body (HEX): {marshaled_body.hex()}")
...
if __name__ == "__main__":
    asyncio.run(process_create_cust_token_account())
