...
TASK_DOCUMENTATION = r"""
    ...
    >>> print(f"First Relevant Rank: {results["first_relevant_rank"]:.2f}")
    ```
...
class CBIREvaluator(Evaluator):
    ...
    def compute(
        ...
        Args:
            ...
            k_values (List[int], *optional*, defaults to [1, 5, 10, 20, 50]):
                K values for Precision@K and Recall@K computation.
    ...
    def _compute_cbir_metrics(
        ...
        if compute_ranking_metrics:
            try:
                ...
                results["first_relevant_rank"] = frr_results["first_relevant_rank"]
        ...
        organized_results = self._organize_results(
            results, k_values, precision_thresholds,
            compute_ranking_metrics, compute_precision_at_k,
            compute_recall_at_k, compute_recall_at_precision, compute_per_NR
        )

        return organized_results, None

    def _compute_summary_statistics(self, predictions, references, ignore_missing):
    ...
    def _organize_results(self, results, k_values, precision_thresholds,
                          compute_ranking_metrics, compute_precision_at_k,
                          compute_recall_at_k, compute_recall_at_precision, compute_per_NR):
        ...
        if compute_ranking_metrics:
            ...
            if "first_relevant_rank" in results:
                organized["ranking_metrics"]["first_relevant_rank"] = results["first_relevant_rank"]
