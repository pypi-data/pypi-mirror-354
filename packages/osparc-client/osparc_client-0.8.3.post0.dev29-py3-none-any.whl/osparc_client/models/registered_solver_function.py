...
class RegisteredSolverFunction(BaseModel):
    """
    RegisteredSolverFunction
    """ # noqa: E501
    function_class: Optional[StrictStr] = 'SOLVER'
    ...
    solver_version: Annotated[str, Field(strict=True)]
    __properties: ClassVar[List[str]] = ["function_class", "title", "description", "input_schema", "output_schema", "default_inputs", "uid", "solver_key", "solver_version"]
    ...
    def function_class_validate_enum(cls, value):
        ...
        if value not in set(['SOLVER']):
            raise ValueError("must be one of enum values ('SOLVER')")
        return value

    @field_validator('solver_key')
    def solver_key_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^simcore\/services\/((comp|dynamic|frontend))\/([a-z0-9][a-z0-9_.-]*\/)*([a-z0-9-_]+[a-z0-9])$", value):
            raise ValueError(r"must validate the regular expression /^simcore\/services\/((comp|dynamic|frontend))\/([a-z0-9][a-z0-9_.-]*\/)*([a-z0-9-_]+[a-z0-9])$/")
        return value

    @field_validator('solver_version')
    def solver_version_validate_regular_expression(cls, value):
        """Validates the regular expression"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "function_class": obj.get("function_class") if obj.get("function_class") is not None else 'SOLVER',
            ...
            "uid": obj.get("uid"),
            "solver_key": obj.get("solver_key"),
            "solver_version": obj.get("solver_version")
        })
        return _obj
