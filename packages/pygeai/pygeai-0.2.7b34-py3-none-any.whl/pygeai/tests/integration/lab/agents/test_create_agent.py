    ...
    def test_create_agent_without_required_data(self):
        ...
        for auto_publish in test_params:
            with self.subTest(input=auto_publish):
                ...
                self.created_agent = self.__create_agent(automatic_publish=auto_publish)
        
                #TODO: Change validation error to a more specific one
                self.assertEqual(self.created_agent.id, 400)
                self.assertEqual(self.created_agent.description, "Bad Request")
    ...
    def test_create_agent_invalid_name(self):
        ...
        for auto_publish in test_params:
            ...
            with self.subTest(input=auto_publish):
                ...
                result2 = self.__create_agent(agent=new_agent2, automatic_publish=auto_publish)
    ...
    def test_create_agent_invalid_scope(self):
        ...
        with self.assertRaises(ValidationError) as exception:
            ...
            self.assertIn(
                ...
                str(exception),
                "Expected a validation error about allowed values for access_scope"
            )
