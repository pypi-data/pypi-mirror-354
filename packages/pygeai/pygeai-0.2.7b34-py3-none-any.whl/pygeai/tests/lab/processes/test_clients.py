...
class TestAgenticProcessClient(unittest.TestCase):
    ...
    def test_update_process_success_with_id(self, mock_put):
    ...
    @patch("pygeai.core.services.rest.ApiService.put")
    def test_update_process_success_with_name(self, mock_put):
    ...
    @patch("pygeai.core.services.rest.ApiService.put")
    def test_update_process_json_decode_error(self, mock_put):
        mock_response = mock_put.return_value
        mock_response.status_code = 200
        mock_response.json.side_effect = JSONDecodeError("Invalid JSON", "", 0)
        mock_response.text = "Invalid JSON response"
        ...
        self.assertEqual(str(context.exception), f"Unable to update process {self.process_id} in project {self.project_id}: Invalid JSON response")
