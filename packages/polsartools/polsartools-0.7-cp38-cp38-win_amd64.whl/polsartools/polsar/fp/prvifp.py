...
def prvifp(infolder,  window_size=1, outType="tif", cog_flag=False, 
    ...
    full-polarimetric SAR data. PRVI is specifically designed to assess vegetation
    density and biomass by utilizing the complete polarimetric information available
    in the coherency (T3) or covariance (C3) matrix.

    Examples
    --------
    >>> # Basic usage with default parameters
    >>> prvifp("/path/to/fullpol_data")
    ...
    None
        ...
        - 'prvi_fp.tif' or 'prvi_fp.bin': PRVI values
    ...
    output_filepaths = []
    if outType == "bin":
        output_filepaths.append(os.path.join(infolder, "prvi_fp.bin"))
    else:
        output_filepaths.append(os.path.join(infolder, "prvi_fp.tif"))
    
    process_chunks_parallel(input_filepaths, list(output_filepaths), 
                            window_size=window_size, write_flag=write_flag,
                        processing_func=process_chunk_prvifp,block_size=block_size, 
