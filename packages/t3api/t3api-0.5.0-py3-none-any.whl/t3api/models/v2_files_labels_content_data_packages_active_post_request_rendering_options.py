...
class V2FilesLabelsContentDataPackagesActivePostRequestRenderingOptions(BaseModel):
    """
    Options for controlling how the barcode will render.
    """ # noqa: E501
    barcode_bar_thickness: Optional[Any] = Field(default=None, description="Controls how thick a barcode's bars will appear. Useful for low-DPI thermal printers. Tested values: - Zebra printers with a Code128 should use 0.94 - Dymo printers with a Code128 should use 0.8 - 1.0 is normal rendering.  - 0.9 is 10% thinner - 1.1 is 10% thicker", alias="barcodeBarThickness")
    label_margin_thickness: Optional[Any] = Field(default=None, description="Controls width of a label's margins. Useful for thermal printers with slighly skewed printing. - 1.0 is normal margin.  - 0.9 is 10% thinner - 1.1 is 10% thicker", alias="labelMarginThickness")
    __properties: ClassVar[List[str]] = ["barcodeBarThickness", "labelMarginThickness"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if barcode_bar_thickness (nullable) is None
        # and model_fields_set contains the field
        if self.barcode_bar_thickness is None and "barcode_bar_thickness" in self.model_fields_set:
            _dict['barcodeBarThickness'] = None

        # set to None if label_margin_thickness (nullable) is None
        # and model_fields_set contains the field
        if self.label_margin_thickness is None and "label_margin_thickness" in self.model_fields_set:
            _dict['labelMarginThickness'] = None
