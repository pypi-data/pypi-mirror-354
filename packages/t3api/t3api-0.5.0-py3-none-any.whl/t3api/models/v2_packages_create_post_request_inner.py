...
class V2PackagesCreatePostRequestInner(BaseModel):
    """
    V2PackagesCreatePostRequestInner
    """ # noqa: E501
    actual_date: date = Field(description="The date of the package creation.", alias="actualDate")
    ingredients: List[V2PackagesCreatePostRequestInnerIngredientsInner] = Field(description="List of package ingredients used to create this new package.")
    item_id: StrictInt = Field(description="Item ID of the item for the new package To find eligible items, use the [Source Items](#/Create%20Package/get_v2_packages_create_source_items) endpoint. ", alias="itemId")
    location_id: StrictInt = Field(description="Location ID of the location for the new package To find eligible source locations, use the [Create Package Inputs](#/Create%20Package/get_v2_packages_create_inputs) endpoint. ", alias="locationId")
    note: Optional[StrictStr] = Field(default=None, description="Notes about the package creation, if needed. ")
    production_batch_number: Optional[StrictStr] = Field(default=None, description="The production batch number of the new package, if needed.", alias="productionBatchNumber")
    quantity: Union[StrictFloat, StrictInt] = Field(description="Quantity of the new package.")
    tag_id: StrictInt = Field(description="Tag ID of the tag used for the new package To find eligible source tags, use the [Create Package Inputs](#/Create%20Package/get_v2_packages_create_inputs) endpoint. ", alias="tagId")
    unit_of_measure_id: StrictInt = Field(description="Unit of measure identifier for the package.", alias="unitOfMeasureId")
    expiration_date: Optional[date] = Field(default=None, description="The expiration date of the package, if needed.  *Only applicable in some Metrc states* ", alias="expirationDate")
    use_by_date: Optional[date] = Field(default=None, description="The use by date of the package, if needed. *Only applicable in some Metrc states* ", alias="useByDate")
    sell_by_date: Optional[date] = Field(default=None, description="The sell by date of the package, if needed. *Only applicable in some Metrc states* ", alias="sellByDate")
    __properties: ClassVar[List[str]] = ["actualDate", "ingredients", "itemId", "locationId", "note", "productionBatchNumber", "quantity", "tagId", "unitOfMeasureId", "expirationDate", "useByDate", "sellByDate"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ingredients": [V2PackagesCreatePostRequestInnerIngredientsInner.from_dict(_item) for _item in obj["ingredients"]] if obj.get("ingredients") is not None else None,
