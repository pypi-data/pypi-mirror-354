...
CUSTOM_TEMPLATE_VERIFY_CONTAINER_PREFIX = "celium-sdk-test-container"
...
def build_and_push_docker_image_from_dockerfile(
    ...
    try:
        logger.debug(f"Building Docker Image and pushing to registry: {image_name}")
        build_cmd = [
            "docker", "build",
            "-f", dockerfile_path,
            "-t", image_name,
        ...
        logger.debug(f"Docker image built successfully")
        ...
        if size_result.returncode == 0:
            ...
            logger.debug(f"Docker image size: {image_size / (1024*1024):.2f} MB")
        else:
        ...
        if docker_username and docker_password:
            ...
            if login_proc.returncode != 0:
                logger.warning(f"[bold red]Docker login failed![/bold red]\n{login_proc.stderr}")
                return False, image_size

            logger.debug(f"Docker login successful")

        # Push the docker image
        push_cmd = ["docker", "push", image_name]
        result = subprocess.run(push_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            logger.warning(f"Docker push failed: {result.stderr}")
            return False, image_size
            
        logger.debug(f"Docker image pushed successfully")
...
def create_docker_container(image_name: str, public_key: str) -> tuple[str, int]:
    ...
    if status.returncode != 0:
        logger.warning(f"Docker container is not running!")
    ...
    try:
        ...
        if container_ids:
            for container_id in container_ids.splitlines():
                ...
                logger.debug(f"Removed container with ID: {container_id}")
        else:
            logger.debug(f"No containers found with '{CUSTOM_TEMPLATE_VERIFY_CONTAINER_PREFIX}' prefix.")
    except Exception as e:
...
def verify_docker_image_validity(image_name: str) -> bool:
    ...
    """
    try:
        logger.debug(f"Verifying docker image validity: {image_name}")
        ...
        logger.debug(f"Docker container created successfully!")

        # Install the openssh-server 
        install_openssh_server(container_name, public_key)
        logger.debug(f"Openssh-server installed successfully!")
        ...
        if not is_verified:
            ...
            return False
        logger.debug(f"SSH connection verified successfully!")
        return True
    except Exception as e:
        logger.error(f"An error occurred: {e}")
        return False    
