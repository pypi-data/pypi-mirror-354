...
ARGUMENT_TARGET_NOISE_STD = "target_noise_std"
ARGUMENT_TARGET_NOISE_CLIP = "target_noise_clip"
...
DEFAULT_CONFIGURATION = {
    ...
    ARGUMENT_TARGET_NOISE_STD: 0.2,
    ARGUMENT_TARGET_NOISE_CLIP: 0.5,
...
def make_td3_components(
    ...
    if not isinstance(observation_space, Box):
        raise ValueError(
            f"This TD3 implementation only supports Box observation spaces, but got {type(observation_space)}"
        )
    if not isinstance(action_space, Box):
        raise ValueError(
            f"This TD3 implementation only supports Box action spaces, but got {type(action_space)}"
        )
    ...
    update_rule = TD3Update(
        ...
        target_noise_std=configuration[ARGUMENT_TARGET_NOISE_STD],
        target_noise_clip=configuration[ARGUMENT_TARGET_NOISE_CLIP],
...
athlete.register(
    id="td3",
    component_factory=make_td3_components,
