...
class TestCluster:
    ...
    def test_cluster_correct(self):
        ...
        # with equal size groups (i.e. 16 samples / 4 = 4 per cluster)
        ...
        assert "juzi_cluster_groups" in adata.uns
        assert "juzi_cluster_linkage" in adata.uns

        groups, counts = np.unique(
            adata.uns["juzi_cluster_groups"], return_counts=True)
    ...
    def test_cluster_overcluster(self):
        ...
        assert len(groups) == len(adata.uns["juzi_names"])
