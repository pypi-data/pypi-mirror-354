class NotRealInterface(Exception):
...
class Connnector:
    ...
    def normalize(side_x):
        ...
        if not isinstance(side_x, str):
            raise NotRealInterface('Use string, Luke')
        side_x = side_x.lower()
        if any(map(vsp, ['ge', 'et', 'gi'])):
            pass
        else:
            raise NotRealInterface("Try to be REAL interface: ['ge', 'et', 'gi']")
        if side_x[-1].isdigit():
            return side_x
        else:
            raise NotRealInterface('Your interface should have a num in the end!')
    ...
    def status(self, value):
        if not isinstance(value, str):
            raise UnrealStatus('Choose visely. Give me the String!')
        if value.capitalize() in self._statuses:
            self._status = value.capitalize()
        else:
            raise UnrealStatus(f'This status is unreal! Use one of supported: {self._statuses}')
