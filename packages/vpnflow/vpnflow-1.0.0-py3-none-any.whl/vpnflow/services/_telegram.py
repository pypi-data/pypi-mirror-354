...
async def broadcast(bot: Bot, message: str, *recipients, photo=None):
    ...
    if photo:
        results = await gather(
            *(bot.send_photo(chat_id=recipient, caption=message, photo=photo) for recipient in recipients),
            return_exceptions=True
        )
    else:
        results = await gather(
            *(bot.send_message(recipient, message) for recipient in recipients),
            return_exceptions=True
        )
