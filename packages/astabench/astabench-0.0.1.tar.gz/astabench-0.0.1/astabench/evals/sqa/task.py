...
from astabench.evals.sqa.citation_eval import JUST_HAS_A_TITLE, CitationEval
from astabench.evals.sqa.precision_eval import PrecisionEval
from astabench.evals.sqa.rubric import (
...
def score_citation(
    ...
    async def score(state: TaskState, target: Target) -> Score:
        def filter_citation(citation: Dict[str, Any], cite_text: str) -> bool:
            ...
            If the snippets are:
            1. present, and
            2. are not same as the title, and
            3. are not exactly present in the section text,
            then return True.
            """
            cite_text_alpha = re.sub(r"[^a-zA-Z]", "", cite_text).lower()
            return (
                citation.get("title")
                and citation.get("snippets")
                and not any(citation["title"] in s for s in citation["snippets"])
                and not any(
                    re.sub(r"[^a-zA-Z]", "", s).lower() in cite_text_alpha
...
def score_all(
    scorer_model: str | Model = "openai_json/gpt-4o-2024-08-06",
...
def load_rubrics_data(split: str = "dev") -> List[Dict]:
    ...
    data_files = f"tasks/sqa/rubrics_{version}.json"
    p = hf_hub_download(
...
def sqa(
    ...
    if with_search_tools:
        ...
        validation_set_inserted_before = "2025-04"  # TODO: set test set to "2025-06"
        tool_setups.append(
            use_tools(
                [
                    ToolDef(
                        make_paper_search(
                            inserted_before=validation_set_inserted_before
                        )
                    ).as_tool(),
                    ToolDef(
                        make_snippet_search(
    ...
    if with_native_search_tools:
        native_search_tools = make_native_search_tools()
