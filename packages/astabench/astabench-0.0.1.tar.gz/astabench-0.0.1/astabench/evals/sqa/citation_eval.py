...
def citation_intersection(supporting_cits, bad_cits, all_cits):
    supporting_cits = {clean_citation(str(c)) for c in supporting_cits}
    bad_cits = {clean_citation(str(c)) for c in bad_cits}
    all_cits = {clean_citation(str(c)) for c in all_cits}
    return list(supporting_cits.intersection(bad_cits)) + list(
        supporting_cits.difference(all_cits)
    )
...
class CitationEval:
    ...
    async def score_citation_group(
        self, citation_group: str, citations: List[Dict[str, str]]
    ):
        if not citations:
            return {
                "n_attributable": 0,
        ...
        for claim in claims:
            ...
            n_extrapolatory += 0 if not supported else 1
            n_half_credit_citations.append(
                len(
                    citation_intersection(
                        claim["supporting"], half_credit, citation_ids
                    )
                )
            )
            supporting_counts.append(len(claim["supporting"]))
            n_half_credit_claims += (
                1
                if supported and supporting_counts[-1] == n_half_credit_citations[-1]
                else 0
            )
            non_supporting_counts.append(len(claim["non_supporting"]))
        prompt_logs = {
    ...
    async def evaluate_citation_groups(
        ...
        for citation_group, citations in citation_group_cit_pairs:
            scores, meta = await self.score_citation_group(citation_group, citations)
