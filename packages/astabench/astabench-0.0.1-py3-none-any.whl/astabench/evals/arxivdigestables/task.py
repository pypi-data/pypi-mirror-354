...
from astabench.tools import snippet_search, table_editor
...
def score_tables(unroller_model: str | Model, scorer_model: str | Model) -> Scorer:
    ...
    async def score(state: TaskState, target: Target) -> Score:
        ...
        unrolled_table = unrolled_table_str.completion.split("\n")
        unrolled_table = [line for line in unrolled_table if line.strip()]
        # Compare each statement against unrolled target statements
        # and use that to compute the recall for the generated table
        final_input = eval_prompt.replace("[UNROLLED_TABLE]", target.text)
        responses = []
        recall = 0.0
        for statement in unrolled_table:
            ...
            if "true" in supported_stmt:
                recall += 1
        target_statement_num = len([x for x in target.text.split("\n") if x.strip()])
...
def setup_snippet_tool() -> Solver:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        async def inner_snippet_search(
            ...
            allowed_corpus_id_list = state.metadata["corpus_ids"]
            corpus_id_list = allowed_corpus_id_list
            if corpus_ids:
                ...
                corpus_id_list = list(
                    set(allowed_corpus_id_list) & set(map(str, corpus_ids))
                )
                if not corpus_id_list:
                    return json.dumps(
                        {
                            "error": "No valid corpus ids found in the input.",
                            "allowed_corpus_ids": allowed_corpus_id_list,
                        }
                    )
            try:
                return str(await tool1(query, limit, corpus_ids=corpus_id_list))
            except Exception as e:
                return json.dumps({"exception": str(e)})

        tool2 = ToolDef(tool1)
        tool2.tool = inner_snippet_search
        return await use_tools(tool2.as_tool())(state, generate)
...
def arxivdigestables(
    unroller_model: str | Model = "openai/gpt-4o-mini",
    scorer_model: str | Model = "openai/gpt-4o-mini",
    ...
    instances = [json.loads(line) for line in open(cached_file)]
    ...
    unrolled_tables = json.loads(open(cached_file).read())
