# -*- coding: utf-8 -*-
"""
Created on Tue Sep 9 13:52:00 2022
...
from softsensor.armax import ARMAX
...
def test_armax_sine():
    ...
    order = (2, 4, 2)
    armax = ARMAX(order=order)
    armax.fit(data_train=[df], input_sensors=inp_sens, output_sensors=out_sens, windows=[(0, 800)])
    assert len(armax.parameters) == order[0] + order[1] * len(inp_sens) + order[2]

    # start at zero
    test_df = df.iloc[50:, :]

    prediction = armax.prediction(test_df)
...
def test_armax_duffing():
    ...
    order = (2, 6, 2)
    armax = ARMAX(order=order)
    armax.fit(train_df, inp_sens, out_sens, verbose=True)
    assert len(armax.parameters) == order[0] + order[1] * len(inp_sens) + order[2]
