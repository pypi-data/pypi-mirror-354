    ...
    try:
        ...
        if azapi_mode:
            system_prompt = (
                ...
                "ALWAYS output a terraform.tfvars section as the last section, containing all variables with example or default values. "
                ...
                "RECHECK IF YOU ADDED ANY ARGUMENTS, ATTRIBUTES, OR BLOCKS THAT ARE NOT PRESENT IN THE DOCUMENTATION. IF YOU DID, REMOVE THEM. CHECK IF EVERY PARAMETER HAS ITS VARIABLES IN VARIABLES.TF AND AN EXAMPLE VALUE IN TERRAFORM.TFVARS. "
                "If you do not include the terraform.tfvars section, your answer will be considered incomplete. Your response MUST always end with the terraform.tfvars section."
            )
        else:
            system_prompt = (
                "Carefully read the documentation below. When generating the Terraform files, you must include ALL arguments, attributes, and blocks that are explicitly present in the documentation, including all optional and nested ones. "
                ...
                "Each section will be saved to a separate file with the corresponding name. "
                "Do not use any code block markers (such as triple backticks or ```hcl). Only output plain Terraform code, no explanations.\n\n"
                "RECHECK IF YOU ADDED ANY ARGUMENTS, ATTRIBUTES, OR BLOCKS THAT ARE NOT PRESENT IN THE DOCUMENTATION. IF YOU DID, REMOVE THEM. CHECK IF EVERY PARAMETER HAS ITS VARIABLES IN VARIABLES.TF, AN OUTPUT IF APPROPRIATE, AND AN EXAMPLE VALUE IN TERRAFORM.TFVARS. "
                "If you do not include the terraform.tfvars section, your answer will be considered incomplete. Your response MUST always end with the terraform.tfvars section."
            )
...
def split_and_save_outputs(gpt_output, module_dir):
    # Split the GPT output into main.tf, variables.tf, and terraform.tfvars using section headers
    sections = {
        'main.tf': '',
        'variables.tf': '',
        'terraform.tfvars': ''
    ...
    for line in gpt_output.splitlines():
        header_match = re.match(r'^#+\s*(main|variables|terraform\.tfvars)\.tf', line.strip().lower())
        if header_match:
            ...
            if section_name == 'terraform.tfvars.tf':
                section_name = 'terraform.tfvars'
    ...
    # Clean up variables.tf: remove any lines after the last closing brace, and any tfvars header/assignments
    variables_path = os.path.join(module_dir, 'variables.tf')
    if os.path.exists(variables_path):
        with open(variables_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        # Remove everything after '### terraform.tfvars' header if present
        cleaned_lines = []
        for line in lines:
            if line.strip().lower().startswith('### terraform.tfvars'):
                break
            cleaned_lines.append(line)
        # If no tfvars header, remove lines after last closing brace
        if len(cleaned_lines) == len(lines):
            ...
            for idx, line in enumerate(cleaned_lines):
                if line.strip() == '}':
                    last_brace_idx = idx
            if last_brace_idx is not None and last_brace_idx + 1 < len(cleaned_lines):
                cleaned_lines = cleaned_lines[:last_brace_idx + 1]
        with open(variables_path, 'w', encoding='utf-8') as f:
            f.writelines(cleaned_lines)

    # Fallback: If terraform.tfvars is empty or missing, auto-generate from variables.tf
    tfvars_path = os.path.join(module_dir, 'terraform.tfvars')
    if not os.path.exists(tfvars_path) or os.path.getsize(tfvars_path) == 0:
        variables_path = os.path.join(module_dir, 'variables.tf')
        if os.path.exists(variables_path):
            print('⚠️  terraform.tfvars missing or empty, generating from variables.tf...')
            with open(variables_path, 'r', encoding='utf-8') as f:
                variables_content = f.read()
            # Regex to extract variable blocks
            var_blocks = re.findall(r'variable\s+"([^"]+)"\s*\{([^}]*)\}', variables_content, re.DOTALL)
            tfvars_lines = []
            for var_name, var_block in var_blocks:
                ...
                if default_match:
                    ...
                    tfvars_lines.append(f'{var_name} = {default_val}')
                else:
                    # Try to guess type for placeholder
                    type_match = re.search(r'type\s*=\s*([a-zA-Z0-9_{}()\[\]"\s]+)', var_block)
                    if type_match:
                        type_val = type_match.group(1).strip()
                        if 'string' in type_val:
                            tfvars_lines.append(f'{var_name} = "example"')
                        elif 'bool' in type_val:
                            tfvars_lines.append(f'{var_name} = false')
                        elif 'number' in type_val:
                            tfvars_lines.append(f'{var_name} = 0')
                        elif 'map' in type_val:
                            tfvars_lines.append(f'{var_name} = {{}}')
                        elif 'list' in type_val:
                            tfvars_lines.append(f'{var_name} = []')
                        else:
                            tfvars_lines.append(f'{var_name} = null')
                    else:
                        tfvars_lines.append(f'{var_name} = null')
            with open(tfvars_path, 'w', encoding='utf-8') as f:
                f.write('\n'.join(tfvars_lines) + '\n')
            print(f'✅ Auto-generated {tfvars_path} from variables.tf ({len(tfvars_lines)} variables)')
