...
from .security_validation.security_validator import SecurityValidator
from .security_profiles import SecurityProfile, get_security_thresholds, get_security_checks, log_security_profile_info
from .memory import DefaultSharedScratchpad, FileBasedPersistentSharedKnowledge
from .security_profiles import get_cached_regex, cache_security_validation
...
class Squad:
    ...
    def __init__(self, experts: List[Expert], operations: List[Operation], process: str = 'sequential',
             ...
             persistent_shared_knowledge_base_path: str = "./.squad_knowledge",
             squad_id: Optional[str] = None, 
             initial_session_id: Optional[str] = "default_session", 
             **kwargs):
        """
        ...
        Args:
            ...
            persistent_shared_knowledge_base_path (str): Path for squad's persistent shared knowledge.
            squad_id (Optional[str]): Unique ID for the squad. Auto-generated if None.
            initial_session_id (Optional[str]): Initial session ID for shared knowledge. Defaults to 'default_session'.
        ...
        # Squad ID management
        self.squad_id = squad_id if squad_id else f"squad_{uuid.uuid4().hex[:8]}"
        ...
        self.security_context = {
            'squad_id': self.squad_id,  # Use the managed squad_id
        ...
        try:
            self.shared_scratchpad = DefaultSharedScratchpad()
            logger.info(f"Squad '{self.squad_id}' initialized with DefaultSharedScratchpad.")
            
            # Initialize Persistent Shared Knowledge if path is provided
            if persistent_shared_knowledge_base_path:
                self.persistent_shared_knowledge = FileBasedPersistentSharedKnowledge(
                    base_path=persistent_shared_knowledge_base_path
                )
                if self.initial_session_id:
                    self.persistent_shared_knowledge.set_active_session(squad_id=self.squad_id, session_id=self.initial_session_id)
                    logger.info(f"Squad '{self.squad_id}' initialized FileBasedPersistentSharedKnowledge. Active session: '{self.initial_session_id}'. Path: '{persistent_shared_knowledge_base_path}'")
                else:
                    # If no initial_session_id, it will operate without a specific session context initially
                    # or FileBasedPersistentSharedKnowledge might have its own default.
                    # Forcing a default session if none provided to ensure consistency:
                    default_init_session = "default_squad_session"
                    self.persistent_shared_knowledge.set_active_session(squad_id=self.squad_id, session_id=default_init_session)
                    logger.warning(f"Squad '{self.squad_id}' initialized FileBasedPersistentSharedKnowledge with default session '{default_init_session}' as no initial_session_id was provided. Path: '{persistent_shared_knowledge_base_path}'")
            else:
                self.persistent_shared_knowledge = None
                logger.info(f"Squad '{self.squad_id}' initialized without Persistent Shared Knowledge (no path provided).")

        except Exception as e:
            logger.error(f"Error initializing shared memory components for Squad '{self.squad_id}': {e}")
            # Decide if this is a critical failure or if the squad can operate without shared memory
            self.shared_scratchpad = None
            self.persistent_shared_knowledge = None
    ...
    def get_squad_id(self) -> Optional[str]:
        """Returns the unique ID of this squad."""
        return self.squad_id

    def set_active_shared_session(self, session_id: str) -> None:
        """Sets the active session for the squad's persistent shared knowledge."""
        if not session_id:
            logger.error(f"Squad '{self.squad_id}': Session ID cannot be empty when setting active shared session.")
            raise ValueError("Session ID cannot be empty.")

        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, 'set_active_session'):
            try:
                self.persistent_shared_knowledge.set_active_session(squad_id=self.squad_id, session_id=session_id)
                logger.info(f"Squad '{self.squad_id}' active shared session changed to '{session_id}'.")
            except Exception as e:
                logger.error(f"Squad '{self.squad_id}': Failed to set active shared session to '{session_id}': {e}")
        elif not self.persistent_shared_knowledge:
            logger.warning(f"Squad '{self.squad_id}' has no persistent shared knowledge base. Cannot set session.")
        else:
            logger.warning(f"Squad '{self.squad_id}'s persistent shared knowledge base does not support session switching.")

    def get_active_shared_session(self) -> Optional[str]:
        """Returns the active session ID of the squad's persistent shared knowledge."""
        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, '_current_session_id'):
            return self.persistent_shared_knowledge._current_session_id
        elif self.persistent_shared_knowledge:
            logger.warning(f"Squad '{self.squad_id}': Persistent shared knowledge active, but session ID attribute not found.")
        return None

    def store_shared_fact(self, key: str, value: Any) -> None:
        """Stores a fact in the squad's persistent shared knowledge for the current session."""
        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, '_shared_knowledge') and hasattr(self.persistent_shared_knowledge, 'save_shared'):
            try:
                # Ensure _shared_knowledge is a dict, though it should be initialized as such
                if not isinstance(self.persistent_shared_knowledge._shared_knowledge, dict):
                    self.persistent_shared_knowledge._shared_knowledge = {} # Re-initialize if not a dict
                    logger.warning(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): _shared_knowledge was not a dict, re-initialized.")
                
                self.persistent_shared_knowledge._shared_knowledge[key] = value
                self.persistent_shared_knowledge.save_shared() # Persist after modification
                logger.info(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Stored shared fact '{key}'.")
            except Exception as e:
                logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Failed to store shared fact '{key}': {e}")
        else:
            logger.warning(f"Squad '{self.squad_id}': Persistent shared knowledge not properly configured for storing fact '{key}'. Missing _shared_knowledge or save_shared.")

    def retrieve_shared_fact(self, key: str) -> Optional[Any]:
        """Retrieves a fact from the squad's persistent shared knowledge for the current session."""
        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, '_shared_knowledge'):
            try:
                # _shared_knowledge should be populated by load_shared() called during set_active_session
                if not isinstance(self.persistent_shared_knowledge._shared_knowledge, dict):
                    logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): _shared_knowledge is not a dict. Cannot retrieve fact.")
                    return None
                
                fact = self.persistent_shared_knowledge._shared_knowledge.get(key)
                logger.info(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Retrieved shared fact '{key}'. Value: {'<present>' if fact is not None else '<not found>'}")
                return fact
            except Exception as e:
                logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Failed to retrieve shared fact '{key}': {e}")
                return None
        else:
            logger.warning(f"Squad '{self.squad_id}': Persistent shared knowledge not properly configured for retrieving fact '{key}'. Missing _shared_knowledge.")
            return None

    def get_all_shared_facts(self) -> Dict[str, Any]:
        """Retrieves all facts from the squad's persistent shared knowledge for the current session."""
        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, '_shared_knowledge'):
            try:
                if not isinstance(self.persistent_shared_knowledge._shared_knowledge, dict):
                    logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): _shared_knowledge is not a dict. Cannot retrieve all facts.")
                    return {}
                facts = self.persistent_shared_knowledge._shared_knowledge.copy() # Return a copy
                logger.info(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Retrieved all {len(facts)} shared facts.")
                return facts
            except Exception as e:
                logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Failed to retrieve all shared facts: {e}")
                return {}
        else:
            logger.warning(f"Squad '{self.squad_id}': Persistent shared knowledge not properly configured for retrieving all facts. Missing _shared_knowledge.")
            return {}

    def clear_shared_session_knowledge(self) -> None:
        """Clears all shared knowledge for the squad's current active session."""
        if self.persistent_shared_knowledge and hasattr(self.persistent_shared_knowledge, '_shared_knowledge') and hasattr(self.persistent_shared_knowledge, 'save_shared'):
            try:
                self.persistent_shared_knowledge._shared_knowledge = {}
                self.persistent_shared_knowledge.save_shared() # Persist the cleared state
                logger.info(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Cleared all shared knowledge for the current session.")
            except Exception as e:
                logger.error(f"Squad '{self.squad_id}' (Session: {self.get_active_shared_session()}): Failed to clear shared knowledge for the current session: {e}")
        else:
            logger.warning(f"Squad '{self.squad_id}': Persistent shared knowledge not properly configured for clearing session knowledge. Missing _shared_knowledge or save_shared.")
