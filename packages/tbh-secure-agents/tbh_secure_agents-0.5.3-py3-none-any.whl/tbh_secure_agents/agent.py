...
import string
import uuid
from datetime import datetime, timezone # Updated for timezone-aware UTC timestamps
import nltk
...
from .security_validation.validators.enhanced_hybrid_validator import EnhancedHybridValidator
from .security_validation.validators.hybrid_validator import HybridValidator
from .security_validation.security_validator import SecurityValidator # Assuming this is the concrete class
from .memory import DefaultWorkingMemory, FileBasedPersistentKnowledgeBase
...
class Expert:
    ...
    Attributes:
        ...
        working_memory_max_entries (Optional[int], optional): Maximum number of entries in working memory. Defaults to 100.
        persistent_knowledge_base_path (str, optional): Path to the persistent knowledge base. Defaults to "./.expert_knowledge".
        memory_mode (str, optional): Specifies which memory systems to use. Options: "both" (default), "short_term", "long_term", "none".
    """
    def __init__(self,
                 specialty: str,
                 objective: str,
                 ...
                 # Memory parameters
                 working_memory_max_entries: Optional[int] = 100,
                 persistent_knowledge_base_path: str = "./.expert_knowledge",
                 memory_mode: str = "both",
                 initial_session_id: Optional[str] = None): # Add initial_session_id parameter
        ...
        # Convert string profile to Enum for fetching settings
        # self.security_profile is already the normalized string (e.g., "minimal", "standard")
        current_profile_enum = SecurityProfile.from_string(self.security_profile)

        # Get thresholds and checks based on the profile
        # For CUSTOM profiles, the profile_name argument in get_security_thresholds/checks expects the original custom name string.
        # The self.security_profile_str stores the original input string which could be a custom profile name.
        custom_profile_name_if_any = self.security_profile_str if current_profile_enum == SecurityProfile.CUSTOM else None
        
        self.security_thresholds = get_security_thresholds(current_profile_enum, profile_name=custom_profile_name_if_any)
        self.security_checks = get_security_checks(current_profile_enum, profile_name=custom_profile_name_if_any)

        # Log the applied security settings for clarity
        log_security_profile_info(current_profile_enum, profile_name=custom_profile_name_if_any)

        # Initialize security components with the string version of the security level
        # The 'security_level' variable (which is self.security_profile) is already correctly set as a string.
        ...
        self.working_memory: Optional[DefaultWorkingMemory] = None
        self.persistent_knowledge: Optional[FileBasedPersistentKnowledgeBase] = None

        logger.info(f"Initializing memory with mode: {self.memory_mode}")

        if self.memory_mode in ["both", "short_term"]:
            try:
                self.working_memory = DefaultWorkingMemory(max_entries=working_memory_max_entries)
                logger.info(f"Expert '{self.expert_id}' initialized with DefaultWorkingMemory (max_entries={working_memory_max_entries}).")
            except Exception as e:
                logger.error(f"Error initializing DefaultWorkingMemory for expert '{self.expert_id}': {e}", exc_info=True)
                # self.working_memory remains None if initialization fails or mode doesn't include it

        if self.memory_mode in ["both", "long_term"]:
            try:
                self.persistent_knowledge = FileBasedPersistentKnowledgeBase(
                    expert_id=self.expert_id,
                    base_path=persistent_knowledge_base_path,
                    initial_session_id=initial_session_id if initial_session_id else "default_expert_session" # Pass session_id
                )
                logger.info(f"Expert {self.expert_id} initialized with FileBasedPersistentKnowledgeBase at {persistent_knowledge_base_path} for session '{self.persistent_knowledge._current_session_id if self.persistent_knowledge and hasattr(self.persistent_knowledge, '_current_session_id') else 'N/A'}'.")
            except Exception as e:
                logger.error(f"Error initializing FileBasedPersistentKnowledgeBase for expert '{self.expert_id}': {e}", exc_info=True)
                # self.persistent_knowledge remains None if initialization fails or mode doesn't include it

        # Initialize visualization components if enabled
        if self.enable_visualization:
            self._initialize_visualization(effective_api_key)

        # Initialize hybrid validation by default
        self._initialize_hybrid_validation(effective_api_key)

        # Log the applied security profile and its settings
        # The variables current_profile_enum and custom_profile_name_if_any are defined earlier in __init__
        log_security_profile_info(current_profile_enum, profile_name=custom_profile_name_if_any)

    def get_active_memory_session(self) -> Optional[str]:
        """
        Returns the active session ID of the persistent knowledge base.
        Returns None if persistent knowledge is not enabled or no session is active.
        """
        if self.memory_mode in ["long_term", "both"] and self.persistent_knowledge:
            # FileBasedPersistentKnowledgeBase stores it in _current_session_id
            if hasattr(self.persistent_knowledge, '_current_session_id'):
                return self.persistent_knowledge._current_session_id
            else:
                # This case should ideally not be hit if using FileBasedPersistentKnowledgeBase
                logger.warning(f"Expert {self.expert_id}: Persistent knowledge base active, but session ID attribute not found.")
        elif self.memory_mode in ["short_term", "none"]:
            logger.debug(f"Expert {self.expert_id}: Persistent knowledge not active (mode: {self.memory_mode}). No session ID to retrieve.")
        elif not self.persistent_knowledge:
            logger.debug(f"Expert {self.expert_id}: No persistent knowledge base initialized. No session ID to retrieve.")
        return None

    def set_active_memory_session(self, session_id: str) -> None:
        """Sets the active session for the persistent knowledge base."""
        if not session_id:
            logger.error("Session ID cannot be empty when setting active memory session.")
            raise ValueError("Session ID cannot be empty.")

        if self.persistent_knowledge and hasattr(self.persistent_knowledge, 'set_active_session'):
            try:
                self.persistent_knowledge.set_active_session(session_id)
                logger.info(f"Expert {self.expert_id} active memory session changed to '{session_id}'.")
            except Exception as e:
                logger.error(f"Expert {self.expert_id}: Failed to set active memory session to '{session_id}': {e}")
    ...
    def execute_task(self, task_description: str, context: Optional[str] = None, inputs: Optional[Dict[str, Any]] = None) -> str:
        ...
        """
        if not self.llm:
            error_message = f"Error: Expert '{self.specialty}' cannot execute operation. LLM not initialized (check API key and configuration)."
            logger.error(error_message)
            if hasattr(self, 'working_memory') and self.working_memory:
                try:
                    log_entry = {
                        "timestamp": datetime.now(timezone.utc).isoformat(), # Modern timezone-aware UTC
                        "type": "task_execution_failure",
                        "task_description": task_description, # Log the template
                        "inputs": inputs, # Log the actual parameters used
                        "reason": "LLM not initialized",
                        "error_message": error_message
                    }
                    self.working_memory.add(log_entry)
                    logger.debug(f"Logged LLM initialization failure to working memory for expert '{self.specialty}'.")
                except Exception as e_mem:
                    logger.error(f"Failed to log LLM initialization failure to working memory for expert '{self.specialty}': {e_mem}")
        ...
        try:
            if hasattr(self, 'working_memory') and self.working_memory:
                recent_work_entries = self.working_memory.get_recent(count=3)
                if recent_work_entries:
                    formatted_recent_work = "\n".join([
                        f"- Previous task hint: {entry.get('task', 'N/A')[:70]}..., Output hint: {str(entry.get('output', 'N/A'))[:70]}..."
                        for entry in recent_work_entries
                    ])
                    memory_context_parts.append(f"Recent activity relevant to your role:\n{formatted_recent_work}")
        except Exception as e:
            logger.warning(f"Expert '{self.expert_id}' failed to recall from working memory: {e}")

        try:
            if hasattr(self, 'persistent_knowledge') and self.persistent_knowledge:
                knowledge_key_parts = [word for word in task_description.lower().split() if len(word) > 3]
                knowledge_key = "_".join(knowledge_key_parts[:5])
                if knowledge_key: # Ensure key is not empty
                    recalled_knowledge = self.persistent_knowledge.recall(key=knowledge_key)
                    if recalled_knowledge:
                        # Ensure recalled_knowledge is treated as a string for consistent formatting
                        knowledge_str = str(recalled_knowledge.get('value', '')) if isinstance(recalled_knowledge, dict) else str(recalled_knowledge)
                        memory_context_parts.append(f"Relevant knowledge for key '{knowledge_key}':\n{knowledge_str[:250]}...") # Truncate long knowledge
        except Exception as e:
            logger.warning(f"Expert '{self.expert_id}' failed to recall from persistent knowledge: {e}")
        
        final_memory_context = "\n\n".join(memory_context_parts)
        if final_memory_context:
            if context:
                context = f"MEMORY & CONTEXT:\n{final_memory_context}\n\n---\nORIGINAL PROVIDED CONTEXT:\n{context}"
            else:
                context = f"MEMORY & CONTEXT:\n{final_memory_context}"
            logger.info(f"Expert '{self.expert_id}' augmented prompt context with recalled memory.")
        # --- End Memory Recall ---
        ...
        while attempts <= max_retries:
            ...
            try:
                    ...
                    # --- Memory Storage (on successful execution) ---
                    if hasattr(self, 'working_memory') and self.working_memory:
                        try:
                            work_entry = {
                                "operation_id": operation_id,
                                "task": task_description,
                                # "inputs": inputs, # Consider implications of storing potentially large/complex inputs
                                "output_snippet": llm_output[:100] + "..." if len(llm_output) > 100 else llm_output, # Store a snippet
                                "timestamp": datetime.utcnow().isoformat()
                            }
                            self.working_memory.add(work_entry)
                            logger.debug(f"Expert '{self.expert_id}' stored task interaction to working memory.")
                        except Exception as e:
                            logger.warning(f"Expert '{self.expert_id}' failed to store to working memory: {e}")

                    # Store in Persistent Knowledge Base if active
                    if hasattr(self, 'persistent_knowledge') and self.persistent_knowledge:
                        try:
                            # Generate a key for PKB storage
                            task_description_words = [word for word in task_description.lower().split() if len(word) > 3]
                            knowledge_store_key = "_".join(task_description_words[:5]) + "_outcome"

                            if knowledge_store_key: # Ensure key is not empty
                                # Sanitize before storing to PKB (TEMPORARILY DISABLED FOR DEBUGGING)
                                sanitized_value_for_pkb = llm_output # Directly use llm_output
                                # if isinstance(llm_output, str):
                                #     sanitized_value_for_pkb = self.data_guardian.sanitize_text(llm_output)
                                #     if sanitized_value_for_pkb != llm_output:
                                #         logger.debug(f"Expert '{self.expert_id}' sanitized value before storing to PKB for key '{knowledge_store_key}'. Original length: {len(llm_output)}, Sanitized length: {len(sanitized_value_for_pkb)}")
                                
                                self.persistent_knowledge.remember(key=knowledge_store_key, value=sanitized_value_for_pkb, category="task_execution_outcome")
                                logger.debug(f"Expert '{self.expert_id}' stored task outcome to persistent knowledge with key '{knowledge_store_key}'.")
                            else:
                                logger.warning(f"Expert '{self.expert_id}' could not generate a valid key for PKB storage for task: {task_description[:50]}...")
                        except Exception as e:
                            logger.warning(f"Expert '{self.expert_id}' failed to store to persistent knowledge: {e}")
