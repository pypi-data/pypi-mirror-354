...
from myVaillant.const import API_URL_BASE, LOGIN_URL
from myVaillant.tests.generate_test_data import DATA_DIR
from myVaillant.utils import get_realm


def _myvaillant_aioresponses():
    class _myvaillant_aioresponses(aioresponses):
        def __init__(
...
async def _mocked_api(*args, **kwargs) -> MyVaillantAPI:
    api = MyVaillantAPI("test@example.com", "test", "vaillant", "germany")
    api.oauth_session = {
        "access_token": "access_token",
        "refresh_token": "refresh_token",
        "expires_in": 3600,
