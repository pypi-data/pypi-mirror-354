...
from myVaillant.const import BRANDS
...
from myVaillant.utils import datetime_parse, prepare_field_value_for_dict

logger = logging.getLogger(__name__)

T = TypeVar("T", bound="myVaillantDataClass")
...
class Home(myVaillantDataClass):
    country_code: str
...
@dataclass(config=config)
class BaseTimeProgramDay(myVaillantDataClass):
    index: int
    weekday_name: str
    start_time: int
    end_time: int
...
@dataclass(config=config)
class BaseTimeProgram(myVaillantDataClass):
    monday: list
    tuesday: list
    wednesday: list
    thursday: list
...
@dataclass(config=config)
class ZoneHeating(myVaillantDataClass):
...
@dataclass(config=config)
class ZoneCooling(myVaillantDataClass):
    control_identifier: ControlIdentifier
...
@dataclass(config=config)
class ZoneGeneral(myVaillantDataClass):
    name: str
    timezone: datetime.tzinfo
...
@dataclass(config=config)
class Circuit(myVaillantDataClass):
    system_id: str
    index: int
...
@dataclass(config=config)
class Zone(myVaillantDataClass):
    system_id: str
    general: ZoneGeneral
    timezone: datetime.tzinfo
    control_identifier: ControlIdentifier
...
@dataclass(config=config)
class DomesticHotWater(myVaillantDataClass):
    system_id: str
    index: int
    control_identifier: ControlIdentifier
...
@dataclass(config=config)
class Ventilation(myVaillantDataClass):
    system_id: str
    index: int
    control_identifier: ControlIdentifier
...
@dataclass(config=config)
class DeviceDataBucket(myVaillantDataClass):
...
@dataclass(config=config)
class DeviceData(myVaillantDataClass):
    operation_mode: str
...
@dataclass(config=config)
class Device(myVaillantDataClass):
    system_id: str
    timezone: datetime.tzinfo
    device_uuid: str
    ebus_id: str
...
@dataclass(config=config)
class AmbisenseDevice(myVaillantDataClass):
    device_type: str
    name: str
    sgtin: str
...
@dataclass(config=config)
class AmbisenseRoomConfiguration(myVaillantDataClass):
    name: str
    timezone: datetime.tzinfo
...
@dataclass(config=config)
class AmbisenseRoom(myVaillantDataClass):
    system_id: str
    room_index: int
...
@dataclass(config=config)
class System(myVaillantDataClass):
    id: str
    state: dict
    configuration: dict
    home: Home
...
@dataclass(config=config)
class SystemReport(myVaillantDataClass):
    file_name: str
    file_content: str
