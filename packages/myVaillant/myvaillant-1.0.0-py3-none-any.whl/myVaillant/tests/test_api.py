...
async def test_login_vaillant(myvaillant_aioresponses) -> None:
    with myvaillant_aioresponses() as _:
        async with MyVaillantAPI(
...
async def test_login_bulex(myvaillant_aioresponses) -> None:
    with myvaillant_aioresponses() as _:
        async with MyVaillantAPI(
...
async def test_login_invalid_country(myvaillant_aioresponses) -> None:
    with pytest.raises(RealmInvalid):
        MyVaillantAPI("test@example.com", "test", "sdbg", "germany")


async def test_login_country_missing(myvaillant_aioresponses) -> None:
    with pytest.raises(RealmInvalid):
        MyVaillantAPI("test@example.com", "test", "sdbg")


async def test_refresh_token(myvaillant_aioresponses, mocked_api) -> None:
    with myvaillant_aioresponses() as _:
...
async def test_systems(
    myvaillant_aioresponses, mocked_api: MyVaillantAPI, test_data
) -> None:
...
async def test_no_system(myvaillant_aioresponses, mocked_api: MyVaillantAPI) -> None:
...
async def test_vrc700_operating_mode(
    myvaillant_aioresponses, mocked_api: MyVaillantAPI
) -> None:
...
async def test_home_without_system(
    myvaillant_aioresponses, mocked_api: MyVaillantAPI, caplog
) -> None:
    test_data = {
        "homes": [
            {
