...
Program server inti MCP Feedback Enhanced, menyediakan fungsionalitas umpan balik interaktif pengguna.
Mendukung deteksi lingkungan cerdas, otomatis memilih antarmuka Qt GUI atau Web UI.

Fungsi Utama:
- Deteksi lingkungan (lokal/remote)
- Pemilihan antarmuka (GUI/Web UI)
- Pemrosesan gambar dan integrasi MCP
- Standardisasi hasil umpan balik

Penulis: Fábio Ferreira (penulis asli)
Peningkatan: Minidoracat (Web UI, dukungan gambar, deteksi lingkungan)
...
def can_use_gui() -> bool:
    """
    Deteksi apakah dapat menggunakan antarmuka grafis

    Lingkungan WSL default menggunakan Web UI, karena sebagian besar instalasi WSL
    adalah lingkungan Linux tanpa dukungan aplikasi desktop, meskipun PySide6 dapat
    dimuat tetap harus menggunakan antarmuka Web.

    Returns:
        bool: True menunjukkan dapat menggunakan GUI, False menunjukkan hanya dapat menggunakan Web UI
    """
    if is_remote_environment():
        return False

    # Lingkungan WSL default menggunakan Web UI
    if is_wsl_environment():
        debug_log("Lingkungan WSL terdeteksi, default menggunakan Web UI")
        return False
    ...
    try:
        ...
        debug_log("Tidak dapat memuat PySide6, menggunakan Web UI")
        return False
    except Exception as e:
...
async def interactive_feedback(
    ...
    Tool ini akan otomatis mendeteksi lingkungan berjalan:
    - Lingkungan remote: menggunakan Web UI
    - Lingkungan lokal: menggunakan Qt GUI
    - Dapat dipaksa menggunakan Web UI melalui variabel lingkungan FORCE_WEB
    ...
    Kontrol antarmuka (berdasarkan prioritas):
    1. **Variabel lingkungan FORCE_WEB**: set "FORCE_WEB": "true" di mcp.json
    2. Deteksi otomatis: pilih otomatis berdasarkan lingkungan berjalan
    ...
    # Periksa variabel lingkungan FORCE_WEB
    force_web = False
    env_force_web = os.getenv("FORCE_WEB", "").lower()
    if env_force_web in ("true", "1", "yes", "on"):
        force_web = True
        debug_log("Variabel lingkungan FORCE_WEB diaktifkan, paksa menggunakan Web UI")
    elif env_force_web in ("false", "0", "no", "off"):
        force_web = False
        debug_log("Variabel lingkungan FORCE_WEB dinonaktifkan, menggunakan logika default")
    ...
    debug_log(f"Hasil deteksi lingkungan - Remote: {is_remote}, GUI tersedia: {can_gui}, Paksa Web UI: {force_web}")
    debug_log(f"Memutuskan menggunakan antarmuka: {'Web UI' if use_web_ui else 'Qt GUI'}")
...
async def launch_web_ui_with_timeout(project_dir: str, summary: str, timeout: int) -> dict:
    """
    Meluncurkan Web UI untuk mengumpulkan umpan balik, mendukung waktu timeout kustom

    Args:
        project_dir: Jalur direktori proyek
        summary: Ringkasan pekerjaan AI
        timeout: Waktu timeout (detik)

    Returns:
        dict: Data umpan balik yang dikumpulkan
    """
    debug_log(f"Meluncurkan antarmuka Web UI, waktu timeout: {timeout} detik")

    try:
        # Gunakan modul web baru
        from .web import launch_web_feedback_ui, stop_web_ui

        # Teruskan parameter timeout ke Web UI
        return await launch_web_feedback_ui(project_dir, summary, timeout)
    except ImportError as e:
        # Gunakan penanganan error terpadu
        error_id = ErrorHandler.log_error_with_context(
            e,
            context={"operation": "Import modul Web UI", "module": "web"},
            error_type=ErrorType.DEPENDENCY
        )
        user_error_msg = ErrorHandler.format_user_error(e, ErrorType.DEPENDENCY, include_technical=False)
        debug_log(f"Import modul Web UI gagal [Error ID: {error_id}]: {e}")
        ...
        debug_log(f"Web UI timeout: {e}")
        # Saat timeout pastikan menghentikan server Web
        try:
            from .web import stop_web_ui
            stop_web_ui()
            debug_log("Server Web UI telah dihentikan karena timeout")
        except Exception as stop_error:
            debug_log(f"Terjadi error saat menghentikan server Web UI: {stop_error}")

        return {
            "command_logs": "",
            "interactive_feedback": f"Pengumpulan umpan balik timeout ({timeout} detik), antarmuka telah ditutup otomatis.",
            "images": []
        }
    except Exception as e:
        # Gunakan penanganan error terpadu
        error_id = ErrorHandler.log_error_with_context(
            e,
            context={"operation": "Peluncuran Web UI", "timeout": timeout},
            error_type=ErrorType.SYSTEM
        )
        user_error_msg = ErrorHandler.format_user_error(e, include_technical=False)
        debug_log(f"❌ Error Web UI [Error ID: {error_id}]: {e}")

        # Saat terjadi error juga harus menghentikan server Web
        try:
            from .web import stop_web_ui
            stop_web_ui()
            debug_log("Server Web UI telah dihentikan karena error")
        except Exception as stop_error:
            ErrorHandler.log_error_with_context(
                stop_error,
                context={"operation": "Penghentian server Web UI"},
                error_type=ErrorType.SYSTEM
            )
            debug_log(f"Terjadi error saat menghentikan server Web UI: {stop_error}")
...
def get_system_info() -> str:
    ...
    system_info = {
        ...
        "Antarmuka yang Disarankan": "Web UI" if is_remote or not can_gui else "Qt GUI",
        "Variabel Lingkungan": {
...
def main():
    ...
    if debug_enabled:
        ...
        debug_log(f"   Antarmuka yang disarankan: {'Web UI' if is_remote_environment() or not can_use_gui() else 'Qt GUI'}")
