...
class ConfigManager:
    ...
    def _load_config(self) -> None:
        ...
        try:
            if self._config_file.exists():
                ...
                debug_log("File konfigurasi berhasil dimuat")
            else:
                self._config_cache = {}
    ...
    def update_partial_config(self, updates: Dict[str, Any]) -> None:
        ...
        try:
            ...
            debug_log(f"Konfigurasi parsial telah diperbarui: {list(updates.keys())}")
    ...
    def get_language(self) -> str:
        """Dapatkan pengaturan bahasa"""
        return self.get('language', 'id')  # Default ke Bahasa Indonesia

    def set_language(self, language: str) -> None:
        """設置語言"""
        self.update_partial_config({'language': language})
        debug_log(f"Pengaturan bahasa: {language}")

    def get_splitter_sizes(self, splitter_name: str) -> list:
        """獲取分割器尺寸"""
        sizes = self.get(f'splitter_sizes.{splitter_name}', [])
        if sizes:
            debug_log(f"Memuat ukuran splitter {splitter_name}: {sizes}")
        return sizes
    ...
    def set_splitter_sizes(self, splitter_name: str, sizes: list) -> None:
        ...
        debug_log(f"Menyimpan ukuran splitter {splitter_name}: {sizes}")

    def get_window_geometry(self) -> dict:
        """獲取窗口幾何信息"""
        geometry = self.get('window_geometry', {})
        if geometry:
            debug_log(f"Memuat informasi geometri jendela: {geometry}")
        return geometry
    ...
    def set_window_geometry(self, geometry: dict) -> None:
        ...
        debug_log(f"Menyimpan informasi geometri jendela: {geometry}")
    ...
    def get_timeout_duration(self) -> int:
        """獲取超時時間（秒）"""
        return self.get('timeout_duration', 1800)  # 預設30分鐘 (lebih lama)
