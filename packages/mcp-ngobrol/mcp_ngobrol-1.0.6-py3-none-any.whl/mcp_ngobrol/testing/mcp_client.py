...
class MCPTestClient:
    ...
    async def start_server(self) -> bool:
        """啟動 MCP 服務器"""
        try:
            debug_log("🚀 Memulai server MCP...")
            self.performance_monitor.start()
            
            # 構建啟動命令
            cmd = [
                "python", "-m", "src.mcp_ngobrol", "server"
            ]
            ...
            debug_log(f"✅ Proses server MCP telah dimulai (PID: {self.process.pid})")
    ...
    async def stop_server(self):
        ...
        if self.process:
            try:
                debug_log("🛑 Menghentikan server MCP...")
                ...
                debug_log("✅ Server MCP telah dihentikan")
    ...
    async def send_message(self, message: MCPMessage) -> Optional[MCPMessage]:
        ...
        try:
            ...
            debug_log(f"📤 Mengirim pesan MCP: {message.method or 'response'}")
            if self.config.test_debug:
                debug_log(f"   Konten: {message_data.strip()}")
        ...
        try:
            while self.process and self.process.poll() is None:
                ...
                try:
                    ...
                    debug_log(f"📨 Menerima pesan MCP: {message.method or 'response'}")
                    if self.config.test_debug:
                        debug_log(f"   Konten: {line.strip()}")
    ...
    async def initialize(self) -> bool:
        """初始化 MCP 連接"""
        try:
            debug_log("🔄 Menginisialisasi koneksi MCP...")
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        """獲取可用工具列表"""
        try:
            debug_log("🔍 Mendapatkan daftar tools...")
    ...
    async def cleanup(self):
        ...
        self.performance_monitor.stop()
        debug_log("🧹 Resource klien MCP telah dibersihkan")
