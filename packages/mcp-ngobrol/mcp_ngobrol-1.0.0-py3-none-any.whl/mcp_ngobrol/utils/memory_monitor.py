...
class MemoryMonitor:
    ...
    def __init__(self, 
        ...
        debug_log("MemoryMonitor inisialisasi selesai")
    ...
    def start_monitoring(self) -> bool:
        ...
        try:
            ...
            debug_log(f"Monitoring memori telah dimulai, interval {self.monitoring_interval} detik")
            return True
    ...
    def _monitoring_loop(self):
        """內存監控主循環"""
        debug_log("Loop monitoring memori dimulai")
    ...
    def _check_memory_usage(self, snapshot: MemorySnapshot):
        ...
        # 檢查警告閾值
            ...
            alert = MemoryAlert(
                level="warning",
                message=f"Penggunaan memori tinggi: {snapshot.system_percent:.1f}%",
                timestamp=snapshot.timestamp,
                memory_percent=snapshot.system_percent,
    ...
    def _handle_alert(self, alert: MemoryAlert):
        ...
        debug_log(f"Peringatan memori [{alert.level}]: {alert.message}")
    
    def _trigger_cleanup(self):
        """觸發清理操作"""
        self.cleanup_triggers_count += 1
        debug_log("Memicu operasi pembersihan memori")
        
        # 執行 Python 垃圾回收
        collected = gc.collect()
        debug_log(f"Garbage collection membersihkan {collected} objek")
    ...
    def add_cleanup_callback(self, callback: Callable):
        ...
        if callback not in self.cleanup_callbacks:
            ...
            debug_log("Menambahkan callback pembersihan")
