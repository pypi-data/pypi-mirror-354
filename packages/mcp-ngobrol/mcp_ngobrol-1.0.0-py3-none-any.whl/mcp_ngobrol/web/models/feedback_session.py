...
# -*- coding: utf-8 -*-
"""
Model Sesi Umpan Balik Web
===========================

Mengelola data dan logika sesi umpan balik Web.
"""
...
class SessionStatus(Enum):
    ...
    ACTIVE = "active"            # Aktif
    FEEDBACK_SUBMITTED = "feedback_submitted"  # Umpan balik telah dikirim
...
class CleanupReason(Enum):
    """Enumerasi alasan pembersihan"""
    TIMEOUT = "timeout"          # Pembersihan timeout
    EXPIRED = "expired"          # Pembersihan kedaluwarsa
    MEMORY_PRESSURE = "memory_pressure"  # Pembersihan tekanan memori
    MANUAL = "manual"            # Pembersihan manual
    ERROR = "error"              # Pembersihan error
    SHUTDOWN = "shutdown"        # Pembersihan shutdown sistem

# Definisi konstanta
MAX_IMAGE_SIZE = 1 * 1024 * 1024  # Batas ukuran gambar 1MB
...
class WebFeedbackSession:
    """Manajemen sesi umpan balik Web"""
    ...
    def __init__(self, session_id: str, project_directory: str, summary: str,
        ...
        self.settings: dict = {}  # Pengaturan gambar
        self.feedback_completed = threading.Event()
        self.process: Optional[subprocess.Popen] = None
        self.command_logs = []
        self._cleanup_done = False  # Mencegah pembersihan berulang

        # Baru: manajemen status sesi
        self.status = SessionStatus.WAITING
        self.status_message = "Menunggu umpan balik pengguna"
        # Gunakan time.time() secara konsisten untuk menghindari inkonsistensi basis waktu
        self.created_at = time.time()
        self.last_activity = self.created_at

        # Baru: konfigurasi auto cleanup (lebih lama)
        self.auto_cleanup_delay = auto_cleanup_delay or 3600  # Default 1 jam (lebih lama)
        self.max_idle_time = max_idle_time or 7200  # Default 2 jam idle (lebih lama)
        self.cleanup_timer: Optional[threading.Timer] = None
        self.cleanup_callbacks: List[Callable] = []  # Daftar fungsi callback pembersihan
