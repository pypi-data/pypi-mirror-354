...
使用方法：
    python -m mcp_ngobrol.test_mcp_enhanced
    python -m mcp_ngobrol.test_mcp_enhanced --scenario basic_workflow
    python -m mcp_ngobrol.test_mcp_enhanced --tags quick
"""
...
示例用法:
  ...
  %(prog)s --debug --verbose                  # 調試模式
    ...
    try:
        ...
        if success:
            debug_log("🎉 Semua tes berhasil!")
            sys.exit(0)
        else:
            debug_log("❌ Sebagian tes gagal")
            sys.exit(1)
    
    except KeyboardInterrupt:
        debug_log("\n⚠️ Tes dibatalkan oleh pengguna")
        sys.exit(130)
    except Exception as e:
        debug_log(f"❌ Eksekusi tes gagal: {e}")
