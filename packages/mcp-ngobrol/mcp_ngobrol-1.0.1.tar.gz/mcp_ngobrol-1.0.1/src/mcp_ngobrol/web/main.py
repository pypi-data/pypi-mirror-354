...
# -*- coding: utf-8 -*-
"""
Manajer Utama Web UI
====================

Kelas manajer utama antarmuka pengguna Web berbasis FastAPI, direfaktor mengacu pada pola desain GUI.
Dirancang khusus untuk lingkungan pengembangan SSH remote, mendukung antarmuka modern dan multi-bahasa.
"""
...
from .routes.auto_completion_routes import router as auto_completion_router
...
class WebUIManager:
    """Manajer Web UI - Direfaktor ke mode sesi aktif tunggal"""

    def __init__(self, host: str = "127.0.0.1", port: int = None):
        self.host = host

        # Tentukan port preferensi: variabel lingkungan > parameter > default 8765
        preferred_port = 8765

        # Periksa variabel lingkungan MCP_WEB_PORT
        ...
        if env_port:
            try:
                ...
                if 1024 <= custom_port <= 65535:
                    ...
                    debug_log(f"Menggunakan port yang ditentukan variabel lingkungan: {preferred_port}")
                else:
                    debug_log(f"Nilai MCP_WEB_PORT tidak valid ({custom_port}), harus dalam rentang 1024-65535, menggunakan port default 8765")
            except ValueError:
                debug_log(f"Format MCP_WEB_PORT salah ({env_port}), harus berupa angka, menggunakan port default 8765")
        else:
            debug_log(f"Variabel lingkungan MCP_WEB_PORT tidak diset, menggunakan port default {preferred_port}")

        # Gunakan manajemen port enhanced, mendukung auto cleanup
        ...
        # Setup middleware kompresi dan cache
        self._setup_compression_middleware()

        # Setup monitoring memori
        self._setup_memory_monitoring()

        # Refaktor: gunakan sesi aktif tunggal bukan dictionary sesi
        self.current_session: Optional[WebFeedbackSession] = None
        self.sessions: Dict[str, WebFeedbackSession] = {}  # Dipertahankan untuk kompatibilitas mundur

        # Manajemen status tab global - dipertahankan lintas sesi
        self.global_active_tabs: Dict[str, dict] = {}

        # Penanda notifikasi update sesi
        self._pending_session_update = False

        # Statistik pembersihan sesi
        self.cleanup_stats = {
            "total_cleanups": 0,
            "expired_cleanups": 0,
        ...
        # Setup file statis dan template
        ...
        # Add auto-completion routes
        self.app.include_router(auto_completion_router)

        debug_log(f"Inisialisasi WebUIManager selesai, akan dimulai di {self.host}:{self.port}")

    def _setup_compression_middleware(self):
        """Setup middleware kompresi dan cache"""
        # Dapatkan manajer kompresi
        compression_manager = get_compression_manager()
        config = compression_manager.config

        # Tambahkan middleware kompresi Gzip
        self.app.add_middleware(
            GZipMiddleware,
            minimum_size=config.minimum_size
        )

        # Tambahkan middleware statistik cache dan kompresi
        @self.app.middleware("http")
        async def compression_and_cache_middleware(request: Request, call_next):
            """Middleware kompresi dan cache"""
            response = await call_next(request)

            # Tambahkan header cache
            if not config.should_exclude_path(request.url.path):
                cache_headers = config.get_cache_headers(request.url.path)
                for key, value in cache_headers.items():
                    response.headers[key] = value

            # Update statistik kompresi (jika memungkinkan)
            try:
                ...
                if content_length > 0:
                    # Estimasi ukuran asli (jika sudah dikompresi, asumsikan rasio kompresi 30%)
                ...
                # Abaikan error statistik, tidak mempengaruhi respons normal
                pass

            return response

        debug_log("Setup middleware kompresi dan cache selesai")
    ...
    def create_session(self, project_directory: str, summary: str) -> str:
        """Buat sesi umpan balik baru - Direfaktor ke mode sesi aktif tunggal, mempertahankan status tab"""
        # Simpan koneksi WebSocket sesi lama untuk mengirim notifikasi update
        old_websocket = None
        if self.current_session and self.current_session.websocket:
            old_websocket = self.current_session.websocket
            debug_log("Menyimpan koneksi WebSocket sesi lama untuk mengirim notifikasi update")

        # Jika sudah ada sesi aktif, simpan dulu status tab ke status global
        if self.current_session:
            debug_log("Menyimpan status tab sesi yang ada dan membersihkan sesi")
            # Simpan status tab ke global
            if hasattr(self.current_session, 'active_tabs'):
                self._merge_tabs_to_global(self.current_session.active_tabs)

            # Pembersihan sinkron sumber daya sesi (tapi pertahankan koneksi WebSocket)
            self.current_session._cleanup_sync()

        session_id = str(uuid.uuid4())
        session = WebFeedbackSession(session_id, project_directory, summary)

        # Wariskan status tab global ke sesi baru
        session.active_tabs = self.global_active_tabs.copy()

        # Set sebagai sesi aktif saat ini
        self.current_session = session
        # Sekaligus simpan ke dictionary untuk menjaga kompatibilitas mundur
        self.sessions[session_id] = session

        debug_log(f"Membuat sesi aktif baru: {session_id}")
        debug_log(f"Mewarisi {len(session.active_tabs)} tab aktif")
