...
def main():
    ...
    # Perintah setup MCP
    setup_parser = subparsers.add_parser('setup', help='Setup konfigurasi MCP otomatis')
    setup_parser.add_argument('--force', action='store_true', help='Force overwrite existing config')
    setup_parser.add_argument('--instructions', action='store_true', help='Show setup instructions only')
...
def run_setup(args):
    """Menjalankan setup MCP otomatis"""
    ...
    print("🔧 Setting up MCP Ngobrol configuration...")
    success = setup.setup_mcp_config(force=args.force)

    if success:
        print("✅ MCP configuration setup completed!")
        print("\n📋 Next steps:")
        print("1. Restart your AI assistant")
        print("2. Test with: interactive_feedback tool")
        print("3. Use 'uvx mcp-ngobrol@latest setup --instructions' for manual setup")
    else:
