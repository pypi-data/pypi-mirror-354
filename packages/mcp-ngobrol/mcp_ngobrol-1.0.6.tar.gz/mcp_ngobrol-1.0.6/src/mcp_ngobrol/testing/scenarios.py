...
class QuickConnectionScenario(TestScenario):
    """Skenario tes koneksi cepat"""

    def __init__(self):
        super().__init__(
            name="quick_connection",
            description="Tes startup dan koneksi cepat server MCP",
            timeout=30,
...
class MockTestScenario(TestScenario):
    """Skenario tes simulasi (untuk demonstrasi)"""

    def __init__(self):
        super().__init__(
            name="mock_test",
            description="Skenario tes simulasi untuk demonstrasi fungsi framework testing",
            timeout=10,
            tags=["mock", "demo", "quick"]
        )

    async def run(self, client: MCPTestClient) -> Dict[str, Any]:
        """Menjalankan tes simulasi"""
        result = {
            "success": True,
            "steps": {
                "mock_step_1": True,
        ...
        debug_log("✅ Tes simulasi selesai")
        return result


class TestScenarios:
    """Manager skenario testing"""
    ...
    def _register_default_scenarios(self):
        """Mendaftarkan skenario testing default"""
        scenarios = [
            MockTestScenario(),  # Menambahkan skenario tes simulasi
    ...
    def register_scenario(self, scenario: TestScenario):
        """Mendaftarkan skenario testing kustom"""
        self.scenarios[scenario.name] = scenario

    def get_scenario(self, name: str) -> Optional[TestScenario]:
        """Mendapatkan skenario testing"""
    ...
    async def run_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        debug_log(f"🧪 Menjalankan skenario tes: {scenario.name}")
        debug_log(f"   Deskripsi: {scenario.description}")
    ...
    async def run_all_scenarios(self, tags: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        debug_log(f"🚀 Mulai menjalankan {len(scenarios)} skenario tes...")
        ...
        for scenario in scenarios:
            ...
            if result.get("success", False):
                passed_count += 1
                debug_log(f"✅ {scenario.name}: Berhasil")
            else:
                failed_count += 1
                debug_log(f"❌ {scenario.name}: Gagal")
        
        overall_success = failed_count == 0
        
        debug_log(f"📊 Tes selesai: {passed_count}/{len(scenarios)} berhasil")
