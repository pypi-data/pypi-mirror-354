...
class TestReporter:
    ...
    def generate_report(self, test_results: Dict[str, Any]) -> TestReport:
        ...
        debug_log(f"📊 Laporan tes berhasil dibuat (waktu: {time.time() - start_time:.2f}s)")
        return report
    ...
    def save_report(self, report: TestReport, output_path: Optional[Path] = None) -> Path:
        ...
        debug_log(f"📄 Laporan tes telah disimpan: {output_path}")
        return output_path
...
"""
    ...
    def print_summary(self, report: TestReport):
        ...
        print("\n" + "="*60)
        print(f"🧪 Ringkasan Laporan Tes MCP {status_icon}")
        print("="*60)
        print(f"📊 Total Tes: {report.total_scenarios}")
        print(f"✅ Tes Berhasil: {report.passed_scenarios}")
        print(f"❌ Tes Gagal: {report.failed_scenarios}")
        print(f"📈 Tingkat Keberhasilan: {report.success_rate:.1%}")
        print(f"⏱️  Total Waktu: {report.performance_summary.get('total_duration_formatted', 'N/A')}")
        print(f"⚡ Rata-rata Waktu: {report.performance_summary.get('avg_duration_formatted', 'N/A')}")
        
        if report.errors:
            print(f"\n❌ Ditemukan {len(report.errors)} kesalahan:")
            for error in report.errors[:5]:  # Hanya tampilkan 5 kesalahan pertama
                print(f"   • {error}")
            if len(report.errors) > 5:
                print(f"   ... masih ada {len(report.errors) - 5} kesalahan lagi")
