...
File ini memungkinkan paket dijalankan melalui `python -m mcp_feedback_enhanced`.

Cara Penggunaan:
  python -m mcp_feedback_enhanced        # Memulai server MCP
  python -m mcp_feedback_enhanced test   # Menjalankan testing
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MCP Feedback Enhanced Enhanced - Server MCP Koleksi Umpan Balik Interaktif"
    )
    ...
    test_parser.add_argument('--web', action='store_true', help='Test Web UI (otomatis berjalan terus)')
...
def run_tests(args):
    ...
    if args.enhanced or args.scenario or args.tags or args.list_scenarios:
        ...
        print("🧪 Menjalankan testing Web UI...")
        from .test_web_ui import test_web_ui, interactive_demo
        success, session_info = test_web_ui()
        if not success:
            sys.exit(1)
        # Testing Web UI otomatis mengaktifkan mode berkelanjutan
        if session_info:
            print("📝 Testing Web UI selesai, masuk mode berkelanjutan...")
            print("💡 Tips: Server akan terus berjalan, dapat menguji fungsi interaktif di browser")
