# ######################################################################################################################
# DH Specific: import locations adjusted + css file location explicitly stated because in a "python -c ..." call
# as command in Circle CI, we've had a failure due to the fact the "Path(__file__).with_suffix(".css")" was not
# returned as:
# '/home/circleci/project/venv/lib/python3.8/site-packages/dataheroes/core/sklearn_extra/sklearn_1_5_1_mask/utils/_estimator_html_repr.css'
# ...but rather as:
# '/home/circleci/project/venv/lib/python3.8/site-packages/dataheroes/core/sklearn_extra/sklearn_1_5_1_mask/utils/_estimator_html_repr.cpython-38-x86_64-linux-gnu.css'
# However, the changed line below is not enough, because we also need to package the .css file into our dist wheel.
# ######################################################################################################################
...
def _get_css_style():
    # return Path(__file__).with_suffix(".css").read_text(encoding="utf-8")  # DH Specific: original line
    return Path(__file__).with_name("_estimator_html_repr.css").read_text(encoding="utf-8")  # DH Specific: replacement
