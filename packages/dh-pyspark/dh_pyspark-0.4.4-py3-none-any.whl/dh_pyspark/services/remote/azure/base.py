...
class RemoteAzureCoresetTreeService(RemoteCoresetTreeService):
    ...
    def upload_main_script(self, job_scripts):
        ...
        """
        # protobuf==3.20.3
        pip_packages = f"  --force-reinstall --no-cache-dir dataheroes dh_pyspark fsspec adlfs {self.get_config_value('python_libraries')}"
        text = f"""
import subprocess
import sys
import os
...
print(f"{pip_packages} installed")
...
broadcast_wheels = spark.sparkContext.broadcast(wheel_files)
def install_all_wheels(_):
    ...
    wheel_files =  broadcast_wheels.value
    def safe(cmd):
        try:
...
"""
    ...
    def upload_main_script(self, job_scripts):
        ...
        Uploads it to cloud storage and returns its path.
        """
        pip_packages = f"protobuf==3.20.3 dataheroes dh_pyspark fsspec adlfs xgboost zipp {self.get_config_value('python_libraries')}"
...
print("🚀 Starting setup on driver")
spark = SparkSession.builder.getOrCreate()
...
storage_key = "{self.get_config_value('storage_account_key')}"
workflow_id = "{self.workflow_id}"
local_wheel_dir = "/tmp/wheelhouse"
remote_wheel_dir = f"run-folder/{{workflow_id}}/wheelhouse-x86-pypi"

# Download wheels from PyPI on driver
os.makedirs(local_wheel_dir, exist_ok=True)
subprocess.run([sys.executable, "-m", "pip", "download",
            "--dest", local_wheel_dir] + "{pip_packages}".split(), check=True)

# Install locally on driver
subprocess.run([sys.executable, "-m", "pip", "install", "--no-index", f"--find-links={{local_wheel_dir}}"] + "{pip_packages}".split(), check=True)
print("✅ Installed packages on driver")

# Upload wheels to Azure Blob Storage
blob_service_client = BlobServiceClient(account_url=f"https://{{account_name}}.blob.core.windows.net", credential=storage_key)
container_client = blob_service_client.get_container_client(container)

wheel_files = []
for filename in os.listdir(local_wheel_dir):
    local_path = os.path.join(local_wheel_dir, filename)
    remote_path = f"{{remote_wheel_dir}}/{{filename}}"
with open(local_path, "rb") as f:
    container_client.upload_blob(name=remote_path, data=f, overwrite=True)
wheel_files.append(remote_path)
print(f"☁️ Uploaded {{remote_path}}")

# Broadcast wheel files to workers
num_executors = spark.sparkContext.defaultParallelism
rdd = spark.sparkContext.parallelize(range(num_executors), num_executors)
broadcast_wheels = spark.sparkContext.broadcast(wheel_files)
...
def install_all_wheels(_):
    ...
    for path in wheel_files:
        ...
        safe([sys.executable, "-m", "pip", "install", local_path])
    return "✅ Worker install complete"
...
spark.sparkContext.parallelize(range(num_executors), num_executors).barrier().mapPartitions(lambda _: [1]).count()
print("✅ All workers confirmed package installation")

# Run workflow scripts
scripts = {str(job_scripts)}
for step in scripts:
    remote_path = step['path']
    local_path = step['name'] + '.py'
    print(f"📥 Downloading {{remote_path}}")
    blob_client = container_client.get_blob_client(blob=remote_path)
    with open(local_path, "wb") as f:
        f.write(blob_client.download_blob().readall())
