...
class overlay_video(StreamingOutput, Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: path or URL for the default value that overlay_video component is going to take. Can also be a tuple consisting of (video filepath, subtitle filepath). If a subtitle file is provided, it should be of type .srt or .vtt. Or can be callable, in which case the function will be called whenever the app loads to set the initial value of the component.
    ...
    def preprocess(self, payload: VideoData | None) -> str | None:
        ...
        if self.min_length is not None or self.max_length is not None:
            ...
            if self.min_length is not None and duration < self.min_length:
                raise gr.Error(
                    f"overlay_video is too short, and must be at least {self.min_length} seconds"
                )
            if self.max_length is not None and duration > self.max_length:
                raise gr.Error(
                    f"overlay_video is too long, and must be at most {self.max_length} seconds"
                )
        ...
        if needs_formatting or flip:
            ...
            if wasm_utils.IS_WASM:
                raise wasm_utils.WasmUnsupportedError(
                    "overlay_video formatting is not supported in the Wasm mode."
                )
    ...
    def postprocess(
        ...
        if not processed_files[0]:
            raise ValueError("overlay_video data missing")
    ...
    def _format_video(self, video: str | Path | None) -> FileData | None:
        ...
        if (
            ...
            warnings.warn(
                "overlay_video does not have browser-compatible container or codec. Converting to mp4."
            )
    ...
    async def stream_output(
        ...
        if not value.endswith(".ts"):
            if not value.endswith(".mp4"):
                raise RuntimeError(
                    "overlay_video must be in .mp4 or .ts format to be streamed as chunks",
                )
...
class OverlayVideo(Component):
    ...
    def postprocess(self, value: Tuple[str | None, str | None] | None) -> OverlayVideoData | None:
        ...
        video_path, json_data = value
        
        try:
            # Handle both dict and file path cases
            if isinstance(json_data, dict):
                full_data = json_data
            else:
                # json_data is a file path
