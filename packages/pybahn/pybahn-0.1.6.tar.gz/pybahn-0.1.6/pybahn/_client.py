...
class PyBahn(object):
    ...
    def arrivals(self, id: Union[str | int, Station], filters: typing.List[Filter] = [Filter.RB_RE], duration: int = 180):
        if isinstance(id, str) or isinstance(id, int):
            ...
            url_base = f"https://www.bahnhof.de/api/boards/arrivals?evaNumbers={id.id}"
        else:
            raise ValueError("id must me only String, Number or Station type value")
