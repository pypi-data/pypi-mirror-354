...
def _fetch_sequence_params(
    cursor, table_name: str, schema_name: str = "public"
    ...
    SELECT
        ...
        s.start_value,
        s.cycle_option,
        COALESCE(seq.last_value, 1) AS current_value
    FROM
        information_schema.sequences s
    JOIN
        pg_sequences seq
        ON s.sequence_name = seq.sequencename
        AND s.sequence_schema = seq.schemaname
    LEFT JOIN
        information_schema.columns c
        ON c.column_default LIKE CONCAT('nextval(', '''', s.sequence_schema, '.', s.sequence_name, '''', '::regclass)')
        AND c.table_name = %s
        AND c.table_schema = %s
    WHERE
        s.sequence_schema = %s
    """
    cursor.execute(query, (table_name, schema_name, schema_name))
...
def _create_constraints(
    ...
    foreign_tables_mapping: Dict[Tuple[str, str], Tuple[str, str]] | None,
...
def _create_table_from_dict(
    ...
    foreign_tables_mapping: Dict[Tuple[str, str], Tuple[str, str]] | None = None,
    schema_name="public",
) -> None:
    """
...
def _compare_table_structure(
    connection,
    table_structure: Dict[str, str | List[Dict[str, str | bool | int | None]]],
    table_name: str,
    foreign_tables_mapping: Dict[Tuple[str, str], Tuple[str, str]] | None = None,
    schema_name: str = "public",
) -> bool:
