...
def gensym(length=32, prefix="gensym_"):
    ...
    Args:
        length: int
            The length of the random part of the symbol. Defaults to 32.
        prefix: str
            The prefix to prepend to the generated random string. Defaults to "gensym_".
...
def load_module(source, module_name=None):
    ...
    Args:
        source: str 
            The path to the Python file to be loaded as a module or to a .kt file.
        module_name: str
            The name under which to register the module in `sys.modules`. If None, a unique name is generated. Defaults to None.
    ...
    if source.is_file() and source.suffix == '.kt': # Loading the module from a ketos file.
        # The custom folder will always be static as "custom"
        zip_module_path = zipimport.zipimporter(source / "custom") # the folder name within the zip archive will always be custom
        spec = zip_module_path.find_spec(module_name)
        if spec is None:
            raise ImportError(f"Could not find module {module_name} in the .kt file.")
        ...
        if spec is None:
            raise ImportError(f"Could not find module {module_name} at {source}.")
        module = importlib.util.module_from_spec(spec)
        sys.modules[module_name] = module
        spec.loader.exec_module(module)
