...
def test_spatial_navigation_left(practice_pdf):
    ...
    assert left_region.bbox[2] <= non_leftmost.bbox[0], "Left region should end where element begins"
    assert left_region.bbox[1] <= non_leftmost.bbox[3], "Left region should overlap vertically with element"
    assert left_region.bbox[3] >= non_leftmost.bbox[1], "Left region should overlap vertically with element"
...
def test_spatial_navigation_right(practice_pdf):
    ...
    assert right_region.bbox[0] >= non_rightmost.bbox[2], "Right region should begin where element ends"
    assert right_region.bbox[1] <= non_rightmost.bbox[3], "Right region should overlap vertically with element"
    assert right_region.bbox[3] >= non_rightmost.bbox[1], "Right region should overlap vertically with element"
...
def test_spatial_navigation_above(practice_pdf):
    ...
    assert above_region.bbox[3] <= non_topmost.bbox[1], "Above region should end where element begins"
    assert above_region.bbox[0] <= non_topmost.bbox[2], "Above region should overlap horizontally with element"
    assert above_region.bbox[2] >= non_topmost.bbox[0], "Above region should overlap horizontally with element"
...
def test_spatial_navigation_below(practice_pdf):
    ...
    assert below_region.bbox[1] >= non_bottommost.bbox[3], "Below region should begin where element ends"
    assert below_region.bbox[0] <= non_bottommost.bbox[2], "Below region should overlap horizontally with element"
    assert below_region.bbox[2] >= non_bottommost.bbox[0], "Below region should overlap horizontally with element"
...
def test_region_contains_elements(practice_pdf):
    ...
    for elem in all_text:
        if (elem.bbox[0] >= region.bbox[0] and elem.bbox[2] <= region.bbox[2] and 
            elem.bbox[1] >= region.bbox[1] and elem.bbox[3] <= region.bbox[3]):
            expected_count += 1
    ...
    # Assertions
    assert len(text_in_region) == expected_count, "Region should find exactly the elements it contains"
...
def test_region_overlaps(practice_pdf):
    ...
    if hasattr(region1, 'overlaps'):
        ...
        def regions_overlap(r1, r2):
            return not (r1.bbox[2] <= r2.bbox[0] or  # r1 is left of r2
                        r1.bbox[0] >= r2.bbox[2] or  # r1 is right of r2
                        r1.bbox[3] <= r2.bbox[1] or  # r1 is above r2
                        r1.bbox[1] >= r2.bbox[3])    # r1 is below r2
