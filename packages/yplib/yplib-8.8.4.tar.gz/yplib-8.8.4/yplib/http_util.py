...
from yplib import *
from yplib import to_list, to_log_file
...
# 是否打印日志
def get_or_set_log_flag(r_flag: bool = None) -> None:
    key = 'set_log_flag'
    if r_flag is not None:
        get_thread_local_http_data()[key] = r_flag
    return get_thread_local_http_data()[key]
...
def do_http_log(url: str = None,
    ...
    if get_or_set_log_flag(None) is False:
        return
    # for log 部分
