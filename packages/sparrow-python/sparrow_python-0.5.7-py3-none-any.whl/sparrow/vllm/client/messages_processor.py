...
async def process_content_recursive(content, session, cache_config: Optional[ImageCacheConfig] = None, **kwargs):
        ...
        if progress_callback:
            try:
                ...
                progress_info = {
                    ...
                    'estimated_remaining_time': (elapsed_time / current * (total - current)) if current > 0 else 0,
                    ...
                    async for messages in messages_to_process:
                        ...
                        if len(pending_tasks) >= max_concurrent:
                            ...
                            for task in done:
                                ...
                                if pbar:
                                    pbar.update(1)
                                report_progress(processed_count, total_count or processed_count, start_time)
                                yield result
                        
                        # 创建新任务
                        task = asyncio.create_task(process_single_batch(messages, semaphore, task_index))
