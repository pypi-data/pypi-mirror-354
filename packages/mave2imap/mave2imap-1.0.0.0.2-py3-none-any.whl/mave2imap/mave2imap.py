...
def main():
    ...
    for n, f in enumerate(fastq_files):
        ...
        parser_cmd = f"""time mpirun -np {ncpus} python {exe_dir}/ReadParser -f {fastq} -o {
        ...
        if even_odd != 0:
            ...
            assembly_cmd = f"""time mpirun -np {ncpus} python {exe_dir}/ReadAssembler --mpi -f {
    ...
    combiner_cmd = f"""time mpirun -np {ncpus} python {exe_dir}/CombineMultiExp -b assembly_{
