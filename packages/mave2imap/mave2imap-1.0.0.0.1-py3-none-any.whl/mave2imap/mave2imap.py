...
def main():
    ...
    for n, f in enumerate(fastq_files):
        ...
        parser_cmd = f"""time mpirun -np {ncpus} {exe_dir}/ReadParser -f {fastq} -o {direction}_{
            ba} -e {direction_l[n % 2].lower()} -r {region_index} -c {ini_file} --mpi"""
        ...
        if even_odd != 0:
            ...
            assembly_cmd = f"""time mpirun -np {ncpus} {exe_dir}/ReadAssembler --mpi -f {
    ...
    combiner_cmd = f"""time mpirun -np {ncpus} {exe_dir
        }/CombineMultiExp -b assembly_{ba_l[0].lower()} -a assembly_{ba_l[1].lower()} -o result_thresh{
        thresh_filter.replace(',', '_')} -c {ini_file} -t {thresh_filter}"""
    run_command(combiner_cmd)
    combiner_cmd = f"""time mpirun -np {ncpus} {exe_dir}/CombineMultiExp -b assembly_{
