...
class Database:
    ...
    class __Table:
        ...
        def getTable(self, db_object:object):
            ...
            """
            try:
                self._cursor.execute(f"Select * from Equipment")
                result=self._cursor.fetchall() if len(self._cursor.fetchall())!=0 else None
...
WHERE tc.constraint_type='FOREIGN KEY' AND tc.table_name = '{self.table_name}';;""")
                        ...
                        if(i==0):
                            ...
                            if (len(foreign_key)!=0):
                                if(not db_object.connect_type):
                                    ...
                                    if (columns[i].strip() in foreign_key[0]):
                                        self.__table.controls[-1].controls[-1].rows[-1].cells[-1].content.data+=f" references {foreign_key[0][1]} {foreign_key[0][2]}"
                return self.__table
            except Exception as ex: 
                raise Exception (f'{ex}')
        ...
        def select_request(self, columns:str, contidion:str):
            try:
                ...
                return self._cursor.fetchall() if (len(self._cursor.fetchall())!=0) else None
            except Exception as ex: 
                raise Exception(ex.args[0])
