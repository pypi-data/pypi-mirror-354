...
class SQLCompiler(compiler.SQLCompiler):
    ...
    def as_sql(self, with_limits=True, with_col_aliases=False, subquery=False):
        ...
        select = SelectCommand(self, self.connection, self.query)
        return (select, tuple())
...
class SQLDeleteCompiler(SQLCompiler, compiler.SQLDeleteCompiler):
    def as_sql(self, with_limits=True, with_col_aliases=False, subquery=False):
        return (DeleteCommand(self, self.connection, self.query), tuple())


class SQLUpdateCompiler(SQLCompiler, compiler.SQLUpdateCompiler):
    def as_sql(self, with_limits=True, with_col_aliases=False, subquery=False):
        self.pre_sql_setup()
        return (UpdateCommand(self, self.connection, self.query), tuple())
