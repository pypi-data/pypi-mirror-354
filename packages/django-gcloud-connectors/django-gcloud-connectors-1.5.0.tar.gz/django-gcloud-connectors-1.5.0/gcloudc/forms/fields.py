...
class TrueOrNoneFormField(forms.BooleanField):
...
class ListWidgetMixin:

    delimiter: str = NotImplemented
    default_help_text: str = NotImplemented

    def __init__(self, *args, **kwargs):
        self.strip_values = kwargs.pop("strip_values", True)
        self.remove_empty_values = kwargs.pop("remove_empty_values", True)
        super().__init__(*args, **kwargs)

    def render(self, name, value, attrs=None, renderer=None):
        if isinstance(value, (list, tuple, set)):
            self._check_for_delimeter(value)
    ...
    def _check_for_delimeter(self, value):
        """ Make sure none of the values in the list contain the delimiter value. """
        for item in value:
            if self.delimiter in item:
                raise ValueError(
                    f"{self.__class__.__name__} cannot render the following value because it "
                    f"contains the delimiter value ({self.delimiter}): {item}"
                )


class CommaSeparatedListWidget(ListWidgetMixin, forms.TextInput):
    """ A widget for being able to edit a ListField/SetField of string values using a text input
        with values separated by commas.
    """

    delimiter = ","
    default_help_text = "Enter values separated by commas."


class LineSeparatedListWidget(ListWidgetMixin, forms.Textarea):
    """ A widget for being able to edit a ListField/SetField of string values using a textarea with
        one value per line.
    """

    delimiter = "\n"
    default_help_text = "Enter one value per line."


class ListFormField(forms.Field):
    """ A form field for being able to display a ListField and SetField. """

    widget = LineSeparatedListWidget

    def __init__(self, *args, **kwargs):
        # This thwarts the ability to override the default help text because passing help_text=""
        # still gives the default. But the trade-off of a nice default seems worth it; they can
        # override the form field/widget if they really don't want it.
        widget = kwargs.get("widget", self.widget)
        default_help_text = getattr(widget, "default_help_text", "")
        kwargs["help_text"] = kwargs.get("help_text") or default_help_text
        super().__init__(*args, **kwargs)
