...
def _binddn_to_username(binddn: str) -> str | None:
    try:
        dn = ldap.dn.str2dn(binddn)[0][0]
        return f"{dn[1]}$" if dn[0] == "cn" else dn[1]
...
try:  # UCS system
    ...
    ucr = dict(_ucr)
    FQDN = "{hostname}.{domainname}".format(**ucr)
    USER_AGENT = "univention.cli/{version/version}-{version/patchlevel}-errata{version/erratalevel}".format(**ucr)
    DEFAULT_USERNAME = _binddn_to_username(ucr["ldap/hostdn"])
    with open("/etc/machine.secret") as _fp:
        DEFAULT_PASSWORD = _fp.read().strip()
except (ImportError, KeyError, OSError):  # docker container
    FQDN = os.getenv("FQDN", "localhost")
    USER_AGENT = f"univention.cli/{os.getenv('VERSION_VERSION', '')}-{os.getenv('VERSION_PATCH', '')}-errata{os.getenv('VERSION_ERRATA', '')}"
...
class CLIClient:
    def init(self, parser, args):
        ...
        self.udm = UDM(args.uri, username, password, bearer_token=args.bearertoken, user_agent=args.user_agent, language=args.language)
    ...
    def print_line(self, key, value="", prefix="", stream=sys.stdout):
        ...
        value = value.replace("\n", f"\n{prefix}")
        print(
            "".join(v for v in value if v not in "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x0b\x0c\r\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f\x7f"),
            file=stream,
        )
    ...
    def get_info(self, args, stream=sys.stdout):
        ...
        for _layout in layout:
            print("  {} - {}:".format(_layout["label"], _layout["description"]), file=stream)
            for sub in _layout["layout"]:
    ...
    def print_layout(self, sub, properties, indent=1, stream=None):
        def _print_prop(prop):
            ...
            def _print(prop):
                ...
                print("\t\t{}{}".format((prop + _get_flags(vals)).ljust(41), vals.get("label")), file=stream)
        ...
        if isinstance(sub, dict):
            print("\t{} {}".format(sub["label"], sub["description"]), file=stream)
    ...
    def handle_server_error(self, error):
        for e in ("title", "message", "traceback"):
            error.setdefault(e, "")
        self.print_error("{title}: {message}\n{traceback}".format(**error))
        self.handle_unprocessible_entity(error)


def parse_known_args(parser, arg_list, client, known_args=None, subparsers=None):
    ...
    try:
        ...
        args, opts = parser.parse_known_args(arg_list)
        args.options = opts
        return args
    except SystemExit:
        if subparsers:
...
Use "univention-directory-manager modules" for a list of available modules.""",
    ...
    parser.add_argument("--uri", default=f"https://{FQDN}/univention/udm/", help=argparse.SUPPRESS)
    parser.add_argument("--user-agent", default=USER_AGENT, help=argparse.SUPPRESS)

    parser.add_argument("object_type")
    known_args = parse_known_args(parser, arg_list, client)
    ...
    parser.formatter_class = argparse_module_help(client, parser, known_args, subparsers)
    parse_known_args(parser, arg_list, client, known_args, subparsers)
...
def add_object_action_arguments(parser, client):
    ...
    def add_subparser(name, *args, **kwargs):
        kwargs.setdefault("description", "{}: {}".format(name, kwargs.get("help", "")))
