...
class OrchestraTelemetryInterface:
    ...
    def get_latest_telemetry_by_element_id(
        ...
        try:
            response: element_telemetry_pb2.GetLatestElementTelemetryResponse = (
                self.element_telemetry_stub.GetLatest(request, timeout=TIMEOUT)
            )
        except grpc.RpcError as e:
    ...
    def get_many_latest_telemetry_by_element_ids(
        ...
        try:
            ...
            ] = self.element_telemetry_stub.GetManyLatest(request, timeout=TIMEOUT)
        except grpc.RpcError as e:
    ...
    def get_many_latest_telemetry_partitions_by_element_ids(
        ...
        try:
            ...
            ] = self.element_telemetry_stub.GetManyPartitionLatest(request, timeout=TIMEOUT)
        except grpc.RpcError as e:
            if e.code() == grpc.StatusCode.UNAVAILABLE:
                raise exceptions.UnavailableError(e.details())
            elif e.code() == grpc.StatusCode.INVALID_ARGUMENT:
        ...
        result: Optional[element_telemetry_pb2.GetManyLatestElementPartitionTelemetryResponse] = None
            ...
            response: element_telemetry_pb2.PushElementTelemetryResponse = (
                self.element_telemetry_stub.Push(
                    ElementTelemetry(
                        ...
                        data=data,
                    ).to_protobuf_model_push_request(partition=partition or self.DEFAULT_PARTITION),
                    timeout=TIMEOUT,
                )
            )
        except grpc.RpcError as e:
