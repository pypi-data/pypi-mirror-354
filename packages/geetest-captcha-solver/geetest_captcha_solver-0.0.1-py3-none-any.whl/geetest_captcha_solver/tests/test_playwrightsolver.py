...
def test_shapes_v2_is_detected(caplog):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        page = browser.new_page()
        page.goto("file:///home/gregb/ToughdataLLC/SadCaptcha/tiktok-captcha-solver/src/geetest_captcha_solver/tests/new_shapes.html")
        sadcaptcha = PlaywrightSolver(page, os.environ["API_KEY"])
...
def test_rotate_v2_is_detected(caplog):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        page = browser.new_page()
        page.goto("file:///home/gregb/ToughdataLLC/SadCaptcha/tiktok-captcha-solver/src/geetest_captcha_solver/tests/new_rotate.html")
        sadcaptcha = PlaywrightSolver(page, os.environ["API_KEY"])
...
def test_puzzle_v2_is_detected(caplog):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        page = browser.new_page()
        page.goto("file:///home/gregb/ToughdataLLC/SadCaptcha/tiktok-captcha-solver/src/geetest_captcha_solver/tests/new_puzzle.html")
        sadcaptcha = PlaywrightSolver(page, os.environ["API_KEY"])
