...
class OllamaClient:
    ...
    def install_ollama_windows(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                console.print("⚠️ Winget не сработал, пробую другой способ...", style="yellow")

        except (FileNotFoundError, subprocess.TimeoutExpired):
            console.print("⚠️ Winget недоступен", style="yellow")

        # Если winget не сработал, показываем инструкции
        console.print(Panel(
            "🔧 Для автоматической установки Ollama:\n\n"
            "1. Скачайте установщик: https://ollama.ai\n"
            "2. Запустите установщик\n"
            "3. Перезапустите offline-ai\n\n"
    ...
    def start_ollama(self) -> bool:
        ...
        try:
            ...
            # Пытаемся установить автоматически на Windows
            if platform.system() == "Windows":
                if Confirm.ask("🤖 Попробовать установить Ollama автоматически?"):
                    if self.install_ollama_windows():
