...
# 6. Строим графики для сравнения
plt.figure(figsize=(15, 10))

# График 1: Исходный и восстановленный сигналы
plt.subplot(2, 1, 1)
plt.plot(t, original_signal, label='Исходный сигнал (1 Гц + 3 Гц)')
plt.plot(t, reconstructed_signal, '--', label='Восстановленный сигнал (после фильтра > 5 Гц)', linewidth=2)
...
# График 2: Спектр до и после фильтрации
plt.subplot(2, 1, 2)
# Показываем только положительную половину спектра для наглядности
positive_freq_indices = np.where(freqs >= 0)
plt.plot(freqs[positive_freq_indices], np.abs(dft_coeffs[positive_freq_indices]), 'o-', label='Спектр исходного сигнала')
plt.plot(freqs[positive_freq_indices], np.abs(filtered_coeffs[positive_freq_indices]), 'x-', label='Спектр после фильтрации', alpha=0.7)
plt.title('Частотный спектр сигнала')
plt.xlabel('Частота (f), Гц')
plt.ylabel('Амплитуда |X(f)|')
plt.axvline(x=5, color='red', linestyle='--', label='Частота среза (5 Гц)')
plt.grid(True)
plt.legend()
plt.xlim(0, 10) # Ограничим ось X для лучшей видимости пиков
