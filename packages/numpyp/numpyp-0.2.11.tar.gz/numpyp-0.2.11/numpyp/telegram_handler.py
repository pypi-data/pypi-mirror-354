...
import os
import dbm
import json  # Будем использовать JSON для хранения списков в базе
...
class _TelegramHandler:
    ...
    Внутренний класс-обработчик.
    Использует встроенный модуль 'dbm' для хранения состояний в файле.
    Накапливает все ответы для каждой задачи.
    """
    ...
    # --- ЗДЕСЬ ОСНОВНЫЕ ИЗМЕНЕНИЯ ---
    async def check_for_reply(self, task_id: str) -> list[dict] | None:
        """
        Проверяет новые ответы, добавляет их к истории и возвращает ПОЛНУЮ историю ответов.
        """
        message_id_to_check_str = self.db.get(f"task:{task_id}")
        if not message_id_to_check_str:
        ...
        try:
            # 1. Получаем новые обновления от Telegram
            offset = self.last_update_id + 1 if self.last_update_id else None
            updates = await self.bot.get_updates(offset=offset, timeout=1)
            ...
            # 2. Загружаем старую историю ответов из нашей базы
            history_key = f"task:{task_id}:all_replies"
            history_json = self.db.get(history_key, b'[]').decode()  # По умолчанию - пустой список в формате JSON
            all_replies_history = json.loads(history_json)

            # Собираем ID уже сохраненных сообщений, чтобы не было дубликатов
            saved_message_ids = {reply['message_id'] for reply in all_replies_history}
            ...
            for update in reversed(updates):
                ...
                if (msg and msg.reply_to_message and
                    ...
                    # Проверяем, что этого ответа еще нет в нашей истории
                    if msg.message_id not in saved_message_ids:
                        ...
                        reply_data = {
                            ...
                            "text": msg.text,
                            "user_id": msg.from_user.id,
                            "message_id": msg.message_id  # Сохраняем ID самого ответа
                        }
                        all_replies_history.append(reply_data)  # Добавляем новый ответ к истории

            # 3. Если были найдены новые ответы, сохраняем обновленную историю в базу
            if new_replies_found:
                print(f"Найдены новые ответы для задачи '{task_id}', история обновлена.")
                self.db[history_key] = json.dumps(all_replies_history)

            # 4. Возвращаем полную историю
            return all_replies_history
