...
from simba_sdk.sessions.base import Base, BaseSession, process_type
...
class ClaimAttribute(Base):
    ...
    def from_dict(cls, kwargs: Dict[str, Any]):
        # the name will be the key, the other fields the value
        in_dict = {}
        for k, v in kwargs.items():
            in_dict["name"] = k
            in_dict = dict(in_dict, **v)
        cls_dict = {}
        for field in fields(cls):
            value = in_dict.get(field.name)
            if not value:
                continue
