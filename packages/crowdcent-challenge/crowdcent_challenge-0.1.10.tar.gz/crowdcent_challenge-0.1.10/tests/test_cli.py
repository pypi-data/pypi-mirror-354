...
def test_download_training_data_success(runner, mock_client, tmp_path):
    ...
    result = runner.invoke(
        cli, ["download-training-data", "--challenge", TEST_SLUG, version, "-o", str(output_file)]
    )
    assert result.exit_code == 0
...
def test_download_training_data_default_output(
    ...
    result = runner.invoke(cli, ["download-training-data", "--challenge", TEST_SLUG, version])
    assert result.exit_code == 0
...
def test_submit_success(runner, mock_client, mock_predictions_file):
    ...
    result = runner.invoke(cli, ["submit", "--challenge", TEST_SLUG, mock_predictions_file])
    assert result.exit_code == 0
    assert "Submission successful!" in result.output
    assert (
        json.loads(result.output.split("\n", 1)[1]) == mock_response
    )  # Check JSON part
    mock_client.submit_predictions.assert_called_once_with(mock_predictions_file, slot=1)


def test_submit_file_not_found(runner):
    # Click's path type validation happens before our command, so expect Click's error
    result = runner.invoke(cli, ["submit", "--challenge", TEST_SLUG, "nonexistent.parquet"])
    assert result.exit_code != 0
