...
class CommandLine(Application[CommandSettings]):
    ...
    def shutdown(self, connection: Connection, _context: CommandContext[None]) -> None:
        """
        Shuts down the station.
        """
        if not self.is_admin(connection):
            # TODO: Make separate routers for admin commands.
            send_message(connection, "You do not have permission to do this.")
            return
        send_message(connection, "Shutting down station.")
        asyncio.ensure_future(self.station.shutdown())

    def whoami(self, connection: Connection, _context: CommandContext[None]) -> None:
        """
    ...
    def swap_jobs(self, connection: Connection) -> None:
        """
        Swaps the foreground and background connections. If we're remotely connected to
        another station, this will bring us back into the CLI. If we're in the cli,
        it will bring us back into the connected session.
        """
        state = self.connections[connection]
        if not state.connection:
            state.remote_state = RemoteConnectionState.BACKGROUND
            ...
            state.remote_state = (
                RemoteConnectionState.BACKGROUND
                if state.remote_state == RemoteConnectionState.FOREGROUND
                else RemoteConnectionState.FOREGROUND
            )
        if state.remote_state.BACKGROUND:
            self.dump_logs_to_connection(connection)
        else:
            self.dump_connection_buffer(connection)
