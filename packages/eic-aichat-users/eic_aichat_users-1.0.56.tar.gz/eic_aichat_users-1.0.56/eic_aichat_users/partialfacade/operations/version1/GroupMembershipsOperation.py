...
# TODO
class GroupMembershipsOperationsV1(RestOperations):
    def __init__(self):
        super().__init__()
        self._group_memberships: IGroupMembershipsService = None
        self._group: IGroupsService = None
        self._dependency_resolver.put('groupmemberships', Descriptor('service-groupmemberships', 'service', '*', '*', '1.0'))
        self._dependency_resolver.put('groups', Descriptor('service-groups', 'service', '*', '*', '1.0'))
    ...
    def register_routes(self, controller: RestController, auth: AuthorizerV1):
            ...
            controller.register_route_with_auth('delete', '/users/groups/memberships', None, auth.signed(), lambda: self.delete_membership_by_filter())

            controller.register_route_with_auth('post', '/users/groups/memberships/:id/activate', None, auth.signed(), lambda id: self.activate_membership_by_id(id))

            controller.register_route_with_auth('post', '/users/groups/memberships/:id/deactivate', None, auth.signed(), lambda id: self.deactivate_membership_by_id(id))
