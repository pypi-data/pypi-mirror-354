...
class GroupMembershipsMemoryPersistence(IdentifiableMemoryPersistence, IGroupMembershipsPersistence):
    ...
    def __compose_filter(self, filter_params: Optional[FilterParams]) -> Callable[[GroupMembershipV1], bool]:
        ...
        def predicate(item: GroupMembershipV1) -> bool:
            ...
            if ids and item.id not in ids:
                return False
            if not_in_org_ids and item.org_id in not_in_org_ids:
                return False
            if profile_ids and item.profile_id not in profile_ids:
                return False
            if offline_ids and item.offline_id not in offline_ids:
                return False
            if group_ids and item.group_id not in group_ids:
                return False
            if org_id and item.org_id != org_id:
                return False
            if profile_id and item.profile_id != profile_id:
                return False
            if offline_id and item.offline_id != offline_id:
                return False
    ...
    def delete_by_filter(self, context: Optional[IContext], filter_params: FilterParams) -> None:
        return super().delete_by_filter(context, self._compose_filter(filter_params))

    async def create(self, context: Optional[IContext], item: GroupMembershipV1) -> GroupMembershipV1:
        await self._check_if_exists(context, item)
        return await super().create(context, item)

    async def update(self, context: Optional[IContext], item: GroupMembershipV1) -> GroupMembershipV1:
        await self._check_if_exists(context, item)
        return await super().update(context, item)

    async def _check_if_exists(self, context: Optional[IContext], item: GroupMembershipV1) -> None:
        page = await self.get_page_by_filter(
            context,
