...
def analyze_functions(db_path, pattern="main", max_results=10):
    ...
    if db.open(db_path, ida_options):
        ...
        display_functions = matching_functions[:max_results] if max_results > 0 else matching_functions
...
def main():
    ...
    parser = argparse.ArgumentParser(description=f"Database exploration example")
    parser.add_argument("-f", "--input-file", help="Binary input file to be loaded", type=str, required=True)
    parser.add_argument("-p", "--pattern", default="main", help="Pattern to search for in function names (default: main)")
    parser.add_argument("-m", "--max-results", type=int, default=10, help="Maximum number of results to display (0 for all, default: 10)")
