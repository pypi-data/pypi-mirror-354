...
def run_subscription():
    ...
    # tick
    response2 = ths.subscribe_tick("USHA600519", callback=callback_tick)
    if response2.errInfo != "":
        print(f"订阅失败: {response2.errInfo}")
    else:
        sub_id2 = response2.payload.result.get("subscribe_id")
        print(f"订阅成功，订阅ID: {sub_id2}, 结果: {response2.errInfo}")

    # l2
    response3 = ths.subscribe_l2("USHA600519", callback=callback_l2)
    if response3.errInfo != "":
        print(f"订阅失败: {response3.errInfo}")
    else:
        sub_id3 = response3.payload.result.get("subscribe_id")
        print(f"订阅成功，订阅ID: {sub_id3}, 结果: {response3.errInfo}")
    ...
    try:
        ...
        if sub_id2 != "":
            print(f"取消订阅ID: {sub_id2}")
            ths.unsubscribe(sub_id2)
        if sub_id3 != "":
            print(f"取消订阅ID: {sub_id3}")
            ths.unsubscribe(sub_id3)
