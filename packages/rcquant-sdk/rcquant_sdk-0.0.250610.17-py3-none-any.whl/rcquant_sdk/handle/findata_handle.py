...
class FinDataHandle():
    def __init__(self, tsocket: TSocket):
        ...
        self.__Financial_Columns_Types = {
            "ID": int, "InstrumentID": str, "StatDate": int, "PubDate": int, "Type": int, "JsonData": str
        }
    ...
    def fin_save_financials(self, df):
    ...
    def __unpack_financial_list(self, reqrsp: ReqRsp):
        ...
        for r in rsp_list:
            if len(r.UData) > 0:
                ...
                for ot in rspparams.DataList:
                    if len(ot) >= 6:
                        jsondt = json.loads(ot[5])
                        jsondt["InstrumentID"] = ot[1]
                        jsondt["Type"] = ot[4]
                        os.append(jsondt)
