...
def fin_save_financials(df) -> Tuple[bool, str]:
    '''
    获取品种信息及交易时段
    Args:
        df:数据dataframe
    Returns:
        _type_: [bool, str]
    '''
    return FinDataClient.instance().findata_handle().fin_save_financials(df=df)


def fin_read_financials(instrument_ids: str, begin_date: int, end_date: int, type: int) -> Tuple[bool, str, pd.DataFrame]:
    '''
    获取品种信息及交易时段
    Args:
        df:数据dataframe
