...
def brute_rdp_patator_args(parser, get_protocol_group):
    ...
    add_argument_once(brute_group, "--rdp-userlist", nargs="+", help="User wordlist(s) for RDP bruteforce (patator)")
    add_argument_once(brute_group, "--rdp-passlist", nargs="+", help="Password wordlist(s) for RDP bruteforce (patator)")

@Scanner.extend
def brute_rdp_patator(self, plugin_results=None):
    """
    Attempt RDP brute-force using patator.
    ...
    userlists = self.options.get("rdp_userlist") or ["/usr/share/wordlists/user.txt"]
    passlists = self.options.get("rdp_passlist") or ["/usr/share/wordlists/rockyou.txt"]
    ...
    for userlist in userlists:
        for passlist in passlists:
            ...
            cmd = (
                f"patator rdp_login host={host} port={port} "
                f"user=FILE0 password=FILE1 0={userlist} 1={passlist} "
                f"-x ignore:mesg='Authentication failed' "
                f"-o {output_path}"
            )
            cmds.append(cmd)
            logging.info(f"[brute_rdp] Executing: {cmd}")
            ...
            try:
                ...
                logging.error(f"[brute_rdp] Error during patator brute-force: {e}")
                results[f"{userlist}|{passlist}"] = {"error": str(e)}

    return {"cmd": cmds, "results": results, "all_found": all_found}

brute_rdp_patator.depends_on = ["scan_tcp_scanner"]