...
def dp_function(noise_multiplier, max_grad_norm, classifier, train_dataset, X_train):
    """
        Parameters:
...
def dp_target(noise_multiplier, max_grad_norm, classifier, X_test, train_dataset):
    """
        Parameters:
            noise_multiplier ([type]): Description.
            max_grad_norm ([type]): Description.
            classifier: e.g., BinaryClassifier, LogisticRegression.
    ...
    # apply DP to ML
    model, optimizer, criterion, train_dataloader, privacy_engine = dp_function(noise_multiplier, max_grad_norm, classifier, train_dataset, X_test)
    # compute accuracy
...
def dp_pareto_front(x1, x2, type):
    """
        Parameters: None
