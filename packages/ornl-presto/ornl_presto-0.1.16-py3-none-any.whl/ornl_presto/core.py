...
def dp_function(noise_multiplier, max_grad_norm, model, train_dataset, X_train):
    """
        Parameters:
...
def dp_target(noise_multiplier, max_grad_norm, model, X_test, train_dataset, y_test):
    """
        Parameters:
            noise_multiplier ([type]): Description.
            max_grad_norm ([type]): Description.
    ...
    # apply DP to ML
    model, optimizer, criterion, train_dataloader, privacy_engine = dp_function(noise_multiplier, max_grad_norm, model, train_dataset, X_test)
    # compute accuracy
...
def dp_pareto_front(x1, x2, model, X_test, train_dataset, y_test):
    """
        Parameters: None
    ...
    for i in range(100):
        ...
        accuracy, privacy_engine, predicted_classes = dp_target(Xg[pt,0], Xg[pt,1], model, X_test, train_dataset, y_test)
