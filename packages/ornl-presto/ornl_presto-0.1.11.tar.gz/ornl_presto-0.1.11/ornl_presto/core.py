...
def dp_function(noise_multiplier, max_grad_norm, algo_type, train_dataset, X_train):
    """
        Parameters:
...
def dp_target(noise_multiplier, max_grad_norm, type, X_test, train_dataset):
    """
        Parameters:
            noise_multiplier ([type]): Description.
            max_grad_norm ([type]): Description.
            type: 0 = BinaryClassifier, 1 = LogisticRegression.
    ...
    # apply DP to ML
    model, optimizer, criterion, train_dataloader, privacy_engine = dp_function(noise_multiplier, max_grad_norm, type, train_dataset, X_test)
    # compute accuracy
