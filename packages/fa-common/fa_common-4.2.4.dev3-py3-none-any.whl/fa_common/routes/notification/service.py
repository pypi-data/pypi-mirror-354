        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if workflow is not None:
                    job_succesful = workflow.status in [JobStatus.SUCCEEDED, JobStatus.COMPLETED]
                    ...
                    status_icon = "✅"
                    status_str = JobStatus.FAILED.value if not job_succesful else JobStatus.SUCCEEDED.value
                    ...
                    {metadata.success_content.body if metadata.success_content and job_succesful else ""}
                    {metadata.failed_content.body if metadata.failed_content and not job_succesful else ""}
                    </html>
                    </body>
                    """

                    # send an additional notification to the failed_email to assist with troubleshooting.
                    if metadata.failed_email and not job_succesful:
                        support_html = f"""
                            <html>
                            <body>
