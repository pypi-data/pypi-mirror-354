...
def shorten_name(name, total_len, random_part_len=AZURE_RANDOM_NAME_PART_LENGTH):
...
class ClusterCreate(ClusterCommand):
    ...
    def generate_bright_conf(self):
        ...
        bs = {
            ...
            "bright": {
                ...
                "wlm": kwargs["wlm"],
                "wlm_slot_count": "AUTO" if BcmVersion(config["version"]).release > (9, 1) else 1,
        ...
        if config["timezone"]:
            bs["bright"]["timezone"] = config["timezone"]
    ...
    def _change_public_access_for_storage_account(self, **kwargs):
        """
    ...
    def deploy_private_endpoint_for_storage_account(self):
        ...
        vnet_resource_group = config['vnet_resource_group'] if config['vnet_resource_group'] else self.resource_group
        vnet_network_name = config['vnet_network_name'] if config['vnet_network_name'] else self.vnet_name
        vnet_subnet_name = (
            config['vnet_subnet_name']
            if config['vnet_subnet_name'] else self.vnet_subnet_name
        )
        ...
        plink_deployment_properties = DeploymentProperties(
            ...
            parameters=plink_deployment_parameters
        )
    ...
    def run(self):
        ...
        inbound_rules = self.generate_inbound_traffic_rules(
            InboundTrafficRule.process_inbound_rules(config["inbound_rule"])
        )
        ...
        self._change_public_access_for_storage_account(public_access_for_cod_ip=False)
