...
class Cluster:
    ...
    @classmethod
    def find_clusters(cls, azure_client, filters):
        ...
        cluster_names = (
            ...
            | set(head_node.name[:-2] for head_node in head_nodes)
        )

        # Construct dictionaries to map cluster names to head nodes and resource groups
        cluster_names_to_head_nodes = defaultdict(list)
        cluster_names_to_rgs = dict()
        ...
        for head_node in head_nodes:
            if tag_name := cls._get_cluster_name_from_tags(head_node):
                ...
                # TODO: Consider checking name only (not tags) since head node name is always cluster_name[a|b]
                cluster_names_to_head_nodes[head_node.name[:-2]].append(head_node)

        for rg in rgs:
            cluster_names_to_rgs[cls._get_cluster_name_from_tags(rg)] = rg
        ...
        for cluster_name in cluster_names:
            ...
            try:
                image_name = next(
                    hn.tags["BCM Image name"] for hn in cluster_head_nodes
                    if hn.tags.get("BCM Image name")
                )
            except (StopIteration, TypeError):
                image_name = cluster_resource_group.tags.get("BCM Image name", None)

            version = (
                parse_image_or_blob_name(image_name)["version"] if image_name else None
            )
            ...
            vms_in_rg = [
                vm
                for vm in all_vms
                if vm.id.split("/")[4].lower() == cluster_resource_group.name.lower()
            ]
            # If cluster is deployed in own RG, then the RG name will be based on the cluster name
            if name_from_r_group(cluster_resource_group.name) == cluster_name:
    ...
    @staticmethod
    def _find_rg_of_vm(azure_client, vm):
        return azure_client.resource_client.resource_groups.get(vm.id.split("/")[4])

    @staticmethod
    def _get_cluster_name_from_tags(resource):
        if not resource.tags:
            return ""
        return resource.tags.get("BCM Cluster", "")

    @classmethod
    def filter_cluster_head_nodes(cls, vms, regexes):
        """
        It's a head node if:
        ...
        filtered_head_nodes = [
            ...
            for vm in vms
            if (
                vm.tags and vm.tags.get("BCM Type") == "Head node"
                and any(
                    re.match(regex, cls._get_cluster_name_from_tags(vm))
                    for regex in regexes
                )
            )
            # TODO: Consider checking name only (not tags) since head node name is always cluster_name[a|b]
            or vm.tags and "BCM Type" not in vm.tags
            ...
            and any(re.match(f"{regex}-[ab]", vm.name) for regex in regexes)
        ]

        return filtered_head_nodes

    @classmethod
    def _filter_cluster_rgs(cls, rgs, regexes):
        cod_rgs = [rg for rg in rgs if (rg.tags and rg.tags.get("BCM Bursting") == "on-demand")]
        filtered_rgs = [
            rg
            for rg in cod_rgs
            if any(
                re.match(regex, cls._get_cluster_name_from_tags(rg))
                for regex in regexes
            )
        ]
    ...
    def _set_primary_secondary_head_nodes(self):
        ...
        try:
            if len(self.head_nodes) == 1:
                ...
                first_hn_ha_tag = self.head_nodes[0].tags.get("BCM HA", None)
                second_hn_ha_tag = self.head_nodes[1].tags.get("BCM HA", None)
