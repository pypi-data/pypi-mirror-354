...
def start_vms(azure_api_client, vms, dry_run):
    log.debug(f"Starting VMs {', '.join(vm.name for vm in vms)}")
    if dry_run:
        return

    async_start_promises = [
    ...
    log.debug(f"Started VMs {', '.join(vm.name for vm in vms)}")
