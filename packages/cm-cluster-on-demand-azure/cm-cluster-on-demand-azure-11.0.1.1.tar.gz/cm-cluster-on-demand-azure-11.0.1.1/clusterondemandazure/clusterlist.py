...
def _get_node_info(azure_api, r_group, vm) -> NodeInfo | None:
    ...
    power_state_name = get_vm_power_state(azure_api, r_group.name, vm)
    return NodeInfo(
        name=vm.name,
        vm=vm,
        vm_size=vm.hardware_profile.vm_size,
        vm_power_state=power_state_name,
...
def get_cluster_data(azure_api, cluster):
    """
...
@cache
def _get_location_vmsize_details(compute_client, location, vmsize_name):
    """
    ...
    return next(
        (
            ...
            if vmsize.name.lower() == vmsize_name.lower()
        ),
        {
