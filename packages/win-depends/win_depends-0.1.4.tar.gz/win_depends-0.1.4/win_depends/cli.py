...
def main():
    ...
    parser.add_argument("--check-invalid", action="store_true", help="Check if dependencies are loadable.")
    parser.add_argument("--check-orphan", action="store_true", help="Check for orphan local DLL/EXE files.")
    ...
    if not (is_dir or (os.path.isfile(target) and target.lower().endswith(('.dll', '.exe')))):
    ...
    if args.tree is not None:
        ...
        dep_graph.print_tree(max_depth=max_depth)
    if args.graph:
