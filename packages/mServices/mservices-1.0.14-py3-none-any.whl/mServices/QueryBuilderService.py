...
class QueryBuilderService:
    ...
    def build_query(self, count_mode=False):
        ...
        if count_mode:
            if self.group_by:
                select_clause = f"COUNT(*) FROM (SELECT {self.select_columns or '*'} FROM {self.table}"
            else:
                select_clause = "COUNT(*)"
        else:
            select_clause = self.select_columns if self.select_columns else "*"
        ...
        if self.conditions:
            ...
            for cond in self.conditions:
                if isinstance(cond, tuple) and len(cond) == 2:
                    condition_strings.append(cond[0])
        ...
        if count_mode and self.group_by:
            query += ") AS grouped_results"  # Closing subquery for grouped count

        if not count_mode and self.order_by:
            query += f" {self.order_by}"

        if not count_mode and self.limit is not None:
