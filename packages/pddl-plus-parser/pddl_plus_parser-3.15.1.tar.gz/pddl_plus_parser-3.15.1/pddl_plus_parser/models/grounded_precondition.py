...
from pddl_plus_parser.models.pddl_precondition import (
...
BinaryOperator = {"and": lambda x, y: x and y, "or": lambda x, y: x or y}
...
class GroundedPrecondition:
    ...
    @staticmethod
    def _ground_equality_objects(
        equality_preconditions: Set[Tuple[str, str]], parameters_map: Dict[str, str]
    ) -> Set[Tuple[str, str]]:
    ...
    def _ground(
        self,
        lifted_conditions: Precondition,
        grounded_conditions: Precondition,
        ...
        for precondition in lifted_conditions.operands:
            if isinstance(precondition, Predicate):
                ...
                grounded_condition.equality_preconditions = self._ground_equality_objects(
                    precondition.equality_preconditions, parameters_map
                )
    ...
    def _validate_numeric_expression_hold(
        self,
        condition: NumericalExpressionTree,
    ...
    def _validate_predicates_hold(
        self,
        condition: GroundedPredicate,
        prev_is_applicable: bool,
        preconditions: Precondition,
        state: State,
    ) -> bool:
    ...
    def _validate_universal_precondition(
        self,
    ...
    def ground_preconditions(self, parameters_map: Dict[str, str]) -> None:
        ...
        :param parameters_map: the mapping between the lifted and the grounded objects.
        """
        self._grounded_precondition.root.equality_preconditions = self._ground_equality_objects(
            self._lifted_precondition.root.equality_preconditions, parameters_map
        )
        ...
        self._ground(
            lifted_conditions=self._lifted_precondition.root,
            grounded_conditions=self._grounded_precondition.root,
