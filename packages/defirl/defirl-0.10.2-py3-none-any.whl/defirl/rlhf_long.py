...
class RLmodel_bids(RLModelBase):
    ...
    model_file_path: str = f'{Path(__file__).resolve().parent}/bids2_q_table.pkl'
    state_index_file: str = f'{Path(__file__).resolve().parent}/bids2_state_to_index.pkl'
    episodes_file: str = f'{Path(__file__).resolve().parent}/bids2_epitrans.pkl' # Set directly
    ...
    def _load_data(self):
        ...
        self.action_mapping = {"go_long": 0, "go_short": 1} # Assuming only these two actions for bids "do_nothing": 0, 
...
print(RLmodel_bids(0.5,0.512,0,1).predict_action())
