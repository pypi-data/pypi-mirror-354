...
def get_agricultural_encyclopedia_vector(text: str) -> list[TextContent]:
    """根据用户的问题：text从农业百科的向量库中查找出几段最近似的内容，需要你归纳总结形成答案给用户。
    返回json格式数据，其中data为jsonarray，里面包含content：近似的内容。
    从content中选取跟用户的问题相关的内容总结后返回给用户，无关的舍弃不要。
    ...
    test_url = api_url+"/getNszdContent"
    test_data = {"text": text,"token":api_key,"tag":"bkqs","mdName":"get_agricultural_encyclopedia_vector"}
...
async def list_tools() -> list[types.Tool]:
    ...
    return [
        ...
        types.Tool(
            name="get_agricultural_encyclopedia_vector",
            description="根据用户的问题：text从农业百科的向量库中查找出几段最近似的内容，需要你归纳总结形成答案给用户。返回json格式数据，其中data为jsonarray，里面包含content：近似的内容。从content中选取跟用户的问题相关的内容总结后返回给用户，无关的舍弃不要。注意：在使用其他工具无法很好的回答用户提出的关于农业方面的问题，可使用该工具。",
            inputSchema={
                "type": "object",
                "properties": {
                    "text": {"type": "string", "description": "用户的问题"}
...
async def call_tool_x(name: str, arguments: dict) -> list[TextContent]:
    ...
    elif name == "get_agricultural_encyclopedia_vector":
        text = arguments.get("text")
        return get_agricultural_encyclopedia_vector(text)
