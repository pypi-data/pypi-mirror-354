...
def GenericXY(x, xname, xunit, y, yname, yunit):
    ...
    """
    return Spectrum(
        Data1D(x, DataType(xname, xunit)), Data1D(x, DataType(xname, xunit))
    )
...
def GenericXMultiY(x, xname, xunit, y, yname, yunit, keys, keyname, keyunit):
    ...
    return SpectrumSeries(
        Data1D(x, DataType(xname, xunit)),
        Data1D(keys, DataType(keyname, keyunit)),
        Data2D(y, DataType(yname, yunit)),
    )
