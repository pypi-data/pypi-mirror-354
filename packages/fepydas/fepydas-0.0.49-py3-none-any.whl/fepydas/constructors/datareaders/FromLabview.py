            ...
            if len(units) > 2:
                ...
                if headers["Measurement Type"][1] == "Time Series":
                    infos["Power Track"] = Data1D(
                        ...
                        DataType("Power Track", "mW"),
                    )
                else:
                    infos["Power Track"] = Data1D(
                        numpy.array(
                            units[1 + int(len(units) / 2) :], dtype=numpy.float64
                        ),
                        DataType("Power Track", "mW"),
                    )
    ...
    if is_map:
        ...
        if is_series:
            ...
            if is_ple:
                ...
                if len(units) > 2:
                    dtype = DataType(headers["Measurement Type"][1], units[2])  # TODO
                else:
                    dtype = DataType(headers["Measurement Type"][1], "a.u.")  # TODO
            keys = Data1D(
