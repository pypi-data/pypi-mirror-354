...
class FitSimple:
    ...
    def fit(self, print_report: bool = True, **kwargs):
        """Performs the fitting procedure for the entire dataset.

        This is the main method to run the fit. It iterates through the dataset
        provided at initialization (handling both 1D and 2D cases) and uses the
        parameters from `self.params`.

        The results are stored in the `self.results` attribute.

        Args:
            print_report (bool): If True, the `lmfit` fit report for each
                successful fit will be printed to the console.
            **kwargs: Additional keyword arguments to be passed to `fitxy` and
                subsequently to `lmfit.model.Model.fit()`.

        Raises:
            TypeError: If the initial parameters (`self.params`) have not been
                set or guessed before calling this method.
        """
        if self.params is None:
            raise TypeError(
                "Initial parameters have not been set. Call 'initguess()' or 'setinitparams()' first."
            )
        ...
        if self.ydims == 1:
            ...
            self.results = []
            for i, singlespectrum in enumerate(self.spectrumdataset.data.values):
                result = self.fitxy(
                    x=self.spectrumdataset.axis.values,
                    y=singlespectrum,
                    p0s=self.params[i],
    ...
    def plot(self, i=None):
        if self.ydims == 1:
            fig, ax = plt.subplots()
            ax.plot(self.spectrumdataset.axis.values, self.spectrumdataset.data.values)
            ax.plot(
            ...
            if i is None:
                for i in range(len(self.spectrumdataset.data.values)):
                    ...
                    ax.legend()
                    ax.set_title(f"{i} - {self.spectrumdataset.keys.values[i]}")
                    plt.show()
                    plt.close(fig)
