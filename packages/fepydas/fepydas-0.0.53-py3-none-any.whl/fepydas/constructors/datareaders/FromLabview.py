    ...
    if "Header Version" in headers.keys() and int(headers["Header Version"][0]) == 2:
        ...
        def read_units(file):
            ...
            if len(units) > 2:
                ...
                if headers["Measurement Type"][1] == "Time Series" or headers["Measurement Type"][1] == "Power":
                    infos["Power Track"] = Data1D(
                        numpy.array(units[3:], dtype=numpy.float64),
                        datatypes.P_mW,
                    )
                else:
                    infos["Power Track"] = Data1D(
                        numpy.array(units[1 + int(len(units) / 2) :], dtype=numpy.float64),
                        datatypes.P_mW,
                    )
    ...
    if is_map:
        ...
        if is_series:
            ...
            if is_ple:
                ...
                if len(units) > 2:
                    dtype = datatypes.generateDataType(headers["Measurement Type"][1], units[2])  # TODO
                else:
                    dtype = datatypes.generateDataType(headers["Measurement Type"][1], "a.u.")  # TODO
            keys = Data1D(
            ...
            if (
                ...
                and int(headers["Dark Offset"][0]) == 0
            ):
