...
class WebSocketClient:
    ...
    async def convert_trade_list_to_rq(self, trades):
        ...
        pd_data.to_csv(file_path, mode='a' if file_exists else 'w', index=False, header=not file_exists)
    ...
    async def save_daily_data(self, data, file_name):
        ...
        # 写入 CSV 文件，根据文件是否存在决定是否包含列头
        df.to_csv(file_path, mode='a' if file_exists else 'w', index=False, header=not file_exists)
    ...
    async def handle_rtn_back_results(self, data):
        ...
        if data[0]["stdReturn"] and data[0]["sharpRatio"] and data[0]["backReturn"] and data[0][
            "annualizedReturn"] and data[0]["calmarRatio"]:
            # 获取数据
            stdReturn = data[0]["stdReturn"] * 100
            # 以数字带2-4位小数显示
            sharpRatio = data[0]["sharpRatio"]
            backReturn = data[0]["backReturn"] * 100
            annualizedReturn = data[0]["annualizedReturn"] * 100
            # calmar应该是正的，最大回撤取绝对值，以数字带2-4位小数显示
            calmarRatio = abs(data[0]["calmarRatio"])
