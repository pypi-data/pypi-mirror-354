...
def format_response(response, type) -> dict:
    ...
    if type == 'responses':
        ...
        if not response or not hasattr(response, 'output_text') or not response.output_text:
...
class CustomOpenAIClient(OpenAI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def custom_responses_create(*args, **kwargs):
            ...
            response.flash_query = format_response(response, 'responses')
            return response
        
        def custom_completions_create(*args, **kwargs):
            response = original_completions_create(*args, **kwargs)
            response.flash_query = format_response(response, 'completions')
            return response
...
class CustomAsyncOpenAIClient(AsyncOpenAI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def get_flash_query_stream(response, type: str) -> dict:
            return {
                "success": True,
                "content": self.full_text.strip() if type == 'completions' else response.response.output_text,
                "model": response.model if getattr(response, "model", "unknown") else response.response.model,
                "usage": {
                    "prompt_tokens": 0,
                    "completion_tokens": 0,
                    "total_tokens": 0
                },
                "finish_reason": response.choices[0].finish_reason if type == 'completions' else ''
            }
        ...
        async def stream_wrapper(stream, type: str):
            ...
            and injects a custom 'flash_query' field into the last item before yielding it.
            ...
            while True:
                try:
                    ...
                    try:
                        setattr(previous, "flash_query", get_flash_query_stream(previous, type))
                    except Exception:
