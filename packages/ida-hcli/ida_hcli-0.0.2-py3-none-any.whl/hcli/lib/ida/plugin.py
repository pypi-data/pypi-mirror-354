...
from hcli.lib.api import plugins as plugins_api
from hcli.lib.ida import get_ida_user_dir
from hcli.lib.util.io import file_exists, dir_exists, remove_dir
...
class DynamicMetadata:
    ...
    def __init__(self, stars: int = 0, forks: int = 0, watchers: int = 0, 
                 open_issues: int = 0, language: str = "", latest_update: str = "",
...
class Metadata:
    ...
    def __init__(self, repository_name: str, repository_description: str, 
                 repository_owner: str, entry_point: Optional[str] = None,
                 install: Optional[str] = None, dynamic_metadata: Optional[DynamicMetadata] = None):
...
class Plugin:
    ...
    def __init__(self, id: str, owner: str, name: str, slug: str, url: str,
                 metadata: Metadata, updated_at: Optional[str] = None,
...
async def get_plugins() -> List[Plugin]:
    ...
    if _PLUGINS_CACHE is None:
        results = await plugins_api.get_plugins()
        _PLUGINS_CACHE = results.get('hits', [])
...
async def get_plugin(slug: str) -> Optional[Plugin]:
    ...
    return await plugins_api.get_plugin(slug)
...
def _generate_plugin_metadata(plugin: Plugin) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin": {
            "name": plugin.name,
            "entryPoint": plugin.metadata.entry_point
        }
    }
