...
def load_plugins():
    eps = entry_points()
    return [ep.load() for ep in eps.select(group="hcli.plugins")]


@click.group(
    help=f"[bold blue]IDA CLI[/bold blue] [dim](v{__version__})[/dim]\n\n[yellow]Command-line interface for IDA Pro license, plugin, and cloud management.[/yellow]"
)
