    ...
    def proxy_request(self, method):
        try:
            ...
            if parsed_url.scheme not in ("http", "https"):
                self.send_error(400, f"Unsupported URL scheme: {parsed_url.scheme}")
                return
...
async def start_proxy(port: int, config: ProxyConfig, handlers: Optional[ServerHandlers] = None) -> None:
