...
from hcli.env import ENV, CONFIG_API_KEY, OAUTH_REDIRECT_URL, OAUTH_SERVER_PORT
...
class AuthService:
    ...
    def __init__(self):
        ...
        self.supabase: Client = create_client(
            ENV.HCLI_SUPABASE_URL,
            ENV.HCLI_SUPABASE_ANON_KEY,
            options
        )
    ...
    def get_auth_type(self) -> AuthType:
        ...
        if self.get_api_key():
            if config_store.has(CONFIG_API_KEY):
                return {"type": "key", "source": "config"}
            elif ENV.HCLI_API_KEY:
    ...
    async def _start_oauth_server(self):
        ...
        if self._oauth_result:
            ...
            self.supabase.auth.set_session(
                self._oauth_result["access_token"],
                self._oauth_result["refresh_token"]
            )
    ...
    def show_login_info(self):
        ...
        if self.is_logged_in():
            ...
            print(f"You are logged in as {user['email']}{suffix}")
        else:
