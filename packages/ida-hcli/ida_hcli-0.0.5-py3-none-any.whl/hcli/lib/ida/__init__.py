...
def is_installable(download: DownloadResource) -> bool:
    ...
    return (
        (src.endswith(".app.zip") and current_os == "mac") or
        (src.endswith(".run") and current_os == "linux") or
        (src.endswith(".exe") and current_os == "windows")
    )
...
def get_ida_binary(ida_dir: str, suffix: str = "") -> str:
    ...
    if get_os() == "windows":
        return str(Path(ida_dir) / f"ida{suffix}.exe")
    else:
        return str(Path(ida_dir) / f"ida{suffix}")


def get_ida_path(ida_dir: str, suffix: str = "") -> str:
    """Get the IDA executable path."""
    if get_os() == "mac":
        return str(Path(ida_dir) / "Contents" / "MacOS" / f"ida{suffix}")
    elif get_os() == "windows":
        return str(Path(ida_dir) / f"ida{suffix}.exe")
    else:
        return str(Path(ida_dir) / f"ida{suffix}")
...
async def install_license(license_file: str, ida_dir: str) -> None:
    """Install a license file to an IDA directory."""
    license_path = Path(license_file)
    target_dir = Path(get_license_dir(ida_dir))
    target_file = target_dir / license_path.name
...
async def install_ida(installer: str, install_dir: str) -> Optional[str]:
    """
...
async def _install_ida_mac(installer: str, prefix: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="hcli_") as temp_unpack_dir:
        with tempfile.TemporaryDirectory(prefix="hcli_") as temp_install_dir:
            ...
            process = await asyncio.create_subprocess_exec(
                "unzip", installer, "-d", temp_unpack_dir
            )
            await process.communicate()
...
async def _install_ida_unix(installer: str, prefix: str) -> None:
    ...
    process = await asyncio.create_subprocess_exec(installer, *args)
    await process.communicate()
