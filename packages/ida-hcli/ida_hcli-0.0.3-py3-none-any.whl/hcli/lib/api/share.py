...
class ShareAPI:
    ...
    async def delete_file(self, code: str) -> SharedFile:
        """Delete a shared file."""
        client = await get_api_client()
        data = await client.delete_json(f"/api/share/{code}")
    ...
    async def get_files(self, filter_params: Optional[PagingFilter] = None) -> Paged:
        ...
        data = await client.get_json(
            f"/api/share?limit={filter_params.limit}&offset={filter_params.offset}"
        )
        return Paged(**data)
