...
class Speech2Text:
    ...
    def __init__(
        ...
        # 
        self.chunk_size = max(decoding_window, 0)
    ...
    def streaming_decode(
        ...
        enc_out, _, self.encoder_states = self.asr_model.encoder(
            feats, feats_length, self.encoder_states,
        ...
        if len(enc_out[0]) > 0:
            nbest_hyps = self.beam_search(enc_out[0], is_final=is_final or is_final_search)

        self.num_processed_frames += self.chunk_size

        if is_final or is_final_search:
            self.reset_streaming_cache()
