...
from polars import Expr, Float64, Int64, DataType
...
class ColumnSchema:
    ...
    def _get_clean_stage(
        ...
        if isinstance(to, CleanColumn):
            ...
            return CleanNumberColumn(**kwargs)
        elif to == "integer":
            return CleanNumberColumn(output_format=Int64, data_type="integer", **kwargs)
        elif to == "decimal":
            return CleanDecimalColumn(**kwargs)
        elif to == "float":
            return CleanNumberColumn(
                output_format=Float64,
                data_type="float",
                **kwargs,
            )
        elif to == "boolean":
            return CleanBooleanColumn(**kwargs)
        elif to == "date":
            return CleanDateColumn(**kwargs)
        # elif data_type == "datetime":
        #     return replace(self, clean=CleanDateTimeColumn(**kwargs))
        elif to == "enum":
            return CleanEnumColumn(**kwargs)
        elif to == "id":
            return CleanIdColumn(**kwargs)
