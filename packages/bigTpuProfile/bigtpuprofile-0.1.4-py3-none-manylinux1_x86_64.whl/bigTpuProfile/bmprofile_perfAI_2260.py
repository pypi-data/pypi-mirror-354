...
class BMProfileParserPerfAI(BMProfileParser):
    ...
    def parse_cmd(self, file_list):
        ...
        for infile in tqdm(file_list):
            ...
            if item.command_info:
                logging.warning("Deprecated. Please update runtime and re-run bmodel.")
    ...
    def __make_mix_pairs(self, cmd, monitor, sys_code, des_cmd):
        ...
        if des_cmd:
            ...
            for i, p in enumerate(des_pairs):
                pairs[idx[i]] = p
        # print("+++++++++++++")
        # print("cmd", [c[0] for c in _cmd])
        # print([[i.inst_id for i in c[1]] for c in _cmd])
        # print("des_cmd", [c[0] for c in _des_cmd])
        # print("pmu", [m[0] for m in _monitor])
        # print([[i.inst_id for i in c[1]] for c in _monitor])
        # for m in _monitor[:9]:
        #     for i in m[1]:
    ...
    def __make_pairs(self, cmd, monitor, sys_code, des_cmd):
        pairs = []
        ...
        for p_monitor, p_cmd in self.__make_mix_pairs(cmd, monitor, sys_code, des_cmd):
            ...
            if p_monitor is None:
                continue
            if p_cmd is None:
                if not self.is_bmodel: # bmodel will drop unknow pairs
                    for m in p_monitor:
                        pairs.append({"monitor": m, "cmd": None})
                continue
    ...
    def __compatiable_make_pairs(self, cmd, monitor, sys_code, des_cmd):
        pairs = []
    ...
    def make_pairs(self, cmd, monitor, sys_code, des_cmd=None, is_cdma=False):
        mix_mode = not self.is_bmodel and mix_mode
