...
def serve(
    ...
    uvicorn.run("p2zyq.app:app", host=host, port=port, reload=reload)
...
def routes():
    """显示所有API路由。"""
    try:
        from p2zyq.app import app as fastapi_app
    except ImportError:
        console.print(
...
def info():
    """显示项目信息。"""
    version = getattr(p2zyq, "__version__", "未知")

    console.print(
        Panel(
            f"""
项目名称: [bold]p2zyq[/bold]
...
def version():
    """显示版本信息。"""
    version = getattr(p2zyq, "__version__", "未知")
    console.print(f"p2zyq v{version}")
