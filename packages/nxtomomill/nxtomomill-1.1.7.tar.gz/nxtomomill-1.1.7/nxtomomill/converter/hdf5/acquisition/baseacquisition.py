...
class BaseAcquisition:
    ...
    def _get_sample_x(self, root_node, n_frame) -> tuple:
    ...
    def _get_sample_y(self, root_node, n_frame) -> tuple:
        """return the list of translation for each frame"""
        for grp in self._get_positioners_node(root_node), root_node:
            try:
    ...
    def _get_translation_y(self, root_node, n_frame) -> tuple:
        """return the list of translation for each frame"""
        for grp in self._get_positioners_node(root_node), root_node:
            try:
                y_tr, unit = self._get_node_values_for_frame_array(
    ...
    @staticmethod
    def get_translation_z_frm(root_node, n_frame: int, configuration: TomoHDF5Config):
    ...
    def _get_translation_z(self, root_node, n_frame) -> tuple:
