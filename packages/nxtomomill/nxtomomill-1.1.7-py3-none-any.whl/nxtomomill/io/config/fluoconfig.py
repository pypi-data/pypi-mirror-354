...
class TomoFluoConfig(ConfigBase, ConfigSourceSection):
    ...
    _valid_metric_values = [str(item) for item in MetricSystem]
    ...
    COMMENTS_GENERAL_SECTION = {
        ...
        INPUT_FOLDER_DK: "Folder containing .tif files. if not provided from the configuration file must be provided from the command line",
        OUTPUT_FILE_DK: "output file name. If not provided from the configuration file must be provided from the command line",
        DETECTOR_NAMES_DK: "names of fluo detectors. Either '' (detects all detectors), or e.g. 'xmap' or 'xmap falcon'",
        ...
        DATASET_BASENAME_DK: f"dataset file prefix. Used to determine projections folder name. Projection folder name is assumed to be DATASET_BASENAME_XXX where XXX denotes the 3-digit proj index. If not provided will take the name of {INPUT_FOLDER_DK}",
