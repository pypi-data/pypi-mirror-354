...
class StandardAcquisition(BaseAcquisition):
    ...
    def _register_machine_current(self, entry: h5py.Group, start_time, end_time):
        ...
        if machine_currents is not None and len(machine_currents) > 0:
            ...
            if len(filtered_new_know_machine_currents) != new_know_machine_currents_am:
    ...
    def _get_pixel_size(self, axis) -> pint.Quantity | None:
    ...
    def to_NXtomos(self, request_input, input_callback, check_tomo_n=True) -> tuple:
        ...
        # x and y pixel size
        try:
            x_pixel_size: pint.Quantity = self._get_user_settable_parameter(
        ...
        try:
            y_pixel_size: pint.Quantity = self._get_user_settable_parameter(
        ...
        if self.translation_y is None or len(self.translation_y) < 1:
            ...
            x_pixel_size = x_pixel_size.to(_ureg.meter).magnitude
