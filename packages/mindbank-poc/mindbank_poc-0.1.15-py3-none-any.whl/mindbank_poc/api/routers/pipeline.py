...
async def get_normalization_stats(token: dict = Depends(get_current_token)):
    """
    Получение статистики нормализации для мониторинга.
    """
    ...
    try:
        ...
        # Статистика сегментов и кластеров
        total_segments = 0
        total_clusters = 0
        
        try:
            if hasattr(knowledge_store, 'list_all_segments'):
                all_segments = await knowledge_store.list_all_segments()
                total_segments = len(all_segments)
            if hasattr(knowledge_store, 'list_clusters'):
                all_clusters = await knowledge_store.list_clusters()
                total_clusters = len(all_clusters)
        except Exception as e:
            logger.warning(f"Could not get segments/clusters stats: {e}")
        
        # Статистика воркера нормализации
        worker_running = False
        worker_stats = {}
        try:
            # Попытка получить статистику воркера через глобальную переменную
            import mindbank_poc.api.main as main_module
            if hasattr(main_module, 'normalization_worker') and main_module.normalization_worker:
                worker_running = main_module.normalization_worker._running
                worker_stats = main_module.normalization_worker.get_stats()
        except Exception as e:
            logger.warning(f"Could not get normalization worker stats: {e}")
        
        return {
            "total_aggregates": total_aggregates,
            "processed_aggregates": len(processed_ids),
            "unprocessed_aggregates": total_aggregates - len(processed_ids),
            "total_normalized_units": len(all_units),
        ...
        return {
            ...
            "unprocessed_aggregates": 0,
            "total_normalized_units": 0,
            "total_segments": 0,
            "total_clusters": 0,
            "worker_running": False,
            "worker_stats": {}
