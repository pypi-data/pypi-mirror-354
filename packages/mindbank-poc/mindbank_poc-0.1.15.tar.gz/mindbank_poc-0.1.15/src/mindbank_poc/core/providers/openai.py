...
def register_normalization_providers():
    """Регистрирует OpenAI провайдеры нормализации."""
    ...
    if "openai-transcription" not in existing_providers:
        transcription_provider = ProviderModel(
            id="openai-transcription",
            name="OpenAI Transcription",
            provider_type=ProviderType.TRANSCRIPTION,
            description="Transcription provider using OpenAI Whisper API",
            config_schema={
                "api_key": {
                    "type": "string",
        ...
        logger.info("Registered new OpenAI transcription provider")
    else:
        logger.info("OpenAI transcription provider already exists, keeping existing configuration")
    
    # OpenAI Caption Provider
    if "openai-caption" not in existing_providers:
        caption_provider = ProviderModel(
            id="openai-caption",
            name="OpenAI Caption",
            provider_type=ProviderType.CAPTION,
            description="Image caption provider using OpenAI Vision API",
            config_schema={
                "api_key": {
                    "type": "string",
                    "description": "API key for OpenAI service"
                ...
                "model": {
                    "type": "string",
                    "description": "Vision model to use for image captioning",
                    "default": "gpt-4o-mini"
                },
                "max_tokens": {
                    "type": "integer",
            ...
            current_config={
                "model": settings.openai.caption_model,
                "max_tokens": settings.openai.caption_max_tokens,
                "temperature": settings.openai.caption_temperature,
                "api_key": settings.openai.api_key or ""
            }
        )
        provider_service.register_provider(caption_provider)
        logger.info("Registered new OpenAI caption provider")
    else:
        logger.info("OpenAI caption provider already exists, keeping existing configuration")
    
    # OpenAI Embedding Provider
    if "openai-embedding" not in existing_providers:
        embedding_provider = ProviderModel(
            id="openai-embedding",
            name="OpenAI Embeddings",
            provider_type=ProviderType.EMBEDDING,
            description="Text embedding provider using OpenAI embeddings API",
            config_schema={
                "api_key": {
                    "type": "string",
                    "description": "API key for OpenAI service"
        ...
        logger.info("Registered new OpenAI embedding provider")
    else:
        logger.info("OpenAI embedding provider already exists, keeping existing configuration")
    
    # OpenAI Classification Provider
    if "openai-classifier" not in existing_providers:
        classifier_provider = ProviderModel(
            id="openai-classifier",
            name="OpenAI Classifier",
            provider_type=ProviderType.CLASSIFICATION,
            description="Content classification provider using OpenAI chat API",
            config_schema={
                "api_key": {
                    "type": "string",
                    "description": "API key for OpenAI service"
                ...
                "model": {
                    "type": "string",
                    "description": "Chat model to use for classification",
                    "default": "gpt-4o-mini"
                },
                "max_tokens": {
                    "type": "integer", 
            ...
            current_config={
                "model": settings.openai.classifier_model,
                "max_tokens": settings.openai.classifier_max_tokens,
                "temperature": settings.openai.classifier_temperature,
                "api_key": settings.openai.api_key or ""
            }
        )
        provider_service.register_provider(classifier_provider)
        logger.info("Registered new OpenAI classifier provider")
    else:
        logger.info("OpenAI classifier provider already exists, keeping existing configuration")
    
    logger.info(f"OpenAI normalization providers registration completed. Total new providers: {4 - len([p for p in ['openai-transcription', 'openai-caption', 'openai-embedding', 'openai-classifier'] if p in existing_providers])}") 