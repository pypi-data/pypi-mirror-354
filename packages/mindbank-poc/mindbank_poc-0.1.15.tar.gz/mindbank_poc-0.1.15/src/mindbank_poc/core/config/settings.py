...
class EnrichmentSettings(BaseSettings):
    ...
    normalization_enabled: bool = Field(True, description="Включить фоновую нормализацию")
    normalization_min_pending: int = Field(5, description="Минимальное количество агрегатов для запуска")
    normalization_timeout_sec: int = Field(1800, description="Таймаут между запусками нормализации (30 мин)")
