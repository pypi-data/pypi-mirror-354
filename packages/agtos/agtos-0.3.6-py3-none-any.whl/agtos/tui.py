...
class AgtOSTUI:
    ...
    def _create_application(self) -> Application:
        ...
        main_kb_filter = auth_dialog_filter & tutorial_filter & command_palette_filter & dashboard_filter & help_overlay_filter
            ...
            try:
                if system == "Darwin":  # macOS
                    ...
                    tell application "Terminal"
                        activate
                        do script "cd {cwd} && {claude_command}"
                    end tell
                    '''
...
"""
    ...
    def _check_updates(self):
        ...
        try:
            ...
            self.spinner.update_message(operation_id, "Contacting GitHub...")
    ...
    def _get_latest_version(self) -> tuple[Optional[str], Optional[str], Optional[Dict[str, Any]]]:
        ...
        try:
            # Try to import github_auth module
            try:
                ...
                # Github auth module not available
                logger.debug("Github auth module not available for update checks")
        ...
        try:
            ...
            if self.pending_update and self.pending_update.get("download_url"):
                ...
                try:
                    ...
                    # Github auth module not available
                    raise RuntimeError(
                        "Update installation requires GitHub authentication. "
                        "Please update using pip directly."
                    )
                ...
                try:
                    self.spinner.update_message(operation_id, f"Downloading v{new_version}...")
