...
try:
    generator_config = config_helper.get_generator_config(name=default_generator)
...
async def process_generate(req: ChatCompletionRequest):
    ...
    try:
        ...
        logger.error(f"main.process_generate: {str(me)}")
        raise
    except ContextLengthExceededException as cle:
        logger.error(f"main.process_generate: {str(cle)}")
        raise
    except GeneratorMismatchException as gme:
        logger.error(f"main.process_generate: {str(gme)}")
        raise
    except Exception as e:
        id=str(uuid.uuid4())
        logger.error(f"main.process_generate: {str(e)} id={id}")
        raise InternalException(id)

@router.post("/gen/v1/chat/pull")
async def post_chat_pull(req: ModelDownloadRequest = Body(...)):
    from gai.chat.lib.generators_utils import download, text_progress_callback
