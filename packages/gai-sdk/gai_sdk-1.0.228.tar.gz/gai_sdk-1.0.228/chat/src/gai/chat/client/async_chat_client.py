...
def attach_extractor(response: ChatCompletion, is_stream: bool):
    if not is_stream:
        ...
        if response.choices[0].message.tool_calls:
            ...
            return response
        raise Exception(
            "completions.attach_extractor: Response is neither content nor toolcall. Please verify the API response."
        )
...
class AsyncChatClient:
    def __init__(
        self,
        config: Optional[Union[GaiClientConfig, dict]] = None,
        name: Optional[str] = "ttt",
        ...
        if self.config.client_type != "gai":
            raise ValueError(
                f"__init__: Invalid client type. client_type={self.config.client_type}"
            )
    ...
    async def _generate_dict(self, **kwargs) -> ChatCompletion:
        ...
        try:
            ...
            raise ApiException(
                code=408, message=f"Request timeout after {timeout} seconds"
            )
    ...
    async def _stream_dict(self, **kwargs) -> AsyncGenerator[ChatCompletionChunk, None]:
        ...
        try:
            async with self._client.stream(
                "POST", url, json={**kwargs}, timeout=timeout
            ) as response:
                response.raise_for_status()
                ...
                async for line in response.aiter_lines():
                    try:
                        ...
                        logger.error(
                            f"completions._stream_dict: error={e}, line={line}"
                        )
                        continue
    ...
    async def pull(self, model_name: str, url: str, timeout: float) -> ChatCompletion:
        ...
        try:
            response = await self._client.post(
                url, json={"model": model_name}, timeout=timeout
            )
            response.raise_for_status()
            jsoned = response.json()
            completion = ChatCompletion(**jsoned)
