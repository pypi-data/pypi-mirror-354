...
class Submodule:
    def __init__(self, path, url, parent = None):
        ...
        self.yaml = {}  
        self.repo = None
        self.submodrepo = Repo(self.url, True, False, False)
        self.parent = parent
    ...
    def getPathFromRoot(self):
        ...
        if self.parent is None:
            ...
            ret = self.path
            ret = ret.replace(self.parent.relativePath,"")
    ...
    def checkFor(self,mods):
        ...
        self.exists = False
        for mod in mods:
            if(self.getPathFromRoot() == mod.getPathFromRoot()):
                self.exists = True
    ...
    def loadRootYaml(self):
        mrtYaml = self.submodrepo.getRootYaml()
        self.loadYaml(mrtYaml)
...
class Repo:
    def __init__(self, path :str, remote = False, createdir=True, fetchDefaultBranch = True):
        ...
        if not self.name == "" and not self.isGroup and fetchDefaultBranch:
    ...
    def getRootYaml(self, url = ""):
        ...
        if url == "":
            ...
            yamlText = getRemoteFileText(url,'mrt.yml')
    ...
    def gatherModuleYamls(self):
        ...
        modules = []
        for mod in self.mods:
            modules.append(mod.getYaml())
        
        dict_yaml['modules'] = modules
        
        return dict_yaml
        

    def getSubModules(self, exclude = ['tools/**']):
        data = ""
        ...
        if self.isRemote:
            ...
            if self.isGroup:
                ...
                if resp.status_code == 200:
                    ...
                    for mod in data:
                        ...
                        if "path_with_namespace" in mod: 
                            ...
                            for ex in exclude:
                                if fnmatch.fnmatch(modPath.lstrip('/'),  ex):
                                    excludeMod = True
                                    break
                            ...
                            if not"mrt-ignore" in mod["tag_list"]:
                                ...
                                newSubMod = Submodule(modPath, modUrl, self)
                                newSubMod.submodrepo.default_branch = mod["default_branch"]
                                self.mods.append(newSubMod)

                    threads = []
                    for mod in self.mods:
                        threads.append(Thread(target=mod.loadRootYaml))
                ...
                for mod in modules:
                    if(mod[1] != "Config"):
                        self.mods.append(Submodule(mod[1], mod[2], self))
        else:
    ...
    def addSubModule(self,mod):
        ...
        subprocess.check_output(['git','submodule','add', '-f',mod.git_url,  self.relativePath + '/' + mod.path.lower()] )
        os.chdir(prev_path)
        mod.exists = True
    ...
    def removeSubModule(self,mod):
        ...
        subprocess.check_output(['git','rm', '-f', self.relativePath + mod.path.lower() ])
        os.chdir(prev_path)
        mod.exists = False
    ...
    def getDefaultBranch(self):
        ...
        try:
            ...
            if self.isRemote:
                ...
                branch = subprocess.check_output("git remote show " + self.path + " | sed -n '/HEAD branch/s/.*: //p'", shell=True)
