...
class RegFieldSpec:
    def __init__(self, node):
        ...
        if 'mask' in node:
            ...
            tmpOffset = self.getOffset()
            self.lowBit = tmpOffset
            self.highBit = tmpSize + tmpOffset - 1

        if 'bit' in node:
            self.mask = 1 << node['bit']
        if 'bits' in node:
            #handle bit range as n-n, n:n, or n

            if type(node['bits']) is int:
                self.lowBit = node['bits']
                self.highBit = node['bits']
            elif type(node['bits']) is str:

                bits = node['bits']
                if bits.startswith(':'):
                    self.bitCount = int(bits[1:])
                else:
                    bits = bits.replace(':','-').split("-")
                    if len(bits) == 1:
                        self.lowBit = int(bits[0])
                        self.highBit = int(bits[0])
                    elif len(bits) == 2:
                        start = int(bits[0])
                        end = int(bits[1])
                        self.lowBit = min(start, end)
                        self.highBit = max(start, end)
                    else:
                        print("Error: Invalid bits format in field: " + self.name)
            else:
                print("Error: Invalid bits format in field: " + self.name)
                self.lowBit = 0
                self.highBit = 0
        ...
        if 'vals' in node:
            if type(node['vals']) is dict:
                tmp = []
                for key, value in node['vals'].items():
                    tmp.append({key: value})
                node['vals'] = tmp
        ...
        if self.lowBit is not None and self.highBit is not None:
            self.bitCount = self.highBit - self.lowBit + 1
            self.mask = 0
        ...
        if self.mask:
            ...
            self.highBit = self.bitCount + self.offset -1
...
class DeviceRegSpec:
    ...
    def fillInFieldSpans(self):
        """
         Find the high and low bits of each field based on size if it is not specified
        """

        last = len(self.fields) 
        bitCount = 0

        highBit = 0

        #start at the end and work backwards so we always start with lowBit = 0
        for i, field in enumerate(reversed(self.fields)):

            if field.lowBit is None:
                if i == 0:
                    field.lowBit = 0
                    field.highBit = field.bitCount - 1
                else:
                    prevField = self.fields[last - i]
                    field.lowBit = prevField.highBit + 1
                    field.highBit = field.lowBit + field.bitCount - 1

            if field.highBit > highBit:
                highBit = field.highBit

        bitCount = highBit + 1
        
        print(f"{self.name} bitCount: {bitCount} size: {self.size * 8} fields: {len(self.fields)}")
        
        if bitCount != self.size * 8:
            self.size = bitCount // 8
            #self.type = "uint" + str(bitCount)
    ...
    def getNextfieldByhighBit(self, highBit):
        ...
        while search >= 0:
            search = search -1 
            for field in self.fields:
                if field.highBit == search:
                    return False , highBit - search
        
        return False, highBit
    ...
    def printFieldMap(self):
        ...
        i = self.size * 8
        while i > 0:
            field , nextStart = self.getNextfieldByhighBit(i)
            if field:
    ...
    def getRstTable(self):
        ...
        for field in self.fields:
            fieldBitMap[field.highBit] = field
...
class DeviceSpec:
    ...
    def parseYAML(self,yamlFile):
        ...
        if 'registers' in objDevice:
            ...
            #if its a dict, convert to a list
            if type(objDevice['registers']) is dict:
                tmp = []
                for key, value in objDevice['registers'].items():
                    tmp.append({key: value})
                objDevice['registers'] = tmp
        ...
        if 'fields' in objDevice:
            ...
            #if its a dict, convert to a list
            if type(objDevice['fields']) is dict:
                tmp = []
                for key, value in objDevice['fields'].items():
                    tmp.append({key: value})
                objDevice['fields'] = tmp
            ...
            for propNode in objDevice['fields']:
                if type(propNode) is dict:
                    ...
                    if type(propItem) is dict:
                        tmp = []
                        for key, value in propItem.items():
                            tmp.append({key: value})
                        propItem = tmp
                    ...
                    if regName not in self.regs:
                        newDevReg = DeviceRegSpec(regName)
                        self.addReg(newDevReg)
    ...
    def fillInAddresses(self):
        ...
        for key, reg in self.regs.items():
            ...
            if reg.addr is None:
                reg.addr = lastRegAddr + lastRegSize

            lastRegAddr = reg.addr
            lastRegSize = reg.size
