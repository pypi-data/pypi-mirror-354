...
# LangGraph-specific enums and models
...
class LangGraphState(BaseModel):
    """LangGraph state at a specific point in execution."""
...
class LangGraphNode(BaseModel):
    """Represents a LangGraph node execution."""

    node_id: str
    node_name: str
    node_type: LangGraphNodeType
    input_state: Optional[LangGraphState] = None
    output_state: Optional[LangGraphState] = None
...
class LangGraphExecution(BaseModel):
    """Complete LangGraph execution record."""

    execution_id: str
    graph_name: str
    graph_version: Optional[str] = None
    initial_state: LangGraphState
    final_state: Optional[LangGraphState] = None
    executed_nodes: List[LangGraphNode] = Field(default_factory=list)
    execution_path: List[str] = Field(default_factory=list)  # sequence of node_ids
    ...
    def add_node_execution(self, node: LangGraphNode) -> None:
    ...
    def get_failed_nodes(self) -> List[LangGraphNode]:
    ...
    def get_successful_nodes(self) -> List[LangGraphNode]:
    ...
    def calculate_success_rate(self) -> float:
        ...
        return successful / len(self.executed_nodes)
