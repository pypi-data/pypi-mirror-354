...
class GoogleDriveClient(CloudStorage):
    ...
    def connect(self):
        ...
        try:
            ...
            logger.authtest(
                f"Failed to connect to Drive API. Response: {e}"
            )
            return 1
    ...
    def upload(
        ...
        if drive:
            ...
            self.drive_id = self.get_drive_id(drive_id=self.shared_drive_name)
            # if the drive is provided, but not found, then the service account doesn't have access and needs it to be shared
            if not self.drive_id:
                raise ExitCodeException(
                    f"Invalid have access to the following drive: {self.shared_drive_name}. Please visit the authorization guide to see how to share the Drive to the service account",
        ...
        try:
            if drive_folder:
                ...
                if not self.folder_id and not self.is_folder_id(self.folder_name):
    ...
    def list_files(
        self, query, drive_id: Optional[str] = None, fields="id"
    ) -> List[Any]:
        ...
        Args:
            ...
            drive_id: The optional ID of the shared drive.
            fields: The fields to return in the response. Defaults to 'id'.
        ...
        if drive_id:
            ...
            results = self.service.files().list(q=query, fields=fields).execute()
