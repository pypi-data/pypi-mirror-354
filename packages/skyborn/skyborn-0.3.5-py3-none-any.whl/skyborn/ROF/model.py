...
class AttributionModel:
    ...
    def ols(self, Cf: np.ndarray, Proj: np.ndarray, Z2: np.ndarray, cons_test: Literal['AT99'] = 'AT99') -> Dict[str, np.ndarray]:
        ...
        beta_hat_inf = beta_hat - 2. * \
            stats.t.cdf(0.95, df=nz2) * np.sqrt(np.diag(Var_beta_hat).T)
        beta_hat_sup = beta_hat + 2. * \
            stats.t.cdf(0.95, df=nz2) * np.sqrt(np.diag(Var_beta_hat).T)

        # consistency check
        epsilon = _y - \
            np.linalg.multi_dot([self.X, np.linalg.inv(Proj), beta_hat.T])

        if cons_test == 'AT99':  # formula provided by Allen & Tett (1999)
            d_cons = np.linalg.multi_dot(
                [epsilon.T, np.linalg.pinv(Var_valid), epsilon]) / (self.nr - self.I)
        ...
        Beta_hat = {
            'beta_hat': beta_hat[0], 'beta_hat_inf': beta_hat_inf[0], 'beta_hat_sup': beta_hat_sup[0]}
    ...
    def ribes(self, Cxi: np.ndarray, Cy: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        y_star_hat = self.y + \
            np.linalg.multi_dot([Cy, np.linalg.inv(Cy + Cx), (X - self.y)])
        for i in range(Xi_star_hat.shape[1]):
            Xi_star_hat[:, i] = self.X[:, i] + \
                np.linalg.multi_dot(
        ...
        print('%30s: %.7f (%.7f)' % ('Internal variability only',
              chi2_test(d_cons, self.nt), np.exp(d_cons / -2.)))

        # (all forcings)
        d_cons = np.linalg.multi_dot(
            [(self.y - X).T, np.linalg.inv(Cy + Cx), (self.y - X)])
        print('%30s: %.7f (%.7f)' % ('All forcings', chi2_test(
            d_cons, self.nt), np.exp(d_cons / -2.)))

        # (individual forcings)
        for i in range(self.X.shape[1]):
            d_cons = np.linalg.multi_dot(
                [(self.y - self.X[:, i]).T, np.linalg.inv(Cy + Cxi[i]), (self.y - self.X[:, i])])
            print('%30s: %.7f (%.7f)' % ('Forcing no %d only' %
