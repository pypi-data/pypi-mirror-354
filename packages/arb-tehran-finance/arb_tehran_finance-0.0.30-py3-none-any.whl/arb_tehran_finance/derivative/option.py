...
def option_speculation(
    ...
    if long_target is not None:
        ...
        # فقط زمانی که long_target عددی باشد، مقایسه را انجام می‌دهیم
        if isinstance(long_target, (int, float)):
    ...
    if short_target is not None:
        ...
        # فقط زمانی که short_target عددی باشد، مقایسه را انجام می‌دهیم
        if isinstance(short_target, (int, float)):
