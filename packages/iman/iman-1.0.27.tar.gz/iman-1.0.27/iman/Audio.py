...
def ReadFFMPEG(filename,sr=16000,start_from=0,dur=-1,mono = False , ffmpeg_path='c:\\ffmpeg.exe',ffprobe_path='c:\\ffprobe.exe'):
           ...
           ffmpeg_command = [ffmpeg_path, '-ss' , str(start_from), '-i', filename,  '-ac' ,str(chnumber) , '-ar', str(sr), '-f', 'wav', 'pipe:1']
           
           if (dur!=-1):
                    ffmpeg_command = [ffmpeg_path, '-ss' , str(start_from), '-t' , str(dur) , '-i', filename, '-ac', str(chnumber), '-ar', str(sr), '-f', 'wav', 'pipe:1']
