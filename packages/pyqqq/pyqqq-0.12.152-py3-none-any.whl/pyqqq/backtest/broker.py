...
from pyqqq.data.domestic import get_ticker_info as get_kr_ticker_info, get_market_cap_by_codes
...
class MockBroker(BaseBroker):
    ...
    def show_trading_history_report(self, make_file: bool = False, filter_side: OrderSide = None, sort: str = None, invested_capital: int = 0):
        ...
        # 컬럼 순서 변경
        df = df[["주문번호", "종목코드", "종목명", "매매구분", "수량", "체결가", "평단가", "매수금액", "세금", "수수료", "손익금액", "손익률(%)", "최고가", "고가시간", "고가손익률(%)", "최저가", "저가시간", "저가손익률(%)", "시가총액", "전일거래대금"]]
        ...
        self.logger.info(f"{datestr} Total {count} trades, Sum PnL(%): {sum_pnl_rate:7.2f}%, Avg PnL/Max/Min(%):{avg_pnl_rate:>7.2f}%/{avg_max_pnl_rate:>7.2f}%/{avg_min_pnl_rate:>7.2f}% Earn/Buy={sum_pnl_value:9.0f}/{sum_buy_value:9.0f} INVESTED_CAPITAL={invested_capital:>9,.0f} ROI(%):{roi_rate:>7.2f}%")
