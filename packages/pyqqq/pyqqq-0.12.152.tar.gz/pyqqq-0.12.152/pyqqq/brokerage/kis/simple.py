...
class KISSimpleDomesticStock:
    ...
    def get_historical_daily_data(self, asset_code: str, first_date: dtm.date, last_date: dtm.date, adjusted_price: bool = True) -> pd.DataFrame:
        """
        일봉 데이터 검색
        ...
        for i in range(0, total_days + 1, max_days_per_request + 1):
            ...
            r = self.stock_api.inquire_daily_itemchartprice(asset_code, search_start, search_end, fid_period_div_code="D", fid_org_adj_prc="0" if adjusted_price else "1")
    ...
    def create_order(
        ...
        Args:
            ...
            order_type (OrderType): 주문 유형
            price (int): 주문 가격 (지정가 주문일 경우에만 필요)
            stop_price (Optional[int]): 스톱 트리거 가격 (스톱 주문일 경우에만 필요)
    ...
    def cancel_order(self, org_order_no: str, quantity: int = 0) -> str:
        ...
        """
        r = self.stock_api.order_rvsecncl(self.account_no, self.account_product_code, org_order_no, "00", "02", quantity, 0, "Y" if quantity == 0 else "N")  # 취소
