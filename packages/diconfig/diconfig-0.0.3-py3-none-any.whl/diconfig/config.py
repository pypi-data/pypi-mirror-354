...
"""
    ...
    def __init__(self, data: dict = None, file: str = "config", way: str = "toml", replace: bool = False,
        ...
        # 先处理文件扩展名再设置格式
        self.file = file  # 先保存原始文件名
        self.way = self.validate_format(way)
        self.file = self.ensure_extension(file)  # 确保扩展名正确
        # 根据文件名后缀自动修正格式类型
        if way == "toml":  # 只有默认格式时才自动检测
            file_way = os.path.splitext(self.file)[1][1:]  # 提取扩展名
            if file_way in ['json', 'toml', 'yaml', 'ini', 'xml']:  # 直接使用支持的格式列表
                self.way = self.validate_format(file_way)
    ...
    def save(self):
        ...
        with self._lock:
            ...
            try:
                ...
                with open(self.file, 'wb' if self.way in ["json", "xml"] else 'w',
                          encoding=None if self.way in ["json", "xml"] else 'utf-8') as f:
                    self.handler.save(self.data.to_dict(), f)
                self._dirty = False
            except Exception as e:
                print(f"保存配置文件失败 {self.file}: {e}")
