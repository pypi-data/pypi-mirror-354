...
from tensorkube.constants import REGION, Events, DEFAULT_NAMESPACE, get_cluster_name, CliColors, \
    get_cfn_base_stack_name, TeardownType, get_base_login_url
...
from tensorkube.services.aws_service import get_aws_account_id, get_aws_user_arn, \
    get_aws_user_principal_arn, get_session_region, check_and_install_aws_cli
...
from tensorkube.services.deploy import deploy_knative_service, DEFAULT_GPUS, DEFAULT_CPU, DEFAULT_MEMORY, \
    ...
    DEFAULT_GITHUB_ACTIONS, DEFAULT_DOMAIN_NAME, DEFAULT_GPU_TYPE, DEFAULT_ENTRYPOINT
...
from tensorkube.services.teardown_service import legacy_teardown, cfn_teardown
from tensorkube.services.s3_service import get_existing_bucket_name
...
def configure(vpc_public_subnets, vpc_private_subnets, legacy, test, override_policy_checks):
    ...
    track_event(Events.CONFIGURED_REGION.value,
            {"current_time": current_time, "region": session_region})    
    
    if session_region != REGION:
        confirmation = click.confirm(click.style( f"The current region is {session_region}. Tensorfuse recommends to use {REGION} region. If you wish to proceed with {session_region}", bold=True, fg = CliColors.WARNING.value) + click.style(" please select \"Yes\".", bold=True, fg = CliColors.WARNING.value), default=False)
        if not confirmation:
            click.echo("Configuration aborted.")
            end_time = time.time() * 1000
            track_event(Events.CONFIGURE_END.value,
                    {"start_time": start_time, "end_time": end_time, "duration": end_time - start_time, "status": "region_declined"})
            return
