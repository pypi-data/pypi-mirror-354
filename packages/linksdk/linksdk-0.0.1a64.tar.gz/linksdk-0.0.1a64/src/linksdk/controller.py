...
class WiimController:
    ...
    async def add_device(self, wiim_device: WiimDevice) -> None:
        ...
        if wiim_device.udn in self._devices:
            ...
            # If the device is already known, update its reference in case of new details
            # Or decide to just return if it's already there
            # For robust updates, you might want to replace the existing object or merge properties.
        ...
        # It's better to update multiroom status for this *new* device individually
        # or rely on async_update_all_multiroom_status being called after a batch of adds.
        # Calling async_update_all_multiroom_status here might be too frequent if many devices
        # are added at once. However, for a single add, it's fine.
    ...
    async def async_update_multiroom_status(self, leader_device: WiimDevice) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to get multiroom list for leader %s (UDN: %s): %s", leader_device.name, leader_device.udn, e)
            # If request fails, assume this device is no longer a leader of a group
            if leader_device.udn in self._multiroom_groups:
                del self._multiroom_groups[leader_device.udn]
        except (ValueError, TypeError, KeyError) as e:
            self.logger.error("Error parsing multiroom data for leader %s (UDN: %s): %s", leader_device.name, leader_device.udn, e, exc_info=True)
            if leader_device.udn in self._multiroom_groups:
                del self._multiroom_groups[leader_device.udn]
    ...
    def _restore_full_udn(self, short_uuid: str, leader_udn: str) -> str | None:
        ...
        from a shortened UUID often found in Linkplay's MULTIROOM_LIST.
        This restoration is heuristic and may need adjustment based on exact Linkplay behavior.
        The short_uuid from MULTIROOM_LIST response is often the last 12 chars of the full UDN,
        sometimes combined with the first 8 chars or a specific model identifier prefix.
        A common pattern is: `last 12 chars + first 8 chars of full UDN`.

        Given the WiimApp.restore_uuid function, let's try to mimic that.
        Original: `slave_uuid = restore_uuid(slave_uuid)` inside `async_update_multiroom_status`
        Where `restore_uuid` is:
        ```python
        def restore_uuid(cleaned):
            ...
            part3 = cleaned[12:16]
            part4 = cleaned[16:20]
            part5 = cleaned[20:] + prefix # This implies cleaned is 20 chars long, and prefix is first 8.
            full_uuid = f"uuid:{part1}-{part2}-{part3}-{part4}-{part5}"
            return full_uuid
        ```
        This looks like `cleaned` is a 20-character string. Let's make sure the short_uuid fits this.
        Example: `short_uuid` might be `12345678ABCD` (last 12 of full UDN)
        And the full UDN is `uuid:ABCDEF01-2345-6789-ABCD-1234567890AB`
        The `restore_uuid` in the SDK seems to assume `cleaned` is a concatenation of `(last 12) + (first 8)`.

        Let's try to reconstruct based on common UDN format `uuid:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`.
        The `short_uuid` from Linkplay's multiroom list often contains the last part of the UDN.
        A simpler heuristic is to just use the leader's UDN structure if the prefix/middle parts are consistent.
        """
        # A common Linkplay pattern for the 'uuid' in MULTIROOM_LIST is the MAC address or a device ID.
        # It's often related to the last 12 characters of the full UDN's last part (after the last '-').
        # If the short_uuid is 12 chars, it could be a MAC address.
        # If it's 20 chars, it might be the 'cleaned' format your previous code suggested.

        # Let's assume the most reliable method: we iterate through all known devices
        # and try to match the short_uuid against any part of their full UDN.
        # This is more robust than a fixed parsing heuristic if the format varies.
        for device_udn, device_obj in self._devices.items():
            # Remove 'uuid:' prefix and hyphens for comparison
            cleaned_udn = device_udn.replace("uuid:", "").replace("-", "")
            if short_uuid in cleaned_udn:
                return device_udn # Found a match

        # If no direct match, try the reconstruction logic from your previous SDK snippet
        # This part is highly dependent on how WiiM devices format their UUIDs in groups
        # If `short_uuid` is exactly 20 chars:
        if len(short_uuid) == 20:
            prefix = short_uuid[:8]
            part1 = short_uuid[0:8]
            part2 = short_uuid[8:12]
            part3 = short_uuid[12:16]
            part4 = short_uuid[16:20]
            # This 'part5' calculation seems specific to how it was cleaned
            # Assuming 'leader_udn' can help deduce the first 8 chars if they are model/batch specific
            leader_prefix = leader_udn.replace("uuid:", "")[:8]
            # If the short_uuid is based on a predictable pattern, this will work.
            # E.g., if the short_uuid is `LAST_12_CHARS` + `FIRST_8_CHARS_OF_FULL_UDN`
            # And `cleaned` in your previous `restore_uuid` is actually `LAST_12_CHARS + FIRST_8_CHARS_OF_FULL_UDN`
            # Then:
            # part5_from_short = short_uuid[12:20] # This would be the "first 8" if it's concat
            # But the logic `part5 = cleaned[20:] + prefix` is puzzling unless `cleaned` is >20
            
            # Let's just try to match based on substrings more simply for now if direct match fails.
            # Fallback to general pattern or search in known devices for similar suffixes.
            # The most reliable way is often that `short_uuid` IS the suffix.
            # Example: short_uuid = "C0A80025D7E8" (MAC)
            # Full UDN = "uuid:00000000-0000-0000-C0A8-0025D7E8xxxx" (last part is MAC, xxxx is unknown)
            # Given that leader UDN is `uuid:ABCDEF01-2345-6789-ABCD-1234567890AB`
            # And follower short_uuid is `1234567890AB` (last 12 chars)
            # You might assume the first parts are shared, or try to find a device ending with this short_uuid.
            for device_udn, device_obj in self._devices.items():
                if device_udn.endswith(short_uuid): # If short_uuid is the suffix of the full UDN
                    return device_udn
            
            # If still not found and short_uuid might be the "prefix" part of a common UDN pattern
            # this needs specific knowledge of WiiM UUID generation.
            # For robustness, we will assume short_uuid is a unique identifier within the SDK.
            # If it's truly a shortened form, your _devices map should eventually contain the full UDN.
            # The most reliable way is to iterate over `self._devices.keys()` and see if `short_uuid` is a substring
            # or if the `short_uuid` is the last part of a full UDN.

        SDK_LOGGER.warning("Could not restore full UDN for short UUID '%s'.", short_uuid)
        return None # Could not restore full UDN
    ...
    async def async_update_all_multiroom_status(self) -> None:
        ...
        # Iterate over a *copy* of self._devices.values() to prevent "dictionary changed size during iteration"
