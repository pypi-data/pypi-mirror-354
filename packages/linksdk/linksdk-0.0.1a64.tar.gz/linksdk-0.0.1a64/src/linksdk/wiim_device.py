...
class WiimDevice:
    ...
    async def async_set_play_mode(self, input_mode: str) -> None:
        ...
        if http_mode_val and self._http_api:
            try:
                ...
                self.logger.error("Device %s: Failed to set play mode to %s via HTTP: %s", self.name, input_mode, e)
                raise
        elif not self._http_api:
            self.logger.error("Device %s: HTTP API unavailable, cannot set play mode %s.", self.name, input_mode)
    ...
    async def async_set_output_mode(self, output_mode:str) -> None:
        ...
        if http_mode_val > 0 and http_mode_val != 64 and self._http_api:
            try:
                ...
                self.logger.error("Device %s: Failed to set output mode to %s via HTTP: %s", self.name, output_mode, e)
                raise
        elif http_mode_val == 64:
            self.logger.debug("other mode no action.")
        elif not self._http_api:
            self.logger.error("Device %s: HTTP API unavailable, cannot set output mode %s.", self.name, output_mode)
    ...
    def _parse_preset_data(self, xml_str: str) -> list:
        ...
        try:
            ...
            self.logger.error("Failed to parse KeyList XML: %s", e)
            return []
    ...
    def _parse_queue_data(self, xml_str: str) -> list:
        ...
        try:
            ...
            self.logger.error("Failed to parse queue XML: %s", e)
            return []
