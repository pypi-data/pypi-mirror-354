...
class LoginRequiredTestCaseMixin:
    ...
    def setUp(self):
        super().setUp()
        assert self.client.login(username="user@wearemoose.com", password="test")
...
class SuperuserRequiredTestCaseMixin(LoginRequiredTestCaseMixin):
    ...
    def setUpTestData(cls):
        ...
        get_user_model().objects.create_superuser(
            email="superuser@wearemoose.com",
            password="test",
            first_name="Super",
            last_name="User",
        )
    ...
    def setUp(self):
        super().setUp()
        assert self.client.login(username="superuser@wearemoose.com", password="test")

    def test_superuser_required(self):
        url = self.get_url()

        self.client.login(username="user@wearemoose.com", password="test")
...
class TemplateViewTestCaseMixin:
    ...
    def test_template_view(self):
        ...
        assert (
            resp.get("X-Frame-Options") == self.x_frame
        ), f"X-Frame-Options should be '{self.x_frame}', but is '{resp.get('X-Frame-Options')}'"
        assert resp.content
