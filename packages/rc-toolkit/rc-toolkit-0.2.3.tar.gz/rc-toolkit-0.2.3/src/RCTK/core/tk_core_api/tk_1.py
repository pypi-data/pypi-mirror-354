...
def tk_107830(name, globals=None, locals=None, fromlist=list(), level=0):
    return _tk_107830(name=name,globals=globals,locals=locals,fromlist=fromlist,level=level)

def tk_107831(name, globals=None, locals=None, fromlist=list(), level=0):
    """Fake import not call"""
    try:
        rt = importlib.__import__(name=name,globals=globals,locals=locals,fromlist=fromlist,level=level)
        return rt
    except ImportError:
