...
def extract_tensor(input_path, tensor_name, output_path="output.gguf"):
    with open(input_path, "rb") as f:
        ...
        for tensor in reader.tensors:
            if tensor.name == tensor_name:
                print(f"Extracting tensor: {tensor.name}")
...
def tensor_extractor():
    ...
    tensor_idx = int(input("\nSelect the tensor to EXTRACT by number: ")) - 1
    tensor_name = reader.tensors[tensor_idx].name
    extract_tensor(input_file, tensor_name)
    print(f"\nTensor '{tensor_name}' extracted. Output saved as 'output.gguf'.")
