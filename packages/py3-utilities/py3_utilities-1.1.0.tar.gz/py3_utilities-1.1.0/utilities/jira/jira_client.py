...
class JiraClient(UtilityBase):
            ...
            while counter < self.retry_cnt:
                async with self.session.get(url, headers=self.headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(f"Failed to fetch `{url}`. Response status: {response.status} ({counter+1}. attempt).")
                counter += 1

            self.logger.error(f"Failed to fetch `{url}` after {self.retry_cnt} retries.")
        except aiohttp.ClientConnectionError as e:
            self.logger.critical(f"Connection error while requesting `{url}`: {e}", exc_info=True)
            raise
        except aiohttp.ClientResponseError as e:
            self.logger.critical(f"Response error {e.status} while requesting `{url}`: {e}", exc_info=True)
            raise
        except aiohttp.ClientPayloadError as e:
            self.logger.critical(f"Payload error while requesting `{url}`: {e}", exc_info=True)
            raise
        except aiohttp.ClientError as e:
            self.logger.critical(f"General client error while requesting `{url}`: {e}", exc_info=True)
            raise
        except Exception as e:
            self.logger.critical(f"Unexpected error while requesting `{url}`: {e}", exc_info=True)
            raise
    ...
    async def read_custom_jql_keys(self, custom_jql: str) -> list[str]:
        ...
        while True:
            ...
            if not response_data or "issues" not in response_data:
                self.logger.error(f"Failed to retrieve result keys for query '{custom_jql}'")
                break
    ...
    async def read_issue(
        ...
        if issue_data and "fields" in issue_data:
            ...
            self.logger.error(f"Could not fetch issue {issue_id}")
    ...
    async def read_board_id(self, board_name: str) -> int | None:
        ...
        if response and len(response) > 0:
            ...
            self.logger.error(f"Could not retrieve ID for board `{board_name}`")
    ...
    async def read_sprint_list(self, 
        ...
        while True:
            ...
            if not response_data or "values" not in response_data:
                self.logger.error(f"Failed to retrieve sprints for board {board_id}")
                return None
