...
from typing import Callable, TypeVar, Any, Optional, Union, cast
            ...
            for attempt in range(retries):
                try:
                    ...
                    if hasattr(self, 'logger'):
                        self.logger.critical(f"Exception in {func.__name__} (attempt {attempt + 1}/{retries}): {e}", exc_info=True)
...
class GitlabClient(UtilityBase):
    ...
    def _warn_large(self, resource, project_path: str, label: str) -> None:
        ...
        if count > self.large_repo_threshold:
            self.logger.warning(f"Project '{project_path}' has {count}+ {label}. all=True may be slow.")
    ...
    @handle_gitlab_errors(default_return=None)
    def get_branch_mr(
        self, project_path: str, branch_name: str, state: str = "all"
    ) -> Optional[MergeRequestMetadata]:
        """
        Get parent merge request metadata for a branch if it exists.
    ...
    def checkout_branch(self, local_path: str, branch: str) -> None:
        ...
        """
        Repo(local_path).git.checkout(branch)
