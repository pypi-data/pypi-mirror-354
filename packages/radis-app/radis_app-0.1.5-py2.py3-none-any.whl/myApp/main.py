...
def show_eula_gui():
    ...
    license_file = importlib.resources.files("myApp").joinpath("LICENSE")
    if license_file.is_file():
        formal_license = license_file.read_text()
...
#        with importlib.resources.files("myApp.data").joinpath("LICENSE").open("r") as license_file:
    ...
    eula_file = importlib.resources.files("myApp").joinpath("EULA")
    if eula_file.is_file():
        eula_license = eula_file.read_text()
        text_widget.delete(1.0, tk.END)  # Clear previous content
        text_widget.insert(tk.END, formal_license)
