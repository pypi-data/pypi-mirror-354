...
class Litefold(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Litefold client instance.

        This automatically infers the `api_key` argument from the `LITEFOLD_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("LITEFOLD_API_KEY")
        if api_key is None:
            raise LitefoldError(
                "The api_key client option must be set either by passing api_key to the client or by setting the LITEFOLD_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("LITEFOLD_BASE_URL")
        if base_url is None:
...
class AsyncLitefold(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncLitefold client instance.
