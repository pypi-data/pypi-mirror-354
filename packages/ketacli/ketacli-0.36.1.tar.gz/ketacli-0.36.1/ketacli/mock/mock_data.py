...
@command
def mock_data(repo="default", data=None, file=None, number=1, batch=None, gzip=False, workers=None, output_type="server", output_file=None, render=False):
    """
    ...
    # 优化批处理大小，根据总数和工作进程数动态调整
    if batch is None:
        # 根据数据量动态调整批次大小
        if number <= 1000:
            batch = min(100, number)  # 小数据量使用较小批次
        elif number <= 10000:
            batch = min(500, number)  # 中等数据量
        else:
            batch = min(2000, number)  # 增加大数据量的批次大小
    
    # 优化工作进程数
    if workers is None:
        # 根据CPU核心数和数据量动态调整工作进程数
        cpu_count = multiprocessing.cpu_count()
        if number <= 1000:
            workers = min(2, cpu_count)  # 小数据量使用较少进程
        elif number <= 10000:
            workers = min(cpu_count - 1, 4)  # 中等数据量
        else:
            workers = min(cpu_count, 8)  # 大数据量使用更多进程，但不超过8个
    ...
    with Progress(console=console) as prog:
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            # 按工作进程ID分组任务，确保每个工作进程的任务被一起提交
            tasks_by_worker = {}
            for task_info in worker_tasks:
                worker_id = task_info["worker_id"]
                if worker_id not in tasks_by_worker:
                    tasks_by_worker[worker_id] = []
                tasks_by_worker[worker_id].append(task_info)
            
            # 先提交每个工作进程的第一个任务，确保所有工作进程同时开始工作
            for worker_id in sorted(tasks_by_worker.keys()):
                if tasks_by_worker[worker_id]:
                    task_info = tasks_by_worker[worker_id][0]
                    futures.append(
                        executor.submit(
                            generate_and_upload, 
                            data, 
                            ...
                            output_type, 
                            output_file,
                            task_info["worker_id"] if output_type == 'file' and workers > 1 else None,  # 传递worker_id用于创建临时文件
                            render
                        )
                    )
                    # 移除已提交的任务
                    tasks_by_worker[worker_id] = tasks_by_worker[worker_id][1:]
                ...
                try:
                    ...
                    if resp:
                        ...
                        if output_type == 'file' and 'temp_file' in resp:
                            temp_files.append(resp['temp_file'])
                except Exception as e:
...
@command
def mock_log(repo="default", data=None, file=None, number=1, batch=None, gzip=False, workers=None, output_type="server", output_file=None, render=False):
...
@command
def mock_metrics(repo="metrics_keta", data=None, file=None, number=1, batch=None, gzip=False, workers=None, output_type="server", output_file=None, render=False):
