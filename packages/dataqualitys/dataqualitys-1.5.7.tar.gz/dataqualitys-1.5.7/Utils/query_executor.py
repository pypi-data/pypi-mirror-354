...
class QueryExecutor:
    def __init__(self, connector, db_type: str, config: Dict):
        ...
        # Local template paths for dev
        local_paths = [
            os.path.join(os.path.dirname(__file__), '..', 'templates', db_type),
            os.path.join(os.path.dirname(__file__), '..', 'templates', 'common')
        ]

        # Normalize paths
        local_paths = [os.path.abspath(path) for path in local_paths]

        # Use both FileSystemLoader (for dev) and PackageLoader (for installed)
        self.env = Environment(
            loader=ChoiceLoader([
                FileSystemLoader(local_paths),
                PackageLoader('dataqualitys', 'templates')
            ]),
            trim_blocks=True,
            lstrip_blocks=True
        )
