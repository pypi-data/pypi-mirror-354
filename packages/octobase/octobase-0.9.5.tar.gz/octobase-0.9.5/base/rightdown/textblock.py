...
class Text(Block):
  ...
  def AdjustAlign(self):
    ...
    lpushr          = self.children[0].tokentype == SNIPTYPE_FORMAT_RIGHT
    lpushl          = self.children[0].tokentype == SNIPTYPE_FORMAT_LEFT
    rpushr          = self.children[-1].tokentype == SNIPTYPE_FORMAT_RIGHT
    rpushl          = self.children[-1].tokentype == SNIPTYPE_FORMAT_LEFT

    # XYZZY:  weird bug:  lpushr is not detected if the text block contains a link
    # (not fixing today, just noting it's here) 2025-05
