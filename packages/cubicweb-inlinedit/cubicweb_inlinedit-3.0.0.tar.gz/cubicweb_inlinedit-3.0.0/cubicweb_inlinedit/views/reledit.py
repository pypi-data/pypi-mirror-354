...
from cubicweb_web.views import uicfg, reledit as old_cw_reledit
    ...
    def call(self, action=None, reload=False, evid=None, **kwargs):
        ...
        with t.div(w, Class=self.__regid__):
            for i in range(len(rset)):
                with t.div(w, Class=f"{self.__regid__}-item"):
                    self.wview(
                        self.__regid__,
                        rset,
                        row=i,
...
class AutoClickAndEditFormView(EntityView):
    ...
    def _may_add_related(self, rschema, role):
        ttypes = rschema.targets(entity_schema=self.entity.e_schema.type, role=role)
...
class ReleditFormHandler(appobject.AppObject):
    ...
    def wrap_form(
        ...
        with t.div(
            w,
            id=f"{divid}-reledit",
            onmouseout=f"jQuery('#{divid}').addClass('hidden')",
            onmouseover=f"jQuery('#{divid}').removeClass('hidden')",
            Class="releditField",
        ):
            with t.div(w, id=f"{divid}-value", Class="editableFieldValue"):
                w(value)
