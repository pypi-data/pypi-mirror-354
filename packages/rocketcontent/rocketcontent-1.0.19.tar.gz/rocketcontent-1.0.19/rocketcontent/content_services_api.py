...
class ContentServicesApi:
    ...
    def get_content_classes(self, output_file="content_classes.json"):
        """
        Retrieves the content classes from the Content Repository and saves them to a JSON file.

        Args:
            output_file (str, optional): Output file name. Defaults to "content_classes.json".
        """
        repo = ContentRepository(self.config)
        repo.get_content_classes(output_file)


    def get_indexes(self, output_file="index.json"):
        """
        Retrieves the index from the Content Repository and saves them to a JSON file.

        Args:
            output_file: [Mandatory]  Output file name. Defaults to "index.json".
        """    
        repo = ContentRepository(self.config)
        repo.get_indexes(output_file)
    ...
    def archive_policy(self, file_path, policy_name, xml_metadata=""):
        """
        ...
        return archive_obj.archive_policy(file_path, policy_name, xml_metadata)
