...
class ContentConfig:
    ...
    def __init__(self, yaml_file):
        ...
        try:
            ...
            self.repo_id = "" 
        # Content source ID for using Content Repository Admin Rest API
        try:
            self.content_source_id = content_config.get('content_source_id')
        except:
            self.content_source_id = ""   
        # Content repository ENC ID for using Content Repository Admin Rest API
        try:
        ...
        # Log Level
        try:
            self.log_level = content_config.get('log_level')
        except:
            self.log_level = "INFO"
        #---------------
        # Log directory
        try:
            self.log_dir = content_config.get('log_dir')

            if not self.log_dir:
                self.log_dir = ""
                main_program_directory = self.get_main_program_directory()
                self.log_dir  = os.path.abspath(os.path.join(main_program_directory, './log'))
                if not os.path.exists(self.log_dir ):
                    os.makedirs(self.log_dir)
        except:
            # Creates default directory for logging
            self.log_dir = ""
            main_program_directory = self.get_main_program_directory()
            self.log_dir  = os.path.abspath(os.path.join(main_program_directory, './log'))
            if not os.path.exists(self.log_dir ):
        ...
        if not os.path.exists(self.log_dir):
            raise FileExistsError(f"Log path '{self.log_dir}' not found.")
        
        # End of Log directory configuration
        #---------------

        #---------------
        #  Set Logging configuration
        if self.log_level not in ContentConfig.VALID_DEBUG_LEVELS:
            self.log_level = "INFO"
        ...
        self.logger.info("                /     __      ___ __     ___     /\    __ ")
        self.logger.info("                |    |  | |\ | | |__ |\ | |     /__\  |__| |")   
        self.logger.info("                \___ |__| | \| | |__ | \| |    /    \ |    |")
        self.logger.info("---------------------------------------------------------------------------")
    ...
    def get_current_config(self):
        ...
        content_config["repository"]["log_dir"] = self.log_dir        
    ...
    # Obtain the directory of main program this is for establish log file directory by default
    def get_main_program_directory(self):
        """
        Gets the directory of the main program.

        Returns:
            str: The absolute path of the main program's directory, or None if not found.
        """
        call_stack = inspect.stack()
        for frame in call_stack:
            if frame.filename == inspect.stack()[-1].filename:
    ...
    def get_repo_id(self):
        ...
        try:
            ...
            self.logger.info("--------------------------------")
            self.logger.debug("Method : get_repo_id")
            self.logger.info("Getting new repo ID from rocketcontent...")
            self.logger.info (f"Request URL: {repositories_url}")               

            response = requests.get(repositories_url, verify=False)  # Make a GET request to the specified URL
    ...
    def get_vdr_content_sources(self):
        try:
            ...
            self.logger.info("--------------------------------")
            self.logger.debug("Method : get_vdr_content_sources")
            self.logger.info(f"VDR Content Sources URL : {content_sources_url}")
            ...
            self.logger.info("--------------------------------")
            self.logger.debug("Method : get_vdr_content_sources")
            self.logger.info(f"VDR Repositories URL : {content_sources_url}")
            self.logger.debug(f"Headers : {json.dumps(headers)}")

            vdrrepositories_url= self.base_url + f"/mobius/adminrest/vdrrepositories?contentsourceid={self.content_source_id}"
            
            headers = {
                'Accept': 'application/vnd.asg-mobius-admin-vdrrepositories.v1+json',
                      'Authorization': f'Basic {self.encoded_credentials}' }
                        
            response = requests.get(vdrrepositories_url, headers=headers, verify=False)
    ...
    def get_repo_capabilities(self):
        ...
        try:
            ...
            self.logger.info("--------------------------------")
            self.logger.debug("Method : get_repo_capabilities")
            self.logger.info("Checking if Content Repository is active ...")
            self.logger.info (f"Request URL: {capabilities_url}")
            self.logger.debug (f"Repository ID: {self.repo_id}")  
