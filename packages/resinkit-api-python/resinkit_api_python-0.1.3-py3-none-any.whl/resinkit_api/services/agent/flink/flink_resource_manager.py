...
class FlinkResourcesResult(BaseModel):
    """
    Result model for processed Flink resources.
    """
...
class FlinkResourceManager:
    ...
    def __init__(self):
        ...
        self.temp_dir = str(get_resource_manager().create_temp_dir("flink_resources_"))
        self.downloaded_resources: Dict[str, str] = {}  # Maps URLs to local paths

    async def process_resources(self, resources_config: Dict[str, Any]) -> FlinkResourcesResult:
        """
        ...
        Returns:
            FlinkResourcesResult with resource types and lists of file paths
        """
        result = FlinkResourcesResult()
    ...
    def cleanup(self):
        ...
        if os.path.exists(self.temp_dir):
            try:
                get_resource_manager().cleanup_dir(Path(self.temp_dir))
