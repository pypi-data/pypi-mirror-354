...
# def plsqlxy(
#     query: str,
#     page_size: int = 10_00_000,
#     verify_tls: bool = True,
#     debug: bool = False,
# ) -> pl.DataFrame:
#     # (1) Build auth/session exactly as your get_connection does…
#     conn_dict = pb.connections.get_conn_dict("[Warehouse] Trino")
#     host, port = conn_dict["host"], int(conn_dict["port"])
#     user       = conn_dict.get("user", conn_dict.get("username"))
#     pwd        = conn_dict["password"]
#     p = urlparse(conn_dict["uri"])
#     catalog, schema = p.path.lstrip("/").split("/",1)
#     source = "jhub" if "JUPYTERHUB_USER" in os.environ else "common_airflow"

#     sess = requests.Session()
#     sess.auth   = requests.auth.HTTPBasicAuth(user, pwd)
#     sess.verify = verify_tls

#     base    = f"https://{host}:{port}/" if verify_tls else f"http://{host}:{port}/"
#     headers = {
#         "X-Trino-Catalog": catalog,
#         "X-Trino-Schema":  schema,
#         "X-Trino-Source":  source,
#         "X-Trino-User":    os.environ.get("JUPYTERHUB_USER", user),
#         "Accept":          "application/json",
#     }

#     # (2) Kick off the query
#     resp = sess.post(urljoin(base, "v1/statement"), data=query, headers=headers)
#     resp.raise_for_status()
#     job = resp.json()

#     # (3) Wait for the first “columns” page
#     while "columns" not in job:
#         resp = sess.get(job["nextUri"], headers=headers); resp.raise_for_status()
#         job = resp.json()
#         if job.get("error"):
#             e = job["error"]
#             raise RuntimeError(f"{e['errorCode']}: {e['message']}")

#     names = [c["name"] for c in job["columns"]]

#     # (4) Stream each page into a Polars chunk
#     chunks = []
#     # first page
#     rows = job.get("data", [])
#     if rows:
...
#         )
#         chunks.append(pl.from_arrow(tbl))
#         if debug: print(f"page… → {len(rows)}")

#     # (5) Concatenate all your Polars chunks (each was a modest‐sized batch)
#     if not chunks:
#         return pl.DataFrame()
