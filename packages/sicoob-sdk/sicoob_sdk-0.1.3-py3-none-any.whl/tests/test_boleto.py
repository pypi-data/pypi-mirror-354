...
def test_emitir_boleto_sucesso(boleto_client):
    ...
    dados = {
        ...
        "modalidade": 1,
        "valor": 100.50,
        "beneficiario": {"nome": "Teste Beneficiário", "documento": "12345678901"},
    }
...
def test_emitir_boleto_erro_http(boleto_client):
    ...
    dados = {"numeroContrato": 123456, "valor": 100.50}
...
def test_emitir_boleto_erro_generico(boleto_client):
...
def test_consultar_boleto_sucesso(boleto_client):
    ...
    assert result == {"nossoNumero": "123456789", "situacao": "REGISTRADO"}
...
def test_consultar_boleto_nao_encontrado(boleto_client):
...
def test_consultar_boleto_erro_http(boleto_client):
...
def test_consultar_boleto_erro_generico(boleto_client):
...
def test_consultar_boletos_por_pagador_sucesso(boleto_client):
    ...
    # Verificações
    assert len(result["boletos"]) == 2
    boleto_client.session.get.assert_called_once()
    args, kwargs = boleto_client.session.get.call_args
    assert "pagadores/12345678901/boletos" in args[0]
...
def test_consultar_boletos_por_pagador_apenas_obrigatorios(boleto_client):
...
def test_consultar_boletos_por_pagador_erro_http(boleto_client):
    ...
    assert "[500] Falha na consulta de boletos por pagador - Status: 500" in str(exc_info.value)
    assert exc_info.value.numero_cpf_cnpj == "12345678901"


def test_consultar_boletos_por_pagador_erro_generico(boleto_client):
    ...
    assert "Falha na comunicação com API de boletos: Erro de conexão" in str(exc_info.value)
    assert exc_info.value.numero_cpf_cnpj == "12345678901"


def test_atualizar_webhook_sucesso(boleto_client):
...
def test_atualizar_webhook_erro_negocio(boleto_client):
    ...
    mock_response.json.return_value = {
        "mensagens": [{
            "mensagem": "URL de webhook inválida",
            "codigo": "ERR001"
        }]
    }
    boleto_client.session.patch.return_value = mock_response
    ...
    # Verificações
    assert "[400] Falha na atualização do webhook: URL de webhook inválida" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.id_webhook == id_webhook
    assert exc_info.value.dados_webhook == webhook


def test_atualizar_webhook_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na atualização do webhook: Email inválido" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.id_webhook == id_webhook
    assert exc_info.value.dados_webhook == webhook


def test_atualizar_webhook_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na atualização do webhook: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.id_webhook == id_webhook
    assert exc_info.value.dados_webhook == webhook


def test_atualizar_webhook_erro_comunicacao(boleto_client):
...
def test_emitir_segunda_via_sucesso(boleto_client):
...
def test_emitir_segunda_via_com_pdf(boleto_client):
    ...
    args, kwargs = boleto_client.session.get.call_args
    assert kwargs["params"]["linhaDigitavel"] == "00190000090123456789012345678901234567890000"
    assert kwargs["params"]["gerarPdf"] == "true"


def test_emitir_segunda_via_com_codigo_barras(boleto_client):
    ...
    assert "linhaDigitavel" in result
    args, kwargs = boleto_client.session.get.call_args
    assert kwargs["params"]["codigoBarras"] == "00190000090123456789012345678901234567890000"
    assert "nossoNumero" not in kwargs["params"]
    assert "linhaDigitavel" not in kwargs["params"]


def test_emitir_segunda_via_erro_http(boleto_client):
...
def test_emitir_segunda_via_erro_generico(boleto_client):
...
def test_consultar_faixas_nosso_numero_sucesso(boleto_client):
    ...
    # Verificações
    assert result["numeroInicial"] == 1000
    assert result["numeroFinal"] == 1100
    assert result["validaDigitoVerificadorNossoNumero"] == 0
    boleto_client.session.get.assert_called_once()
    args, kwargs = boleto_client.session.get.call_args
...
def test_consultar_faixas_nosso_numero_sem_contrato(boleto_client):
    ...
    # Verificações
    assert result["validaDigitoVerificadorNossoNumero"] == 1
    args, kwargs = boleto_client.session.get.call_args
    assert "numeroContratoCobranca" not in kwargs["params"]


def test_consultar_faixas_nosso_numero_erro_http(boleto_client):
...
def test_consultar_faixas_nosso_numero_erro_generico(boleto_client):
...
def test_consultar_faixas_nosso_numero_estrutura_resultado(boleto_client):
    ...
    # Verificações
    assert result["numeroInicial"] == 1
    assert result["numeroFinal"] == 10
    assert result["validaDigitoVerificadorNossoNumero"] == 1  # Deve converter True para 1


def test_consultar_faixas_nosso_numero_erro_negocio(boleto_client):
...
def test_consultar_faixas_nosso_numero_erro_dados(boleto_client):
...
def test_consultar_faixas_nosso_numero_sem_resultados(boleto_client):
...
def test_alterar_boleto_sucesso(boleto_client):
...
def test_alterar_boleto_erro_negocio(boleto_client):
    ...
    dados_alteracao = {
        "seuNumero": {
            "seuNumero": "123",
            "identificacaoBoletoEmpresa": "123"
        }
    }
    client_id = "client-id-123"
...
def test_alterar_boleto_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na alteração do boleto: Dados inconsistentes" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.nosso_numero == str(nosso_numero)
    assert exc_info.value.dados_alteracao == dados_alteracao


def test_alterar_boleto_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na alteração do boleto: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.nosso_numero == str(nosso_numero)
    assert exc_info.value.dados_alteracao == dados_alteracao


def test_alterar_boleto_erro_comunicacao(boleto_client):
...
def test_baixar_boleto_sucesso(boleto_client):
...
def test_baixar_boleto_erro_negocio(boleto_client):
...
def test_baixar_boleto_erro_dados_inconsistentes(boleto_client):
...
def test_baixar_boleto_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na baixa do boleto: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.nosso_numero == nosso_numero
    assert exc_info.value.dados_boleto == dados_boleto


def test_baixar_boleto_erro_comunicacao(boleto_client):
...
def test_alterar_pagador_sucesso(boleto_client):
    ...
    # Chama o método
    result = boleto_client.alterar_pagador(
        pagador=pagador,
        client_id=client_id
    )
...
def test_alterar_pagador_erro_negocio(boleto_client):
    ...
    with pytest.raises(BoletoAlteracaoPagadorError) as exc_info:
        boleto_client.alterar_pagador(
            pagador=pagador,
            client_id=client_id
        )

    # Verificações
    assert "[400] Falha na alteração do pagador: Pagador não encontrado" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.numero_cpf_cnpj == "12345678901"
    assert exc_info.value.dados_pagador == pagador


def test_alterar_pagador_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na alteração do pagador: Dados inconsistentes" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.numero_cpf_cnpj == "12345678901"
    assert exc_info.value.dados_pagador == pagador


def test_alterar_pagador_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na alteração do pagador: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.numero_cpf_cnpj == "12345678901"
    assert exc_info.value.dados_pagador == pagador


def test_alterar_pagador_erro_comunicacao(boleto_client):
...
def test_cadastrar_webhook_sucesso(boleto_client):
    ...
    # Chama o método
    result = boleto_client.cadastrar_webhook(
        webhook=webhook,
        client_id=client_id
    )
...
def test_cadastrar_webhook_erro_negocio(boleto_client):
    ...
    # Verifica se exceção é levantada
    with pytest.raises(BoletoWebhookError) as exc_info:
        boleto_client.cadastrar_webhook(
            webhook=webhook,
            client_id=client_id
        )

    # Verificações
    assert "[400] Falha no cadastro do webhook: URL de webhook inválida" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.url == "url-invalida"
    assert exc_info.value.dados_webhook == webhook


def test_cadastrar_webhook_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha no cadastro do webhook: Tipo de movimento inválido" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.url == webhook["url"]
    assert exc_info.value.dados_webhook == webhook


def test_cadastrar_webhook_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha no cadastro do webhook: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.url == webhook["url"]
    assert exc_info.value.dados_webhook == webhook


def test_cadastrar_webhook_erro_comunicacao(boleto_client):
...
def test_consultar_webhook_sucesso(boleto_client):
...
def test_consultar_webhook_erro_negocio(boleto_client):
    ...
    # Verificações
    assert "[400] Falha na consulta do webhook: Webhook não encontrado" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_webhook_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na consulta do webhook: Tipo de movimento inválido" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_webhook_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na consulta do webhook: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_webhook_erro_comunicacao(boleto_client):
...
def test_excluir_webhook_sucesso(boleto_client):
    ...
    # Chama o método
    result = boleto_client.excluir_webhook(
        id_webhook=id_webhook,
        client_id=client_id
    )
...
def test_excluir_webhook_erro_negocio(boleto_client):
    ...
    # Verifica se exceção é levantada
    with pytest.raises(BoletoWebhookError) as exc_info:
        boleto_client.excluir_webhook(
            id_webhook=id_webhook,
            client_id=client_id
        )

    # Verificações
    assert "[400] Falha na exclusão do webhook: Webhook já excluído" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.id_webhook == id_webhook


def test_excluir_webhook_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na exclusão do webhook: Webhook não encontrado" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.id_webhook == id_webhook


def test_excluir_webhook_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na exclusão do webhook: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.id_webhook == id_webhook


def test_excluir_webhook_erro_comunicacao(boleto_client):
...
def test_consultar_solicitacoes_webhook_sucesso(boleto_client):
    ...
    assert "resultado" in result
    assert len(result["resultado"]) == 1
    assert result["resultado"][0]["webhookSolicitacoes"][0]["codigoSolicitacaoSituacao"] == 3
    boleto_client.session.get.assert_called_once()
    args, kwargs = boleto_client.session.get.call_args
...
def test_consultar_solicitacoes_webhook_erro_negocio(boleto_client):
    ...
    mock_response.json.return_value = {
        "mensagens": [{
            "mensagem": "Data de solicitação inválida",
            "codigo": "ERR001"
        }]
    }
    boleto_client.session.get.return_value = mock_response
    ...
    # Verificações
    assert "[400] Falha na consulta das solicitações do webhook: Data de solicitação inválida" in str(exc_info.value)
    assert exc_info.value.code == 400
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_solicitacoes_webhook_erro_dados_inconsistentes(boleto_client):
    ...
    # Verificações
    assert "[406] Falha na consulta das solicitações do webhook: Webhook não encontrado" in str(exc_info.value)
    assert exc_info.value.code == 406
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_solicitacoes_webhook_erro_interno(boleto_client):
    ...
    # Verificações
    assert "[500] Falha na consulta das solicitações do webhook: Erro interno no servidor" in str(exc_info.value)
    assert exc_info.value.code == 500
    assert exc_info.value.id_webhook == id_webhook


def test_consultar_solicitacoes_webhook_erro_comunicacao(boleto_client):
