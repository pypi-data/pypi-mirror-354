...
def test_extrato_sucesso(conta_corrente_client):
    ...
    result = conta_corrente_client.extrato(
        ...
        dia_inicial=1,
        dia_final=30,
        numero_conta_corrente=12345
    )

    # Verificações
    assert len(result) == 1
    assert result[0]["transacao"] == "DEPOSITO"
    ...
    assert str(kwargs["params"]["numeroContaCorrente"]) == "12345"  # Aceita int ou string

def test_extrato_erro(conta_corrente_client):
...
def test_saldo_sucesso(conta_corrente_client):
    ...
    assert result == {"saldo": 1500.75}
    conta_corrente_client.session.get.assert_called_once()
    args, kwargs = conta_corrente_client.session.get.call_args
    assert "saldo" in args[0]
    assert kwargs["params"]["numeroConta"] == "12345"

def test_saldo_sem_numero_conta(conta_corrente_client):
...
def test_transferencia_sucesso(conta_corrente_client):
    ...
    assert result == {"status": "PROCESSADA"}
...
def test_transferencia_minima(conta_corrente_client):
...
def test_transferencia_erro(conta_corrente_client):
    ...
    with pytest.raises(TransferenciaError) as exc_info:
        conta_corrente_client.transferencia(
            valor=500.00,
            conta_destino="54321"
        )
    assert "Erro API" in str(exc_info.value)
