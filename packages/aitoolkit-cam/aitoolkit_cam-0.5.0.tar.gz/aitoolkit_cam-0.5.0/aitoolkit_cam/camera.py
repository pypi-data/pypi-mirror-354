...
class Camera:
    ...
    def find_first_camera_fast(max_test=5):
        ...
        """
        logger.info(f"🚀 正在快速检测摄像头 (最多测试 {max_test} 个索引)...")
    ...
    def get_default_camera():
        ...
        返回:
            int: 第一个可用的摄像头索引，如果未找到则引发RuntimeError
        """
        result = Camera.find_first_camera_fast(max_test=10)
        if result is None:
            logger.error("无法找到任何可用摄像头！请检查：")
            logger.error("1. 摄像头是否正确连接且未被其他程序占用。")
            logger.error("2. 摄像头驱动是否已正确安装。")
            logger.error("3. 当前程序是否有权限访问摄像头。")
            if sys.platform.startswith('linux'):
                logger.error("4. Linux系统可尝试: sudo usermod -a -G video $USER")
            raise RuntimeError("自动检测失败，未找到可用摄像头。")
        return result
