...
from .utils import quantize_2bit_with_qmap, set_block_size
...
QMAP_SIGNED_LINEAR = torch.linspace(-1, 1, 4).tolist()
QMAP_UNSIGNED_LINEAR = torch.linspace(0, 1, 5)[1:].tolist()


class LinearSROptimState2bit(TorchAOBaseTensor):
...
if not TORCH_VERSION_AT_LEAST_2_4:
    ...
    LinearSROptimState2bit.to = _to
    del _to  # make sure to not re-use


@LinearSROptimState2bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, LinearSROptimState2bit) and isinstance(src, LinearSROptimState2bit):
        assert (
    ...
    elif isinstance(dst, LinearSROptimState2bit):
        scaled_src, scale = scale_tensor(src.view(-1), dst.block_size)
        codes = quantize_2bit_with_qmap(scaled_src, dst.qmap, stochastic_rounding=True)
        dst.codes.copy_(
...
@LinearSROptimState2bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = LinearSROptimState2bit(
...
@LinearSROptimState2bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, LinearSROptimState2bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@LinearSROptimState2bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return LinearSROptimState2bit(x.codes, x.scale, x.qmap, x.signed, x._shape)

    if len(shape) == 1 and shape[0] == -1:
        return LinearSROptimState2bit(x.codes, x.scale, x.qmap, x.signed, (x.numel(),))
...
@LinearSROptimState2bit.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, LinearSROptimState2bit):
        raise ValueError(f"expecting a LinearSROptimState2bit but found {type(x)}")
    ...
    return LinearSROptimState2bit(codes, scale, x.qmap.clone(), x.signed, shape)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@LinearSROptimState2bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@LinearSROptimState2bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([LinearSROptimState2bit])