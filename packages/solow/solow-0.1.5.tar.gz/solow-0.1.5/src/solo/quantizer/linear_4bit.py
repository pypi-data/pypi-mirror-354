...
class LinearOptimState4bit(TorchAOBaseTensor):
...
if not TORCH_VERSION_AT_LEAST_2_4:
    ...
    LinearOptimState4bit.to = _to
    del _to  # make sure to not re-use


@LinearOptimState4bit.implements(aten.copy_.default)
def _(func, types, args, kwargs):
    dst = args[0]
    src = args[1]

    if isinstance(dst, LinearOptimState4bit) and isinstance(src, LinearOptimState4bit):
        assert (
    ...
    elif isinstance(dst, LinearOptimState4bit):
        scaled_src, scale = scale_tensor(src.view(-1), dst.block_size)
        codes = quantize_4bit_with_qmap(scaled_src, dst.qmap, stochastic_rounding=False)
...
@LinearOptimState4bit.implements(aten._to_copy.default)
def _(func, types, args, kwargs):
    # ignore dtype
    device = kwargs.get("device", None)
    out = LinearOptimState4bit(
...
@LinearOptimState4bit.implements(aten.lerp.Scalar)
def _(func, types, args, kwargs):
    args = [x.dequantize() if isinstance(x, LinearOptimState4bit) else x for x in args]
    return func(*args, **kwargs)


# this is needed for DTensor.from_local() and for flattening tensor
@LinearOptimState4bit.implements(aten.view.default)
def _(func, types, args, kwargs):
    x, shape = args

    if tuple(x.shape) == tuple(shape):
        return LinearOptimState4bit(x.codes, x.scale, x.qmap, x.signed, x._shape)

    if len(shape) == 1 and shape[0] == -1:
        return LinearOptimState4bit(x.codes, x.scale, x.qmap, x.signed, (x.numel(),))
...
@LinearOptimState4bit.implements(
    [
...
def _(func, types, args, kwargs):
    x = args[0]
    if not isinstance(x, LinearOptimState4bit):
        raise ValueError(f"expecting a LinearOptimState4bit but found {type(x)}")
    ...
    return LinearOptimState4bit(codes, scale, x.qmap.clone(), x.signed, shape)


# required by torch.distributed.checkpoint.save
# note that we don't actually implement pin memory for this tensor subclass
# (pin_memory argument is ignored in aten._to_copy)
@LinearOptimState4bit.implements(aten.is_pinned.default)
def _(func, types, args, kwargs):
    return (
...
@LinearOptimState4bit.implements(aten.slice.Tensor)
def _(func, types, args, kwargs):
...
if TORCH_VERSION_AT_LEAST_2_5:
    ...
    add_safe_globals([LinearOptimState4bit])
