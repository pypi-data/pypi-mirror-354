...
class Disc_class:
    ...
    def calculate_2D(self, time=None, ngrain=NGRAIN, nchem=NCHEM, nz=NZ, summed=True):
        ...
        self.z_over_r = np.linspace(0, 0.8, nz)                 # z/r array
        ...
        # Setting a floor
        self.twodee.rho_ice = np.where(self.twodee.rho_ice == 0, 1e-50, self.twodee.rho_ice)
        self.twodee.rho_dust = np.where(self.twodee.rho_dust == 0, 1e-50, self.twodee.rho_dust)
