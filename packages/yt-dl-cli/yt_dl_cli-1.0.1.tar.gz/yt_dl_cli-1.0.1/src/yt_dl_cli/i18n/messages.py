...
Note:
    This module requires the colorama library for cross-platform colored output
    and expects the global _() translation function to be available when messages
    are displayed (typically set up by calling setup_i18n() from the i18n module).
"""
...
class MessagePrinter(IMessage):
    """
    Console message printer with color support.

    This class implements the IMessage interface to provide colorized console
    ...
    including Windows systems that don't natively support ANSI sequences.

    The printer supports a predefined set of colors and automatically resets
    formatting after each message to prevent color bleeding between different
    output operations.

    Attributes:
        _COLORS (Dict[str, str]): Mapping of color names to colorama color codes.
            Supports: red, green, yellow, blue, magenta, cyan, white, reset.

    Example:
        >>> printer = MessagePrinter()
        >>> printer.printout("red", "Error message")
        >>> printer.printout("green", "Success message")
        >>> printer.printout("yellow", "Warning message")
    """
    ...
    """Dictionary mapping color names to colorama foreground color codes."""

    def __init__(self):
        """
        Initialize the message printer with colorama support.

        This constructor ensures that colorama is properly initialized for
        the current platform. The autoreset=False parameter is used because
        we manually reset colors after each message for more precise control
        over formatting.

        On Windows systems, this initialization enables ANSI escape sequence
        processing, allowing colored output to work correctly in the console.
        """
        # Ensure colorama is initialized (important for Windows)
    ...
    def printout(self, colour: str, msg: str) -> None:
        ...
        This method outputs the provided message with the specified color
        formatting. The color is applied using colorama codes, and formatting
        is automatically reset after the message to prevent color bleeding.

        If an unknown color name is provided, the method falls back to the
        default (reset) color instead of raising an error, ensuring robust
        operation even with invalid color specifications.

        Args:
            colour (str): The name of the color to use for the message.
                Supported values: "red", "green", "yellow", "blue", "magenta",
                "cyan", "white", "reset". Case-insensitive.
            msg (str): The message text to display. The message is printed
                exactly as provided, without adding newlines or other formatting.
        ...
        Example:
            >>> printer = MessagePrinter()
            >>> printer.printout("red", "Error: Something went wrong")
            >>> printer.printout("GREEN", "Success: Operation completed")  # Case insensitive
            >>> printer.printout("invalid", "This uses default color")     # Graceful fallback

        Note:
            The method prints without adding a newline character at the end,
            allowing callers to control line endings as needed. Use end=""
            parameter if you need to suppress the newline that print() normally adds.
        """
        color_code = self._COLORS.get(colour.lower(), Fore.RESET)
        print(color_code + msg + Style.RESET_ALL, end="")


# Global instance for convenient access throughout the application
console_printer = MessagePrinter()
"""
Global MessagePrinter instance for convenient access throughout the application.

This singleton instance can be imported and used directly without needing to
create MessagePrinter instances in each module. It provides a consistent
interface for colored console output across the entire application.

Example:
    >>> from messages import console_printer, Messages
    >>> error_msg = Messages.Core.ERROR_RESOURCE_CLOSE(error="Connection timeout")
    >>> console_printer.printout("red", error_msg)
"""
