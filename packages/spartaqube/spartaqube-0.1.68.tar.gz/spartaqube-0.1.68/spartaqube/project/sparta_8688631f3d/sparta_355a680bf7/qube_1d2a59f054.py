...
def sparta_d520f02464(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_9ebb70fe6e(G,F,A,B);H={_A:1,_C:C};return H
def sparta_9ebb70fe6e(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_ee623b4022(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_1c22139619(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_9ab111c9a6(json_data,user_obj):A=sparta_ee623b4022(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_d4407b436b(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_7171c7ce53(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_9ebb70fe6e(G,A,I,C)
	J={_A:1};return J
def sparta_2ae04f8194(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_166ef094d9(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_848479966a(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_781ba9ea70(json_data,user_obj):
