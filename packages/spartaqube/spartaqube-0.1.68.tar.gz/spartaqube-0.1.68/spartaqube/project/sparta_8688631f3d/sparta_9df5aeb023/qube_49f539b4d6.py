...
def sparta_317b7ae397(path) ->bool:
    """
...
def sparta_d2b5c8e205(json_data, user_obj):
    notebookProjectId = json_data['notebookProjectId']
    sqNotebookProjectSharedObj, _ = qube_ece8a50f88.get_notebookProjectObj(
...
def sparta_737fedf474(remoteBranchToTrack):
    """
...
def sparta_a74ee88291(repo, user_obj):
    """
...
def sparta_5d6808835e(webSocket, json_data, user_obj):
    """
...
def sparta_c717238807(json_data, user_obj):
    """
...
def sparta_76ce6282a3(json_data, user_obj):
    """
    Add remote origin
    """
    ...
    logger.debug(json_data)
    project_path = json_data['projectPath']
    is_git_repository = sparta_317b7ae397(project_path)
    if is_git_repository:
        remote_url = json_data['remoteUrl']
        remote_name = json_data['remoteName']
        repo = git.Repo(project_path)
        sparta_a74ee88291(repo, user_obj)
        ...
        for this_remote in repo.remotes:
            if remote == this_remote:
                ...
                if len(refs_arr) > 0:
                    ...
                    sparta_737fedf474(this_branch)
                    os.chdir(dir_path)
    return {'res': 1}


def sparta_08202cb490(json_data, user_obj):
    """
...
def sparta_da0ef949c1(json_data, user_obj):
    """
    Set track remote
    """
    logger.debug('*******************************************')
    ...
    if is_git_repository:
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        os.chdir(project_path)
        sparta_737fedf474(remoteBranchToTrack)
        os.chdir(dir_path)
    return {'res': 1}


@sparta_923d1f596f
def sparta_e5bcc0181e(json_data, user_obj):
    """
...
def sparta_41b33a4d64(json_data, user_obj):
    """
    Pull repo
    """
    project_path = json_data['projectPath']
...
def sparta_122b743981(json_data, user_obj):
    """
...
def sparta_9d83679840(json_data, user_obj):
    """
    Pull repo
    """
    project_path = json_data['projectPath']
...
def sparta_9691b3f8e7(json_data, user_obj):
    """
...
def sparta_6c414d016f(json_data, user_obj):
    """
    Load all commits
    """
    ...
    def get_git_commits_at_folder(folder_path) ->dict:
        ...
        is_git_repository = sparta_317b7ae397(folder_path)
...
def sparta_11838ecd3d(json_data, user_obj):
    """
...
def sparta_c3f8da0b09(json_data, user_obj):
    """
    Run commit
    """
    project_path = json_data['projectPath']
...
@sparta_923d1f596f
def sparta_d34849fc9f(json_data, user_obj):
    """
    Delete repository
    """
    return {'res': 1}
...
def sparta_122505e47f(json_data, user_obj):
    """
    Delete Remote
    """
...
def sparta_559fbc43f1(json_data, user_obj):
    """
    Load all branches
    """
    return {'res': 1}
...
def sparta_64df81ef17(json_data, user_obj):
    """
    Create new branch
    """
    project_path = json_data['projectPath']
...
def sparta_b45c771b4d(json_data, user_obj):
    """
    Checkout branch
    """
    project_path = json_data['projectPath']
...
def sparta_3247a6831a(json_data, user_obj):
    """
    Merge branch
    """
    project_path = json_data['projectPath']
...
def sparta_ac534c843f(json_data, user_obj):
    """
    Delete branch
    """
    project_path = json_data['projectPath']
...
def sparta_00f44c345b(json_data, user_obj):
    """
    Load files diff
    """
