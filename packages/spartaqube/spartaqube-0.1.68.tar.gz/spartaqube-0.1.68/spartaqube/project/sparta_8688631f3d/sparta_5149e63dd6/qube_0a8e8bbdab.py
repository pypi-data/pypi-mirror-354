...
def sparta_883abce43d():0
def sparta_8d2d3bcd82():A='name';B='https://api.github.com/repos/SpartaQube/spartaqube-version/tags';C=requests.get(B,proxies=proxies_dict,verify=_A);D=json.loads(C.text);E=max(D,key=lambda t:parse(t[A]));return E[A]
def sparta_e396effff9():A='https://spartaqube-version.pages.dev/latest_version.txt';B=requests.get(A,proxies=proxies_dict,verify=_A);return B.text.split('\n')[0]
def sparta_bd75674a09():A='https://spartaqube-version.pages.dev/latest_features.json';B=requests.get(A,proxies=proxies_dict,verify=_A);return json.loads(B.text)
def sparta_3fb153773c():
...
def sparta_8c5bc8c8c4():
	...
	return A
def sparta_71968722c2():
	I='res'
	try:
		C=sparta_8c5bc8c8c4();A=sparta_e396effff9();D=AppVersioning.objects.all();E=datetime.now().astimezone(UTC)
		...
		F=not C==A;G=[]
		if F:J=sparta_bd75674a09();G=J['features']
