...
from project.sparta_8688631f3d.sparta_c77f2d3c37.qube_8f0cad92aa import convert_to_dataframe,convert_dataframe_to_json,sparta_226d9606de,process_dataframe_components
...
def sparta_16c790fbfc(user_obj):from project.sparta_8688631f3d.sparta_34d32fb8c6.qube_cbe6ad2077 import sparta_5fddc0f473 as A;return A(user_obj)
def sparta_09abdd9532(user_obj):
	A=qube_1d2a59f054.sparta_1c22139619(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_07bdd2c5f0(json_data,user_obj):
	D=user_obj;E=sparta_09abdd9532(D)
...
def sparta_3183e9c619():return{_A:1,'available_engines':qube_4c1ed82c32.sparta_3183e9c619()}
def sparta_87b02f1547(json_data,user_obj):
	...
	return{_A:1}
def sparta_f1cff99503(json_data):
...
def sparta_504c735840(json_data):
...
def sparta_17299a7543(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_09abdd9532(C)
...
def sparta_89b79f0642(json_data,user_obj):
	B=_C;C=[];D=json_data['pip_cmds']
	for E in D:
		A=sparta_dd015eda03(E)
...
def sparta_3f7d33c485(obj):
	A=obj
	if isinstance(A,dict):
		B=DotDict()
		for(C,D)in A.items():B[C]=sparta_3f7d33c485(D)
		return B
	elif isinstance(A,list):return[sparta_3f7d33c485(A)for A in A]
	else:return A
def sparta_d7943f4d47(json_data,user_obj):
	B=json_data;D=B.get(_AY,_C)
	if D:A=sparta_3f7d33c485(B)
	else:
		E=B[_I];A=sparta_8aea4a0475(E,user_obj)
...
def sparta_f1b9ba9958(json_data,user_obj):
	A=json_data;G=A[_I];E=A[_M];H=int(A.get(_j,'0'))==1;B=[];C=sparta_8aea4a0475(G,user_obj)
...
def sparta_a0fce8113d(json_data,db_connector_obj):
	B=db_connector_obj
...
def sparta_5a118bb77c(json_data,user_obj):
	...
	if G=='spartaqube-data-store':
		...
		if A.get(_j,_C):A[_K]=sparta_126dfbb816(A[_x])
		F=qube_d70996b7fa.sparta_db94f6cb87(A,J)
		if F[_A]==1:
			try:
	...
	if Q:E=sparta_3f7d33c485(A)
	else:
		E=sparta_8aea4a0475(P,J)
		if E is _B:return{_A:-1,_E:_i}
	sparta_a0fce8113d(A,E);C=Connector(db_engine=E.db_engine);C.init_with_model(E)
	if K is not _B:
		if K:
			R=A[_x]
	...
	return{_A:1,_F:convert_dataframe_to_json(B)}
def sparta_c05712932e(json_data,user_obj):
	B=qube_d70996b7fa.sparta_b9341eb929({},user_obj);A=[]
	if B[_A]==1:A=B['available_df']
	return{_A:1,'dataframes_explorer':A,'dataframes_explorer_name':[A[_H]for A in A]}
def sparta_183c9bc9ca(json_data,user_obj):
	...
	if C.get(_R,_B)is not _B:A=qube_d70996b7fa.sparta_094806e994(C,F)
	else:C[_M]=C.get(_H,_B);A=qube_d70996b7fa.sparta_db94f6cb87(C,F)
	if A[_A]==1:
		G=qube_d70996b7fa.sparta_78e00a5a30(C,F);H=dict();I=_l;J=_B;K=_B;L=_B
		if G is not _B:
	...
	return{_A:-1,_E:_Ab}
def sparta_126dfbb816(dispo):A=pickle.dumps(dispo);return base64.b64encode(A).decode(_A0)
def sparta_70fbc85d75(json_data,user_obj):
	E=user_obj;A=json_data;print('load_spartaqube_data_store_preview_explorer json_data 123');print(A);A[_K]=sparta_126dfbb816(A[_K])
	if A.get(_R,_B)is not _B:C=qube_d70996b7fa.sparta_094806e994(A,E)
	else:A[_M]=A.get(_H,_B);C=qube_d70996b7fa.sparta_db94f6cb87(A,E)
	if C[_A]==1:
		try:
	...
	return{_A:-1,_E:_Ab}
def sparta_da4ad5e234(json_data,user_obj):
	A=json_data;F=A[_I];D=A.get(_M,'');G=int(A.get(_j,'0'))==1;K=A.get(_Q,_B);B=sparta_8aea4a0475(F,user_obj)
	if B is _B:return{_A:-1,_E:_i}
	sparta_a0fce8113d(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if G:
		H=A[_x]
...
def sparta_dc64dd8d5c(json_data,user_obj):
	E=json_data
	...
	return{_A:1,'table':D}
def sparta_b25175f7a8(json_data,user_obj):
	A=json_data;D=A[_I];F=A.get(_M,_B);G=int(A.get(_j,'0'))==1;B=sparta_8aea4a0475(D,user_obj)
	if B is _B:return{_A:-1,_E:_i}
	sparta_a0fce8113d(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_2b42cd2e90(json_data,user_obj,b_return_model=_C):
	A=json_data;print('save_plot');O=A[_N];D=_B
	if O:D=A[_A5];D=qube_c71ace27e3.sparta_b4548ea0cb(D)
...
def sparta_7882d22565(json_data,user_obj):
	...
	if H.count()>0:
		...
		if I.count()>0:
			...
			if M.is_admin or M.has_write_rights:
				P=A.get(_N,_C);C=_B
				if P:C=A[_A5];C=qube_c71ace27e3.sparta_b4548ea0cb(C)
				D=_B;E=A.get(_Ac,_B)
				if E is not _B:
	...
	return{_A:1,_G:K}
def sparta_bca6ba59e2(json_data,user_obj):0
def sparta_e77488c453(json_data,user_obj):
	D=user_obj;F=sparta_09abdd9532(D)
...
def sparta_b3f41d38f3(json_data,user_obj):
	...
	if J.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				K=Q[0];e=K.user;f=sparta_16c790fbfc(e);R=K.user;L=[];A=K.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if M:L.append(B[b])
							...
							if _J in B:
								h=B[_I];S=sparta_8aea4a0475(h,R);T=[]
							...
							B[_AZ]=1;V=sparta_5a118bb77c(B,R)
...
def sparta_b186c4442d(json_data,user_obj):
	...
	return{_A:1}
def sparta_c357bc7ae1(user_obj,widget_id):
...
def sparta_0b410e4cd7(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_09abdd9532(K)
...
def sparta_0186c72bd0(json_data,user_obj):
	...
	return{_A:1}
def sparta_b0d5903f1f(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _D
				...
				try:
					if qube_c71ace27e3.sparta_5b66dfafff(A.widget_password_e)==password_widget:return _D
					else:return _C
				except:return _C
		else:return _C
	return _C
def sparta_b64f60322d(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_L:A}
					...
					try:
						if qube_c71ace27e3.sparta_5b66dfafff(A.widget_password_e)==F:return{_A:1,_L:A}
	...
	return{_A:-1}
def sparta_e21c76d7c7(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_D
		...
		if A.count()==1:D=_D
	if D:
		B=A[A.count()-1];G=sparta_09abdd9532(C)
	...
	return{_A:1,_r:_C}
def sparta_26622c30d7(plot_db_chart_obj):
	...
	return C
def sparta_95ea31ddd6(json_data,user_obj):
	B=user_obj;D=sparta_09abdd9532(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_N];C=_B
				if N:C=B[_A5];C=qube_c71ace27e3.sparta_b4548ea0cb(C)
...
def sparta_002d2fb558(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_N];C=_B
				if K:C=B[_A5];C=qube_c71ace27e3.sparta_b4548ea0cb(C)
	...
	return{_A:1}
def sparta_e9e7a01163(json_data,user_obj):
...
def sparta_160e05c464(json_data,user_obj):
	...
	return{_A:1}
def sparta_634d594a58(user_obj):
	B=user_obj;C=sparta_09abdd9532(B)
	...
	return E
def sparta_a92e0ee4e8(json_data,user_obj):
	try:A=sparta_e21c76d7c7(json_data[_Am],user_obj);B=A[_r];return{_A:1,_r:B}
	except:pass
	return{_A:-1}
def sparta_ac5be681d6(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_e21c76d7c7(A[_Am],B);D=C[_r]
	if D:E=C[_L];A[_G]=E.plot_chart_id;F=sparta_b3f41d38f3(A,B);return{_A:1,_F:[A[_F]for A in F[_m]]}
	return{_A:-1}
def sparta_49cde73e5b(data_df,json_data):
	...
	return B
def sparta_b100967e9d(json_data):
	...
	B=sparta_49cde73e5b(B,E);return B,D,F
def sparta_e212f2515f(json_data,user_obj):
	j='cusum';i='You must add at least two series';h='Kmean';g='n_estimators';f='Relationships explorer data_df';e='model';d='Error matpltlib';c='matplotlib';b='Error quantstats';Z='nbComponents';W=user_obj;V='bNormalizeDecisionTree';U='maxDepth';S='trainTest';P='bInSample';K='paramsDict';G='B_DARK_THEME';E='relationship_explorer';A=json_data;from project.sparta_8688631f3d.sparta_5d2f5154f8 import qube_df7c163638 as X;Q=A['service']
	if Q=='quantstats':
		try:return X.sparta_5402951f60(A,W)
		except Exception as R:logger.debug(b);logger.debug(traceback.format_exc());logger.debug(b);return{_A:-1,_E:str(R)}
	elif Q==c:
		try:return X.sparta_c510242651(A,W)
		except Exception as R:logger.debug(d);logger.debug(traceback.format_exc());return{_A:-1,_E:str(R)}
	elif Q==c:
		try:return X.sparta_c510242651(A,W)
		except Exception as R:logger.debug(d);logger.debug(traceback.format_exc());return{_A:-1,_E:str(R)}
	elif Q=='relationshipsExplorer':
		D=A[e];C,O,L=sparta_b100967e9d(A);print(f);print(C);print(_Al);print(A);print(A.keys());M=_B
		if len(O)>0:M=O[0]
		if D=='LinearRegression':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_ab594fe27f as k;H=M;F=L;print(f"target_y_str > {H}");print(f"x_features_list > {F}");B=k(C,H,F,int(A.get('rw_beta',30)),int(A.get('rw_corr',30)),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='PolynomialRegression':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_f1405f1445 as l;H=M;F=L;B=l(C,H,F,degree=int(A.get('degree',2)),standardize=bool(A.get('standardize',_D)),in_sample=bool(A.get(P,_D)),test_size=float(A.get('test_size',.2)),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='DecisionTreeRegression':
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_58d34bbe0e as m;H=M;F=L;I=A.get(U,-1)
			if str(I)==_l:I=_B
			else:I=int(I)
			...
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_828e889664 as n;H=M;F=L;print(f"target_y_str > {H}");print(f"x_features_list > {F}");I=A.get(U,-1)
			if str(I)==_l:I=_B
			else:I=int(I)
			...
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_13098e3963 as o;H=M;F=L;print(f"target_y_str > {H}");print(f"x_features_list > {F}");I=A.get(U,-1)
			if str(I)==_l:I=_B
			else:I=int(I)
			...
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_46971fbe3c as p;H=M;F=L;I=A.get(U,-1)
			if str(I)==_l:I=_B
			else:I=int(I)
			...
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_20c38de387 as q,sparta_cebc7a6fa4 as r;H=M;F=L
		...
		elif D=='correlation_network':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_12e59f8c45 as t;H=M;F=L;B=t(C,F,float(A.get('threshold',.5)),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='pca':
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_d37279503b as u;H=M;F=L;T=int(A.get(Z,3));T=min(T,len(F))
			...
			from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_b18dc1eca5 as v;H=M;F=L
		...
		elif D=='features_importance':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_f1dbb32097 as w;H=M;F=L;B=w(C,H,F,B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='mutual_information':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_a8bd61a647 as x;H=M;F=L;B=x(C,H,F,B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='QuantileRegression':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_cb6ad1128b as y;H=M;F=L;B=y(C,H,F,quantiles=A.get('selectedQuantiles',[.1,.5,.9]),standardize=bool(A.get('bNormalizeQuantileReg',_D)),in_sample=bool(A.get(P,_D)),test_size=1-float(A.get(S,80))/100,B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='RollingRegression':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_30c82c506c as z;H=M;F=L;B=z(C,H,F,A.get('window',20),bool(A.get('bNormalizeRollingReg',_D)),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='RecursiveRegression':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_7397b20443 as A0;H=M;F=L;B=A0(C,H,F,standardize=bool(A.get('bNormalizeRecursiveReg',_D)),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='statisticsSummary':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_414188f292 import sparta_0b3f11ae69 as A1;a=L;print('selected_columns');print(a);B=A1(C,a,B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B),'data_df_json':C[[A for A in list(C.columns)if A!='__sq_index__']].to_json(orient=_A4)}
	elif Q=='tsa':
		D=A[e];C,O,L=sparta_b100967e9d(A);M=O[0];N=M;J=L;print(f);print(C);print(f"dates_col_str > {N}");print(f"variables_list > {J}");print(f"x_col_list > {O}");print(A.get(G,'NOT DEFINED'))
		if D=='STL':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_aaa67c8beb as A2;B=A2(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='wavelet':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_b31582ba62 as A3;B=A3(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='hmm':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_a1b8205d91 as A4;B=A4(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='ruptures':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_9113fda66f as A5;B=A5(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D==j:from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_85ada6b2e9 as Y;B=Y(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D==j:from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_85ada6b2e9 as Y;B=Y(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='zscore':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_7b77175e3f as A6;B=A6(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='isolation_forest':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_2d743610de as A7;B=A7(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='mad':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_6a58641189 as A8;B=A8(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='prophet_outlier':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_4b85f428e7 as A9;B=A9(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='granger':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_cd935bb29f as AA;B=AA(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='cointegration':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_93698c5868 as AB;B=AB(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='canonical_correlation':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_2949500dfc as AC;print('X data_df[x_col_list]');print(C[O]);print('Y data_df[variables_list]');print(C[J]);B=AC(C[J],C[O],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='sarima':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_a32431de61 as AD;B=AD(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='ets':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_54a5d4960b as AE;B=AE(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='prophet_forecast':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_d227332dbc as AF;B=AF(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='var':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_ae44c4a420 as AG;B=AG(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
		elif D=='adf':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_fa8803935a as AH;B=AH(C[J],params_dict=A.get(K,_B));return{_A:1,E:json.dumps(B)}
		elif D=='kpss':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_34e6c46007 as AI;B=AI(C[J],params_dict=A.get(K,_B));return{_A:1,E:json.dumps(B)}
		elif D=='perron':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_0bbdb27917 as AJ;B=AJ(C[J],params_dict=A.get(K,_B));return{_A:1,E:json.dumps(B)}
		elif D=='za':from project.sparta_8688631f3d.sparta_8c6a44fbc0.qube_054e97a968 import sparta_ff2c453a66 as AK;B=AK(C[J],C[N],params_dict=A.get(K,_B),B_DARK_THEME=A.get(G,_C));return{_A:1,E:json.dumps(B)}
	return{_A:1}
def sparta_9e9cc0a95a(json_data,user_obj):
...
def sparta_14cf8a229f(json_data,user_obj):
