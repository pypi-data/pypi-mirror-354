...
def sparta_602131557c(df):
	A=df
	if pd.api.types.is_datetime64_any_dtype(A.index):
		if A.index.tz is not _B:A.index=A.index.tz_localize(_B)
	return A
def sparta_edfdab564d(series):
	A=series
	if(A>=0).all():
		if A.max()>1.:return _A
	return _C
def sparta_3150a20363(fig):A=io.BytesIO();fig.savefig(A,format=_F);A.seek(0);B=base64.b64encode(A.getvalue()).decode(_D);A.close();return B
def sparta_7dc9d32859(fig):A=BytesIO();fig.savefig(A,format=_F);B=base64.b64encode(A.getvalue()).decode(_D);return B
import quantstats._plotting.core as qscore,matplotlib.pyplot as _plt
from matplotlib.ticker import FuncFormatter as _FuncFormatter
import pandas as _pd,numpy as _np
def sparta_538011ff15(x):return(x+1).cumprod()
class _Stats:compsum=staticmethod(_compsum)
try:from quantstats import stats as _stats
except ImportError:_stats=_Stats()
def sparta_4fb1eac3f9(grayscale):
	...
	return A,B,C
def sparta_330bb1eac6(x,_):return f"{x*100:.0f}%"
def safe_plot_timeseries(returns,benchmark=_B,title='Returns',compound=_A,cumulative=_C,fill=_A,returns_label=_H,hline=_B,hlw=_B,hlcolor='red',hllabel='',percent=_C,match_volatility=_A,log_scale=_A,resample=_B,lw=1.5,figsize=(10,6),ylabel='',grayscale=_A,fontname='Arial',subtitle=_C,savefig=_B,show=_C):
	V='gray';U="%e %b '%y";T='bold';S=fontname;R=ylabel;Q=match_volatility;P=hlcolor;O='white';L=resample;K=hline;J=compound;I='black';H=savefig;G=grayscale;B=benchmark;A=returns;D,W,M=sparta_4fb1eac3f9(G);A=A.copy().fillna(0)
...
def sparta_5402951f60(json_data,user_obj):
	...
	if d in A:
		X=A[d]
		if len(X)>0:W=X
	H=pd.DataFrame(C[0]);H[P]=pd.to_datetime(V);H.set_index(P,inplace=_C);H.columns=[e];H=sparta_602131557c(H);B=H[e]
	if not sparta_edfdab564d(B):B=B.pct_change().dropna()
	D=_B
	if len(C)==2:
		I=pd.DataFrame(C[1]);I[P]=pd.to_datetime(V);I.set_index(P,inplace=_C);I.columns=[_E];I=sparta_602131557c(I);D=I[_E]
		if not sparta_edfdab564d(D):D=D.pct_change().dropna()
	...
	if K==0:
		def L(data,benchmark=_B):
			C=benchmark;A=[];D=qs.plots.snapshot(data,show=_A,strategy_title=E,benchmark_title=G);A.append(sparta_7dc9d32859(D));B=qs.plots.monthly_heatmap(data,show=_A,ylabel=_A,returns_label=E);A.append(sparta_7dc9d32859(B))
			if C is not _B:B=qs.plots.monthly_heatmap(C,show=_A,ylabel=_A,returns_label=G);A.append(sparta_7dc9d32859(B))
			return A
		...
		def L(data,benchmark=_B):
			...
			if A['returns']:F=qs.plots.returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(F))
			if A['logReturns']:G=qs.plots.log_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(G))
			if A['yearlyReturns']:H=qs.plots.yearly_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(H))
			if A['dailyReturns']:I=qs.plots.daily_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(I))
			if A['histogram']:J=qs.plots.histogram(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(J))
			if A['rollingVol']:K=qs.plots.rolling_volatility(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(K))
			if A['rollingSharpe']:L=qs.plots.rolling_sharpe(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(L))
			if A['rollingSortino']:M=qs.plots.rolling_sortino(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(M))
			if A['rollingBeta']:
				if D is not _B:N=qs.plots.rolling_beta(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(N))
			if A['distribution']:O=qs.plots.distribution(C,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(O))
			if A['heatmap']:P=qs.plots.monthly_heatmap(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(P))
			if A['drawdowns']:Q=qs.plots.drawdown(C,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(Q))
			if A['drawdownsPeriod']:R=qs.plots.drawdowns_periods(C,show=_A,ylabel=_A,title=E);B.append(sparta_7dc9d32859(R))
			if A['returnQuantiles']:S=qs.plots.distribution(C,show=_A,ylabel=_A);B.append(sparta_7dc9d32859(S))
			return B
		if len(C)==1:F=L(B);J=F
		elif len(C)==2:F=L(B,D);J=F
	elif K==3:k=[E];M=B;M.columns=k;l=qs.reports._calc_dd(M);m=qs.stats.drawdown_details(M).sort_values(by='max drawdown',ascending=_C)[:10];T=[];n=qs.plots.drawdown(M,show=_A,ylabel=_A);T.append(sparta_7dc9d32859(n));o=qs.plots.drawdowns_periods(M,show=_A,ylabel=_A,title=E);T.append(sparta_7dc9d32859(o));J=[l.to_json(orient=U),m.to_json(orient=U),T]
	return{_G:1,Z:K,_K:J}
def sparta_6ee6feb168():import matplotlib,matplotlib.pyplot as A;from matplotlib.path import Path;from matplotlib.patches import PathPatch;from matplotlib.patches import Patch;import matplotlib.patches as B
def sparta_c510242651(json_data,user_obj):
	R='alpha';Q='sq_index';P='None';O='data_df';N='column_renamed';C=json_data;sparta_6ee6feb168();S=json.loads(C['opts']);D=json.loads(C[_I])[0];E=json.loads(C[_J]);F=json.loads(C['chartParamsEditorDict']);print('MATPLOTIB DEBUGGER BACKJEND SERIVCE');print('x_data_arr');print(D);print(type(D));print('y_data_arr');print(E);print(type(E));print('common_props');print(S);print('chart_params_editor_dict');print(F);A=pd.DataFrame(E).T;A.index=D;H=[]
