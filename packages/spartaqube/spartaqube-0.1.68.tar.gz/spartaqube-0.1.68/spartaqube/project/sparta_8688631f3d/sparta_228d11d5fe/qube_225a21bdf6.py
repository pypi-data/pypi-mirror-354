...
from project.sparta_8688631f3d.sparta_5d2f5154f8.qube_82ff246dc8 import sparta_e77488c453
...
def sparta_09abdd9532(user_obj):
	...
	return B
def sparta_7f36810bfb(json_data,user_obj):
	K='%Y-%m-%d';J='Recently used';D=user_obj;F=sparta_09abdd9532(D)
...
def sparta_0a17386495(json_data,user_obj):
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_4009e9a33a.sparta_1b0c5846fa(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_8d7c31307e(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_02a45fa177(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_c71ace27e3.sparta_b4548ea0cb(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_226d9606de(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
	...
	logger.debug('SAVE DASHBOARD dashboard_notebook');logger.debug(_R);logger.debug(L);e=Dashboard.objects.create(dashboard_id=K,name=T,slug=B,description=U,is_expose_dashboard=V,is_public_dashboard=W,has_password=H,password_e=E,lumino_layout=S,project_path=D,dashboard_venv=X,main_ipynb_fullpath=L,grid_config=json.dumps(O),plot_db_programmatic_attributes=json.dumps(Z),plot_db_dependencies=json.dumps(F),thumbnail_path=I,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_8c5bc8c8c4());f=ShareRights.objects.create(is_admin=_D,has_write_rights=_D,has_reshare_rights=_D,last_update=C);DashboardShared.objects.create(dashboard=e,user=J,share_rights=f,is_owner=_D,date_created=C);return{_A:1,_H:K}
def sparta_386127410c(lumino_dict,entrypoint_full_path):
	B=entrypoint_full_path;A=lumino_dict;C=A['type']
	if C=='split-area':
		D=A['children']
		for E in D:return sparta_386127410c(E,B)
	else:
		F=A['widgets']
		for G in F:
			if sparta_226d9606de(G[_f])==B:return _D
	return _C
def sparta_02a45fa177(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_09abdd9532(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_c71ace27e3.sparta_b4548ea0cb(G);A.password_e=G;A.has_password=_D
			else:A.has_password=_C
			A.save()
	return{_A:1,_H:I}
def sparta_c919424ef3(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_C).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_09abdd9532(B)
	...
	return{_A:1}
def sparta_9e0534ad19(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_09abdd9532(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_D
		if H:K=sparta_226d9606de(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_4829f2be86(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_c71ace27e3.sparta_5b66dfafff(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_627f14534b(json_data,user_obj):A=sparta_e77488c453(json_data,user_obj);return A
