...
from project.sparta_8688631f3d.sparta_34d32fb8c6.qube_cbe6ad2077 import sparta_5fddc0f473,sparta_bed904ab12,sparta_fae256f2cf
from project.sparta_8688631f3d.sparta_2a93ddec7a.qube_9b8a1e243b import SenderKernel
from project.logger_config import logger
def sparta_4fec8fb2cc():
...
class SqKernelManager:
	...
	def create_kernel(A,django_settings_module=_A):
		if A.notebook_exec_id!=_G:A.user_kernel=sparta_bed904ab12(A.notebook_exec_id)
		if A.dashboard_exec_id!=_G:A.user_kernel=sparta_fae256f2cf(A.dashboard_exec_id)
		G=os.path.dirname(__file__);H=sparta_5fddc0f473(A.user_kernel);C=sparta_4fec8fb2cc();I=sys.executable;J=A.venv_name if A.venv_name is not _A else _G
		def L(pipe):
			for A in iter(pipe.readline,''):logger.debug(A,end='')
			pipe.close()
		E=os.environ.copy();E['ZMQ_PROCESS']='1';logger.debug(f"SPAWN PYTHON KERNEL {C}");K=subprocess.Popen([I,_K,str(H),str(C),J],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_D,cwd=G,env=E);F=K.pid;D=datetime.now().astimezone(UTC);B=sparta_1b457f9dcf(A.user,A.kernel_manager_uuid)
		...
		return{_B:1,_O:B}
def sparta_c4f6e3590c(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_size()
def sparta_1c34243c81(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_workspace_variables()
def sparta_d00f34aa2f(kernel_process_obj,venv_name):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_activate_venv(venv_name)
def sparta_60aabf3792(kernel_process_obj,kernel_varname):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_variable_repr(kernel_varname)
def sparta_3d6346fd2d(kernel_process_obj,var_name,var_value):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_set_workspace_variable(var_name,var_value)
...
def sparta_c31e868dba(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_cloudpickle_kernel_variables()
def sparta_49a753c819(pid):
	logger.debug('Force Kill Process now from kernel manager')
	if platform.system()==_P:return sparta_b71877faf8(pid)
	else:return sparta_0112486a91(pid)
def sparta_b71877faf8(pid):
...
def sparta_0112486a91(pid):
	try:subprocess.run(['kill','-9',str(pid)],check=_D,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
	except subprocess.CalledProcessError:logger.debug(f"Failed to kill process {pid}. It may not exist.")
def sparta_8ae5d90704(kernel_process_obj):A=kernel_process_obj.pid;sparta_49a753c819(A)
def sparta_1b457f9dcf(user_obj,kernel_manager_uuid):
	A=KernelProcess.objects.filter(user=user_obj,kernel_manager_uuid=kernel_manager_uuid,is_delete=_F)
	if A.count()>0:return A[0]
def sparta_55dfcd3407(json_data,user_obj,b_return_model=_F):
	...
	return{_B:1}
def sparta_177d86d125(json_data,user_obj):
	C=user_obj;D=json_data[_C];A=sparta_1b457f9dcf(C,D)
	if A is not _A:
		sparta_8ae5d90704(A);B=A.type;F=A.name;G=A.project_folder;H=A.notebook_exec_id;I=A.dashboard_exec_id;J=A.user_kernel;K=A.venv_name;E=SqKernelManager(D,B,F,C,user_kernel=J,project_folder=G,notebook_exec_id=H,dashboard_exec_id=I,venv_name=K)
		if B==3 or B==4 or B==5:E.create_kernel(django_settings_module=_S)
		else:E.create_kernel()
	return{_B:1}
def sparta_4e420ba879(json_data,user_obj):
	A=json_data
	if _C in A:
		C=A[_C];D=A['env_name'];B=sparta_1b457f9dcf(user_obj,C)
		if B is not _A:sparta_d00f34aa2f(B,D)
	return{_B:1}
def sparta_f1ce8c8138(json_data,user_obj):
	B=json_data[_C];A=sparta_1b457f9dcf(user_obj,B)
	if A is not _A:C=sparta_c4f6e3590c(A);D=sparta_1c34243c81(A);return{_B:1,'kernel':{_T:D,_H:B,'kernel_size':C,'type':A.type,_E:A.name,_U:str(A.date_created.strftime(_W)),_V:naturalday(parser.parse(str(A.date_created)))}}
	return{_B:-1}
def sparta_179bc1bbee(json_data,user_obj):
	A=json_data;C=A[_C];D=A['varName'];B=sparta_1b457f9dcf(user_obj,C)
	if B is not _A:E=sparta_60aabf3792(B,D);return{_B:1,'htmlReprDict':E}
	return{_B:-1}
def sparta_ea4d049e3a(json_data,user_obj):
	C=json_data;D=C[_C];A=sparta_1b457f9dcf(user_obj,D)
	if A is not _A:
		B=C.get(_E,_A)
		if B is not _A:A.name=B;A.save();sparta_3d6346fd2d(A,_E,B)
	return{_B:1}
def sparta_0d3573fa0a():
	if platform.system()==_P:return sparta_6a1f139ee8()
	else:return sparta_eda9d8b215()
def sparta_b94068ee3c(command):
...
def sparta_6a1f139ee8():return sparta_6fb97881c6()
def sparta_20e0dafc5b():
...
def sparta_321cb96038():
	try:
		E='wmic process where "name=\'python.exe\'" get ProcessId,ParentProcessId,CommandLine /FORMAT:CSV';F=sparta_b94068ee3c(E);C=[];G=F.splitlines()
		for H in G[2:]:
...
def sparta_eda9d8b215():
	try:
		F=sparta_b94068ee3c("ps -eo pid,ppid,command | grep '[s]pawnKernel.py'");A=[];G=F.split('\n')
		for H in G:
...
def sparta_cfad2581d7(json_data,user_obj):
	...
	if K:from project.sparta_8688631f3d.sparta_a02d4b9d5b import qube_ba243020c4 as L;E=L.sparta_8a4abdd59a(D)
	M=sparta_0d3573fa0a();F=[];C=[(A[_I],A[_J])for A in M]
	if len(C)>0:
		N=KernelProcess.objects.filter(pid__in=[A[0]for A in C],port__in=[A[1]for A in C],user=D).distinct()
		for A in N:
			G=_A
			if I:G=sparta_c4f6e3590c(A)
			H=[]
			if J:H=sparta_1c34243c81(A)
	...
	return{_B:1,_N:F}
def sparta_e375d35b20(json_data,user_obj):
	B=user_obj;from project.sparta_8688631f3d.sparta_a02d4b9d5b import qube_ba243020c4 as D;A=D.sparta_930a02b3af(B);C=sparta_cfad2581d7(json_data,B)
	if C[_B]==1:E=C[_N];F=[A[_H]for A in E];A=[A for A in A if A[_H]not in F];return{_B:1,'kernel_library':A}
	return{_B:-1}
def sparta_1fa768e739(json_data,user_obj):
	B=json_data[_C];A=sparta_1b457f9dcf(user_obj,B)
	if A is not _A:sparta_8ae5d90704(A)
	return{_B:1}
def sparta_0c314266a6(json_data,user_obj):
	A=user_obj;B=sparta_cfad2581d7(json_data,A)
	if B[_B]==1:
		C=B[_N]
		for D in C:E={_C:D[_H]};sparta_1fa768e739(E,A)
	return{_B:1}
def sparta_ff2e12028b():
	B='cypress_tests@gmail.com';C=KernelProcess.objects.filter(user__email=B,is_delete=_F)
	for A in C:print(f"Kill kernel: {A}");sparta_8ae5d90704(A)
	return{_B:1}
def sparta_6193bc5f34(json_data,user_obj):
	C=user_obj;B=json_data;D=B[_C];from project.sparta_8688631f3d.sparta_a02d4b9d5b import qube_ba243020c4 as I;G=I.sparta_5c59e02c10(C,D);A=sparta_1b457f9dcf(C,D)
	if A is not _A:
		E=A.venv_name
		if E is _A:E=''
		B={_Q:100,_C:D,_E:A.name,_R:E};F=sparta_55dfcd3407(B,C,_D)
		if F[_B]==1:
			A=F[_O]
		...
		return{_B:F[_B]}
	return{_B:-1}
def sparta_1a313e1fdf(json_data,user_obj):return{_B:1}