...
from project.sparta_8688631f3d.sparta_f4c6bfee03 import qube_d3e40f443f as qube_d3e40f443f
def sparta_8858ae4314(json_data,userObj):
...
def sparta_47b0e8a1a6(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_1d2a59f054.sparta_1c22139619(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_1d2a59f054.sparta_1c22139619(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_2d50ced66a(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_1d2a59f054.sparta_1c22139619(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_1d2a59f054.sparta_1c22139619(A);C=[A.userGroup for A in H]
...
def sparta_b8d3d55c55(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_1d2a59f054.sparta_1c22139619(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_1d2a59f054.sparta_1c22139619(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_1d2a59f054.sparta_1c22139619(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_487a99402e(json_data,user_obj):
