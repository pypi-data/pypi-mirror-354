...
class db_connection_mssql(db_connection_sql):
    ...
    def create_connection(self):
        if self.bPrint:
            logger.debug('MSSQL')
            logger.debug('create_connection for MS SQL now')
        ...
        if len(str(self.port)) > 0:
            self.connection = pymssql.connect(host=self.hostname, user=self.user, password=self.password, database=self.db, port=self.port
                )
        else:
            self.connection = pymssql.connect(host=self.hostname, user=self.user, password=self.password, database=self.db)

    def getAllTables(self):
        """
            Get all available tables
        """
        strReq = 'SELECT * FROM sys.Tables'
    ...
    def pd2DB(self, tableName, thisDf):
        ...
        """
        engine = create_engine('mssql://' + str(self.user) + ':' + str(self.password) + '@' + str(self.hostname) + ':' + str(self.port) +
            '/' + str(self.db))
    ...
    def getDataFrameLimit(self, tableName, limit=100
        ...
        try:
            strReq = 'SELECT TOP ' + str(limit) + ' * FROM "' + tableName + '"'
            if self.bPrint:
                logger.debug('strReq => ' + str(strReq))
            self.create_connection()
            res_df = pd.read_sql(strReq, con=self.connection)
