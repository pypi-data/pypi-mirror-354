...
def sparta_69200f9475(project_path, has_django_models=True) ->dict:
    """
    ...
    src_folder = os.path.join(sparta_b6a401eb72()['django_app_template'],
        'notebook', 'template')
...
def sparta_f1d932221c(json_data, user_obj) ->dict:
    """
    ...
    try:
        ...
        notebook_path_infos_dict = sparta_69200f9475(project_path,
            has_django_models)
...
def sparta_ac79e38472(json_data, user_obj) ->dict:
    """
    Validate project path init git
    """
    json_data['bAddGitignore'] = True
    json_data['bAddReadme'] = True
    return qube_8ecd6dcb08.sparta_95f2a59d86(json_data, user_obj)


def sparta_6cbe29e36e(json_data, user_obj) ->dict:
    """
...
def sparta_fc86b92763(json_data, user_obj) ->dict:
    """
...
def sparta_7f047bd795(json_data, user_obj) ->dict:
    """
    ...
    if not user_obj.is_anonymous:
        ...
        notebook_access_dict = sparta_23dd5f5513(notebook_id, user_obj,
...
def sparta_70cc8f3968(json_data, user_obj) ->dict:
    """
    Save notebook view
    """
    ...
    if not is_new:
        return sparta_536bdf8e4e(json_data, user_obj)
    date_now = datetime.now().astimezone(UTC)
    ...
    if has_password:
        ...
        notebook_password = qube_9c73ae35fa.sparta_331d5499ac(
            notebook_password)
    ...
    notebook_obj = Notebook.objects.create(notebook_id=notebook_id, name=
        ...
        date_now, spartaqube_version=sparta_8d863c145d())
...
def sparta_536bdf8e4e(json_data, user_obj) ->dict:
    """
        ...
        if has_edit_rights:
            ...
            if has_password:
                ...
                if len(notebook_password) > 0:
                    notebook_password = (qube_9c73ae35fa.sparta_331d5499ac(notebook_password))
...
def sparta_b2f5d9f90b(json_data, user_obj) ->dict:
    """
...
def sparta_b8d3733e52(json_data, user_obj) ->dict:
    """
    Delete notebook
    """
    notebook_id = json_data['notebookId']
...
def sparta_23dd5f5513(notebook_id, user_obj, password_notebook=None) ->dict:
    """
...
def sparta_9a731cd1f1(json_data, user_obj) ->dict:
    """
...
def sparta_6f03452699(json_data, user_obj) ->dict:
    """
    Open terminal 
    """
...
def sparta_cf1f18a193(json_data, user_obj) ->dict:
    """
    ...
    if notebook_set.count() == 1:
        ...
        if notebook_shared_set.count() > 0:
            ...
            if share_rights_obj.is_admin or share_rights_obj.has_write_rights:
                has_edit_rights = True
        if has_edit_rights:
            main_ipynb_fullpath = sparta_ad557db230(json_data['fullPath'])
        ...
        if await notebook_set.acount() > 0:
            ...
            if full_path is None:
                full_path = os.path.join(notebook_obj.project_path,
                    'main.ipynb')
            ipynb_dict = qube_9bc385c03d.sparta_ae46d9cd4a(full_path)
...
def sparta_c288f42a77(json_data, user_obj) ->dict:
    """
    Copy template
    """
...
def sparta_49efe522a3(project_path, python_executable='python'):
    """
...
def sparta_e4bf083526(json_data, user_obj) ->dict:
    """
    ...
    """
    project_path = sparta_ad557db230(json_data['projectPath'])
    dest_path = project_path
    src_path = os.path.join(sparta_b6a401eb72()['django_app_template'],
        'notebook', 'template')
...
def sparta_2261dacae8(json_data, user_obj) ->dict:
    """
    Return if there are pending model migrations
    """
    path = sparta_ad557db230(json_data['projectPath'])
    path = os.path.join(path, 'app')
    is_migration_needed, stdout, has_error = sparta_49efe522a3(path)
...
def sparta_bd90ad234e(project_path, python_executable='python'):
    """
...
def sparta_cf1afa243a(json_data, user_obj) ->dict:
    """
    Migrate (apply makemigrations and migrate) django migrations
    """
    path = sparta_ad557db230(json_data['projectPath'])
    path = os.path.join(path, 'app')
    res_migration, stdout = sparta_bd90ad234e(path)
    res = 1
    errorMsg = ''
...
def sparta_70dd42288d(json_data, user_obj) ->dict:
    """
...
def sparta_a0e0726d91(json_data, user_obj) ->dict:
    """
