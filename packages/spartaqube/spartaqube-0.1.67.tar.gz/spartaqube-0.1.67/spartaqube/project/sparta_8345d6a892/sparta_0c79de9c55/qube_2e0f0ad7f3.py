...
from project.sparta_8345d6a892.sparta_26ea98fb42.qube_b61b0eabde import sparta_45a6ac7b8d
...
def sparta_bcefba0d6f(user_obj):
	...
	return B
def sparta_44027f9af9(json_data,user_obj):
	K='%Y-%m-%d';J='Recently used';D=user_obj;F=sparta_bcefba0d6f(D)
...
def sparta_61e2cadea1(json_data,user_obj):
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_9bc385c03d.sparta_475bbafe40(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_ab24d70d29(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_2500e30c5c(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_9c73ae35fa.sparta_331d5499ac(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_ad557db230(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
	...
	logger.debug('SAVE DASHBOARD dashboard_notebook');logger.debug(_R);logger.debug(L);e=Dashboard.objects.create(dashboard_id=K,name=T,slug=B,description=U,is_expose_dashboard=V,is_public_dashboard=W,has_password=H,password_e=E,lumino_layout=S,project_path=D,dashboard_venv=X,main_ipynb_fullpath=L,grid_config=json.dumps(O),plot_db_programmatic_attributes=json.dumps(Z),plot_db_dependencies=json.dumps(F),thumbnail_path=I,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_8d863c145d());f=ShareRights.objects.create(is_admin=_D,has_write_rights=_D,has_reshare_rights=_D,last_update=C);DashboardShared.objects.create(dashboard=e,user=J,share_rights=f,is_owner=_D,date_created=C);return{_A:1,_H:K}
def sparta_79a876b386(lumino_dict,entrypoint_full_path):
	B=entrypoint_full_path;A=lumino_dict;C=A['type']
	if C=='split-area':
		D=A['children']
		for E in D:return sparta_79a876b386(E,B)
	else:
		F=A['widgets']
		for G in F:
			if sparta_ad557db230(G[_f])==B:return _D
	return _C
def sparta_2500e30c5c(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_bcefba0d6f(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_9c73ae35fa.sparta_331d5499ac(G);A.password_e=G;A.has_password=_D
			else:A.has_password=_C
			A.save()
	return{_A:1,_H:I}
def sparta_84dc5da335(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_C).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_bcefba0d6f(B)
	...
	return{_A:1}
def sparta_e2afeec53b(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_bcefba0d6f(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_D
		if H:K=sparta_ad557db230(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_b4522d94ca(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_9c73ae35fa.sparta_201801731a(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_0994193208(json_data,user_obj):A=sparta_45a6ac7b8d(json_data,user_obj);return A
