...
def sparta_15f582becd():A=_F;type='notebook';return[{_A:f"{type.capitalize()}",_B:'Example to display a python Notebook',_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_84d20211aa():A=_F;B="{'reportType': 1}";C="{'reportType': 2}";D="{'reportType': 3}";type='quantstats';return[{_A:f"{type.capitalize()} basic report",_B:_E,_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_24206cb972():A=_F;type='dataframe';return[{_A:f"{type.capitalize()}",_B:_E,_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
...
plot_example"""}]
def sparta_96bf1b50bc():A=_F;type='summary_statistics';return[{_A:f"{type.capitalize()}",_B:'Summary Statistics Example',_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
