...
def sparta_8d01a79d2a(type='candlestick'):B='Example to plot a simple candlestick chart with lightweight chart';A=_E;C={_F:_I,_G:_J,_H:_K};D=_M;return[{_A:f"{type.capitalize()}",_B:B,_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_79b4e71c85(type='line2'):A=_E;B={_F:_I,_G:_J,_H:_K};C=_M;return[{_A:f"{type.capitalize()}",_B:f"Example to plot a simple {type} chart with lightweight chart",_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
...
plot_example"""}]
def sparta_115bbb1aa3():return sparta_79b4e71c85('line2')
def sparta_c70b968a15():return sparta_79b4e71c85('bar2')
def sparta_f3b2e3fa5e():return sparta_79b4e71c85('area2')
def sparta_100f12cb58():return sparta_79b4e71c85('lollipop2')
def sparta_f34e056ca1():B='Example to plot a simple baseline chart with lightweight chart';A=_E;D={_F:_I,_G:_J,_H:_K};C='{\n    "baseline": [\n      \t{\n          "defaultBaselinePrice": 200,\n        },\n    ]\n    }';type='ts_baseline';return[{_A:f"{type.capitalize()}",_B:B,_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
...
plot_example"""}]
def sparta_3e425dc8ea():A=_E;B={_F:_I,_G:_J,_H:_K};C=_M;type='performance';return[{_A:f"{type.capitalize()}",_B:f"Example to plot a simple {type} chart with lightweight chart",_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")
