...
def sparta_d11bf43eca(df):
	A=df
	if pd.api.types.is_datetime64_any_dtype(A.index):
		if A.index.tz is not _B:A.index=A.index.tz_localize(_B)
	return A
def sparta_f0a678551f(series):
	A=series
	if(A>=0).all():
		if A.max()>1.:return _A
	return _C
def sparta_6416389b13(fig):A=io.BytesIO();fig.savefig(A,format=_F);A.seek(0);B=base64.b64encode(A.getvalue()).decode(_D);A.close();return B
def sparta_112751791a(fig):A=BytesIO();fig.savefig(A,format=_F);B=base64.b64encode(A.getvalue()).decode(_D);return B
import quantstats._plotting.core as qscore,matplotlib.pyplot as _plt
from matplotlib.ticker import FuncFormatter as _FuncFormatter
import pandas as _pd,numpy as _np
def sparta_cd5e1a27a6(x):return(x+1).cumprod()
class _Stats:compsum=staticmethod(_compsum)
try:from quantstats import stats as _stats
except ImportError:_stats=_Stats()
def sparta_f2cd66a92e(grayscale):
	...
	return A,B,C
def sparta_a3298c3966(x,_):return f"{x*100:.0f}%"
def safe_plot_timeseries(returns,benchmark=_B,title='Returns',compound=_A,cumulative=_C,fill=_A,returns_label=_H,hline=_B,hlw=_B,hlcolor='red',hllabel='',percent=_C,match_volatility=_A,log_scale=_A,resample=_B,lw=1.5,figsize=(10,6),ylabel='',grayscale=_A,fontname='Arial',subtitle=_C,savefig=_B,show=_C):
	V='gray';U="%e %b '%y";T='bold';S=fontname;R=ylabel;Q=match_volatility;P=hlcolor;O='white';L=resample;K=hline;J=compound;I='black';H=savefig;G=grayscale;B=benchmark;A=returns;D,W,M=sparta_f2cd66a92e(G);A=A.copy().fillna(0)
...
def sparta_938c80fd91(json_data,user_obj):
	...
	if d in A:
		X=A[d]
		if len(X)>0:W=X
	H=pd.DataFrame(C[0]);H[P]=pd.to_datetime(V);H.set_index(P,inplace=_C);H.columns=[e];H=sparta_d11bf43eca(H);B=H[e]
	if not sparta_f0a678551f(B):B=B.pct_change().dropna()
	D=_B
	if len(C)==2:
		I=pd.DataFrame(C[1]);I[P]=pd.to_datetime(V);I.set_index(P,inplace=_C);I.columns=[_E];I=sparta_d11bf43eca(I);D=I[_E]
		if not sparta_f0a678551f(D):D=D.pct_change().dropna()
	...
	if K==0:
		def L(data,benchmark=_B):
			C=benchmark;A=[];D=qs.plots.snapshot(data,show=_A,strategy_title=E,benchmark_title=G);A.append(sparta_112751791a(D));B=qs.plots.monthly_heatmap(data,show=_A,ylabel=_A,returns_label=E);A.append(sparta_112751791a(B))
			if C is not _B:B=qs.plots.monthly_heatmap(C,show=_A,ylabel=_A,returns_label=G);A.append(sparta_112751791a(B))
			return A
		...
		def L(data,benchmark=_B):
			...
			if A['returns']:F=qs.plots.returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(F))
			if A['logReturns']:G=qs.plots.log_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(G))
			if A['yearlyReturns']:H=qs.plots.yearly_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(H))
			if A['dailyReturns']:I=qs.plots.daily_returns(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(I))
			if A['histogram']:J=qs.plots.histogram(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(J))
			if A['rollingVol']:K=qs.plots.rolling_volatility(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(K))
			if A['rollingSharpe']:L=qs.plots.rolling_sharpe(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(L))
			if A['rollingSortino']:M=qs.plots.rolling_sortino(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(M))
			if A['rollingBeta']:
				if D is not _B:N=qs.plots.rolling_beta(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(N))
			if A['distribution']:O=qs.plots.distribution(C,show=_A,ylabel=_A);B.append(sparta_112751791a(O))
			if A['heatmap']:P=qs.plots.monthly_heatmap(C,benchmark=D,show=_A,ylabel=_A);B.append(sparta_112751791a(P))
			if A['drawdowns']:Q=qs.plots.drawdown(C,show=_A,ylabel=_A);B.append(sparta_112751791a(Q))
			if A['drawdownsPeriod']:R=qs.plots.drawdowns_periods(C,show=_A,ylabel=_A,title=E);B.append(sparta_112751791a(R))
			if A['returnQuantiles']:S=qs.plots.distribution(C,show=_A,ylabel=_A);B.append(sparta_112751791a(S))
			return B
		if len(C)==1:F=L(B);J=F
		elif len(C)==2:F=L(B,D);J=F
	elif K==3:k=[E];M=B;M.columns=k;l=qs.reports._calc_dd(M);m=qs.stats.drawdown_details(M).sort_values(by='max drawdown',ascending=_C)[:10];T=[];n=qs.plots.drawdown(M,show=_A,ylabel=_A);T.append(sparta_112751791a(n));o=qs.plots.drawdowns_periods(M,show=_A,ylabel=_A,title=E);T.append(sparta_112751791a(o));J=[l.to_json(orient=U),m.to_json(orient=U),T]
	return{_G:1,Z:K,_K:J}
def sparta_7ba6eb1a70():import matplotlib,matplotlib.pyplot as A;from matplotlib.path import Path;from matplotlib.patches import PathPatch;from matplotlib.patches import Patch;import matplotlib.patches as B
def sparta_37ca61e590(json_data,user_obj):
	R='alpha';Q='sq_index';P='None';O='data_df';N='column_renamed';C=json_data;sparta_7ba6eb1a70();S=json.loads(C['opts']);D=json.loads(C[_I])[0];E=json.loads(C[_J]);F=json.loads(C['chartParamsEditorDict']);print('MATPLOTIB DEBUGGER BACKJEND SERIVCE');print('x_data_arr');print(D);print(type(D));print('y_data_arr');print(E);print(type(E));print('common_props');print(S);print('chart_params_editor_dict');print(F);A=pd.DataFrame(E).T;A.index=D;H=[]
