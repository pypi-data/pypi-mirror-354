...
def sparta_3b21db79ac(function):
	def A(request,*E,**D):
		A=request;B=_B
		...
		if not B:
			if C is not _E:F=qube_137201374c.sparta_bdb91d408c(C);login(A,F)
		else:0
		return function(A,*E,**D)
	return A
def sparta_f83f234832(function):
	def A(request,*C,**D):
	...
	return A
def sparta_36715732c5(function):
	def A(request,*B,**C):
	...
	return A
def sparta_18288591d7(function):
	C=function
	def A(request,*D,**E):
		A=request;F=_C
		try:
			G=json.loads(A.body);H=json.loads(G[_J]);I=H[_I];B=qube_137201374c.sparta_bdb91d408c(I)
...
def sparta_7fc94e0326():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_90cd84bfc0(json_data):
	D='ip_addr';A=json_data;del A[_K];del A[_L]
	try:A[D]=sparta_7fc94e0326()
	except:A[D]=-1
...
def sparta_5d7b05cc58(file_path,text):
	A=file_path
	try:
...
def sparta_879014132f(json_data,hostname_url):
	P='emailExist';O='passwordConfirm';K='email';B=json_data;F={O:'The two passwords must be the same...',K:'Email address is not valid...','form':'The form you sent is not valid...',P:'This email is already registered...'};E=_C;Q=B['firstName'].capitalize();R=B['lastName'].capitalize();C=B[K].lower();L=B[_K];S=B[_L];T=B['code'];M=B['captcha'];B['app_id']=sparta_44d63045bf()
	if M=='cypress'and C=='cypress_tests@gmail.com':
		if int(os.environ.get('CYPRESS_TEST_APP','0'))==1:
			try:from project.sparta_b16f09cf2c.sparta_d1706a9539.qube_2e517c0aa5 import sparta_00b83c0ad2 as U;U()
			except Exception as V:W='C:\\Users\\benme\\Desktop\\LOG_DEBUG_CYPRESS.txt';sparta_5d7b05cc58(W,str(V))
	else:
		X=sparta_3f690c3f81(M)
		if X[_A]!=1:return{_A:-1,_D:_H}
	if not sparta_b642c5bfc5(T):return{_A:-1,_D:'Invalid spartaqube code, please contact your administrator'}
	if L!=S:E=_B;G=F[O]
	if not re.match('[^@]+@[^@]+\\.[^@]+',C):E=_B;G=F[K]
	if User.objects.filter(username=C).exists():E=_B;G=F[P]
	if not E:
		Y=sparta_90cd84bfc0(B);N=_B;Z=Y[_G]
		if not Z:N=_C
...
def sparta_2ec8653d6e():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_8b4d2baf0e(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_579e653b92(guest_code,user_obj):
	...
	return _C
def sparta_f46f307073(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_b90c120ecc(email,captcha):
	D=sparta_3f690c3f81(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_602f569d05(A.user,C);return{_A:1}
def sparta_602f569d05(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_01d32ec398(captcha,token,email,password):
	D=sparta_3f690c3f81(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
