...
def sparta_beb2d82e55(project_path) ->dict:
    """
...
"""
    empty_notebook = qube_9bc385c03d.sparta_cb4948e954(
...
def sparta_bf1e093bd7(json_data, user_obj) ->dict:
    """
    ...
    try:
        if not os.path.exists(project_path):
            os.makedirs(project_path)
        notebook_path_infos_dict = sparta_beb2d82e55(project_path)
...
def sparta_72f20a3ba5(json_data, user_obj) ->dict:
    """
...
def sparta_34a0253954(user_obj, json_data) ->dict:
...
def sparta_de60c824d9(main_path: str, test_path: str) ->bool:
...
def sparta_8ad485d094(json_data, user_obj) ->dict:
    """
    Create a new resource
    """
    logger.debug('Create resources')
    logger.debug(json_data)
    res_check_perform_actions_dict = sparta_34a0253954(
        user_obj, json_data)
    ...
    try:
        ...
        if int(resource_type) == 1:
            ...
            path_resource_norm = sparta_ad557db230(path_resource)
            if len(path_resource) > 0:
                if not sparta_de60c824d9(project_path, path_resource_norm):
...
def sparta_2c7c2bcf76(json_data, user_obj) ->dict:
    """
    Rename a resource
    """
    ...
    if rename_type == 1:
        ...
        old_name = sparta_ad557db230(old_name)
        new_name = sparta_ad557db230(new_name)
...
def sparta_770d960d5d(json_data, user_obj) ->dict:
    """
    ...
    folder_location = sparta_ad557db230(folder_location)
    ...
    for file_dict in file_path_to_move_arr:
        ...
        old_file_location = sparta_ad557db230(old_file_location)
        new_file_location = sparta_ad557db230(new_file_location)
...
def sparta_5a1eefba3a(json_data, user_obj) ->dict:
    """
...
def sparta_c539cb215f(json_data, user_obj
    ) ->dict:
    """
    ...
    for file_dict in files_path_to_move_arr:
        ...
        file_path = sparta_ad557db230(file_path)
    ...
    for file_dict in folder_path_to_move_arr:
        ...
        this_folder_path = sparta_ad557db230(this_folder_path)
...
def sparta_91e4c34830(json_data, user_obj):
    """
        Download resource
    """
    res_check_perform_actions_dict = sparta_34a0253954(
...
def sparta_2ae29b78a0(json_data, user_obj):
    """
        Download resource
    """
...
def sparta_e74173774a(json_data, user_obj):
    """
...
def sparta_d1dd25468c(json_data, user_obj, file_obj):
    """
        Upload resources
    """
    logger.debug('**********************************')
    ...
    if len(path_resource) == 0:
        ...
        path_resource = sparta_ad557db230(path_resource)
    ...
    if len(path_folder) > 0:
        ...
        folder_upload_path = sparta_ad557db230(folder_upload_path)
...
def sparta_d6bfc15f76(fileName) ->bool:
    """
...
def sparta_fb46cabed8(full_path) ->dict:
    """
...
def sparta_e11f66db92(json_data, user_obj) ->dict:
    """
...
def sparta_53ef909ea5(json_data, user_obj) ->dict:
    ...
    code_mirror_types_list = sparta_36221615a2()
    ...
    if is_previewable:
        ...
        if sparta_d6bfc15f76(full_path):
            file_extension = full_path.split('.')[-1]
            resource_dict = sparta_fb46cabed8(full_path)
...
def sparta_b17462e3c2(json_data, user_obj) ->dict:
    """
...
def sparta_015a48123c(json_data, user_obj) ->dict:
    """
    ...
    res_saved_ipynb_raw_file_dict = (qube_9bc385c03d.save_ipnyb_from_notebook_cells(notebook_cells_arr, full_path,
        dashboard_id))
