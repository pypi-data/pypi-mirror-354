...
class SenderKernel:
	...
	async def send_zmq_request(C,sender_dict,b_send_websocket_msg=True):
		await C.zmq_socket.send_string(json.dumps(sender_dict));print('ZMQ sent...')
		while True:
			A=json.loads(await C.zmq_socket.recv_string());B=A[_A];logger.debug(f"service >>> {B}")
			if B in['exec','execute_code','execute_shell','execute',_E,'deactivate_venv','reset_kernel_workspace',_F,'set_workspace_variables','set_workspace_variable_from_datasource','set_workspace_variable_from_paste_modal']:
				if b_send_websocket_msg:await C.websocket.send(json.dumps(A))
			elif B in[_G,_H]:return A[_C]
			elif B in['get_workspace_variable']:D=base64.b64decode(A[_C]);E=cloudpickle.loads(D);return E
