...
from project.sparta_8345d6a892.sparta_f1a366f59f.qube_137201374c import sparta_374e43cb4b,sparta_8c77512101,sparta_8c10b3eae6
from project.sparta_8345d6a892.sparta_490625ab5b.qube_dc2c1eaf05 import SenderKernel
from project.logger_config import logger
def sparta_20ebde55f0():
...
class SqKernelManager:
	...
	def create_kernel(A,django_settings_module=_A):
		if A.notebook_exec_id!=_G:A.user_kernel=sparta_8c77512101(A.notebook_exec_id)
		if A.dashboard_exec_id!=_G:A.user_kernel=sparta_8c10b3eae6(A.dashboard_exec_id)
		G=os.path.dirname(__file__);H=sparta_374e43cb4b(A.user_kernel);C=sparta_20ebde55f0();I=sys.executable;J=A.venv_name if A.venv_name is not _A else _G
		def L(pipe):
			for A in iter(pipe.readline,''):logger.debug(A,end='')
			pipe.close()
		E=os.environ.copy();E['ZMQ_PROCESS']='1';logger.debug(f"SPAWN PYTHON KERNEL {C}");K=subprocess.Popen([I,_K,str(H),str(C),J],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_D,cwd=G,env=E);F=K.pid;D=datetime.now().astimezone(UTC);B=sparta_a5c947582d(A.user,A.kernel_manager_uuid)
		...
		return{_B:1,_O:B}
def sparta_1b0d2f97b7(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_size()
def sparta_cc7eba327f(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_workspace_variables()
def sparta_f075dfd832(kernel_process_obj,venv_name):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_activate_venv(venv_name)
def sparta_906353910b(kernel_process_obj,kernel_varname):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_kernel_variable_repr(kernel_varname)
def sparta_309cca667a(kernel_process_obj,var_name,var_value):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_set_workspace_variable(var_name,var_value)
...
def sparta_25e041278f(kernel_process_obj):A=SenderKernel(websocket=_A,port=kernel_process_obj.port);return A.sync_get_cloudpickle_kernel_variables()
def sparta_2237eb5412(pid):
	logger.debug('Force Kill Process now from kernel manager')
	if platform.system()==_P:return sparta_fdf46fa9d0(pid)
	else:return sparta_1bc22bc1fc(pid)
def sparta_fdf46fa9d0(pid):
...
def sparta_1bc22bc1fc(pid):
	try:subprocess.run(['kill','-9',str(pid)],check=_D,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
	except subprocess.CalledProcessError:logger.debug(f"Failed to kill process {pid}. It may not exist.")
def sparta_0a82ed5cc0(kernel_process_obj):A=kernel_process_obj.pid;sparta_2237eb5412(A)
def sparta_a5c947582d(user_obj,kernel_manager_uuid):
	A=KernelProcess.objects.filter(user=user_obj,kernel_manager_uuid=kernel_manager_uuid,is_delete=_F)
	if A.count()>0:return A[0]
def sparta_63e4e4ab5e(json_data,user_obj,b_return_model=_F):
	...
	return{_B:1}
def sparta_411e5dc124(json_data,user_obj):
	C=user_obj;D=json_data[_C];A=sparta_a5c947582d(C,D)
	if A is not _A:
		sparta_0a82ed5cc0(A);B=A.type;F=A.name;G=A.project_folder;H=A.notebook_exec_id;I=A.dashboard_exec_id;J=A.user_kernel;K=A.venv_name;E=SqKernelManager(D,B,F,C,user_kernel=J,project_folder=G,notebook_exec_id=H,dashboard_exec_id=I,venv_name=K)
		if B==3 or B==4 or B==5:E.create_kernel(django_settings_module=_S)
		else:E.create_kernel()
	return{_B:1}
def sparta_b21ccc50f9(json_data,user_obj):
	A=json_data
	if _C in A:
		C=A[_C];D=A['env_name'];B=sparta_a5c947582d(user_obj,C)
		if B is not _A:sparta_f075dfd832(B,D)
	return{_B:1}
def sparta_227c1409fa(json_data,user_obj):
	B=json_data[_C];A=sparta_a5c947582d(user_obj,B)
	if A is not _A:C=sparta_1b0d2f97b7(A);D=sparta_cc7eba327f(A);return{_B:1,'kernel':{_T:D,_H:B,'kernel_size':C,'type':A.type,_E:A.name,_U:str(A.date_created.strftime(_W)),_V:naturalday(parser.parse(str(A.date_created)))}}
	return{_B:-1}
def sparta_16c30373dc(json_data,user_obj):
	A=json_data;C=A[_C];D=A['varName'];B=sparta_a5c947582d(user_obj,C)
	if B is not _A:E=sparta_906353910b(B,D);return{_B:1,'htmlReprDict':E}
	return{_B:-1}
def sparta_63ecd50876(json_data,user_obj):
	C=json_data;D=C[_C];A=sparta_a5c947582d(user_obj,D)
	if A is not _A:
		B=C.get(_E,_A)
		if B is not _A:A.name=B;A.save();sparta_309cca667a(A,_E,B)
	return{_B:1}
def sparta_59e51e8dcd():
	if platform.system()==_P:return sparta_4e4ee2d160()
	else:return sparta_276f766120()
def sparta_cfad30e7d4(command):
...
def sparta_4e4ee2d160():return sparta_356d55c4f2()
def sparta_809be28850():
...
def sparta_aff2dc8e1d():
	try:
		E='wmic process where "name=\'python.exe\'" get ProcessId,ParentProcessId,CommandLine /FORMAT:CSV';F=sparta_cfad30e7d4(E);C=[];G=F.splitlines()
		for H in G[2:]:
...
def sparta_276f766120():
	try:
		F=sparta_cfad30e7d4("ps -eo pid,ppid,command | grep '[s]pawnKernel.py'");A=[];G=F.split('\n')
		for H in G:
...
def sparta_bf6b5f6b51(json_data,user_obj):
	...
	if K:from project.sparta_8345d6a892.sparta_e8bd2ac893 import qube_0efb58479b as L;E=L.sparta_43ea349d81(D)
	M=sparta_59e51e8dcd();F=[];C=[(A[_I],A[_J])for A in M]
	if len(C)>0:
		N=KernelProcess.objects.filter(pid__in=[A[0]for A in C],port__in=[A[1]for A in C],user=D).distinct()
		for A in N:
			G=_A
			if I:G=sparta_1b0d2f97b7(A)
			H=[]
			if J:H=sparta_cc7eba327f(A)
	...
	return{_B:1,_N:F}
def sparta_ae0f523d3c(json_data,user_obj):
	B=user_obj;from project.sparta_8345d6a892.sparta_e8bd2ac893 import qube_0efb58479b as D;A=D.sparta_7ad66911af(B);C=sparta_bf6b5f6b51(json_data,B)
	if C[_B]==1:E=C[_N];F=[A[_H]for A in E];A=[A for A in A if A[_H]not in F];return{_B:1,'kernel_library':A}
	return{_B:-1}
def sparta_abe1d1a2a0(json_data,user_obj):
	B=json_data[_C];A=sparta_a5c947582d(user_obj,B)
	if A is not _A:sparta_0a82ed5cc0(A)
	return{_B:1}
def sparta_47b2ba406d(json_data,user_obj):
	A=user_obj;B=sparta_bf6b5f6b51(json_data,A)
	if B[_B]==1:
		C=B[_N]
		for D in C:E={_C:D[_H]};sparta_abe1d1a2a0(E,A)
	return{_B:1}
def sparta_4dbfae428b():
	B='cypress_tests@gmail.com';C=KernelProcess.objects.filter(user__email=B,is_delete=_F)
	for A in C:print(f"Kill kernel: {A}");sparta_0a82ed5cc0(A)
	return{_B:1}
def sparta_01c73ff9df(json_data,user_obj):
	C=user_obj;B=json_data;D=B[_C];from project.sparta_8345d6a892.sparta_e8bd2ac893 import qube_0efb58479b as I;G=I.sparta_3a64b1186a(C,D);A=sparta_a5c947582d(C,D)
	if A is not _A:
		E=A.venv_name
		if E is _A:E=''
		B={_Q:100,_C:D,_E:A.name,_R:E};F=sparta_63e4e4ab5e(B,C,_D)
		if F[_B]==1:
			A=F[_O]
		...
		return{_B:F[_B]}
	return{_B:-1}
def sparta_85e0ed15b6(json_data,user_obj):return{_B:1}