...
def sparta_dc6a2751cf():
    ...
    if is_virtual_env:
        ...
        return {'is_virtual_env': True, 'virtual_env_name':
            virtual_env_name, 'virtual_env_path': sparta_ad557db230(
            virtual_env_path)}
    else:
...
def sparta_425d69124a(bat_file_path, shortcut_path, icon_path):
    """
...
def sparta_21d70c11cb(shell_script_path, shortcut_name, icon_path):
    """
...
def sparta_0cfee47f87(shortcut_name, shell_script_path, icon_path):
    """
...
def sparta_1e60dc3dad(json_data, user_obj) ->dict:
    """
    Create desktop launcher
    """
    api_token_dict = qube_137201374c.sparta_ade4939e10(json_data, user_obj)
    if api_token_dict['res'] == 1:
        token = api_token_dict['token']
        api_path = sparta_b6a401eb72()['api']
        spartaqube_exec = sparta_ad557db230(os.path.join(api_path,
            'spartaqube_exec.py'))
...
"""
        ...
        spartaqube_launcher = sparta_ad557db230(os.path.join(api_path,
...
"""
        ...
        spartaqube_path = sparta_b6a401eb72()['spartaqube_path']
        if conf_settings.IS_DEV:
            icon_path = sparta_ad557db230(os.path.join(spartaqube_path,
        ...
        if platform_system == 'Windows':
            spartaqube_platform = sparta_ad557db230(os.path.join(api_path,
                'SpartaQube.bat'))
            spartaqube_lnk = sparta_ad557db230(os.path.join(api_path,
                'SpartaQube.lnk'))
            if os.path.exists(spartaqube_lnk):
                try:
                    ...
                    return {'res': -1, 'errorMsg': str(e)}
            sparta_425d69124a(spartaqube_platform, spartaqube_lnk, icon_path)
            desktop_path = os.path.join(os.path.expanduser('~'), 'Desktop')
            shortcut_path = os.path.join(desktop_path, 'SpartaQube.lnk')
            sparta_425d69124a(spartaqube_platform, shortcut_path, icon_path)
        elif platform_system == 'Darwin':
            spartaqube_platform = sparta_ad557db230(os.path.join(api_path,
                'SpartaQube.sh'))
            sparta_21d70c11cb(shell_script_path=
                spartaqube_platform, shortcut_name='SpartaQube Launcher',
                icon_path=icon_path)
        else:
            spartaqube_platform = sparta_ad557db230(os.path.join(api_path,
                'SpartaQube.sh'))
            sparta_0cfee47f87(shell_script_path=spartaqube_platform,
