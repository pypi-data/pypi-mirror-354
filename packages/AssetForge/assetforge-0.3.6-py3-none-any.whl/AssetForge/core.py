...
class AssetTool:
    ...
    def build_cmd(self, input_file: Path) -> str:
        return f"@$(PYTHON) {str(Path(inspect.getfile(type(self))).resolve())} {self.tool_name()} {str(input_file.relative_to(self.build_folder))} {self.input_folder.relative_to(self.build_folder)} {self.output_folder.relative_to(self.build_folder)} >> log.txt 2>&1"
...
def Build(input_folder: Path, output_folder: Path, build_folder: Path, req: Path, recursive: bool = False, parallel: bool = False, debug: bool = False, quiet: bool = True):
    ...
    if debug:
        ...
        viz_dependency_graph(graph_copy, bipartite_order_copy, original_input_folder / Path("output"))
