...
class CommonAssetTool(AssetTool):
    ...
    def build_cmd(self, input_file: Path) -> str:
        return f"@$(PYTHON) -m AssetForge.tool {self.tool_name()} {str(input_file.relative_to(self.build_folder))} {self.input_folder.relative_to(self.build_folder)} {self.output_folder.relative_to(self.build_folder)} >> log.txt 2>&1"
