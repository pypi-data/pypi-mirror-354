...
from .util import invert_graph, topological_sort, viz_dependency_graph, hash_file, combine_hashes, in_folder, relative_path, Graph, JobDict, ThreadPool
...
class AssetTool:
    ...
    def build_cmd(self, input_file: Path) -> str:
        return f"$(PYTHON) {str(Path(inspect.getfile(type(self))).resolve())} {self.tool_name()} {str(input_file.relative_to(self.build_folder))} {self.input_folder.relative_to(self.build_folder)} {self.output_folder.relative_to(self.build_folder)}"
...
def Build(input_folder: Path, output_folder: Path, build_folder: Path, req: Path, recursive: bool = False, parallel: bool = False, debug: bool = False, quiet: bool = True):
    ...
    if debug:
        ...
        viz_dependency_graph(graph_copy, bipartite_order_copy, build_folder / Path("output"))
    ...
    with make_file_path.open("w") as make_file:
        ...
        make_file.write(f"INPUT_FOLDER := {str(input_folder.relative_to(build_folder))}:\n")
        make_file.write(f"OUTPUT_FOLDER := {str(output_folder.relative_to(build_folder))}:\n\n")
        make_file.write(f".PHONY: all venv\n\n")
        make_file.write(f"all: venv {' '.join([str(Path(item).relative_to(build_folder)) for sublist in bipartite_order[2::2] for item in sublist])}\n\n")
        
        make_file.write(f"$(VENV_STAMP): $(REQUIREMENTS)\n")
        make_file.write(f"\t@python3 -m venv $(VENV)\n")
        make_file.write(f"\t@$(VENV) -m pip install -r $(REQUIREMENTS)\n")
        ...
        for job, cmd in jobs.items():
                make_file.write(f"{' '.join([str(Path(dep).relative_to(build_folder)) for dep in inv_graph[job]])}: {' '.join([str(Path(inp).relative_to(build_folder)) for inp in graph[job]])}\n")
