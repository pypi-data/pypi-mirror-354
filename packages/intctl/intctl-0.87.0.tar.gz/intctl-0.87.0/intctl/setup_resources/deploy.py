...
def transfer_and_deploy(cfg: dict, status: StatusManager):
    ...
    while True:
        ...
        with Spinner("Patching Kubernetes secret..."):
            ...
            patch_payload_dict = {
                "data": {
                    "GCP_SERVICE_ACCOUNT_KEY": key,  # This is already base64 encoded
                    "ENVIRONMENT": env_encoded       # This is now base64 encoded
                }
            }
            patch_payload_json = json.dumps(patch_payload_dict)

            # 4. Construct the command as a list of arguments to avoid shell issues
            patch_cmd_list = [
