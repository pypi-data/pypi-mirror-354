...
def transfer_and_deploy(cfg: dict, status: StatusManager):
    ...
    while True:
        print("🔧 Patching gcp-creds secret using the 'data' field...")
        with Spinner("Patching Kubernetes secret..."):
            # We use 'data' because our values are already base64-encoded.
            patch_cmd = (
                f"kubectl patch secret gcp-creds -n intellithing --type='merge' "
                f"-p='{{\"data\": {{\"GCP_SERVICE_ACCOUNT_KEY\": \"{key_b64}\", \"ENVIRONMENT\": \"{env_b64}\"}}}}'"
            )
            patch = run(patch_cmd)
