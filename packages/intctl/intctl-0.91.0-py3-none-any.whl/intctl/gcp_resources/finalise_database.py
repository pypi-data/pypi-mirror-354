...
def finalise_database(cfg: dict, status: StatusManager) -> None:
    # --- NO CHANGES in this outer function ---
    ...
    # STEP 3: Fetch credentials (No changes here)
    ...
    async def execute_sql():
        # Instantiate the Connector and the connection object *inside* the async function.
        # This ensures they are bound to the event loop that is actually running this coroutine.
        connector = Connector()
        ...
        try:
            ...
            # Clean up the connection and the connector gracefully.
            if conn:
                await conn.close()
            # The connector's cleanup is async, so we must await it.
