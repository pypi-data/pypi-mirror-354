...
def transfer_and_deploy(cfg: dict, status: StatusManager):
    ...
    # Step 4: Patch secret
    key_b64 = cfg.get("gcp_key_b64")
    env_plain = cfg.get("environment", "dev") # Get the plain text value
    env_b64 = base64.b64encode(env_plain.encode("utf-8")).decode("utf-8")
    if not key_b64:
        print("❌ Pre-encoded GCP_SERVICE_ACCOUNT_KEY is missing from the configuration.")
        status.fail("deploy")
        return
    
    patch_payload = {
    "data": {
        "GCP_SERVICE_ACCOUNT_KEY": key_b64,
        "ENVIRONMENT": env_b64
    }
}
    fd, patch_file_path = tempfile.mkstemp(suffix=".json")
    with os.fdopen(fd, 'w') as temp_f:
        json.dump(patch_payload, temp_f)
    ...
    print("🔧 Patching gcp-creds secret from a temporary patch file...")
    patch_cmd = (
        f"kubectl patch secret gcp-creds -n intellithing "
        f"--type='merge' --patch-file={patch_file_path}"
    )
    while True:
        with Spinner("Patching Kubernetes secret from file..."):
            patch = run(patch_cmd)
        
        if patch.returncode == 0:
            print("✅ Secret patched successfully from file.")
            break
        print(f"❌ Failed to patch secret from file. Retrying in 10s...\n{patch.stderr.strip()}")
        time.sleep(10)
