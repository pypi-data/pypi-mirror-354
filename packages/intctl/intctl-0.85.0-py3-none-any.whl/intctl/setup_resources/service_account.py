...
""")
    ...
    stripped_key_bytes = key_bytes.strip()    
    encoded_key_for_k8s = base64.b64encode(stripped_key_bytes).decode("utf-8")
    cfg['gcp_key_b64'] = encoded_key_for_k8s
    try:
        os.remove(key_file)
        os.remove(stripped_key_bytes)
        print(f"🧹 Cleaned up temporary key file: {stripped_key_bytes}")
    except OSError as e:
