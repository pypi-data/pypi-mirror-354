...
def execute_sql_job(cfg: dict, status: StatusManager):
    ...
    async def run_sql():
        try:
            # --- Manual Credential Loading to Handle Double-Encoding ---
            print("Manually loading service account credentials...")
            # 1. Get the path to the key file, which contains a singly-base64-encoded string.
            key_file_path = os.environ["GOOGLE_APPLICATION_CREDENTIALS"]
            
            # 2. Read the singly-encoded string from the file.
            with open(key_file_path, 'r') as f:
                singly_encoded_key = f.read()

            # 3. Decode it one time to get the actual JSON key string.
            ...
            # 5. Create a credentials object from this dictionary.
            credentials = service_account.Credentials.from_service_account_info(service_account_info)
            project_id = service_account_info['project_id']
            print("✅ Credentials loaded successfully.")
            
            # --- Proceed with the rest of the script, using the explicit credentials ---
            db_instance_name = os.environ["DB_INSTANCE_NAME"]
            db_name = os.environ["DB_NAME"]
            org_id = os.environ["ORG_ID"]
            ws_id = os.environ["WS_ID"]

            # Pass the manually-loaded credentials to the GCP client.
            ...
            conn = await asyncpg.connect(user=pg_user, password=pg_pass, database=db_name, host=db_instance_name, port=5432)
    ...
    pip install --quiet --no-cache-dir --break-system-packages "asyncpg==0.29.0" "google-cloud-secretmanager==2.19.0" "google-auth==2.29.0" "google-oauth2"
