...
def execute_sql_job(cfg: dict, status: StatusManager):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            # Write scripts and manifest to files to avoid shell quoting issues
            runner_path = os.path.join(tmpdir, "runner.py")
            sql_path = os.path.join(tmpdir, "status.sql")
            entrypoint_path = os.path.join(tmpdir, "entrypoint.sh")
            ...
            run(
                ...
                f"--from-file={entrypoint_path} "
                f"--from-file={runner_path} "
                f"--from-file={sql_path}"
            )
            ...
            if "✅✅✅ SQL SCRIPT EXECUTED SUCCESSFULLY ✅✅✅" in logs:
                ...
                print("❌ Job completed, but success message was not found in logs.")
                print("--- POD LOGS ---")
                print(logs)
                print("--- END POD LOGS ---")
