...
""")
    ...
    key_file = ... # This is the path to your temp key file
    print("🔑 Reading and Base64-encoding the service account key for deployment...")
    with open(key_file, "r") as f:
        key_content = f.read()

    # Encode the string content to bytes, then base64 encode it
    encoded_key_for_k8s = base64.b64encode(key_content.encode("utf-8")).decode("utf-8")

    # Store the pre-encoded key in the config dictionary
    cfg['gcp_key_b64'] = encoded_key_for_k8s

    # Clean up the temporary key file as we now have its content in memory
    try:
        os.remove(key_file)
        print(f"🧹 Cleaned up temporary key file: {key_file}")
    except OSError as e:
        print(f"⚠️ Warning: Could not remove temporary key file: {e}")

    print("✅ Service account setup complete and key prepared for deployment.")
