...
def __generate_table_ddl(
    ...
    for field in ddl_fields:
        ...
        if isinstance(feature_type, dict):
            feature_type = "_float8"
        else:
            postgres_element_type = spark_to_postgres_map.get(feature_type)
            if postgres_element_type is not None:
                feature_type = postgres_element_type
...
def __alter_table_ddl(schema_name: str, table_name: str, create_ddl: str, partition_column: str):
    ...
    md = "**Create Table SQL Query for PostgreSQL:** Run the following query, to create a new table\n"
    md += create_ddl + "\n\n"
    md += "**Alter Table SQL Queries for PostgreSQL:** Run the following queries, in the given order, to add a partition column in your existing table.\n"
    md += "1. `ALTER TABLE \"{0}\".\"{1}\" ADD COLUMN \"{2}\" BIGINT GENERATED ALWAYS AS IDENTITY;`\n".format(schema_name, table_name, partition_column)
    md += "2. `VACUUM ANALYZE \"{0}\".\"{1}\";`\n".format(schema_name, table_name)
...
def __generate_table_ddl_from_schema(schema_name: str, table_name: str, table_schema: dict, primary_key_cols: list = [], partition_column: str = None):
    ...
    for column, column_meta in table_schema["columns"].items():
        postgres_element_type = spark_to_postgres_map.get(column_meta["type"])
        if postgres_element_type is None:
            postgres_element_type = column_meta["type"]

        column_string = "\"{}\" {}".format(column, postgres_element_type.upper())
