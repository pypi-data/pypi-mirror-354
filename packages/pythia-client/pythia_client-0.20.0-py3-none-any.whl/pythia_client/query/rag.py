...
class QueryService(BaseService):
    ...
    def __init__(self, url: str, api_key: str):
        ...
        self.search_stream_endpoint = urljoin(str(self.url), "search/query/stream")
    ...
    def query_stream(
        ...
        """Query the API with a user question to get a LLM-generated answer based
            on context from the document store. The difference with the query method is that here the answer is streamed
        ...
        response = session.post(
            self.search_stream_endpoint, headers=headers, json=payload, stream=True
        )
        if response.status_code != 200:
            response.close()
            session.close()
