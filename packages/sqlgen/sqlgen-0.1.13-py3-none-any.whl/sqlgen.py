...
def escape(val: str, null_values: List[str]) -> str:
    ...
    if val is None or val.strip().lower() in null_values:
        return 'NULL'
    if val.endswith(".0") and not val.startswith("0"):
        try:
...
def data_to_sql_lines(data: List[List[str]], table_name: str, cols: List[str], null_values: List[str], use_progress_bar: bool, compact_sql: bool = False, rows_per_insert: int = 1000) -> List[str]:
    ...
    for start in loop:
        ...
        sql_header = f"INSERT INTO `{table_name}` ({column_string}) VALUES"
...
def split_sql_files(sql_header: str, lines: List[str], output_dir: str, base_name: str, max_file_size_mb: int, use_progress_bar: bool):
    ...
    # 一度仮に全ファイル数を見積もるため、行サイズ合計を計算
    total_bytes = sum(len((line + '\n').encode('utf-8')) for line in lines)
    estimated_num_files = math.ceil(total_bytes / max_bytes)
    digit_len = len(str(estimated_num_files))  # 何桁にするか
    ...
    for line in loop:
        ...
        if current_size + line_bytes > max_bytes and current_lines:
            ...
            out_path = os.path.join(output_dir, f"{base_name}_{file_index:0{digit_len}d}.sql")
    ...
    if current_lines:
        ...
        # 1ファイルだけの場合は index を付けない
        if file_index == 1 and len(out_paths) == 0:
            out_path = os.path.join(output_dir, f"{base_name}.sql")
        else:
            out_path = os.path.join(output_dir, f"{base_name}_{file_index:0{digit_len}d}.sql")
...
def to_sql(
    ...
    """統一されたCSV/DF→SQL変換関数"""
    print("start generating SQL files...")
    ...
    sql_header, sql_lines = data_to_sql_lines(data, table_name, cols, null_values, use_progress_bar, compact_sql, rows_per_insert)
    ...
    split_sql_files(sql_header, sql_lines, output_dir, base_name, max_file_size_mb, use_progress_bar)
