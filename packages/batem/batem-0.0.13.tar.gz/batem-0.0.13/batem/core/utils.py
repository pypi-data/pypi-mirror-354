...
class TimeSeriesPlotter:
        ...
        for variable_name in variable_values:
            if variable_name != 'datetime' and variable_name != 'epochtimems' and variable_name != 'stringdate':
                ...
                if not is_all_none:
                    ...
                    self._variable_min_max[variable_name] = self.minmax([variable_name])
    ...
    def minmax(self, variables: list[str]) -> tuple[float, float]:
        ...
        min_value: float = min(min(self._variable_values[v]) for v in variables)
        max_value: float = max(max(self._variable_values[v]) for v in variables)
            ...
            if len(variable_groups) == 0:  # if no group, create one
                ...
                for variables_in_group in variable_groups:  # check all existing groups
                    ...
                    for variable_name in variables_in_group:
                        same_group = same_group and self.same_group([selected_variable], variable_name)
                    if same_group:
        ...
        if len(variable_groups) > 0:
            ...
            for i, variables_in_group in enumerate(variable_groups):
                for variable_name in variables_in_group:
                    ...
                    fig.add_trace(go.Scatter(x=self._datetimes, y=selected_variable_values[variable_name], name=displayed_variable_name, showlegend=True, line_shape='hv'), row=1+i, col=1)
            fig.show()
    ...
    def same_group(self, group_variables: list[str], variable: str) -> bool:
        rho_comparability: float = 5
        min_group_original, max_group_original = self.minmax(group_variables)
        min_variable, max_variable = self.minmax([variable])
        min_group_final, max_group_final = self.minmax(group_variables + [variable])
        if (max_group_final - min_group_final) > (max_group_original - min_group_original) * rho_comparability:
            return False
        elif max_group_final - min_group_final > (max_variable - min_variable) * rho_comparability:
            return False
        return True
