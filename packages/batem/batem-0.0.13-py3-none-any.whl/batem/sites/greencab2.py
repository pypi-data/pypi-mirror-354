...
def make_data_and_signals(starting_stringdate="1/1/2022", ending_stringdate="5/1/2023") -> DataProvider:
    ...
    temperature_signal_generator = SignalGenerator(dp.series('datetime'), None)
    temperature_signal_generator.daily([0, 1, 2, 3, 4], {0: None, 7: 20, 19: None}, merger=Merger(min, 'r'))
    heating_period: list[int] = temperature_signal_generator.seasonal('16/11', '15/3', 1, merger=Merger(max, 'b'))
    summer_temperature_signal_generator = SignalGenerator(dp.series('datetime'), None)
    summer_temperature_signal_generator.daily([0, 1, 2, 3, 4], {0: None, 7: 22, 19: None}, merger=Merger(min, 'r'))
    cooling_period: list[int] = summer_temperature_signal_generator.seasonal('16/3', '15/11', 1, merger=Merger(max, 'b'))
    temperature_signal_generator.combine(summer_temperature_signal_generator(), merger=Merger(min, 'n'))
    dp.add_external_variable('TZcabinet_setpoint', temperature_signal_generator())

    hvac_modes_signal_generator = SignalGenerator(dp.series('datetime'))
    hvac_modes_signal_generator.combine(heating_period, merger=Merger(max, 'l'))
    hvac_modes_signal_generator.combine(cooling_period, merger=Merger(lambda x, y: x - y, 'n'))
    ...
    ventilation_signal_generator = SignalGenerator(dp.series('datetime'))
    ventilation_signal_generator.daily([0, 1, 2, 3, 4], {0: 0, 7: 1, 18: 0})
