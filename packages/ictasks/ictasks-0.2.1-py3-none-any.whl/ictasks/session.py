...
def _get_launch_command(config: Config, host: Host, worker: Worker, task: Task):
    if config.slurm_job:
        ...
        cmd += (
            f"{config.task_dist.cores_per_task} --host {host.address} {task.launch_cmd}"
        )
        return cmd
...
def _setup_worker_queue(
    ...
    for host in ictasks.worker.load(nodes, config.task_dist):
...
def taskfarm(config_path: Path, tasklist: Path, work_dir: Path):
    """
    ...
    config = Config(**read_yaml(config_path)) if config_path.exists() else Config()
    write_task_func = partial(ictasks.task.write, work_dir)

    ictasks.run(
        tasks=ictasks.task.queue_from_taskfile(tasklist),
        work_dir=work_dir,
        config=config,
