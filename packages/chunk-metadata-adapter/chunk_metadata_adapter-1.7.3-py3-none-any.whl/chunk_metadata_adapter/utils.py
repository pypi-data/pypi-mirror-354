        ...
        # Convert objects to dict (if not a built-in type)
            ...
            for obj_k, obj_v in obj_dict.items():
                ...
                if isinstance(obj_v, (list, tuple)):
                    ...
                    items[obj_key] = str(obj_v.value) if for_redis else obj_v.value
                else:
                    items[obj_key] = str(obj_v) if for_redis and not isinstance(obj_v, (int, float)) else obj_v
            continue
        # Dict: recurse
...
def from_flat_dict(flat: dict, sep: str = '.', enums: dict = None) -> dict:
    ...
    Ключи вида <prefix>.created_at (где prefix не пустой) игнорируются, чтобы created_at не попадал во вложенные dict.
    Теперь: если значение — строка и успешно парсится как list или dict, всегда преобразуем обратно.
    Если enums передан — для соответствующих полей восстанавливаем Enum.
    """
    import json
    result = {}
    enums = enums or {}
    for flat_key, value in flat.items():
        ...
        if isinstance(value, str) and value:
            ...
            d[keys[-1]] = value
        # Восстановление Enum, если указано
        if field_path in enums and isinstance(d[keys[-1]], str):
            enum_cls = enums[field_path]
            try:
                d[keys[-1]] = enum_cls(d[keys[-1]])
            except Exception:
                pass  # если невалидное значение, оставляем строку
    return result
