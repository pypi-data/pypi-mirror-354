...
def to_flat_dict(data: dict, parent_key: str = '', sep: str = '.') -> dict:
    """
    Recursively flattens a nested dictionary. All values are serialized via json.dumps (including strings, numbers, bool, None, arrays, dicts, enums, objects).
...
def from_flat_dict(flat: dict, sep: str = '.') -> dict:
    ...
    All values are parsed from JSON strings (json.loads).
    Example: {"a.b.c": '["c0", "c1"]'} -> {"a": {"b": {"c": ["c0", "c1"]}}}
    """
    result = {}
