...
def test_to_flat_dict_and_from_flat_dict_basic():
    ...
    assert flat["a.b.h"] is None  # None as is
...
def test_to_redis_dict_empty_and_edge():
    ...
    assert out['b'] == ''
    assert out['c'] == '[]'
    # d — вложенный пустой dict, не должен появиться как отдельный ключ
    assert all(not k.startswith('d.') for k in out if k != 'created_at')
