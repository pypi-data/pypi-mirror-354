...
def test_to_flat_dict_empty_fields():
    ...
    assert "link_parent" in d and d["link_parent"] is None
    assert "link_related" in d and d["link_related"] is None
    assert ("source_lines_start" not in d) or (d["source_lines_start"] in (None, 0))
    assert ("source_lines_end" not in d) or (d["source_lines_end"] in (None, 0))
