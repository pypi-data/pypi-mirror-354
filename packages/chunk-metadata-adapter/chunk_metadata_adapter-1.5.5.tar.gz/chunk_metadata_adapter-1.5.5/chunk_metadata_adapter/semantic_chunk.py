...
class SemanticChunk(BaseModel):
    ...
    def to_flat_dict(self) -> dict:
        """
        Преобразует SemanticChunk в плоский словарь для записи в БД.
        Списки -> строки, ссылки -> link_parent/link_related, source_lines -> start/end, метрики -> flat.
        """
        d = self.model_dump()
        # tags: List[str] -> str
        ...
        if self.source_lines:
            d["source_lines_start"] = self.source_lines[0]
            d["source_lines_end"] = self.source_lines[1]
        # Метрики (ChunkMetrics) -> flat
        if self.metrics:
            d.update(self.metrics.model_dump())
        return d
    ...
    def from_flat_dict(cls, data: dict) -> "SemanticChunk":
        ...
        Строки -> списки, link_parent/link_related -> links, source_lines_start/source_lines_end -> source_lines, flat метрики -> dict.
        """
        # tags: str -> List[str]
        tags = data.get("tags", "").split(",") if data.get("tags") else []
        # links: link_parent/link_related -> List[str]
        links = []
        if data.get("link_parent"):
            links.append(f"parent:{data['link_parent']}")
        if data.get("link_related"):
            links.append(f"related:{data['link_related']}")
        # source_lines_start/source_lines_end -> source_lines
        source_lines = None
        if data.get("source_lines_start") is not None and data.get("source_lines_end") is not None:
            source_lines = [data["source_lines_start"], data["source_lines_end"]]
        # Метрики (flat) -> dict
        metrics_fields = ["quality_score", "coverage", "cohesion", "boundary_prev", "boundary_next", "used_in_generation", "feedback_accepted", "feedback_rejected"]
        metrics_data = {k: data.get(k) for k in metrics_fields if k in data}
        metrics = metrics_data if any(metrics_data.values()) else None
        return cls(
    ...
    def model_post_init(self, __context):
        ...
        if self.metrics is not None and isinstance(self.metrics, ChunkMetrics):
            for field in ChunkMetrics.model_fields:
                value = getattr(self.metrics, field)
            ...
            for field in ChunkMetrics.model_fields:
                if hasattr(self, field):
                    metrics_kwargs[field] = getattr(self, field)
            if any(v is not None for v in metrics_kwargs.values()):
                self.metrics = ChunkMetrics(**metrics_kwargs)
