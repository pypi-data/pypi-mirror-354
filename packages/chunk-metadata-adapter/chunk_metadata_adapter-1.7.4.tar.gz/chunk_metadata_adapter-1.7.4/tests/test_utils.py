...
def test_to_flat_dict_and_from_flat_dict_basic():
    ...
    # for_redis=True: None не сохраняется
    flat_redis = to_flat_dict(data, for_redis=True)
    assert "a.b.h" not in flat_redis
    restored_redis = from_flat_dict(flat_redis)
    assert "h" not in restored_redis["a"]["b"]
...
def test_adapter_list_dict_roundtrip_for_redis():
    ...
    flat = to_flat_dict(meta, for_redis=True)
    # Все значения — строки
    for v in flat.values():
        assert isinstance(v, str) or v == '', f"Value {v} is not str for Redis!"
    # Симуляция чтения из Redis (все строки)
    redis_data = dict(flat)
    restored = from_flat_dict(redis_data)
    assert isinstance(restored['embedding'], list), 'embedding should be list after deserialization'
    assert isinstance(restored['tags'], list), 'tags should be list after deserialization'
    assert isinstance(restored['links'], list), 'links should be list after deserialization'
    assert isinstance(restored['text'], str), 'text should be str after deserialization' 
...
def test_from_flat_dict_bytes_and_enum_uuid_json():
    ...
    flat = to_flat_dict(data, for_redis=True)
    # Эмулируем Redis: все значения bytes
    ...
    assert isinstance(restored["embedding"], list)
