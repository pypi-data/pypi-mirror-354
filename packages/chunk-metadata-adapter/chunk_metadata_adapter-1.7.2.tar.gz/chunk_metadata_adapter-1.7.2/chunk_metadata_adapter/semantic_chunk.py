    ...
    def from_flat_dict(cls, data: dict) -> "SemanticChunk":
        ...
        for field in ["tags", "links", "embedding"]:
            ...
            if isinstance(val, str):
                ...
                restored[field] = []
        # block_meta — dict
        if "block_meta" in restored and isinstance(restored["block_meta"], str):
            if restored["block_meta"].strip() == "" or restored["block_meta"].strip() == "null":
                restored["block_meta"] = {}
            else:
                try:
                    parsed = json.loads(restored["block_meta"])
                    if isinstance(parsed, dict):
                        restored["block_meta"] = parsed
                    else:
                        raise ValueError(f"Field 'block_meta' must be a dict, got: {restored['block_meta']}")
                except Exception:
                    raise ValueError(f"Field 'block_meta' must be a dict, got: {restored['block_meta']}")
        # Удаляем created_at из block_meta, если он там есть
        if "block_meta" in restored and isinstance(restored["block_meta"], dict):
            restored["block_meta"].pop("created_at", None)
        return cls(**restored)
