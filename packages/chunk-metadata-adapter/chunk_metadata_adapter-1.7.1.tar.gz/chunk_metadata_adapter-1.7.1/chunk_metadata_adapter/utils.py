...
def to_flat_dict(data: dict, parent_key: str = '', sep: str = '.', for_redis: bool = False) -> dict:
    """
    Recursively flattens a nested dictionary. If for_redis=True, all values are serialized as strings (ready for Redis):
    ...
    If for_redis=False, old behavior is preserved.
    """
    import json, enum
    ...
    for k, v in data.items():
        ...
        # Convert objects to dict (if not a built-in type)
            ...
            for obj_k, obj_v in obj_dict.items():
                ...
                if isinstance(obj_v, (list, tuple)):
                    ...
                    items.update(to_flat_dict(obj_v, obj_key, sep=sep, for_redis=for_redis))
        ...
        if isinstance(v, dict):
            items.update(to_flat_dict(v, new_key, sep=sep, for_redis=for_redis))
