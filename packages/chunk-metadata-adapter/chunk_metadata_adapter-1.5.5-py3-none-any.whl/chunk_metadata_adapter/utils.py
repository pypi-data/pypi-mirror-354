...
def dict_prop_to_flat_dict(value, separator='=', allow_none=True) -> dict:
    ...
    - Supports serialization of bool, int, float, ChunkId, uuid.UUID as strings.
    - For special keys (tags, links, block_meta), auto-convert single values to list of strings.
    - Raises ValueError with detailed info if type is invalid.
    """
    SPECIAL_LIST_KEYS = {"tags", "links"}
    SPECIAL_DICT_KEYS = {"block_meta"}

    if (value is None and not allow_none) or not isinstance(value, dict):
        raise ValueError(f"value must be a dict, got: {type(value)} (value={value!r})")
    if value is None:
        return {}

    flat_dict = {}
    for k, v in value.items():
        # Special handling for keys that must be list of strings
        if k in SPECIAL_LIST_KEYS:
            # Auto-convert single value to list of strings
            if isinstance(v, str):
                v = [v]
                ...
                if v is None:
                    ...
                    v = [str(v)] if isinstance(v, (int, float, bool, uuid.UUID)) or type(v).__name__ == 'ChunkId' else v
            # Now v must be a list
            if not isinstance(v, list):
                raise ValueError(f"Field '{k}' must be a list of strings, got {type(v).__name__}: {v!r}")
            # Check all elements are strings
            for elem in v:
                if not isinstance(elem, str):
                    raise ValueError(f"Field '{k}' must be a list of strings, got element of type {type(elem).__name__}: {elem!r}")
            flat_dict[k] = list_to_str(v, separator=',', allow_none=allow_none)
        elif k in SPECIAL_DICT_KEYS:
            # block_meta must be a dict
            if v is None:
                v = {}
            if not isinstance(v, dict):
                raise ValueError(f"Field '{k}' must be a dict, got {type(v).__name__}: {v!r}")
            for k2, v2 in v.items():
                if not isinstance(k2, str):
                    raise ValueError(f"Field '{k}.{k2}' key must be a string, got {type(k2).__name__}: {k2!r}")
                if not isinstance(v2, (str, int, float, bool, uuid.UUID)) and type(v2).__name__ != 'ChunkId':
                    raise ValueError(f"Field '{k}.{k2}' must be a string, int, float, bool, or ChunkId, got {type(v2).__name__}: {v2!r}")
            ...
            for k2, v2 in v.items():
                if isinstance(v2, (str, int, float, bool, uuid.UUID)) or type(v2).__name__ == 'ChunkId':
                    ...
                    raise ValueError(f"Field '{k}.{k2}' must be a string, int, float, bool, ChunkId, or list of strings, got {type(v2).__name__}: {v2!r}")
        elif isinstance(v, (str, int, float, bool, uuid.UUID)) or type(v).__name__ == 'ChunkId':
            flat_dict[k] = str(v)
        elif isinstance(v, list):
            flat_dict[k] = list_to_str(v, separator=',', allow_none=allow_none)
        else:
            raise ValueError(f"Field '{k}' must be a string, int, float, bool, ChunkId, list of strings, or dict, got {type(v).__name__}: {v!r}")
    return flat_dict
