...
class LangnameUtils():
    ...
    # and the local version has not. Hence we can't just pickle it (class not found)
        if info != None:
            self.fromdict(info)
        else:
            self.iso639 = {}
    ...
    def todict(self):
        return {'iso639': self.iso639, 'two2three': self.two2three, 'iso_conv': self.iso_conv, 'glot_conv': self.glot_conv, 'altname_conv': self.altname_conv, 'macros': self.macros}
    ...
    def update(self, data_folder: str = 'data/'):
        ...
        # TODO cache all possible three character codes?
        self.iso639 = {}
        self.two2three = {}
                ...
                if file.endswith('ini'):
                    ...
                    if True:
                        ...
                        if 'iso639-3' not in config['core']:
                            continue
                        iso639_code = self.toISO(config['core']['iso639-3'])
    ...
    def toISO(self, code, also_search_fullnames=False):
        if code == None:
            return code
