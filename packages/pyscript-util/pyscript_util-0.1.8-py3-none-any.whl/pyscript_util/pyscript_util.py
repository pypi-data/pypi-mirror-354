...
def chdir_to_cur_file():
    ...
    while frame_index < 10:  # Limit search to prevent infinite loops
        try:
            ...
            abs_potential_file = os.path.abspath(potential_file)
            abs_current_file = os.path.abspath(__file__)
    ...
    if caller_file is None:
        ...
        if len(sys.argv) > 0 and sys.argv[0]:
            potential_main = os.path.abspath(sys.argv[0])
...
def find_file_upwards(filename) -> typing.Optional[str]:
    ...
    current_path = os.path.normpath(os.path.abspath(os.getcwd()))
