...
class CanonMap:
    ...
    def match_entity(
        ...
        Args:
            ...
            threshold (float): Minimum score threshold for matches (default: 0.0)
            field_filter (Optional[List[str]]): List of field names to restrict matching to
            user_semantic_search (bool): Whether to enable semantic search (default: False)
        ...
        logger.info(f"Parameters: top_k={top_k}, threshold={threshold}, semantic_search={user_semantic_search}")

        # build the matcher, injecting our embedder so users don't need to supply one
        ...
        # compute a query embedding only if semantic search is enabled
        q_emb = None
        ...
        # call match with keyword args so field_filter is mapped correctly
