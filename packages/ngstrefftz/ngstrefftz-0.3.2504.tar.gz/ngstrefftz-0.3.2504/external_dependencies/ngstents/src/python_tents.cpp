...
auto ExportTimeSlab(py::module &m) {
  ...
  py::class_<TentPitchedSlab, shared_ptr<TentPitchedSlab>>(m, pyname, pydocu)
      .def(py::init([](shared_ptr<MeshAccess> ma, string method_name,
                       int heapsize) {
             ngstents::PitchingMethod method = [method_name] {
               if (method_name == "edge")
                 ...
                 return ngstents::EVolGrad;
                    ...
                    // case
               {
                 cout << "Invalid method! Setting edge algorithm as default..."
                      << endl;
                 return ngstents::EEdgeGrad;
               }
             }();
      ...
      .def(
          ...
          [](shared_ptr<TentPitchedSlab> self, const double dt,
            ...
            case 3:
              ...
              break;
            default:
              throw Exception("TentPitchedSlab not available for dimension " +
                              ToString(dim));
            }
            return success;
          },
