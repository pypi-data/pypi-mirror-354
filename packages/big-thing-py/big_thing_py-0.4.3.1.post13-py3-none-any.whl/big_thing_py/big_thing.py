...
class MXBigThing:
    ...
    async def _setup(self) -> 'MXBigThing':
        ...
        # Recreate zeroconf instance to ensure it's bound to the current event loop
        if self._zeroconf:
    ...
    async def _BROKER_SETUP_state_process(self):
        ...
        if not (self.is_manager or self.is_builtin):
            rst = await self._register_mdns_service()
            if not rst:
    ...
    async def _register_mdns_service(self) -> bool:
        ...
        try:
            # Ensure zeroconf is working with current event loop
            if not self._zeroconf:
                self._zeroconf = Zeroconf()
                
            await self._zeroconf.async_register_service(self._mqtt_mdns_service)
            await self._zeroconf.async_register_service(self._mqtt_ssl_mdns_service)
            MXLOG_DEBUG('mDNS services registered successfully')
            return True
        except zeroconf.NonUniqueNameException as e:
            MXLOG_WARN(f'mDNS service name already exists, but continuing: {e}')
            return True
        except RuntimeError as e:
            if "attached to a different loop" in str(e):
                MXLOG_WARN('Event loop mismatch detected, recreating zeroconf instance')
                try:
                    # Close and recreate zeroconf with current loop
                    self._zeroconf.close()
                    self._zeroconf = Zeroconf()
                    await self._zeroconf.async_register_service(self._mqtt_mdns_service)
                    await self._zeroconf.async_register_service(self._mqtt_ssl_mdns_service)
                    MXLOG_DEBUG('mDNS services registered successfully after recreating zeroconf')
                    return True
                except Exception as retry_e:
                    MXLOG_ERROR(f'Failed to register mDNS services after retry: {retry_e}')
                    print_error(retry_e)
                    return False
            else:
                MXLOG_ERROR(f'RuntimeError in mDNS registration: {e}')
                print_error(e)
                return False
        except Exception as e:
            MXLOG_ERROR(f'Unexpected error in mDNS registration: {e}')
            print_error(e)
            return False

    async def _unregister_mdns_service(self) -> None:
        try:
            if self._mqtt_mdns_service and self._zeroconf:
                await self._zeroconf.async_unregister_service(self._mqtt_mdns_service)
                MXLOG_DEBUG('mDNS service unregistered successfully')
        except Exception as e:
            MXLOG_WARN(f'Failed to unregister mDNS service: {e}')
            
        try:
            if self._mqtt_ssl_mdns_service and self._zeroconf:
                await self._zeroconf.async_unregister_service(self._mqtt_ssl_mdns_service)
                MXLOG_DEBUG('mDNS SSL service unregistered successfully')
        except Exception as e:
            MXLOG_WARN(f'Failed to unregister mDNS SSL service: {e}')
