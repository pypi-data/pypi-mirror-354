...
from scoutsdk.api.constants import SCOUT_CUSTOM_FUNCTION_CONTENT_TYPE
...
class PkgModule(BaseModule):
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        self._generate_package(pkg_src_path, zip_path)
    ...
    def _upload_package(self, zip_path: str, assistant_id: str | None) -> None:
        ...
        try:
            assistant_files = scout_api.list_assistant_files(assistant_id=assistant_id)
        ...
        for assistant_file in assistant_files:
            if (
                ...
                try:
                    scout_api.delete_assistant_file(
        ...
        # upload the new file
        try:
            result = scout_api.upload_assistant_file(
                assistant_id=assistant_id, file_path=zip_path
            )
            logger.info(f"File uploaded successfully to assistant: {result.file_id}")
        except Exception as e:
    ...
    def _generate_package(self, pkg_src_path: str, zip_path: str) -> None:
