...
class ProjectData(TypedDict):
    """pyproject.toml 数据结构定义."""

    project: NotRequired[dict[str, dict]]  # PEP 621项目元数据
    tool: NotRequired[dict[str, dict]]  # 工具特定配置
    build_system: NotRequired[dict[str, dict]]  # 构建系统要求
...
class Project:
    ...
    def _parse_config(self) -> None:
        ...
        if not config_path.is_file():
            logger.error(
                f"路径下未找到 pyproject.toml 文件: {self.project_dir}",
            )
            return

        try:
            with config_path.open("rb") as f:
                self.data = ProjectData(**tomllib.load(f))
    ...
    def _parse_pep621(self, project_data: dict) -> None:
        ...
        if not self.name:
            logger.error(f"未设置项目名称, 路径: [red]{self.project_dir}")
    ...
    def _parse_poetry(self, project_data: dict | None) -> None:
