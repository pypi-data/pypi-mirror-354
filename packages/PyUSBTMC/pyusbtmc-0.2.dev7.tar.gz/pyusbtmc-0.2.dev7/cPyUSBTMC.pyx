...
(C) 2012-2025, Noboru Yamamot, Accl. Lab, KEK, JAPAN
...
def find_tmc_devices(**args):
    ...
    devs=usb.core.find(find_all=True, custom_match=is_usbtmc, **args)  # now returns  a generator. Not a list
...
class USBTMC_device:
    ...
    def device_write(self,  message:Union[str,bytes]):
        ...
        while (remaining > 0):
            ...
            if ((transferSize % 4) == 0):
                command=b"%s%s"%(h, message[:transferSize])
            else:# padding data for 4bytes boundary
                command=b"%s%s%s"%(h ,message[:transferSize],(4-(transferSize % 4))*b'\0')
    ...
    def device_read(self,
        ...
        try:
            ...
            raw_resp=self.bulk_in.read(transferSize+USBTMC_HEADER_SIZE+4, timeout = timeout)
...
def mkReqDevDepMSGINHeader(tsize,
                           TermCharEnable,
                           TermChar=b'\n'):
...
class BulkIn_Header:
    ...
    def __init__(self, bdata):
        self.msgID=bdata[0]
        self.bTag=bdata[1]
        self.bTagInverse=bdata[2]
        self.resMsg=bdata[4:12]
        try:
            self.data=bdata[12:]
        except:
            self.data=None
...
class DevDep_Responce(BulkIn_Header):
    def __init__(self,bdata):
        BulkIn_Header.__init__(self, bdata)
...
class Interrupt_IN_Data:# Table 6, USB488
    def __init__(self, bdata):
        self.SBR=None
        self.statusBdata=None
        if (bdata[0] & 0x80):# USB488
            self.bTag=bdata[0]&0x7f # 0x01 for SRQ responce or bTag_id for READ_Status
            self.SBR=IEEE488_Status_Byte_Reg(bdata[1])
        elif (bdata[0] & 0x40): # vendor specific
            self.statusBytes=bdata[1:]
        else:
            self.statusBytes=bdata[1:]
#
def test(**args):
    ...
    try:
        ...
        if (tmcdev.device.idVendor == 1689): #0x0699 == Tektronix
...
def DSO6K_test(dev=None):
    ...
    n,d=dev.ask0("*IDN?;")
    sys.stdout.write(d.bytes());sys.stdout.write("\n")
    maker,model,sn,vers=d.tobytes().split(",")
...
def DSO6K_test_pylab(dev=None):
    ...
    dev.write(":STOP;:SINGLE;:WAV:POINTS:MODE MAX;:WAV:POINTS 50000;")
    n,d=dev.ask("*IDN?;")
    sys.stdout.write(d.tobyes());sys.stdout.write("\n")
    maker,model,sn,vers=d.tobyes().split(",")
    dev.write(":WAV:SOURCE CH1;\n")
    dev.write(":WAV:POINTS 100000;\n")
...
def SL1K_test():
    ...
    res=dev.ask(":WAV:LENGTH?")[1].tobyes()
...
def AFG3K_test():
    ...
    dev.write("*RST\n");time.sleep(3); 
    sys.stdout.write( dev.IDN()[1].tobyes());sys.stdout.write("\n")
