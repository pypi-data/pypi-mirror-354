...
class DerivativesViews:
    ...
    def derivatives_futures_curve(  # noqa: PLR0912
        **kwargs,
        ...
        -------
        Tuple[KozmoAIFigure, Dict[str, Any]]
            Tuple with the KozmoAIFigure object, and the JSON-serialized content.
        ...
        if df.empty:
            raise KozmoAIError("Error: No data to plot.")

        if "expiration" not in df.columns:
            raise KozmoAIError("Expiration field not found in the data.")
        ...
        figure = KozmoAIFigure().create_subplots(shared_xaxes=True)
