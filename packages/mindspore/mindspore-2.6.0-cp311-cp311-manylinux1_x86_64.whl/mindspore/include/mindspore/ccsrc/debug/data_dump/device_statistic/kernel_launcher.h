...
std::vector<DeviceAddressPtr> CalStatisticAsync(const std::string &, const DeviceContext *, KernelTensor *,
                                                const std::uint32_t);

bool CalCheckOverflow(const DeviceContext *, const std::vector<KernelTensor *> &, const std::uint32_t);

DeviceAddressPtr CalCheckOverflowAsync(const DeviceContext *, std::vector<KernelTensor *>, const std::uint32_t);
}  // namespace datadump
