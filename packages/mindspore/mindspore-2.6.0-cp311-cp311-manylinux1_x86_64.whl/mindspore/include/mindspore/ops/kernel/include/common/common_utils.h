...
class OPS_KERNEL_COMMON_API MatrixInfo {
 public:
...
OPS_KERNEL_COMMON_API int CalDiagOffset(int diag_index, int max_diag_len, int inner_rows, int inner_cols,
                                        const std::pair<MatrixDiag::Alignment, MatrixDiag::Alignment> &alignment);
OPS_KERNEL_COMMON_API TypeId DtypeToTypeId(const std::string &dtypes);
OPS_KERNEL_COMMON_API std::string Dtype2ShortType(const std::string &dtype);
OPS_KERNEL_COMMON_API size_t GetDtypeNbyte(const std::string &dtype);
OPS_KERNEL_COMMON_API bool IsSameShape(const ShapeVector &shape_a, const ShapeVector &shape_b);
OPS_KERNEL_COMMON_API bool CheckShapesSame(const ShapeArray &shape_array);
template <typename T>
...
class OPS_KERNEL_COMMON_API KernelAttr {
 public:
  KernelAttr() = default;
  ~KernelAttr() = default;
...
OPS_KERNEL_COMMON_API size_t GetOutputNum(const AnfNodePtr &node);
OPS_KERNEL_COMMON_API std::ostream &operator<<(std::ostream &os, KernelAttr kernel_attr);

OPS_KERNEL_COMMON_API std::pair<bool, size_t> MatchKernelAttr(const KernelAttr &kernel_attr,
                                                              const std::vector<KernelAttr> &kernel_attr_list);
OPS_KERNEL_COMMON_API std::pair<bool, size_t> MatchKernelAttrStrict(const KernelAttr &kernel_attr,
                                                                    const std::vector<KernelAttr> &kernel_attr_list);
OPS_KERNEL_COMMON_API KernelAttr GetKernelAttrFromBuildInfo(const KernelBuildInfoPtr &build_info);
OPS_KERNEL_COMMON_API bool IsFoldKernelBuildInfo(const KernelBuildInfoPtr &kernel_build_info);
OPS_KERNEL_COMMON_API KernelAttr GetKernelAttrFromTensors(const std::vector<KernelTensor *> &inputs,
                                                          const std::vector<KernelTensor *> &outputs);
// Synchronize the output and input reference map between two kernel attrs.
OPS_KERNEL_COMMON_API void SyncOutInRef(const KernelAttr &from_kernel_attr, KernelAttr *to_kernel_attr);
OPS_KERNEL_COMMON_API std::string FetchPrintInfoByKernelAttr(KernelAttr selected_kernel_attr);
OPS_KERNEL_COMMON_API std::vector<TypeId> GetInputObjectTypeListFromKernelAttr(const KernelAttr &kernel_attr);
OPS_KERNEL_COMMON_API std::vector<TypeId> GetOutputObjectTypeListFromKernelAttr(const KernelAttr &kernel_attr);
OPS_KERNEL_COMMON_API std::pair<std::vector<DataType>, std::vector<DataType>> GetInOutDataTypesFromKernelAttr(
  const KernelAttr &kernel_attr);
// Tuple --> Tuple.
OPS_KERNEL_COMMON_API KernelObjectType TypeIdToKernelObjectType(const TypeId &type_id);
OPS_KERNEL_COMMON_API std::vector<KernelObjectType> TypeIdToKernelObjectType(const std::vector<TypeId> &type_ids);
// Tuple --> TupleUnfold.
OPS_KERNEL_COMMON_API KernelObjectType TypeIdToKernelObjectTypeForTupleUnfold(const TypeId &type_id);
OPS_KERNEL_COMMON_API std::vector<KernelObjectType> TypeIdToKernelObjectTypeForTupleUnfold(
  const std::vector<TypeId> &type_ids);
OPS_KERNEL_COMMON_API TypeId KernelObjectTypeToTypeId(const KernelObjectType &object_type);

OPS_KERNEL_COMMON_API bool CheckAttrForAllSameInput(const size_t input_num,
                                                    const std::vector<mindspore::TypeId> &input_types,
                                                    const KernelAttr &cur_kernel_attr);

OPS_KERNEL_COMMON_API std::vector<KernelObjectType> CalOutputElementObjectTypes(const AnfNodePtr &kernel_node,
                                                                                const KernelAttr &selected_kernel_attr);
OPS_KERNEL_COMMON_API std::vector<KernelObjectType> CalKernelObjectTypes(
