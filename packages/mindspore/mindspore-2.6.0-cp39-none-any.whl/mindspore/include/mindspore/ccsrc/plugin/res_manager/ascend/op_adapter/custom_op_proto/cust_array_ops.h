...
REG_CUST_OP(SliceGrad)
  .INPUT(dy, TensorType({DT_BOOL, DT_INT8, DT_INT16, DT_INT32, DT_INT64, DT_UINT8, DT_UINT16, DT_UINT32, DT_UINT64,
                         DT_FLOAT16, DT_FLOAT, DT_DOUBLE, DT_COMPLEX64, DT_COMPLEX128, DT_BFLOAT16}))
  ...
  .OUTPUT(dx, TensorType({DT_FLOAT16, DT_FLOAT, DT_DOUBLE, DT_BFLOAT16}))
...
REG_CUST_OP(TopKRouter)
  .INPUT(x, TensorType({DT_INT32, DT_INT64}))
  .INPUT(capacity, TensorType::IndexNumberType())
  .INPUT(expert_num, TensorType::IndexNumberType())
  .INPUT(drop_type, TensorType::IndexNumberType())
  .OUTPUT(dispatch_index, TensorType({DT_INT32, DT_INT64}))
  .OUTPUT(combine_index, TensorType({DT_INT32, DT_INT64}))
  .CUST_OP_END_FACTORY_REG(TopKRouter)
