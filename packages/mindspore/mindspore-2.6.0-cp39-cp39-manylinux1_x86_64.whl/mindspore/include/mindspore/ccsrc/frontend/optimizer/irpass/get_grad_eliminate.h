...
class GetGradEliminater : public AnfVisitor {
 public:
  AnfNodePtr operator()(const OptimizerPtr &, const AnfNodePtr &node) override {
    ...
    if (result_ == nullptr) {
      MS_LOG_WITH_NODE(INTERNAL_EXCEPTION, node) << "Can not find the gradient for position or Parameter provided";
    }
    return result_;
  }
    ...
    if (IsPrimitiveCNode(node, prim::kPrimMakeTuple)) {
      ...
      if (input[1]->isa<Parameter>()) {
        ...
        if (para->abstract()->isa<abstract::AbstractRefTensor>()) {
          ...
          auto ref_str = ref_tensor->ref_key_value()->cast_ptr<StringImm>();
          MS_EXCEPTION_IF_NULL(ref_str);
          name_ = ref_str->value();
        }
        ...
        MS_LOG_WITH_NODE(EXCEPTION, node) << "Suppose to get tensor or parameter, but got: " << input[1]->DebugString();
      }
    }
  }
