...
class OPS_KERNEL_COMMON_API KernelMod {
 public:
  KernelMod() = default;
...
OPS_KERNEL_COMMON_API std::vector<std::vector<int64_t>> GetShapes(const std::vector<KernelTensor *> &tensors);

OPS_KERNEL_COMMON_API void ConvertLaunchInfoToAddr(const KernelLaunchInfo &launch_info, KernelLaunchAddr *mem_info);
