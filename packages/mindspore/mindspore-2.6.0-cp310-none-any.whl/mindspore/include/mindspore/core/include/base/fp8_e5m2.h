 ...
 */
#ifndef MINDSPORE_CORE_BASE_FLOAT8_E5M2_H_
#define MINDSPORE_CORE_BASE_FLOAT8_E5M2_H_
...
class Float8_e5m2 {
 public:
  ...
  Float8_e5m2(const Float8_e5m2 &other) noexcept = default;
  Float8_e5m2(Float8_e5m2 &&other) noexcept = default;

  Float8_e5m2 &operator=(const Float8_e5m2 &other) noexcept = default;
  Float8_e5m2 &operator=(Float8_e5m2 &&other) noexcept = default;

  static Float8_e5m2 FromRaw(uint8_t v) {
    Float8_e5m2 f;
    f.value_ = v;
    return f;
  }

  explicit Float8_e5m2(float f) : value_(FromFloat32(f)) {}
  explicit Float8_e5m2(bool b) : value_(b ? true_value : 0) {}
  template <typename T>
  ...
  Float8_e5m2 &operator+=(const Float8_e5m2 &b) {
  ...
  static float ToFloat32(const Float8_e5m2 &e5m2) {
    ...
    constexpr unsigned int exponent_bits = 21;
    constexpr unsigned int sign_bit_shift = 24;
    // Exponent/mantissa bits.
    Union32 f32;
    f32.u = (static_cast<uint32_t>(e5m2.value_ & value_mask) << exponent_bits);
    // Just the exponent.
    unsigned int exp = (shifted_exp & f32.u);
    f32.u += exponent_adjust;
    // Handle exponent special cases.
    if (exp == shifted_exp) {
      // NaN, extra exp adjust.
    ...
    // Set sign bit.
    f32.u |= ((e5m2.value_ & sign_mask) << sign_bit_shift);
    return f32.f;
  }
 ...
 private:
  static uint8_t FromFloat32(float f32) {
    ...
    constexpr uint32_t rouding_bias_part1 = (static_cast<unsigned int>(15 - 127) << 23) + 0xfffff;
...
inline Float8_e5m2 operator+(const Float8_e5m2 &a, const Float8_e5m2 &b) {
...
// accuracy issues in converting the denominator to Float8_e5m2.
inline Float8_e5m2 operator/(const Float8_e5m2 &a, size_t b) {
  return Float8_e5m2(static_cast<float>(a) / static_cast<float>(b));
}
...
inline bool operator==(const Float8_e5m2 &a, const Float8_e5m2 &b) {
...
inline std::ostream &operator<<(std::ostream &os, const Float8_e5m2 &v) { return (os << static_cast<float>(v)); }
...
template <>
struct numeric_limits<const mindspore::Float8_e5m2> : private numeric_limits<mindspore::Float8_e5m2> {};
template <>
struct numeric_limits<volatile mindspore::Float8_e5m2> : private numeric_limits<mindspore::Float8_e5m2> {};
template <>
struct numeric_limits<const volatile mindspore::Float8_e5m2> : private numeric_limits<mindspore::Float8_e5m2> {};
}  // namespace std

#endif  // MINDSPORE_CORE_BASE_FLOAT8_E5M2_H_
