...
class OutlookCalendarPlugin:
    ...
    def _get_access_token(self) -> str:
        ...
        if isinstance(access_token, str):
            ...
            logger.info("User access token  %s", access_token)
        return access_token
        ...
        with tracer.start_as_current_span("SemanticKernelOrchestrator_orchestrate.get_calendar_events"):
            ...
            try:
                ...
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain the user in his language(if unclear en-US) that you failed to get calendar appointment due to an error.",
                )
                return answer
            ...
            if resp.status_code != 200:
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain the user in his language(if unclear en-US) that you failed to fetch calendar events due to an error.",
                )
                return answer
            ...
            if not events:
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain the user in his language(if unclear en-US) that no events were found in the calendar.",
                )
                return answer
            ...
            answer = TextProcessingTool().answer_question(
                    ...
                    chat_history=self.chat_history,
                    text=events_text,
                    operation="Summarize the calendar schedule in the user's language(if unclear en-US).",
                )
            return answer
        ...
        with tracer.start_as_current_span("SemanticKernelOrchestrator_orchestrate.schedule_appointment"):
            ...
            try:
                ...
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain the user in his language(if unclear en-US) that you failed to schedule a calendar appointment due to an error.",
                )
                return answer
            headers = {
            ...
            if resp.status_code == 201:
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain to the user in his language(if unclear en-US) that the appointment was scheduled successfully. And summarize the appointment details. And all appointments shortly before and after the scheduled appointment.",
                )
                return answer
            else:
                answer = TextProcessingTool().answer_question(
                    ...
                    operation="Explain to the user in his language(if unclear en-US) that the appointment scheduling failed.",
                )
                return answer