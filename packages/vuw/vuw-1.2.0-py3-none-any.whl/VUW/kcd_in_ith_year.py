...
def kcd_in_ith_year(df, id_var, kcd_var, from_var, to_var, kcd_code, udate, year, col=None):
    """
    ...
    # compiled_regex = re.compile(kcd_code)

    # dm = df[df[kcd_var].apply(lambda x: bool(compiled_regex.fullmatch(str(x))))]
    ...
    if col is None:
        if year < 0:
            col = f"{kcd_code}_pst_{abs(year)}"
        else:
            col = f"{kcd_code}_ftr_{abs(year)}"

    # 날짜 조건에 맞는 데이터 필터링
    if year < 0:
        df_filtered = dm[(pd.to_datetime(df[from_var]).dt.date < datetime.strptime(udate, '%Y-%m-%d').date()) & (pd.to_datetime(dm[to_var]) >= pd.to_datetime(add_year(udate, year)))]
    else:
        df_filtered = dm[(pd.to_datetime(dm[from_var]) < pd.to_datetime(add_year(udate, year))) & (pd.to_datetime(dm[to_var]).dt.date >= datetime.strptime(udate, '%Y-%m-%d').date())]
...
if __name__ == '__main__':
    ...
    udate = '2023-03-01'
    # 함수 호출
    result = kcd_in_ith_year(df, id_var='id', kcd_var='kcd_code', from_var='from', 
