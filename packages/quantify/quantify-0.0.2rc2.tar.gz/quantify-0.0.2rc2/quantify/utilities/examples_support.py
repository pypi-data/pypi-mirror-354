...
import quantify.data.dataset_attrs as dd
...
def mk_iq_shots(  # noqa: D417
    num_shots: int = 128,
    sigmas: tuple[float, float] | NDArray[np.float64] = (0.1, 0.1),
    centers: tuple[complex, complex] | NDArray[np.complex128] = (
        -0.2 + 0.65j,
        0.7 + 4j,
    ),
    probabilities: tuple[float, float] | NDArray[np.float64] = (0.4, 0.6),
...
def mk_dataset_attrs(
    tuid: TUID | Callable[[], TUID] = dh.gen_tuid, **kwargs
) -> dict[str, Any]:
    """
    A factory of attributes for Quantify dataset.

    See :class:`~quantify.data.dataset_attrs.QDatasetAttrs` for details.

    Parameters
    ----------
    tuid
        If no tuid is provided a new one will be generated.
        See also :attr:`~quantify.data.dataset_attrs.QDatasetAttrs.tuid`.
    **kwargs
...
def mk_main_coord_attrs(
    ...
    **kwargs,
    ...
    See :class:`~quantify.data.dataset_attrs.QCoordAttrs` for details.

    Parameters
    ----------
    uniformly_spaced
        See :attr:`quantify.data.dataset_attrs.QCoordAttrs.uniformly_spaced`.
    is_main_coord
        See :attr:`quantify.data.dataset_attrs.QCoordAttrs.is_main_coord`.
    **kwargs
        Any other items used to update the output dictionary.
...
def mk_main_var_attrs(
    ...
    **kwargs,
    ...
    See :class:`~quantify.data.dataset_attrs.QVarAttrs` for details.

    Parameters
    ----------
    grid
        See :attr:`quantify.data.dataset_attrs.QVarAttrs.grid`.
    uniformly_spaced
        See :attr:`quantify.data.dataset_attrs.QVarAttrs.uniformly_spaced`.
    is_main_var
        See :attr:`quantify.data.dataset_attrs.QVarAttrs.is_main_var`.
    has_repetitions
        See :attr:`quantify.data.dataset_attrs.QVarAttrs.has_repetitions`.
    **kwargs
        Any other items used to update the output dictionary.
