...
def hanger_func_complex_SI(  # noqa: N802
    f: float,
    fr: float,
    Ql: float,  # noqa: N803
    Qe: float,  # noqa: N803
    ...
    r"""
    Complex function for a hanger (lambda/4 resonator).
...
def exp_damp_osc_func(  # noqa: D417
    t: float,
    tau: float,
    n_factor: float,
    frequency: float,
...
class LorentzianModel(lmfit.model.Model):
    ...
    def guess(
        self, data: NDArray, x: NDArray | None = None, **kws
    ) -> lmfit.parameter.Parameters | None:
...
class ResonatorModel(lmfit.model.Model):
    ...
    # pylint: disable=missing-function-docstring
    def guess(  # noqa: D102
        self, data: np.ndarray, x: NDArray | None = None, **kws
    ) -> lmfit.parameter.Parameters | None:
        if x is None:
            f = kws.get("f")
            if f is None:
        ...
        self.set_param_hint("A", value=np.mean(abs(data)), min=0)  # type: ignore
...
class ExpDecayModel(lmfit.model.Model):
    ...
    # pylint: disable=missing-function-docstring
    def guess(  # noqa: D102
        self, data: np.ndarray, x: np.ndarray | None = None, **kws
    ) -> lmfit.parameter.Parameters | None:
        if x is None:
            delay = kws.get("delay")
            if delay is None:
...
def resonator_phase_guess(s21: np.ndarray, freq: np.ndarray) -> tuple[float, float]:
    ...
    """
    phase: np.ndarray = np.angle(s21)

    med_diff: float = float(np.median(np.diff(phase)))
    freq_step: float = float(np.median(np.diff(freq)))

    phi_v: float = med_diff / freq_step

    phi_0: float = phase[0] - phi_v * freq[0]
