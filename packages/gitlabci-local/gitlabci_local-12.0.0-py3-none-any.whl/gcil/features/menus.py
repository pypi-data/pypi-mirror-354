...
class MenusFeature:
    ...
    def configure(
        ...
        for variable in configurations:
            ...
            variable_finds: Optional[Dicts.Finds]
            ...
            # Parse configuration types: json
                ...
                if not variable_set:
                    ...
                    with open(
                        ...
                        variable_finds = Dicts.find(
                            configuration_dict,
                            configuration_key,
                        )
                        if not variable_finds:
                            raise ValueError(
                                f'Unknown "{configuration_key}" key in' \
                                    f' {configuration_path} for f"{variable}"'
                            )
                        if isinstance(variable_finds, list):
                            variable_values = variable_finds
                        elif isinstance(variable_finds, str):
                            variable_values = [variable_finds]
                        else:
                            assert isinstance(
                                variable_finds,
