...
class GitLab:
    ...
    # Local deprecation
    @staticmethod
    def __local_deprecation(key: str, issue: str) -> None:
        print(' ')
        print(f'  {Colors.YELLOW}‣ WARNING: '
              f'{Colors.RED}.local: {key}: '
              f'{Colors.BOLD}YAML configuration is deprecated, see: '
              f'{Colors.GREEN}{Bundle.REPOSITORY}/-/issues/{issue}'
              f'{Colors.RESET}')
        Platform.flush()
    ...
    def __local(
        ...
        if GitLab.LOCAL_NODE in data and data[GitLab.LOCAL_NODE]:
            ...
            # Parse local after
            if 'after' in local and options.after:
                GitLab.__local_deprecation('after', '295')
                pipeline.variables.globals[Bundle.FLAG_AFTER] = Flags.string(
                    local['after'])
            ...
            # Parse local bash
            if 'bash' in local and not options.bash:
                GitLab.__local_deprecation('bash', '295')
                pipeline.variables.globals[Bundle.FLAG_BASH] = Flags.string(local['bash'])

            # Parse local before
            if 'before' in local and options.before:
                GitLab.__local_deprecation('before', '295')
                pipeline.variables.globals[Bundle.FLAG_BEFORE] = Flags.string(
                    local['before'])

            # Parse local debug
            if 'debug' in local and not options.debug:
                GitLab.__local_deprecation('debug', '295')
                pipeline.variables.globals[Bundle.FLAG_DEBUG] = Flags.string(
                    local['debug'])

            # Parse local defaults
            if 'defaults' in local and not options.defaults:
                GitLab.__local_deprecation('display', '299')
                options.defaults = local['defaults']

            # Parse local display
            if 'display' in local and not options.display:
                GitLab.__local_deprecation('display', '295')
                pipeline.variables.globals[Bundle.FLAG_DISPLAY] = Flags.string(
                    local['display'])

            # Parse local engine
            if 'engine' in local and options.engine_default:
                GitLab.__local_deprecation('engine', '295')
                options.engine_default = False
                pipeline.variables.globals[Bundle.FLAG_ENGINE] = Flags.string(
                    local['engine'])
            ...
            # Parse local manual
            if 'manual' in local and not options.manual:
                GitLab.__local_deprecation('manual', '295')
                pipeline.variables.globals[Bundle.FLAG_MANUAL] = Flags.string(
                    local['manual'])
            ...
            # Parse local network
            if 'network' in local and not options.network:
                GitLab.__local_deprecation('network', '286')
                options.network = local['network']

            # Parse local no_console
            if 'no_console' in local and not options.no_console:
                GitLab.__local_deprecation('no_console', '295')
                pipeline.variables.globals[Bundle.FLAG_NO_CONSOLE] = Flags.string(
                    local['no_console'])
            ...
            if 'no_verbose' in local and not options.no_verbose:
                GitLab.__local_deprecation('no_verbose', '295')
                pipeline.variables.globals[Bundle.FLAG_NO_VERBOSE] = Flags.string(
                    local['no_verbose'])

            # Parse local notify
            if 'notify' in local and not options.notify:
                GitLab.__local_deprecation('notify', '295')
                pipeline.variables.globals[Bundle.FLAG_NOTIFY] = Flags.string(
                    local['notify'])
            ...
            # Parse local quiet
            if 'quiet' in local and not options.quiet:
                GitLab.__local_deprecation('quiet', '295')
                pipeline.variables.globals[Bundle.FLAG_QUIET] = Flags.string(
                    local['quiet'])

            # Parse local random_paths
            if 'random_paths' in local and not options.random_paths:
                GitLab.__local_deprecation('random_paths', '295')
                pipeline.variables.globals[Bundle.FLAG_RANDOM_PATHS] = Flags.string(
                    local['random_paths'])

            # Parse local real_paths
            if 'real_paths' in local and not options.real_paths:
                GitLab.__local_deprecation('real_paths', '295')
                pipeline.variables.globals[Bundle.FLAG_REAL_PATHS] = Flags.string(
                    local['real_paths'])

            # Parse local shell
            if 'shell' in local and not options.shell:
                GitLab.__local_deprecation('shell', '295')
                pipeline.variables.globals[Bundle.FLAG_SHELL] = Flags.string(
                    local['shell'])

            # Parse local sockets
            if 'sockets' in local and not options.sockets:
                GitLab.__local_deprecation('sockets', '295')
                pipeline.variables.globals[Bundle.FLAG_SOCKETS] = Flags.string(
                    local['sockets'])

            # Parse local ssh
            if 'ssh' in local and not options.ssh: # pragma: no cover
                GitLab.__local_deprecation('ssh', '295')
                if isinstance(local['ssh'], bool) and local['ssh']:
                    pipeline.variables.globals[
                        Bundle.FLAG_SSH] = Bundle.ARGUMENT_SSH_USER_DEFAULT
                elif isinstance(local['ssh'], str):
                    pipeline.variables.globals[Bundle.FLAG_SSH] = Flags.string(
                        local['ssh'])
            ...
            # Parse local workdir
            if 'workdir' in local and not options.workdir:
                GitLab.__local_deprecation('workdir', '295')
                pipeline.variables.globals[
                    Bundle.FLAG_WORKDIR] = Volumes.LOCAL_FLAG + Flags.string(
                        local['workdir'])

            # Parse local configurations
            if 'configurations' in local:
                GitLab.__local_deprecation('configuration', '295')
                ...
                configured_variables = MenusFeature(
                    pipeline=None,
                    options=options,
                ).configure(local['configurations'])
                pipeline.variables.parameters.update(configured_variables)
