...
import furigrego.process_ebook


def main():
    parser = ArgumentParser(
        description="The fastest converter to add furigrego(readings) to Japanese epub eBooks! (Using Mecab and Unidic)"
    )
...
def process_ebooks(arg_paths: List[str]):
    for arg_path in arg_paths:
        ...
        output_path = path.join(file_dir, f"{file_name}_furigana")
        ...
        with open(file_path, "rb") as f_reader, open(output_path, "wb") as f_writer:
            ...
            process_ebook.process_ebook(f_reader, f_writer)
