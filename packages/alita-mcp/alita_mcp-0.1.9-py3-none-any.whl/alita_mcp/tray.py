...
class MCPTrayApp:
    ...
    def create_icon_image(self):
        ...
        try:
            ...
            print(f"Warning: Could not load logo icon ({e}), using fallback")
            return self._create_fallback_icon()
    ...
    def _load_icon_data(self):
        ...
        try:
            ...
            print(f"Failed to load icon data: {e}")
    ...
    def get_menu_items(self):
        ...
        menu_items = [
            pystray.MenuItem("Alita MCP Client", lambda: None, enabled=False),
        ...
        if is_configured:
            if not self.is_server_running:
                ...
                # Show stop option when server is running
                status = self.server_controller.get_server_status()
                mode = " (Daemon)" if status.get("daemon_mode", False) else " (Embedded)"
                menu_items.extend([
                    pystray.MenuItem(f"Stop MCP Server{mode}", self.stop_server),
                ])
    ...
    def open_config_folder(self, icon=None, item=None):
        ...
        print(f"Opening config folder: {config_folder}")
    ...
    def bootstrap_config(self, icon=None, item=None):
        ...
        print("\n" + "="*50)
        print("Running Bootstrap Configuration...")
        print("="*50)
    ...
    def view_config(self, icon=None, item=None):
        ...
        print("="*50)
        print(f"Deployment URL: {config.get('deployment_url', 'Not set')}")
    ...
    def show_about(self, icon=None, item=None):
        ...
        print("\n" + "="*50)
        print("About Alita MCP Client")
        print("="*50)
        print("Version: 0.1.6")
        print("A Model Context Protocol client for the ELITEA platform.")
        print("\nThis tray application provides easy access to:")
        print("• Configuration management")
        print("• MCP server control")
        print("• Quick status monitoring")
        print("="*50)
    ...
    def run(self):
        ...
        print("Starting Alita MCP Client tray application...")
        print("Right-click the tray icon for options")
        print("Use 'Open Config File' to edit configuration")
        self.icon.run()
    ...
    def _hide_from_dock(self):
        ...
        if platform.system().lower() == "darwin":
            try:
                ...
                print("Successfully hidden from macOS dock")
                
            except ImportError:
                print("Warning: PyObjC not available, cannot hide from dock")
                print("Install with: pip install pyobjc-framework-Cocoa")
            except Exception as e:
                print(f"Warning: Failed to hide from dock: {e}")
        else:
