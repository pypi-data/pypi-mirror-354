...
class EvinVerify:
    ...
    @staticmethod
    def verify_cmpdinm(d_otex: TyDic, doaod_vfy: TyDoAoD) -> TyBool:
        """
        Verify Company display name
        """
        _cmpdinm = Dic.get(d_otex, Cfg.otex_key_cmpdinm)
        if not _cmpdinm:
            DoA.append_unique_by_key(doaod_vfy, 'cmpdinm_is_empty', d_otex)
            return False
        return True

    @staticmethod
    def verify_regno(d_otex: TyDic, doaod_vfy: TyDoAoD) -> TyBool:
        """
        Verify Registration number
        """
        _cmpdinm = Dic.get(d_otex, Cfg.otex_key_regno)
        if not _cmpdinm:
            DoA.append_unique_by_key(doaod_vfy, 'regno_is_empty', d_otex)
            return False
        return True
    ...
    def verify_d_otex(
        ...
        _sw_vfy_cmpdinm = kwargs.get('sw_vfy_cmpdinm', True)
        if _sw_vfy_duns:
            _d_sw['cmpdinm'] = cls.verify_cmpdinm(d_otex, doaod_vfy)

        # Verify Country display name
        _sw_vfy_regno = kwargs.get('sw_vfy_regno', True)
        if _sw_vfy_duns:
            _d_sw['regno'] = cls.verify_regno(d_otex, doaod_vfy)
