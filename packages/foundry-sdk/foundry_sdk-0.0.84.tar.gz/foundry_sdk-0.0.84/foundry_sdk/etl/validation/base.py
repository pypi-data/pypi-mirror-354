...
def encode_boolean_values(df: pd.DataFrame) -> pd.DataFrame:
    """
    Converts boolean-like values in the DataFrame to string "1"/"0",
    ...
    Returns:
        pd.DataFrame: The DataFrame with boolean values encoded as strings
                      and all other values as strings (except nulls).
    """
    ...
    def convert_value(val):
        return mapping.get(val, val)  # Leave non-boolean and nulls as-is

    return df.applymap(convert_value).astype("string")