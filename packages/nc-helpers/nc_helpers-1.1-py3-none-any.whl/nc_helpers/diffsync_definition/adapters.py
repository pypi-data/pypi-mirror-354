    ...
    def load(self):
        ...
        for interface in interfaces:
            interface_item = self.interface(
                hostname=interface.get('device', {}).get('name'),
                interface=interface.get('name'),
                database_pk=interface.get('id'),
            )
            self.add(interface_item)
            parent_device_id = interface.get('device', {}).get('name')
        ...
        for ip in ips:
            if ip.get('assigned_object_type', '') == 'dcim.interface':
                assigned_object_device_name = ip.get('assigned_object', {}).get('device', {}).get('name', '')
