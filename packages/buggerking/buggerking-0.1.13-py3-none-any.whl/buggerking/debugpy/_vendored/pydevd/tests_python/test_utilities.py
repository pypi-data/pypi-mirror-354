...
import sys
from _pydevd_bundle.pydevd_constants import IS_WINDOWS, IS_PYPY, IS_JYTHON, PYDEVD_USE_SYS_MONITORING
import pytest
import os
...
def test_expression_to_evaluate():
    ...
    assert _expression_to_evaluate("  expr") == "expr"
    assert _expression_to_evaluate("  for a in expr:\n  pass") == "for a in expr:\npass"
...
def test_find_thread():
    ...
    assert pydevd_find_thread_by_id(get_current_thread_id(threading.current_thread())) is threading.current_thread()
...
def test_convert_dap_log_message_to_expression():
    ...
    assert check_dap_log_message("a {a}", "'a %s' % (a,)", "a value", {"a": "value"})
    assert check_dap_log_message("a {1}", "'a %s' % (1,)", "a 1")
    assert check_dap_log_message("a {  }", "'a '", "a ")
    assert check_dap_log_message(
        "a {1} {2}",
        "'a %s %s' % (1, 2,)",
        "a 1 2",
    )
    assert check_dap_log_message("a {{22:22}} {2}", "'a %s %s' % ({22:22}, 2,)", "a {22: 22} 2")
    assert check_dap_log_message("a {(22,33)}} {2}", "'a %s} %s' % ((22,33), 2,)", "a (22, 33)} 2")

    assert check_dap_log_message("a {{1: {1}}}", "'a %s' % ({1: {1}},)", "a {1: {1}}")

    # Error condition.
    assert check_dap_log_message("a {{22:22} {2}", "'Unbalanced braces in: a {{22:22} {2}'", "Unbalanced braces in: a {{22:22} {2}")
...
def test_pydevd_logging_files(tmpdir):
    ...
    with log_context(0, stream=stream):
        ...
        for d in (d1, d2):
            ...
            assert os.path.normpath(_LoggingGlobals._debug_stream_filename) == os.path.normpath(
                os.path.join(d, "file.%s.txt" % os.getpid())
            )

            assert os.path.exists(_LoggingGlobals._debug_stream_filename)

            assert pydev_log.list_log_files(DebugInfoHolder.PYDEVD_DEBUG_FILE) == [_LoggingGlobals._debug_stream_filename]
