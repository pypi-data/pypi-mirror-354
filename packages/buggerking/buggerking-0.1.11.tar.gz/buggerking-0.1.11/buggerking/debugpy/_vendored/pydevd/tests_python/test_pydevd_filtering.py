...
def test_in_project_roots(tmpdir):
    ...
    if IS_WINDOWS:
        assert files_filtering._get_library_roots() == [os.path.normcase(x) + "\\" for x in files_filtering._get_default_library_roots()]
    elif IS_MAC:
        assert files_filtering._get_library_roots() == [x.lower() + "/" for x in files_filtering._get_default_library_roots()]
    else:
        assert files_filtering._get_library_roots() == [os.path.normcase(x) + "/" for x in files_filtering._get_default_library_roots()]
    ...
    for check_path, find in check:
        assert files_filtering.in_project_roots(check_path) == find, "Expected: %s to be a part of the project: %s" % (check_path, find)

    sys.path.append(str(site_packages))
    try:
        default_library_roots = files_filtering._get_default_library_roots()
        assert len(set(default_library_roots)) == len(default_library_roots), "Duplicated library roots found in: %s" % (
