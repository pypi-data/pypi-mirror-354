...
'use strict';
{
    ...
    $.fn.formset = function(opts) {
        ...
        const $this = $(this);
        const template_default = $this.parent().find('textarea').val();
        ...
        const addInlineClickHandler = function(e) {
            ...
            //  随机一个item_id,格式exampleitem_set-,后面数字随机
            // const randomNumber = Math.floor(Math.random() * 1000);
            // const item_id = 'exampleitem_set-' + randomNumber
            const item_id = options.prefix+'-'+nextIndex.toString()
            const template_default_new = template_default
                .replace('empty-form', 'dynamic-'+options.prefix)
                .replace(options.prefix+'-empty', item_id)
                .replace(/__prefix__/g, nextIndex.toString())
                
            if(nextIndex > 0){
                // template_default_new.replace(options.prefix+'-0', options.prefix+'-'+nextIndex.toString())
                let regex = new RegExp(`${options.prefix}-0`, 'g');
                const template_default_change = template_default_new.replace(regex, options.prefix+'-'+nextIndex.toString())
                $(this).parent().parent().before(template_default_change)
            }else{
                $(this).parent().parent().before(template_default_new)
            }
