    ...
    def _generate_system_matrixSTRUCT(self, fieldDataPath, fieldParamPath):
        ...
        for i in progress_bar:
            ...
            if not pathField is None and os.path.exists(pathField):
                    ...
                    AcousticField.load_field(pathField, self.params.acoustic['typeSim'])
            
            elif not os.path.exists(pathField):
            ...
            elif not pathField is None and not os.path.exists(pathField):
    ...
    def _generate_AO_signal(self):
        ...
        for i in range (len(self.AcousticFields)):
            for t in range(self.AcousticFields[i].field.shape[0]):
                interaction = self.OpticImage * self.AcousticFields[i].field[t, :, :]
                AOsignal[t,i] = np.sum(interaction)
        
        return AOsignal
    
    def _cutAcousticFields(self):


        minShape = min([field.field.shape[0] for field in self.AcousticFields])

        for i,field in enumerate (self.AcousticFields):
            if field.field.shape[0] != minShape:
                self.AcousticFields[i].field = field.field[:minShape, :, :]
