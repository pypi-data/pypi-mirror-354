...
# Import Termux configuration
from mcp_open_client.termux_config import (
    is_termux,
    get_termux_safe_nicegui_config,
    log_termux_info,
    get_safe_port,
    initialize_termux_compatibility
)

# Initialize Termux compatibility FIRST - before any other imports that might use webview
initialize_termux_compatibility()

# Define the storage path for NiceGUI
from mcp_open_client.state.conversations import get_safe_home_directory

# Log Termux environment info for debugging
log_termux_info()
...
def main() -> None:
    ...
    # Get Termux-safe configuration
    termux_config = get_termux_safe_nicegui_config()
    
    # Determine the port to use
    port = get_safe_port() if is_termux() else args.port
    
    # Run the application
    try:
        ui.run(
            title='MCP-open-client',
            favicon='assets/favicon.ico' if not is_termux() else None,  # Skip favicon in Termux
            dark=True,
            show=False,
            port=port,
        ...
        if e.errno == 10048:  # Address already in use
            ...
            if not is_termux():
                ...
                print("In Termux, you can set the NICEGUI_PORT environment variable.")
                print("Example: export NICEGUI_PORT=8083")
        else:
            print(f"An error occurred: {e}")
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        if is_termux():
            print("Note: If you're running in Termux, some features may be limited.")
