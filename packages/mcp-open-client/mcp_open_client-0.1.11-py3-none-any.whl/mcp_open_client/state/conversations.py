...
def get_safe_home_directory() -> str:
    """Get a safe home directory path that works in Termux and other environments"""
    # Check if we're in Termux
    if 'com.termux' in os.environ.get('PREFIX', ''):
        # In Termux, use the Termux-specific paths
        termux_home = os.environ.get('HOME')
        if termux_home and os.path.exists(termux_home):
            return termux_home
        # Fallback to Termux default
        return '/data/data/com.termux/files/home'
    
    # Try standard home directory expansion
    try:
...
def get_conversations_path():
    """Get the conversations path with Termux compatibility"""
    safe_home = get_safe_home_directory()
    return os.path.join(safe_home, ".mcp-open-client", "conversations")

# Path for storing conversations
CONVERSATIONS_PATH = get_conversations_path()
