...
def migrate_settings_to_home_directory():
    ...
    try:
        ...
        default_theme_path = os.path.join("mcp_open_client", "settings", "default_theme.css")
        ...
        if os.path.exists(current_settings_path) and not os.path.exists(SETTINGS_FILE):
        ...
        if os.path.exists(current_tools_path) and not os.path.exists(TOOLS_FILE):
        ...
        # Copy default theme to user's home directory if it doesn't exist there yet
        if os.path.exists(default_theme_path) and not os.path.exists(THEME_FILE):
