...
def load_settings():
    ...
    # Copy default theme if user theme doesn't exist
    copy_default_theme_if_needed()
...
def copy_default_theme_if_needed():
    """Copy default theme to user config if it doesn't exist"""
    import shutil
    
    if not os.path.exists(THEME_FILE):
        # Find the default theme file in the application directory
        app_dir = os.path.dirname(os.path.dirname(__file__))  # Go up from state/ to mcp_open_client/
        default_theme_src = os.path.join(app_dir, "settings", "default_theme.css")
        
        if os.path.exists(default_theme_src):
            try:
                shutil.copy2(default_theme_src, THEME_FILE)
                print(f"Copied default theme to {THEME_FILE}")
            except Exception as e:
                print(f"Error copying default theme: {e}")
        else:
