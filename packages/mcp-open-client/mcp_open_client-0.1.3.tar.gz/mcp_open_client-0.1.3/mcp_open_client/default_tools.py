...
DEFAULT_TOOLS = [
    ...
    # Ejecutar el código Python usando Pyodide
    js_code = f"""
    (async () => {{
        try {{
            // Mostrar mensaje de carga
            document.getElementById('{element_id}').innerHTML = '<div class="text-blue-500">Cargando Pyodide...</div>';
            ...
            document.getElementById('{element_id}').innerHTML = '<div class="text-blue-500">Ejecutando código...</div>';
            ...
            let result;
            try {{
                result = await window.pyodide.runPythonAsync(`{escaped_code}`);
            }} catch (error) {{
    ...
    # Asegurarse de que Pyodide esté disponible
    ui.add_head_html("""
...
def execute_eval_python_code(params):
    """Ejecuta código Python en el navegador usando Pyodide
