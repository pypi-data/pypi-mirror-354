...
def test_correctly_setup():
	...
	hack.run_in_pty_shell_minimal(Shell().exec_path, command)
...
class ShellConfig:
	...
	def generate_extension_file(self):
		...
		if shell.is_bash():
			...
			path = LIMINAL_DIR / 'bash-preexec.sh'
		...
		content += '\n. "$HOME/.atuin/bin/env"\n' # NOTE: atuin adds this to the shell profile too
...
def uninstall_atuin():
	run_command(['rm', '-r', '-f', Path.home() / '.atuin'])
	run_command(['rm', '-r', '-f', Path.home() / '.local/share/atuin'])
	# TODO: remove from shell profile
...
def install_atuin():
	...
	run_command([shellconfig.shell.exec_path, installer_script])
