...
def _try_get_monitor(agent_endpoint_name: str, experiment_id_opt: Optional[str]):
    ...
    try:
        ...
        # If _get_monitor_internal failed with a 404, the monitor does not exist, so continue to
        # check for an external monitor. Otherwise, rethrow the exception
        if e.response.status_code != 404:
            raise

    # If _get_monitor_internal failed, try to get the monitor from the experiment ID, if
    # specified
    if experiment_id_opt:
        try:
            return get_external_monitor(experiment_id=experiment_id_opt)
...
def _try_create_or_get_monitor_for_agent_deploy(
    ...
    try:
        ...
        monitor = _try_get_monitor(
            agent_endpoint_name=agent_endpoint_name, experiment_id_opt=experiment_id
        )
    except Exception:
        # If _try_get_monitor fails, swallow the exception and don't
        # attempt to create a new monitor
        return None
    if monitor:
...
def _get_experiment_id_for_tracing(serving_endpoint_opt):
    ...
    if serving_endpoint_opt:
        # First check for a legacy monitor
        if monitor := _try_get_monitor(
            agent_endpoint_name=serving_endpoint_opt.name, experiment_id_opt=None
        ):
        ...
        tags_dict = (
            {tag.key: tag for tag in serving_endpoint_opt.tags}
            if serving_endpoint_opt.tags
            else {}
        )
