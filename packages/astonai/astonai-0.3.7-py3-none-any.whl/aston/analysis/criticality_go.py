...
class GoCriticalityScorer:
    ...
    def _find_binary(self) -> Optional[Path]:
        ...
        if platform_suffix:
            bundled_binary = Path(__file__).parent.parent / "bin" / f"aston-rank-{platform_suffix}"
    ...
    def _validate_binary(self) -> None:
        ...
        if not os.access(self.binary_path, os.X_OK):
            ...
            if not (sys.platform == 'win32' and self.binary_path.suffix.lower() == '.exe'):
        ...
        try:
            ...
            if result.returncode != 0:
                raise BinaryNotFoundError(f"Binary version check failed: {result.stderr}")
    ...
    def get_algorithm_info(self, algorithm: str = "degree") -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                raise GoCriticalityError(f"Failed to get algorithm info: {result.stderr}")
...
def create_scorer(scorer_type: str = "auto") -> Union['GoCriticalityScorer', 'CriticalityScorer']:
    """
    ...
    if scorer_type.lower() == "go":
        if go_available:
            ...
            raise BinaryNotFoundError("Go criticality scorer was requested but binary not found")
