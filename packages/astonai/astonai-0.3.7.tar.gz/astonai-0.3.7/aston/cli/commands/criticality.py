...
@click.option('--scorer', type=click.Choice(['auto', 'go', 'python']), default='auto',
              help='Scorer implementation to use (auto: use Go if available, otherwise Python)')
@click.option('--algorithm', type=str, default='degree',
    ...
    try:
        ...
        if scorer == 'go':
            try:
                ...
                if not go_available:
                    console.print("[yellow]Warning:[/yellow] Go scorer requested but not available. Install aston-rank binary or use --scorer python.")
                    console.print("See https://github.com/astonai/aston-rank for installation instructions.")
                    raise click.Abort()
            except ImportError:
                console.print("[red]Error:[/red] Failed to check Go scorer availability.")
                raise click.Abort()
        ...
        if scorer in ('auto', 'go'):
            try:
                ...
                if scorer == 'go':
                    ...
                    raise click.Abort()
                console.print("[yellow]Warning:[/yellow] Failed to initialize Go scorer, falling back to Python.")
        ...
        with console.status("[cyan]Calculating criticality scores..."):
            ...
            if is_go:
                ...
                top_critical_nodes = scorer_instance.get_top_critical_nodes(nodes, edges, top_k=top)
        ...
        if output_format == 'json':
            ...
            _display_criticality_table(console, top_critical_nodes, duration, scorer_instance, is_go)
...
def _display_criticality_table(console: Console, top_nodes: List[Dict[str, Any]], 
                               duration: float, scorer: Any, is_go: bool) -> None:
    ...
    for i, node in enumerate(top_nodes[:20], 1):  # Limit to first 20 for display
        if isinstance(node, dict):
            ...
            node_type = node.get("type", "unknown") if "type" in node else node.get("node_type", "unknown")
