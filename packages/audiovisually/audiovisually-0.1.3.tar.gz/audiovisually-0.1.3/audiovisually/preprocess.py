...
def mp3_to_text(
    ...
    Example:
        >>> from audiovisually.preprocess import audio_to_text
        >>> df = audio_to_text("audio.mp3", api_key="your_api_key", engine="assemblyai")
        >>> df = audio_to_text("audio.mp3", engine="whisper", timestamps=True)
    """
    try:
...
def translate_data(
    ...
    """
    Translate text in a DataFrame column or a single string to a target language.

    Args:
        data (pd.DataFrame or str): DataFrame containing text to translate, or a single string.
