    ...
    def _on_conflict_action_sql(self, **kwargs: Any) -> str:
        if self._on_conflict_do_nothing:
            ...
            for field, value in self._on_conflict_do_updates:
                if value:
                    updates.append(
                        f"{field.get_sql(**kwargs)}={value.get_sql(with_namespace=True, **kwargs)}",
                    )
                else:
                    updates.append(
                        f"{field.get_sql(**kwargs)}=EXCLUDED.{field.get_sql(**kwargs)}",
                    )
...
class ClickHouseQueryBuilder(QueryBuilder):
    ...
    @builder
    def distinct_on(self, *fields: str | Term) -> "ClickHouseQueryBuilder":
        for field in fields:
            if isinstance(field, str):
                self._distinct_on.append(Field(field))
            elif isinstance(field, Term):
    ...
    @builder
    def limit_by(self, n, *by: str | Term) -> "ClickHouseQueryBuilder":
        self._limit_by = (n, 0, [Field(field) if isinstance(field, str) else field for field in by])

    @builder
    def limit_offset_by(self, n, offset, *by: str | Term) -> "ClickHouseQueryBuilder":
