...
get from BK"
    ...
    def getPercentUsableSpace(self, se_name: str):
        """Get percentage of usable space in an SE.
        ...
        * getOccupancy is in TiB (or MiB)
        ...
        percent_free = 1 - current_occupancy / total_space
        # Keep some margin
        percent_free -= 0.05
    ...
    def rankPercentageSEs(self, candSEs):
        """Ranks the SEs according to their percentage of free space."""
        if len(candSEs) <= 1:
            return candSEs
        weightForSEs = {}
        for se in candSEs:
            se_weight = self.getPercentUsableSpace(se)
            if se_weight:
    ...
    def setTargetSEs(self, numberOfCopies, archiveSEs, mandatorySEs, secondarySEs, existingSEs, exclusiveSEs=False):
        ...
        if nbArchive:
            ...
            candidateSEs = self.selectSEs(
                archiveExisting + self.rankPercentageSEs(archiveActiveSEs), nbArchive, existingSEs
            )
        ...
        candidateSEs += [
            se
            for se in self.rankPercentageSEs(secondaryActiveSEs)
