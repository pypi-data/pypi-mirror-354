...
class InlineQueryResultVoicesFactory(TypesFactory):
    ...
    def register_inline_query_result_voice(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = inline_query_result_voice_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.INLINEQUERYRESULTVOICES_REGISTRY[name] = (
                        inline_query_result_voice_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.INLINEQUERYRESULTVOICES_REGISTRY.get(trigger_value)
