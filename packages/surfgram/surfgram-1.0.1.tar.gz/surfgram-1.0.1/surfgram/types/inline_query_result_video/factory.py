...
class InlineQueryResultVideosFactory(TypesFactory):
    ...
    def register_inline_query_result_video(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = inline_query_result_video_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.INLINEQUERYRESULTVIDEOS_REGISTRY[name] = (
                        inline_query_result_video_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.INLINEQUERYRESULTVIDEOS_REGISTRY.get(trigger_value)
