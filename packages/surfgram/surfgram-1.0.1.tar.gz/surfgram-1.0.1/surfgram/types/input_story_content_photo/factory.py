...
class InputStoryContentPhotosFactory(TypesFactory):
    ...
    def register_input_story_content_photo(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = input_story_content_photo_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.INPUTSTORYCONTENTPHOTOS_REGISTRY[name] = (
                        input_story_content_photo_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.INPUTSTORYCONTENTPHOTOS_REGISTRY.get(trigger_value)
