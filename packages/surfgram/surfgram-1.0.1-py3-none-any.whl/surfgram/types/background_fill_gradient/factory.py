...
class BackgroundFillGradientsFactory(TypesFactory):
    ...
    def register_background_fill_gradient(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = background_fill_gradient_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.BACKGROUNDFILLGRADIENTS_REGISTRY[name] = (
                        background_fill_gradient_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.BACKGROUNDFILLGRADIENTS_REGISTRY.get(trigger_value)
