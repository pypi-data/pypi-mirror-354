...
class InlineQueryResultVenuesFactory(TypesFactory):
    ...
    def register_inline_query_result_venue(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = inline_query_result_venue_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.INLINEQUERYRESULTVENUES_REGISTRY[name] = (
                        inline_query_result_venue_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.INLINEQUERYRESULTVENUES_REGISTRY.get(trigger_value)
