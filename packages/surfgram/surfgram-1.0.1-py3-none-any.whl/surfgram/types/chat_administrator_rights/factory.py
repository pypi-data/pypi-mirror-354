...
class ChatAdministratorRightsFactory(TypesFactory):
    ...
    def register_chat_administrator_rights(
        ...
        if not names or None in names or "" in names:
            cls.__fallback_handler__ = chat_administrator_rights_cls
        else:
            for name in names:
                if name:  # Skip empty/None names
                    cls.CHATADMINISTRATORRIGHTS_REGISTRY[name] = (
                        chat_administrator_rights_cls
                    )
    ...
    async def create(cls, update: Any) -> Optional[Any]:
        ...
        handler_cls = cls.CHATADMINISTRATORRIGHTS_REGISTRY.get(trigger_value)
