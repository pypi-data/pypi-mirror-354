...
def sppa_t3000_pre_deal(file, sep):
    def deal_page(p: pd.DataFrame):
        ...
        if types_p == "说明页":
            ...
            p = p.drop(index=[0])
            p = p.replace(" ", np.nan).dropna(axis=1, how="all")
            tags = {}
            if sppa_type == 2:
    ...
    if df.iloc[0, 0] == "Analog Grid Report":  # 河南中原燃机的数据格式，只有一个说明页和一个数据页
...
def pre_deal_and_cutting_period(file, time_dict, bound, config: dict):
    ...
    if ext_ == ".xlsx":
        ...
        if type_ == "power":
            ...
            outfile1 = sppa_t3000_pre_deal(file, ";")
            return deal_dcs_pre_file(outfile1, time_dict)
        elif type_ == "SPPA-T3000 and , sep":
            outfile1 = sppa_t3000_pre_deal(file, ",")
