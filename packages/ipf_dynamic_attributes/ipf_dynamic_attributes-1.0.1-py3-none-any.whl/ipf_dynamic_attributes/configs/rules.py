...
Rule, ConfigRule = ForwardRef("Rule"), ForwardRef("ConfigRule")
DefaultRule, DefaultConfigRule = ForwardRef("DefaultRule"), ForwardRef("DefaultConfigRule")


def merge_default(
    original: Union[Rule, ConfigRule], default: Union[DefaultRule, DefaultConfigRule]
) -> Union[Rule, ConfigRule]:
    if not default:
        return original
...
class DefaultRule(BaseRule, BaseModel):
    attribute: Optional[str] = ATTRIBUTE
    ...
    delete_attribute: Optional[bool] = DELETE_ATTRIBUTE
...
class Rule(DefaultRule, BaseModel):
    name: str = RULE_NAME
    merge_default_filters: bool = Field(
        True,
        description="Merge 'default[filters]' with Table Rule filters or "
        "'default[filter_string]' to Table Rule filter_string.",
        title="Merge Default Filters",
    )
    merge_default_device_filters: bool = Field(
        True,
        description="Merge 'default[device_filters]' with Table Rule device_filters.",
        title="Merge Default Device Filters",
    )
    merge_default_attribute_filters: bool = Field(
        True,
        description="Merge 'default[attribute_filters]' with Table Rule attribute_filters.",
        title="Merge Default Attribute Filters",
    )

    def merge_default(self, default: DefaultRule) -> Rule:
        if not isinstance(default, DefaultRule):
            return self
        rule = merge_default(self.model_copy(deep=True), default)
        # Copy Default Value Config to Rule:
        rule.value = rule.value or default.value
        # Copy Default SN Column to Rule:
        rule.value.sn_column = rule.value.sn_column or (default.value.sn_column if default.value else "sn")
        if default.delete_attribute is not None and rule.delete_attribute is None:
            # Copy Default Delete Attribute to Rule if Rule does not have delete_attribute set:
            rule.delete_attribute = default.delete_attribute
        if not rule.filter_string:
            if default.device_filters and rule.merge_default_device_filters:
                # Copy Default Device Filters to Rule if merge_default_filters is True and filter_string is not set:
                rule.device_filters.extend(default.device_filters)
            if default.attribute_filters and rule.merge_default_attribute_filters:
                # Copy Default Attribute Filters to Rule if merge_default_attribute_filters is True:
                rule.attribute_filters.extend(default.attribute_filters)
        return rule


class DefaultConfigRule(FilterRule, BaseModel):
    attribute: Optional[str] = ATTRIBUTE
    delete_attribute: Optional[bool] = DELETE_ATTRIBUTE
    filters: list[Filter] = Field(
        default_factory=list,
...
class ConfigRule(DefaultConfigRule, BaseModel):
    name: str = RULE_NAME
    ...
    merge_default_filters: bool = Field(
        True,
        description="Merge 'default[config_filters]' with Configuration Rule filters.",
        title="Merge Default Configuration Filters",
    )

    def merge_default(self, default: DefaultConfigRule) -> ConfigRule:
        """Merge Default Config Rule with Config Rule"""
        return merge_default(self.model_copy(deep=True), default) if isinstance(default, DefaultConfigRule) else self
