...
SEO_LANDING_PAGE_CHOICES = Choices(
    (LANDING_PAGE_NEW, "new", _("New website")),
    (LANDING_PAGE_CUSTOMER, "customer", _("Website from another provider")),
    (
...
DISPLAY_CREATIVES_FORMAT_CHOICES = Choices(
    ...
    (
        ...
        _(
            "Billboard + Medium Rectangle Presenter "
            "(includes the following formats: 800x250, 970x250, 770x250, 300x250)"
            " - %(price)d €",
        )
        % {"price": 399},
    ),
    ...
    (
        ...
        _(
            "Dynamic Sitebar + Medium Rectangle Presenter "
            "(includes the following formats: 301x601, 300x250) - %(price)d €",
        )
        % {"price": 599},
    ),
    (
...
class OrderLineGoogleAdsPremiumSerializer(serializers.Serializer):
    ...
    """
    keywords_with_zero_search_volume = serializers.ListField(
...
class DisplayNativeCreativeSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        TYPES_WHICH_REQUIRE_ADVERTORIAL = [
            NATIVE_AD_TYPES.native_ad_in,
            NATIVE_AD_TYPES.native_ad_flow_ad_in,
        ]
...
class OrderLineDisplayBasicSerializer(serializers.Serializer):
    ...
    """
    impressions_per_month = serializers.ChoiceField(
        choices=DISPLAY_IMPRESSIONS_PER_MONTH_CHOICES,
        required=False,
        allow_null=True,
    )
    ...
    """
    banner_color_selection = serializers.ChoiceField(
        choices=DISPLAY_BANNER_COLOR_SELECTION_CHOICES,
        required=False,
        allow_blank=True,
    )
...
class OrderLineDisplayPremiumSerializer(serializers.Serializer):
    ...
    interest_targeting = serializers.MultipleChoiceField(
        choices=DISPLAY_TARGETING_CHOICES,
        required=False,
        allow_empty=True,
    )
...
class OrderLineSeoSerializer(serializers.Serializer):
    ...
    """
    topics = serializers.ListField(child=serializers.CharField(max_length=100, required=True), required=True)
...
class Serializer(serializers.Serializer):
    ...
    def validate_productdetail_exists(self, data):
        ...
        for orderline in data["orderlines"]:
            if orderline["product_type"] in mapping and not mapping[orderline["product_type"]].intersection(
                orderline.keys(),
            ):
                raise serializers.ValidationError(
