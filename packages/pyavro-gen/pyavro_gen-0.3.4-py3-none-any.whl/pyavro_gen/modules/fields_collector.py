...
class FieldsCollector(PreprocessorModule):  # type: ignore
    ...
    def fill_fields_of_node(self, node: Avro.Node) -> None:
        ...
        elif Avro.Name in node and Avro.Fields in node:
            self.fill_fields_of_standard_record(node)

        elif Avro.Name in node and Avro.Symbols in node:
            self.fill_fields_of_enum(node)
    ...
    def fill_fields_of_standard_record(self, node: Avro.Node) -> None:
        ...
        :param node: The Avro node
        """
        fully_qualified_name = self._get_fully_qualified_name(node)
    ...
    def fill_fields_of_enum(self, node: Avro.Node) -> None:
        """
        Fills attributes and documentation of a ClassWriter for enums.
        :param node: The Avro node
        """
        fully_qualified_name = self._get_fully_qualified_name(node)
        writer = self.get_class_writer_from_name(fully_qualified_name)
        ...
        if not writer.attributes:
            ...
            writer.attributes.append(
                Attribute(
                    name='_schema',
                    doc=Docstring('The Avro Schema associated to this class'),
                    default_value=f'nonmember("""{json.dumps(node, indent=4)}""")'
                )
            )
        ...
        if avro_type == 'boolean':
            ...
            if Avro.Type in avro_type:
                if avro_type[Avro.Type] in [Avro.Record, Avro.Enum]:
                    name = self._get_fully_qualified_name(avro_type)
    ...
    def _get_fully_qualified_name(self, node: Avro.Node) -> str:
        if Avro.Namespace in node:
            fqn = node[Avro.Namespace] + '.' + node[Avro.Name]
        elif '.' in node[Avro.Name]:
            fqn = node[Avro.Name]
        else:
            fqn = namespace_name(self.current_schema_name) + '.' + node[Avro.Name]
