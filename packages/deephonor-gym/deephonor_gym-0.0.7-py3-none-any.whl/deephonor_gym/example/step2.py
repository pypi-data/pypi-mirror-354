...
class myEnv(DHgym) :
    ...
    def init(self):
        ...
        self.upload('models','models') # 上传模型文件夹path
        add_obj = {} # 以下参数均可选，除了path
        add_obj['world'] = {'gravity':[0, -9.81, 0],'speed':1 } # optional 可选 speed > 0.01
        add_obj['Heightground'] = {'size':[30,30,1],'position':[0,0,0],'path': '/models/heightground.jpeg', 'detail': 100 } # optional 可选 detail >= 1
        add_obj['urdf'] = [
                            # {'scale':1,'position':[0,1,0],'path': '/models/T12/urdf/T12.URDF','debug':False, 'JointDamping': 1000}, #  dae 文件需要 root 路径 eg. 'root':'models' 
