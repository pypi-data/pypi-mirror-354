...
_CACHE: dict[type[Model] | type[TypeModel], ObjectSchema] = {}
...
def model_to_object_schema(model: type[Model] | type[TypeModel]) -> ObjectSchema:
    ...
    object_schema.primary_key = getattr(model, PRIMARY_KEY, None)  # type: ignore[attr-defined]
    ...
    for key, value in object_schema.properties.items():
        ...
        if isinstance(model_field, ReferenceFieldInfo):
            ...
            value.db_field = _db_field  # type: ignore[assignment]
        elif key in object_schema.foreign_keys:  # type: ignore[attr-defined]
            value.db_field = list(object_schema.foreign_keys[key][0].keys())  # type: ignore[attr-defined]
