...
class IntervalsICUTextParser(BaseParser[str]):
        ...
        while i < len(lines):
            ...
            if line.startswith('- '):
                ...
                if repeat_count:
                    ...
                    repeat = Repeat(
                        type="repeat",
                        count=FixedValue(value=repeat_count, quantity=RepeatQuantity.NUMBER),
    ...
    def _parse_interval_line(self, line: str) -> Union[Interval, Instruction]:
        ...
        if match:
            ...
            volume = FixedValue(value=duration_minutes * 60, quantity=VolumeQuantity.duration)
    ...
    def _format_item(self, item: Union[Interval, Repeat, Section, Instruction]) -> str:
        ...
        if isinstance(item, Instruction):
            ...
            if len(item.content) == 1 and isinstance(item.content[0], Repeat):
                ...
                count = repeat.count.value if isinstance(repeat.count, FixedValue) else 1
    ...
    def _format_volume(self, volume) -> str:
        ...
        if isinstance(volume, FixedValue):
            value = volume.value
    ...
    def _format_intensity(self, intensity) -> str:
        ...
        if isinstance(intensity, FixedValue):
            if intensity.fraction and intensity.variable == "threshold":
                return f"{int(intensity.fraction * 100)}%"
            elif intensity.fraction:
                return f"{int(intensity.fraction * 100)}%"
            elif intensity.value:
                return f"{int(intensity.value)}W"
        elif isinstance(intensity, RangeValue):
            if intensity.min_fraction and intensity.max_fraction:
                min_pct = int(intensity.min_fraction * 100)
                max_pct = int(intensity.max_fraction * 100)
                return f"{min_pct}-{max_pct}%"
            elif intensity.min_value and intensity.max_value:
                return f"{int(intensity.min_value)}-{int(intensity.max_value)}W"
