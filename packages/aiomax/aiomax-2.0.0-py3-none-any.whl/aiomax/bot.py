    ...
    async def handle_update(self, update: dict):
        ...
        if update_type == "message_created":
            ...
            for handler in self.handlers['message_created']:
                filters = [handler.filter(message)] if handler.filter else []
                if handler.router:
                    filters.extend(
                        [i(message) for i in handler.router.filters['message_created']]
                    )
        ...
        if update_type == 'message_edited': 
            ...
            for handler in self.handlers[update_type]:
                ...
                if handler.router:
                    filters.extend(
                        [i(message) for i in handler.router.filters['message_edited']]
                    )
        ...
        if update_type == 'message_removed': 
            ...
            # handling
            for handler in self.handlers[update_type]:
                filters = [handler.filter(payload)] if handler.filter else []
                if handler.router:
                    filters.extend(
                        [i(payload) for i in handler.router.filters['message_removed']]
                    )
        ...
        if update_type == 'message_callback':
            ...
            for handler in self.handlers[update_type]:
                filters = [handler.filter(callback)] if handler.filter else []
                if handler.router:
                    filters.extend(
                        [i(callback) for i in handler.router.filters['message_callback']]
                    )
