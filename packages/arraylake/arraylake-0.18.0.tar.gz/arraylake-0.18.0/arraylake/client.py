    ...
    async def get_repo(
        ...
        *,
        ...
        Args:
            ...
            virtual_container_nicknames: A dictionary mapping virtual chunk container names to bucket nicknames.
        ...
        if kind == RepoKind.V1:
            ...
            try:
                ...
                ic_repo: IcechunkRepository = (
                    IcechunkRepository.open(icechunk_storage, config=config, virtual_chunk_credentials=container_creds)
                    if import_existing
                    else IcechunkRepository.create(icechunk_storage, config=config, virtual_chunk_credentials=container_creds)
                )
                ic_repo.set_default_commit_metadata({"author_name": author.name, "author_email": author.email})
                return ic_repo
    ...
    async def modify_repo(
        ...
        Args:
            ...
            update_metadata: Optional dictionary of metadata to update on the repo.
                Dictionary values can be a scalar (string, int, float, bool, or None) or a list of scalars.
            maintenance_window: Optional maintenance window to set for the repo.
            gc_config: Optional garbage collection configuration for the repo.
            expiration_config: Optional expiration configuration for the repo.
        """
        org, repo_name = _parse_org_and_repo(name)
        mstore = self._metastore_for_org(org)
