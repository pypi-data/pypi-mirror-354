...
async def test_modify_repo(token, isolated_org_with_bucket, repo_kind_to_test: RepoKind) -> None:
    ...
    assert updated_repo_obj.maintenance_window == maintenance_window
    assert updated_repo_obj.gc_config == gc_config
    assert updated_repo_obj.expiration_config == expiration_config
...
async def test_modify_repo_no_change(token, isolated_org_with_bucket, repo_kind_to_test: RepoKind) -> None:
    ...
    assert updated_repo_obj.updated == current_repo_obj.updated
    assert updated_repo_obj.maintenance_window is None
    assert updated_repo_obj.gc_config is None
    assert updated_repo_obj.expiration_config is None
