...
def test_get_icechunk_storage_obj_from_env(s3_bucket_config: BucketResponse, monkeypatch):
    ...
    storage = _get_icechunk_storage_obj(
        bucket_config=s3_bucket_config, prefix=None, credentials=None, credential_type=CredentialType.ENV, credential_refresh_func=None
    )
    assert isinstance(storage, icechunk.Storage)
...
def test_get_icechunk_storage_obj_gcs_no_credentials(gcs_bucket_config: BucketResponse):
    storage = _get_icechunk_storage_obj(
        bucket_config=gcs_bucket_config, prefix=None, credentials=None, credential_type=CredentialType.ENV, credential_refresh_func=None
    )
    assert isinstance(storage, icechunk.Storage)
