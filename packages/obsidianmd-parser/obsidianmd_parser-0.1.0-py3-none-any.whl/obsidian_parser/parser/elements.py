...
from obsidian_parser.note import Callout, Embed, Section, Tag, Task, WikiLink
...
def parse_tags(content: str) -> list[Tag]:
    ...
    for match in re.finditer(pattern, content_no_code, re.MULTILINE | re.UNICODE):
...
def parse_sections(content: str) -> list[Section]:
    ...
    top_level_sections = [s for s in sections if not any(s in other.subsections for other in sections)]

    return sections  # For now, return all sections flat
