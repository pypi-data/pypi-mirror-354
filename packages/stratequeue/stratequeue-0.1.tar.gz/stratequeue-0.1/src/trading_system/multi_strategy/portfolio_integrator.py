...
class PortfolioIntegrator:
    ...
    def _initialize_portfolio_manager(self):
        ...
        try:
            ...
            logger.info(f"Initialized portfolio manager with {len(self.strategy_allocations)} strategy allocations")
        ...
        try:
            ...
            for strategy_id, strategy_info in status.get('strategies', {}).items():
                ...
                if allocation > 0.5:  # More than 50% to one strategy
                    return False, f"Strategy {strategy_id} has excessive allocation: {allocation:.1%}"
