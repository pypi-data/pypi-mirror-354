    ...
    def wrapper(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso) -> tuple[LoginReturn, str, str]:
        while True:
            ...
            if loginReturn != LoginReturn.CAPTCHA_INCORRETO:
                ...
                newImgName = f"{timestamp}_{captcha}.png"
                newImgPath = os.path.join(os.path.dirname(imgPath), newImgName)
                
                os.rename(imgPath, newImgPath)
                
                try:
                    pass
                    # storeCaptcha(newImgPath, enumBanco, enumProcesso)
                except Exception as e:
...
def loginItau(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return checkLogin(), EnumBanco.ITAU
...
def loginCrefisaCP(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return LoginReturn.LOGIN_COM_SUCESSO, EnumBanco.CREFISA_CP
...
def loginC6(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return checkLogin(), EnumBanco.C6
...
def loginDigio(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return checkLogin(), EnumBanco.DIGIO
...
def loginFacta(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    def checkLogin() -> LoginReturn:
        ...
        if esperarElemento(driver, '//*[@id="divAlertaMsg"][contains(text(), "Usuário/senha incorretos!")]', tempo_espera=3):
            return LoginReturn.CREDENCIAIS_INVALIDAS
    ...
    return checkLogin(), EnumBanco.FACTA
...
def loginCashCard(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return LoginReturn.LOGIN_COM_SUCESSO, EnumBanco.MEU_CASH_CARD
...
def loginVirtaus(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return loginReturn, EnumBanco.VIRTAUS
...
def loginPaulista(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return LoginReturn.LOGIN_COM_SUCESSO, EnumBanco.ITAU


@login_decorator
def loginSafra(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    
    def checkLogin() -> LoginReturn:
        if esperarElemento(driver, '//*[@id="toast-container"]', tempo_espera=3):
            return LoginReturn.USUARIO_INATIVO
        if esperarElemento(driver, '//*[@id="lblMensagemErro"]', tempo_espera=3):
            return LoginReturn.CREDENCIAIS_INVALIDAS
    ...
    carregando = esperarElemento(driver, '//*[@id="sec-overlay" and @style="display: block;"]')

    while carregando:
        carregando = esperarElemento(driver, '//*[@id="sec-overlay" and @style="display: block;"]', tempo_espera=1)
        time.sleep(3)
    
    buttonLogin = esperarElemento(driver, '//*[@id="btnEntrar"]').click()

    return checkLogin(), EnumBanco.SAFRA
...
def loginMaster(driver: Chrome, usuario: str, senha: str) -> LoginReturn:
    ...
    return LoginReturn.LOGIN_COM_SUCESSO, EnumBanco.MASTER
...
def loginIBConsig(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso) -> tuple[LoginReturn, str, str, EnumBanco]:
    ...
    def checkLogin() -> LoginReturn:
        if esperarElemento(driver, '//*[@id="Table_01"]/tbody/tr[3]/td/table[2]/tbody/tr[1]/td/font[contains(text(), "A palavra de verificação está inválida")]', tempo_espera=5):
            return LoginReturn.CAPTCHA_INCORRETO
        if esperarElemento(driver, '//*[@id="Table_01"]/tbody/tr[3]/td/table[2]/tbody/tr[1]/td/font[contains(text(), "acesso simultâneo")]', tempo_espera=5):
...
def loginOle(driver: Chrome, usuario: str, senha: str) -> LoginReturn: 
    ...
    return LoginReturn.LOGIN_COM_SUCESSO, EnumBanco.OLE
