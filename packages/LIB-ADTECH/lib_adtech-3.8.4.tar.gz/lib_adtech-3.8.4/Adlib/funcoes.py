...
from .api import EnumBanco, EnumProcesso
from .utils import loginChatIdMapping
...
def esperarElemento(driver: Chrome | TabDriver, xpath: str, tempo_espera=10) -> WebElement | None:
    """
    ...
    if isinstance(driver, TabDriver):
        driver = driver.driver
    try:
        return WebDriverWait(driver, tempo_espera).until(EC.visibility_of_element_located((By.XPATH, xpath)))
    except:
        print(f"Elemento não encontrado: {xpath}")
 

def aguardarElemento(driver: Chrome | TabDriver, xpath: str, tempo_espera=10) -> WebElement | None:
    """
    ...
    if isinstance(driver, TabDriver):
        driver = driver.driver
    try:
        return WebDriverWait(driver, tempo_espera).until(EC.presence_of_element_located((By.XPATH, xpath)))
    except:
        print(f"Elemento não encontrado: {xpath}")
...
def esperarElementos(driver: Chrome, xpath: str, tempo_espera=10) -> list[WebElement]:
    ...
    if isinstance(driver, TabDriver):
        driver = driver.driver
    try:
        return WebDriverWait(driver, tempo_espera).until(EC.visibility_of_all_elements_located((By.XPATH, xpath)))
    except:
        return []


def aguardarElementos(driver: Chrome, xpath: str, tempo_espera=10) -> list[WebElement]:
    """
    ...
    if isinstance(driver, TabDriver):
        driver = driver.driver
    try:
        return WebDriverWait(driver, tempo_espera).until(EC.presence_of_all_elements_located((By.XPATH, xpath)))
    except:
        return []


def clickarElemento(driver: Chrome, xpath: str, time_wait=10) -> WebElement | None:
    """
    ...
    if isinstance(driver, TabDriver):
        driver = driver.driver
    try:
        return WebDriverWait(driver, time_wait).until(EC.element_to_be_clickable((By.XPATH, xpath)))
    except:
        print(f"Elemento não encontrado: {xpath}")
...
def esperar_elemento(driver: Chrome, xpath: str, tempo_espera=10):
    warnings.warn(
        "esperar_elemento() está depreciada e será removida numa futura versão.",
        DeprecationWarning,
        stacklevel=2
    )
    if isinstance(driver, TabDriver):
...
if __name__=="__main__":
    ...
    # email = "raniele.santos@adpromotora.com.br"

    # pprint(coletarDadosEmail(email, "Ação judicial", r"Processo\s+(.*?)\s.*Adeso: (\d+)", separador=' '))