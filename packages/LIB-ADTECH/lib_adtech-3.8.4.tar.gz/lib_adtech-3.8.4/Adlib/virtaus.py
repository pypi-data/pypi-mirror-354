...
def assumirSolicitacao(virtaus: Chrome, nomeFerramenta: str, enumBanco: EnumBanco, tipoFiltro: FiltrosSolicitacao, localizacao: str = None, 
                       HORA_FINALIZACAO: str = 19, resetSessao: Callable[[Chrome], None] = None, driverReset: Chrome = None, resetEvent: threading.Event = None) -> FormularioSolicitacao:
    """
    ...
    while True:
        try:
            ...
            tarefasEmPool = clickarElemento(virtaus, f'//*[@id="centralTaskMenu"]/li[{idxBtn}]/a')

            if tarefasEmPool:
                tarefasEmPool.click()

                # Seleciona o filtro de "Emissão De Nova Senha Login Externo"
                filtroProcesso = clickarElemento(virtaus, f'//*[@id="centralTaskMenu"]/li[{idxBtn}]/ul//a[contains(text(), "{tipoFiltro}")]')
                ...
                if filtroProcesso:
                    ...
                    clickarElemento(virtaus, '//*[@id="inputSearchFilter"]').send_keys(nomeFerramenta)
                    time.sleep(5)
                    ...
                    solicitacao = clickarElemento(virtaus, f'//td[@title="{localizacao}"]')
                    if solicitacao:
                        solicitacao.click()
                        break
