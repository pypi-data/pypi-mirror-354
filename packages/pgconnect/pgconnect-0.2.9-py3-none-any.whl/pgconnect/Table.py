    ...
    async def create(self):
        ...
        try:
            ...
            for index in existing_indexes:
                ...
                if not any(index_name == idx.get("name", f"idx_{self.name}_{'_'.join(idx['columns'])}") for idx in self.indexes):
            ...
            if self.indexes:
                ...
                for index in self.indexes:
                    index_name = index.get("name", f"idx_{self.name}_{'_'.join(index['columns'])}")
                    unique = "UNIQUE" if index.get("unique", False) else ""
                    columns = ", ".join(index["columns"])
                    index_query = f"CREATE {unique} INDEX IF NOT EXISTS {index_name} ON {self.name} ({columns});"
                    await connection.execute(index_query, timeout=self.timeout)
