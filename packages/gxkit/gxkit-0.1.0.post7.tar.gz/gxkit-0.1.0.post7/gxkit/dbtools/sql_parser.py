...
from gxkit.dbtools._base import SQLParseError
from gxkit.dbtools._utils import MULTIWORD_COMMAND_OPS, QUERY_OPS, STATEMENT_OPS, FIXED_COLUMNS
...
class SQLParser:
    ...
    def __init__(self, sql: str, db_type: str = "mysql"):
        ...
        # Store IoTDB paths that were automatically quoted during cleaning so
        # they can be restored when generating SQL
    ...
    def _quote_iotdb_paths(sql: str) -> Tuple[str, List[str]]:
        """Quote IoTDB paths that contain wildcards or numeric parts.

        Returns the modified SQL and a list of paths that were quoted.
        """
        ...
        for match in pattern.finditer(sql):
            ...
            if start > 0 and sql[start - 1] in "'\"`":
                continue
            path = match.group(1)
            if '*' in path or re.search(r"\.[0-9]", path):
    ...
    def _restore_iotdb_paths(self, sql: str) -> str:
        """Restore IoTDB paths that were auto-quoted during parsing."""
