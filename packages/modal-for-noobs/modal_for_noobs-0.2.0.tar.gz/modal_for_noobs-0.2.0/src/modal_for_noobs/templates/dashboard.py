    ...
    with gr.Blocks(title="Modal Deployment Dashboard", theme=MODAL_THEME, css=MODAL_CSS) as dashboard:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("📋 Logs"):
                ...
                dashboard.load(refresh_logs, inputs=[log_limit], outputs=[log_output], every=5)
            
            # Deployment Info Tab
            with gr.Tab("ℹ️ Deployment Info"):
                gr.Markdown("### Deployment Configuration")
                deployment_info = gr.JSON(
                    label="Deployment Information",
                    value=dashboard_state.get_deployment_info()
                )
            ...
            with gr.Tab("📊 Metrics"):
                ...
                dashboard.load(get_metrics, outputs=[uptime, log_count, memory_usage], every=10)
