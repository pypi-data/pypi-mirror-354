...
# 初始化mcp服务
mcp = FastMCP('xhs-bailian-mcp-server-hailey', log_level='ERROR', settings=settings)
# 定义工具
@mcp.tool(name='小红书内容安全审核专家', description='小红书内容安全审核专家,输入小红书文案')
async def red_book_moderator(prompt: str = Field(..., description='小红书文案')) -> str:
    """小红书内容审核专家
    Args:
        prompt (str): 小红书文案
    Returns:
        str: 审核后的内容
    """
    logger.info(f'小红书内容安全审核专家: {prompt}')
