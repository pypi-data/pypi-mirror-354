...
mcp = FastMCP(SERVER_NAME, **fastmcp_settings)
...
async def interactive_feedback(
    ...
    Returns:
        Dict[str, Any]: 包含文本內容和圖片列表的字典
    """
    debug_log(f"啟動互動式回饋收集 MCP 服務器")
    ...
    try:
        ...
        # 處理圖片並轉換為 MCPImage 對象
        mcp_images = []
        if result.get("images"):
            mcp_images = process_images(result["images"])
            debug_log(f"已處理 {len(mcp_images)} 張圖片")
        
        # 返回包含文本和圖片的字典
        # fastmcp 2.x 會自動處理這種格式
        content_items = []
        ...
        # 添加图片内容
        if mcp_images:
            content_items.extend(mcp_images)
        
        debug_log(f"回饋收集完成，內容項目數: {len(content_items)}")
        return {
