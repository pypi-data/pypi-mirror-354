...
class Download(MagmaSeismic):
    ...
    def _hours(self, hour_ranges: pd.DatetimeIndex, period: int) -> list[dict]:
    ...
    def _idds(self, date: datetime, period: int, use_merge: bool = False) -> None:
        ...
        for hour in self._hours(hour_ranges, period):
    ...
    def _sds_chunking(self, date: datetime, chunk_size: int, chunk_dir: str, filename: str) -> list[str]:
        chunk_files = []
        end_date = date + timedelta(days=1) - timedelta(milliseconds=1)
        hour_ranges = pd.date_range(start=date, end=end_date, freq=f'{str(chunk_size)}min')

        for hour in self._hours(hour_ranges, chunk_size):
            index = hour['index']
            start_hour = hour['start_hour']
            end_hour = hour['end_hour']
            start_hour_str = hour['start_hour'].strftime('%H:%M:%S')
            ...
            try:
                ...
                if os.path.isfile(chunk_filepath) and self.overwrite is False:
                    print(f'ℹ️✅ {start_hour_str} to {end_hour_str} - {self.nslc} exists. Skipping')
                    chunk_files.append(chunk_filepath)
                    continue

                if self.verbose:
                    print(f'ℹ️⌚ Chunk Downloading :: {start_hour_str} to {end_hour_str}')

                stream = self.client.get_waveforms(
                    network=self.network, station=self.station, location=self.location,
                    channel=self.channel, starttime=start_hour, endtime=end_hour)
    ...
    def _sds(self, date: datetime, use_merge: bool = False, chunk_size: int = None) -> None:
        ...
        try:
            ...
            if chunk_size is not None:
                if self.verbose:
                    print(f'🔢 {start_date_str} - {self.nslc} :: Using {chunk_size} minutes of chunking')

                tmp_dir = os.path.join(sds_dir, '.tmp')
                os.makedirs(tmp_dir, exist_ok=True)
                chunk_files = self._sds_chunking(date=date, chunk_size=chunk_size, chunk_dir=tmp_dir,
                                                 filename=filename)
    ...
    def to_sds(self, use_merge: bool = False, chunk_size: int = None) -> None:
