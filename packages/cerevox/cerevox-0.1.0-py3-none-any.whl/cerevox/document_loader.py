        ...
        for i, element_data in enumerate(elements_data):
            try:
                ...
                if not sentences and text_content:
                    sentences = len(
                        re.split(r"[.!?]+(?=\s|$|[*_`\]])", text_content.strip())
                    )
...
def chunk_markdown(
    ...
    for section in header_sections:
        if len(section) <= max_size:
            ...
            paragraph_chunks = _split_by_paragraphs(section, min_size, max_size)
...
def chunk_text(text: str, target_size: int = 500, tolerance: float = 0.1) -> List[str]:
    ...
    chunks = _split_by_paragraphs(text, min_size, max_size)
...
def _split_by_paragraphs(text: str, min_size: int, max_size: int) -> List[str]:
