...
from haruka_parser.magic_html.config import *
from haruka_parser.magic_html.readability_plus import Document as DocumentPlus
...
class BaseExtractor:
    ...
    def get_content_html(self, cleaned_tree_backup, xp_num="others", base_url=""):
        ...
        try:
            data = readablity_go_parse(
                tostring(cleaned_tree_backup, encoding=str), base_url
            )
    ...
    def clean_unique_id(self, raw_element, content_html):
        ids = re.findall(rf' {Unique_ID}="(\d+)"', content_html)
        ...
        for x in ids:
            if int(x) > int(skip_ids[-1]):
                ...
                drop_node = raw_element.xpath(f"//*[@{Unique_ID}='{x}']")
                if drop_node:
                    new_div = Element("div")
        ...
        content_html = re.sub(rf' {Unique_ID}="\d+"', "", content_html)
    ...
    def math_latex_processing(self, node):
        ...
        if node.tag == "span" and node_class and "x-ck12-mathEditor" in node_class:
            try:
                ...
                if text_strip(expr):
                    expr = unquote(expr).replace(r"\&quot;", "").replace(r"&quot;", "")
                    # Replace with a span
                    new_span = Element("span")
                    wrapped_expr = wrap_math(expr)
                    new_span.text = wrapped_expr
                ...
                if j.xpath(".//a"):
                    if tagname == "p":
                        ...
                        if tagname == "li":
                            if (
                                text_len(
                                    "".join(j.xpath(".//text()[not(ancestor::a)]"))
                                )
                                > 50
                            ):
                                continue
