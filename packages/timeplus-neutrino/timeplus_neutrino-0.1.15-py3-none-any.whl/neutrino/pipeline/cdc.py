...
from .ddl_translation import translate_ddl_sync
...
4. in case the target stream is clickhouse or mysql accroding to type field of external stream settings, call generate_ddl_on_target_stream() to also generate the ddl on the target stream
   using target_stream ddl from step 3 as input
...
"target_stream_external_ddl": the ddl on the target stream when the target system is clickhouse or mysql
...
def generate_ddl_on_target_stream(
    ddl: Annotated[str, "the target stream DDL"],
    db_type: Annotated[str, "the database type, could be clickhouse or mysql"],
):
...
async def _build_debezium_pipeline(
    ...
    try:
        agent_tools: List[Tool] = [
            ...
            FunctionTool(
                generate_ddl_on_target_stream,
                name="generate_ddl_on_target_stream",
                description="generate the ddl on the target stream from the target stream DDL.",
            ),
        ]
