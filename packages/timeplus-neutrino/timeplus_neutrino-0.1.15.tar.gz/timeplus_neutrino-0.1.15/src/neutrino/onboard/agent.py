...
class ExternalStreamSchemaInferenceAgent(SchemaInferenceAgent):
    ...
    * No need return table engine and settings in the DDL
    * Put description into the comment
    * Check if raw formant is set to True, in such case, return the raw DDL with only one field raw in string type.
    ...
    * map
    ...
    if it is raw_format = True, the output should only contain one string fields, and the DDL should be like this:
    ```sql
    CREATE EXTERNAL STREAM car_live_data
    (
      `raw` string
