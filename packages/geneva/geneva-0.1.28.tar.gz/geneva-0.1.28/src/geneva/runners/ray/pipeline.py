...
from geneva.table import JobFuture, TableReference
...
def dispatch_run_ray_add_column(
    ...
    **kwargs,
    ...
    return RayJobFuture(obj_ref=obj_ref)
...
def run_ray_add_column(
    ...
    **kwargs,
    ...
    _LOG.info(f"carry_forward_cols {carry_forward_cols}")
    ...
    _LOG.debug(
        f"kicking off add column pipeline for {transforms} where='{where}'"
...
class RayJobFuture(JobFuture):
    ...
    def done(self) -> bool:
        ready, _ = ray.wait([self.obj_ref], timeout=0)
        return bool(ready)

    def result(self, timeout: float | None = None) -> Any:
        return ray.get(self.obj_ref, timeout=timeout)
