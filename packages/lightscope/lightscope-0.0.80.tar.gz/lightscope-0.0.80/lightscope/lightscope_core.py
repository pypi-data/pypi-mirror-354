...
class Ports:
    ...
    def Report_unwanted_traffic(self,pkt_info,reason,confidence):
        ...
        if pkt_info.tcp_dport not in self.closed_honeypots:
    ...
    def rotate_honeypots(self, K=10):
            ...
            new_set = set(topk) - self.open_honeypots
            to_close = self.open_honeypots - set(topk)

            for port in new_set:
                self.hp_cmd_parent.send(("open", port))
            for port in to_close:
                self.hp_cmd_parent.send(("close", port))
                self.closed_honeypots.add(port)
                self.port_counts.pop(port)

            self.open_honeypots = set(topk)
            #If we have tried 1,000 ports, clear the closed_honeypots set and we can start over, about 17 days
            if len(self.closed_honeypots) > 1000:
                self.closed_honeypots.clear()
