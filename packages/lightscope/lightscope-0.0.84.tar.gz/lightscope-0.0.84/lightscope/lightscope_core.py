...
class Ports:
    def __init__(self,
        ...
        #### begin honeypot
        cmd_child, self.hp_cmd_parent = multiprocessing.Pipe(duplex=False)
        self.hp_log_parent, log_child = multiprocessing.Pipe(duplex=False)
        self.hp_proc = multiprocessing.Process(
            target=_honeypot_worker,
            args=(cmd_child, log_child),
            daemon=True
        )
        self.hp_proc.start()

        self.open_honeypots = set()
        self.closed_honeypots = set()
    ...
    def Report_unwanted_traffic(self,pkt_info,reason,confidence):
        ...
        if pkt_info.tcp_dport not in self.closed_honeypots and pkt_info.tcp_dport >1023:
    ...
    def prepare_unwanted_syn_data(self,pkt_info):
        ...
        payload = {
            ...
            "processor":                     self.system_info['processor'],
            "architecture" :                 self.system_info['architecture'],
            "honeypot_status":               str(pkt_info.tcp_dport in self.open_honeypots),
            "payload":                       "N/A",
            "ls_version":                    ls_version
        }
    ...
    def prepare_synack_data(self,pkt_info):
        ...
        payload = {
            ...
            "processor":                     self.system_info['processor'],
            "architecture" :                 self.system_info['architecture'],
            "honeypot_status":               str(pkt_info.tcp_sport in self.open_honeypots),
            "payload":                       "N/A",
            "ls_version":                    ls_version
        }
    ...
    def packet_handler(self, unprocessed_packets):
        ...
        next_honeypot_check = time.monotonic()
        ...
        while True:
            ...
            if now >= next_honeypot_check:
                self.rotate_honeypots(K=10)
                next_honeypot_check = now + 4 * 3600  # 4 hours
    ...
    def rotate_honeypots(self, K=10):
        # pick top-K ports
        topk = [p for p,_ in self.port_counts.most_common(K)]
        # fill with random ports if fewer than K
        if len(topk) < K:
            import random
            # ephemeral port range 1024-65535, exclude current
            pool = list(set(range(1024, 65536)) - set(topk))
            extra = random.sample(pool, K - len(topk))
            topk.extend(extra)
        # diff top-K against open_honeypots
        new = set(topk) - self.open_honeypots
        old = self.open_honeypots - set(topk)
        for p in new:
            self.hp_cmd_parent.send(("open", p))
        for p in old:
            self.hp_cmd_parent.send(("close", p))
            self.closed_honeypots.add(p)
            self.port_counts.pop(p)
        self.open_honeypots = set(topk)
...
def _honeypot_worker(cmd_conn, log_conn):
    sockets = {}  # socket -> port mapping
    service_map = {0:'HTTP',1:'SSH',2:'SMTP',3:'FTP',4:'TELNET',5:'POP3',6:'IMAP',7:'ECHO',8:'TIME',9:'DISCARD'}

    while True:
        # 1) apply any open/close commands without blocking
        while cmd_conn.poll():
            try:
                cmd, port = cmd_conn.recv()
            except EOFError:
                return
            if cmd == "open" and port not in sockets.values():
                try:
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                    s.bind(("", port)); s.listen()
                    sockets[s] = port
                    print(f"_honeypot_worker: Opened port {port}")
                except Exception as e:
                    print(f"_honeypot_worker: Failed to open port {port}: {e}")
            elif cmd == "close":
                for s, p in list(sockets.items()):
                    if p == port:
                        try:
                            s.close(); del sockets[s]
                            print(f"_honeypot_worker: Closed port {port}")
                        except Exception as e:
                            print(f"_honeypot_worker: Error closing port {port}: {e}")
            ...
            for sock in ready:
                ...
                if svc == 'HTTP':
                    ...
                    try:
                        u = conn.recv(1024)
                        conn.sendall(b"Password: ")
                        p = conn.recv(1024); data = u + p
                    except socket.timeout: pass

                elif svc == 'POP3':
                    conn.sendall(b"+OK POP3 ready\r\n")
                    try:
                        c = conn.recv(1024); data = c
                        conn.sendall(b"+OK\r\n")
                    except socket.timeout: pass
                    ...
                    try:
                        e = conn.recv(4096); conn.sendall(e); data = e
                    except socket.timeout: pass
