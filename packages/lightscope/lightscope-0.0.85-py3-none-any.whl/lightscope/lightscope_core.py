    ...
    def packet_handler(self, unprocessed_packets):
        ...
        while True:
            ...
            if now >= honeypot_send_top_ports:
                ...
                honeypot_send_top_ports = now + 5  # Send every 5 minutes for better data
            ...
            if now >= honeypot_open_ports_updates:
                ...
                honeypot_open_ports_updates = now + 5  # Check every minute
...
def _honeypot_worker(top_unwanted_ports_consumer, shared_open_honeypots, hp_upload_producer, external_network_information, config_settings, system_info):
    ...
    # Auto-rotation state
    next_rotation = time.time() + 10  # Start first rotation in 10 seconds for testing
    ...
    # Open initial 10 random ports on startup
    import random
    initial_ports = random.sample(range(1024, 65536), 10)
    ...
    def process_honeypot_connection_data(port, service, remote_ip, remote_port, data, connection_start_time, connection_end_time, bytes_received):
        ...
        payload_info = {
            ...
            "initial_data": data.decode('utf-8', errors='ignore')[:200] if data else "None",
            "connection_time": connection_start_time
        }
        payload = f"Service: {payload_info['service']}, Duration: {payload_info['duration_seconds']}s, Bytes: {payload_info['bytes_received']}, Data: {payload_info['initial_data']}"
    ...
    def rotate_honeypots():
        ...
        # Get top ports > 1023, filter out system ports
        valid_ports = {p: c for p, c in aggregated_counts.items() if p > 1023}
        ###print(f"_honeypot_worker: Valid ports (>1023): {len(valid_ports)}")
        
        top_ports = sorted(valid_ports.items(), key=lambda x: x[1], reverse=True)[:10]
        target_ports = [p for p, _ in top_ports]
        
        ###print(f"_honeypot_worker: Target ports from traffic: {target_ports}")
        
        # If first run or insufficient data, fill with random ports > 1023
        if len(target_ports) < 10:
            import random
            existing = set(target_ports)
            available_ports = [p for p in range(1024, 65536) if p not in existing]
            needed = 10 - len(target_ports)
            random_ports = random.sample(available_ports, min(needed, len(available_ports)))
            target_ports.extend(random_ports)
            print(f"_honeypot_worker: First run or insufficient data, using {len(random_ports)} random ports: {random_ports}")
    ...
    while True:
        ...
        if loop_count % 100 == 0:
            print(f"_honeypot_worker: Loop {loop_count}, now={now:.1f}, next_rotation={next_rotation:.1f}, diff={next_rotation-now:.1f}")
        ...
        if now >= next_rotation:
            ...
            rotate_honeypots()
            next_rotation = now + 10  # Rotate every 30 minutes for testing
