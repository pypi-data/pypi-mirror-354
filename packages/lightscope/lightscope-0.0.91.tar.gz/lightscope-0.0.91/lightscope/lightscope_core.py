...
def _honeypot_worker(top_unwanted_ports_consumer, shared_open_honeypots, hp_upload_producer, external_network_information, config_settings, system_info):
    ...
    def rotate_honeypots():
        ...
        if len(target_ports) < 10:
            ...
            # First add priority ports that aren't already in target_ports
            available_priority = [p for p in priority_ports if p not in existing]
            ...
            # If still need more ports after priority, add random ones
            still_needed = 10 - len(target_ports)
            if still_needed > 0:
                all_existing = set(target_ports)
                available_random = [p for p in range(1024, 65536) if p not in all_existing]
                random_ports = random.sample(available_random, min(still_needed, len(available_random)))
