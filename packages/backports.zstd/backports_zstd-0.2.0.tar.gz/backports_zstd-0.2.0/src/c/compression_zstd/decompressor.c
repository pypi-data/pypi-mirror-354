...
{
    assert(BACKPORTSZSTD_LOCK_isLocked(&self->lock));
    ZSTD_DDict *ret;
...
_zstd_load_d_dict(ZstdDecompressor *self, PyObject *dict)
{
    ...
    if (ret < 0) {
        ...
        BACKPORTSZSTD_LOCK_lock(&zd->lock);
        ret = _zstd_load_impl(self, zd, mod_state, type);
        BACKPORTSZSTD_LOCK_unlock(&zd->lock);
        return ret;
    }
...
ZstdDecompressor_dealloc(PyObject *ob)
{
    ...
    assert(!BACKPORTSZSTD_LOCK_isLocked(&self->lock));
    BACKPORTSZSTD_LOCK_free(self->lock);
...
/*[clinic end generated code: output=f3a20940f11b6b09 input=54d41ecd681a3444]*/
{
    ...
    BACKPORTSZSTD_LOCK_lock(&self->lock);

    if (!self->eof) {
        BACKPORTSZSTD_LOCK_unlock(&self->lock);
...
PyType_Spec zstd_decompressor_type_spec = {
    .name = "backports.zstd.ZstdDecompressor",
