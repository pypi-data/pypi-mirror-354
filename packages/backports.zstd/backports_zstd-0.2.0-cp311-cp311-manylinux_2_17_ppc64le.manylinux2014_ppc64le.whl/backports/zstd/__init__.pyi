...
def finalize_dict(
    zstd_dict: ZstdDict,
    /,
    samples: Iterable[ReadableBuffer],
    dict_size: int,
    level: int,
) -> ZstdDict: ...
def compress(
...
class ZstdFile(_streams_BaseStream):
    ...
    def flush(  # type: ignore[override]
        self, mode: _ZstdCompressorFlushBlock | _ZstdCompressorFlushFrame = 1
    ) -> bytes: ...
...
class ZstdCompressor:
    ...
    def compress(
        ...
        /,
        data: ReadableBuffer,
        mode: _ZstdCompressorContinue
        | _ZstdCompressorFlushBlock
        | _ZstdCompressorFlushFrame = 0,
    ) -> bytes: ...
    def flush(
