...
@_ods_cext.register_operation(_Dialect)
class AllocatorSelectAttrOp(_ods_ir.OpView):
  OPERATION_NAME = "hal.allocator.select.attr"

  _ODS_REGIONS = (0, True)

  def __init__(self, optimal_set, memory_types, buffer_usage, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    _ods_context = _ods_get_default_loc_context(loc)
    attributes["optimal_set"] = (optimal_set if (
    isinstance(optimal_set, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('HAL_DeviceOptimalAttr')) else
      _ods_ir.AttrBuilder.get('HAL_DeviceOptimalAttr')(optimal_set, context=_ods_context))
    attributes["memory_types"] = (memory_types if (
    isinstance(memory_types, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('HAL_MemoryTypeBitfieldAttr')) else
      _ods_ir.AttrBuilder.get('HAL_MemoryTypeBitfieldAttr')(memory_types, context=_ods_context))
  ...
  def optimal_set(self):
    return self.operation.attributes["optimal_set"]

  @optimal_set.setter
  def optimal_set(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["optimal_set"] = value
...
def allocator_select_attr(optimal_set, memory_types, buffer_usage, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return AllocatorSelectAttrOp(optimal_set=optimal_set, memory_types=memory_types, buffer_usage=buffer_usage, loc=loc, ip=ip).results
...
class ExecutableConditionOp(_ods_ir.OpView):
  ...
  def __init__(self, function_type, *, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_918')) else
      _ods_ir.AttrBuilder.get('anonymous_918')(function_type, context=_ods_context))
