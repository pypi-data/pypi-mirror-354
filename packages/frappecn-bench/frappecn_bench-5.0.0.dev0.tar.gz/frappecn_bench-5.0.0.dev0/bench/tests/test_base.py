...
class TestBenchBase(unittest.TestCase):
	...
	def init_bench(self, bench_name, **kwargs):
		...
		if not os.path.exists(frappe_tmp_path):
			exec_cmd(
				f"git clone https://github.com/frappecn/frappe -b {FRAPPE_BRANCH} --depth 1 --origin upstream {frappe_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.benches_path, bench_name)):
			...
			exec_cmd(
				"git remote set-url upstream https://github.com/frappecn/frappe",
