...
class TestBenchInit(TestBenchBase):
	...
	def test_init(self, bench_name="test-bench", **kwargs):
		...
		self.assertTupleEqual(
			...
			("/tmp/frappe", "file:///tmp/frappe", "frappe", "frappe", "frappecn"),
		)
	...
	@unittest.skipIf(FRAPPE_BRANCH != "panda", "only for panda branch")
	...
	def test_switch_to_branch(self):
		...
		# * chore: change to 14 when available
		prevoius_branch = "panda"
		if FRAPPE_BRANCH.startswith("version-"):
			try:
				prevoius_branch = f"version-{int(FRAPPE_BRANCH.split('-')[1]) - 1}"
			except (IndexError, ValueError):
				prevoius_branch = "panda"
		# 如果不是 version- 开头，直接跳过分支切换测试或给出提示
		else:
			print(f"FRAPPE_BRANCH={FRAPPE_BRANCH}，不是 version- 格式，跳过 prevoius_branch 切换测试。")
			return
