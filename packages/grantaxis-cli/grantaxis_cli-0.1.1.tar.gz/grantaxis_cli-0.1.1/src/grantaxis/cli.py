...
@click.option(
    ...
    help="Path to Snowflake connector‑kwargs JSON file",
)
@click.option(
    "--output",
    "-o",
...
@click.option(
    "--creds",
    type=click.Path(exists=True, dir_okay=False),
    help="Snowflake credentials JSON (required if --snapshot not provided)",
)
@click.option(
    "--config",
    "-c",
...
def diff_cmd(
    ...
    try:
        ...
        if snapshot:
            ...
            if not creds:
                raise CredentialsError(
                    "Either --snapshot file or --creds must be "
                    "provided for live snapshot"
                )
            creds_dict = _load_creds(creds)
            if not quiet:
                click.echo("📸 Capturing live snapshot...", err=True)
        ...
        if config:
            # User explicitly provided --config
            cfg = Config.load(config)
            logger.info(f"Using configuration: {config}")
        else:
            # No --config provided - try auto-discovery or use defaults
            if Path("grantaxis.yml").exists():
                cfg = Config.load("grantaxis.yml")
                logger.info("Auto-discovered grantaxis.yml")
            else:
                cfg = Config.load(None)  # Built-in defaults
        ...
        added, removed, changed = cfg.filter_drift(added, removed, changed)
