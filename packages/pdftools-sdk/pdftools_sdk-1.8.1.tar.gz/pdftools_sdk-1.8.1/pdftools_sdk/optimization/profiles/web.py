...
class Web(pdftools_sdk.optimization.profiles.profile.Profile):
    """
    ...
    `pdftools_sdk.optimization.profiles.web.Web.resolution_d_p_i` has influence on both values.
    ...
    `pdftools_sdk.optimization.profiles.profile.Profile.image_recompression_options`.
    ...
    def __init__(self):
        ...
        ret_val = _lib.PdfToolsOptimizationProfiles_Web_New()
        if ret_val is None:
    ...
    def resolution_d_p_i(self) -> Optional[float]:
        ...
        Images with a resolution above `pdftools_sdk.optimization.profiles.web.Web.threshold_d_p_i` are down-sampled.
        ...
        ret_val = c_double()
        if not _lib.PdfToolsOptimizationProfiles_Web_GetResolutionDPI(self._handle, byref(ret_val)):
    ...
    def resolution_d_p_i(self, val: Optional[float]) -> None:
        ...
        if not _lib.PdfToolsOptimizationProfiles_Web_SetResolutionDPI(self._handle, byref(c_double(val)) if val is not None else None):
            _NativeBase._throw_last_error(False)
    ...
    def threshold_d_p_i(self) -> float:
        ...
        Valid values are in the range 1.0 to 10000.
        To deactivate down-sampling of images set `pdftools_sdk.optimization.profiles.web.Web.resolution_d_p_i` to `None`.
         
        Default: `1.4` times `pdftools_sdk.optimization.profiles.web.Web.resolution_d_p_i`.
        ...
        ret_val = _lib.PdfToolsOptimizationProfiles_Web_GetThresholdDPI(self._handle)
        if ret_val == 0:
            _NativeBase._throw_last_error()
        return ret_val
    ...
    def threshold_d_p_i(self, val: float) -> None:
        ...
        if not _lib.PdfToolsOptimizationProfiles_Web_SetThresholdDPI(self._handle, val):
            _NativeBase._throw_last_error(False)
    ...
    def _from_handle(cls, handle):
        ...
        """
        instance = Web.__new__(cls)  # Bypass __init__
