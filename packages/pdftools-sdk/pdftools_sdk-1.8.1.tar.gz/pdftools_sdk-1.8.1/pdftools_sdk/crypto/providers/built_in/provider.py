...
class Provider(pdftools_sdk.crypto.providers.provider.Provider):
    ...
    Signing certificates with private keys can be loaded using `pdftools_sdk.crypto.providers.built_in.provider.Provider.create_signature_from_certificate`.
    ...
    *Certificates Directory*:
    ...
    def create_timestamp(self) -> TimestampConfiguration:
        ...
        Note that to create time-stamps, the `pdftools_sdk.crypto.providers.built_in.provider.Provider.timestamp_url` must be set.
    ...
    def create_prepared_signature(self, size: int, format: str, name: str) -> SignSignatureConfiguration:
        ...
        The process using an external signature handler is:
            - `pdftools_sdk.crypto.providers.built_in.provider.Provider.create_prepared_signature`: Create the signature configuration.
            - `pdftools_sdk.sign.signer.Signer.add_prepared_signature`: Create the document with the prepared signature.
            - `pdftools_sdk.sign.prepared_document.PreparedDocument.get_hash`: Calculate the hash from the document and create the signature using an
        external signature handler.
            - `pdftools_sdk.crypto.providers.built_in.provider.Provider.read_external_signature`: Create signature configuration for the external signature.
            - `pdftools_sdk.sign.signer.Signer.sign_prepared_signature`: Insert the external signature into the document with the prepared signature.
    ...
    def read_external_signature(self, signature: List[int]) -> SignSignatureConfiguration:
        ...
        See `pdftools_sdk.crypto.providers.built_in.provider.Provider.create_prepared_signature` for more information on the signing process using an external signature handler.
