...
if TYPE_CHECKING:
    from pdftools_sdk.image.document import Document


class DocumentList(_NativeObject, list):
    """
    ...
    def __init__(self):
        ...
        ret_val = _lib.PdfToolsImage_DocumentList_New()
        if ret_val is None:
    ...
    def __len__(self) -> int:
        ...
        ret_val = _lib.PdfToolsImage_DocumentList_GetCount(self._handle)
        if ret_val == -1:
            _NativeBase._throw_last_error()
        return ret_val

    def clear(self) -> None:
        raise NotImplementedError("Clear method is not supported in DocumentList.")
    ...
    def remove(self, index: int) -> None:
        raise NotImplementedError("Removing elements is not supported in DocumentList.")

    def extend(self, items: DocumentList) -> None:
        if not isinstance(items, DocumentList):
            raise TypeError(f"Expected type {DocumentList.__name__}, but got {type(items).__name__}.")
        for item in items:
            self.append(item)

    def insert(self, index: int, value: Any) -> None:
        raise NotImplementedError("Insert method is not supported in DocumentList.")

    def pop(self, index: int = -1) -> Any:
        raise NotImplementedError("Pop method is not supported in DocumentList.")

    def copy(self) -> DocumentList:
        raise NotImplementedError("Copy method is not supported in DocumentList.")

    def sort(self, key=None, reverse=False) -> None:
        raise NotImplementedError("Sort method is not supported in DocumentList.")

    def reverse(self) -> None:
        raise NotImplementedError("Reverse method is not supported in DocumentList.")
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[Any, List[Any]]:
        ...
        ret_val = _lib.PdfToolsImage_DocumentList_Get(self._handle, index)
        if ret_val is None:
            _NativeBase._throw_last_error(False)
        return Document._create_dynamic_type(ret_val)

    def __setitem__(self, index: int, value: Any) -> None:
        raise NotImplementedError("Setting elements is not supported in DocumentList.")

    def append(self, value: Document) -> None:
        from pdftools_sdk.image.document import Document

        if not isinstance(value, Document):
            raise TypeError(f"Expected type {Document.__name__}, but got {type(value).__name__}.")

        _lib.PdfToolsImage_DocumentList_Add.argtypes = [c_void_p, c_void_p]
        _lib.PdfToolsImage_DocumentList_Add.restype = c_bool
        ret_val = _lib.PdfToolsImage_DocumentList_Add(self._handle, value._handle)
        if not ret_val:
            _NativeBase._throw_last_error(False)

    def index(self, value: Document, start: int = 0, stop: Optional[int] = None) -> int:
        from pdftools_sdk.image.document import Document
    ...
    def _create_dynamic_type(handle):
        return DocumentList._from_handle(handle)
    ...
    def _from_handle(cls, handle):
        ...
        """
        instance = DocumentList.__new__(cls)  # Bypass __init__
