...
class WarningCategory(IntEnum):
    ...
    The category of the warning of `pdftools_sdk.sign.signer.Signer.Warning`.



    Attributes:
        PDF_A_REMOVED (int):
            PDF/A conformance of input file removed due to file encryption (i.e. `pdftools_sdk.pdf.output_options.OutputOptions.encryption` is not `None`).
        ...
        SIGNED_DOC_ENCRYPTION_UNCHANGED (int):
            ...
            Therefore, the property `pdftools_sdk.pdf.output_options.OutputOptions.encryption` has no effect.
             
            This warning is generated so that the following situations can be detected:
                - If `pdftools_sdk.pdf.output_options.OutputOptions.encryption` is `None` and the input document is encrypted.
            The output document is also encrypted.
                - If `pdftools_sdk.pdf.output_options.OutputOptions.encryption` not `None` and the input document is encrypted using different encryption parameters.
            The output document is also encrypted, preserving the encryption parameters of the input document.
                - If `pdftools_sdk.pdf.output_options.OutputOptions.encryption` not `None` and the input document is not encrypted.
            The output document is not encrypted.
             
            Encryption parameters of signed documents can be changed by removing all existing signatures using the property `pdftools_sdk.sign.output_options.OutputOptions.remove_signatures`.
        ...
        ADD_VALIDATION_INFORMATION_FAILED (int):
            ...
            stored in the user's `Certificates` directory (see `pdftools_sdk.crypto.providers.built_in.provider.Provider`).
                - *Network problem:*
            The network must allow OCSP and CRL responses to be downloaded from the certificate authority's server.
            Make sure your proxy configuration (see `pdftools_sdk.sdk.Sdk.proxy`) is correct.
