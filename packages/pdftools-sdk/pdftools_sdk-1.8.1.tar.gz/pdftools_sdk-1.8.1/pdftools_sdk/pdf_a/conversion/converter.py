...
if TYPE_CHECKING:
    ...
    Report a conversion event that occurred in `pdftools_sdk.pdf_a.conversion.converter.Converter.convert`.
    These events can be used to:
        - Generate a detailed conversion report.
        - Detect and handle critical conversion events.
     
    Note that if a document cannot be converted to the requested conformance, the `pdftools_sdk.pdf_a.conversion.converter.Converter.convert` throws an exception.
    ...
    Review the suggested severity of each `pdftools_sdk.pdf_a.conversion.event_category.EventCategory` and determine the `pdftools_sdk.pdf_a.conversion.event_severity.EventSeverity` to be used in your process.
        - *Handle events according to their severity*.
            - *Events of severity* `pdftools_sdk.pdf_a.conversion.event_severity.EventSeverity.ERROR`:
    The conversion must be considered as failed.
             
            - *Events of severity* `pdftools_sdk.pdf_a.conversion.event_severity.EventSeverity.WARNING`:
    ...
    The properties `message`, `context`, and `page` in combination with the output file are helpful to make this decision.
            ...
            If a manual review is not feasible, critical warnings should be classified as an `pdftools_sdk.pdf_a.conversion.event_severity.EventSeverity.ERROR`.
    ...
    In this case, the conversion result can be verified using representative test files and the event severity chosen accordingly.
            - *Events of severity* `pdftools_sdk.pdf_a.conversion.event_severity.EventSeverity.INFORMATION`:
    ...
    Args:
        ...
        severity (EventSeverity): 
            ...
            See the documentation of `pdftools_sdk.pdf_a.conversion.converter.Converter.ConversionEvent` for a more detailed description.
...
class Converter(_NativeObject):
    ...
    def add_invoice_xml(self, invoice_type: InvoiceType, invoice: io.IOBase, af_relationship: Optional[AFRelationship] = None) -> None:
        ...
        Args:
            ...
            afRelationship (Optional[AFRelationship]): 
    ...
    def add_associated_file(self, embedded_file: io.IOBase, name: str, associate: Optional[int] = None, af_relationship: Optional[AFRelationship] = None, mime_type: Optional[str] = None, description: Optional[str] = None, modification_date: Optional[datetime] = None) -> None:
        ...
        Args:
            ...
            afRelationship (Optional[AFRelationship]): 
                ...
                If `None`, the default value is `pdftools_sdk.pdf_a.conversion.a_f_relationship.AFRelationship.UNSPECIFIED`.
    ...
    def convert(self, analysis: AnalysisResult, document: Document, out_stream: io.IOBase, options: Optional[ConversionOptions] = None, out_options: Optional[OutputOptions] = None) -> Document:
        ...
        Note that it is highly recommended to use `pdftools_sdk.pdf_a.conversion.converter.Converter.ConversionEvent` to detect critical conversion events.



        Args:
            analysis (AnalysisResult): 
                The result of the document's analysis using `pdftools_sdk.pdf_a.validation.validator.Validator.analyze`.
            ...
            outOptions (Optional[OutputOptions]): 
