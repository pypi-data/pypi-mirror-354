...
class TimestampConfiguration(_NativeObject, ABC):
    ...
    def field_name(self) -> Optional[str]:
        ...
        The `pdftools_sdk.pdf.signature_field.SignatureField.field_name` of an existing, unsigned signature field to time-stamp.
        Note that when an existing signature field is used, the appearance's position is defined by the existing field.
        Therefore, make sure the `pdftools_sdk.sign.timestamp_configuration.TimestampConfiguration.appearance` fits into the `pdftools_sdk.pdf.signature_field.SignatureField.bounding_box`.
