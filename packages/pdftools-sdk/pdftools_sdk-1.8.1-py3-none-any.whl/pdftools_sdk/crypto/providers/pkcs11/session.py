...
class Session(pdftools_sdk.crypto.providers.provider.Provider):
    ...
    To acquire a session, the following steps must be performed:
        - Load the PKCS#11 driver module using `pdftools_sdk.crypto.providers.pkcs11.module.Module.load`.
        - Get the appropriate cryptographic device from the module's `pdftools_sdk.crypto.providers.pkcs11.module.Module.devices`.
    If it can be assumed that there is only a single device available, the `pdftools_sdk.crypto.providers.pkcs11.device_list.DeviceList.get_single` can be used.
        - Create a session to the device using `pdftools_sdk.crypto.providers.pkcs11.device.Device.create_session`.
    ...
    def create_signature(self, certificate: Certificate) -> SignatureConfiguration:
        ...
        Args:
            certificate (Certificate): 
                The signing certificate from `pdftools_sdk.crypto.providers.pkcs11.session.Session.certificates`
    ...
    def create_signature_from_name(self, name: str) -> SignatureConfiguration:
        ...
        Args:
            name (str): 
                The name of the signing certificate (`pdftools_sdk.crypto.providers.certificate.Certificate.name`)
        ...
        Raises:
            pdftools_sdk.not_found_error.NotFoundError:
                If the certificate cannot be found in `pdftools_sdk.crypto.providers.pkcs11.session.Session.certificates`
    ...
    def create_signature_from_key_id(self, id: List[int], certificate: io.IOBase) -> SignatureConfiguration:
        ...
        The certificates of the trust chain should be added using `pdftools_sdk.crypto.providers.pkcs11.signature_configuration.SignatureConfiguration.add_certificate`.
    ...
    def create_timestamp(self) -> TimestampConfiguration:
        ...
        Note that to create time-stamps, the `pdftools_sdk.crypto.providers.pkcs11.session.Session.timestamp_url` must be set.
    ...
    def timestamp_url(self) -> Optional[str]:
        ...
        If a web proxy is used (see `pdftools_sdk.sdk.Sdk.proxy`), make sure the following MIME types are supported:
    ...
    def certificates(self) -> CertificateList:
        ...
        Note that some certificates or their private keys (see `pdftools_sdk.crypto.providers.certificate.Certificate.has_private_key`) might only be visible
        after `pdftools_sdk.crypto.providers.pkcs11.session.Session.login`.
