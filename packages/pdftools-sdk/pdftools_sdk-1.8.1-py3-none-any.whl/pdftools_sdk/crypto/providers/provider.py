...
class Provider(_NativeObject, ABC):
    ...
    Soft certificates can be used with the `pdftools_sdk.crypto.providers.built_in.provider.Provider`, where they can be loaded using
    `pdftools_sdk.crypto.providers.built_in.provider.Provider.create_signature_from_certificate`.
         
        - *Hardware Security Module (HSM)*:
         
            HSMs always offer very good PKCS#11 support, so the `pdftools_sdk.crypto.providers.pkcs11.session.Session` is suitable.
    For more information and installation instructions, consult the separate document "TechNotePKCS11.pdf".
        ...
        - *USB Token or Smart Card*:
            ...
            These devices typically offer a PKCS#11 interface, so the recommended provider is the `pdftools_sdk.crypto.providers.pkcs11.session.Session`.
    ...
    So these devices cannot be used in a service or web application environment.
        ...
        - *Swisscom Signing Service*:
            ...
            The `pdftools_sdk.crypto.providers.swisscom_sig_srv.session.Session` supports both static and on-demand signing certificates.
         
        - *GlobalSign Digital Signing Service*:
         
            The `pdftools_sdk.crypto.providers.global_sign_dss.session.Session` supports all features of the service.
