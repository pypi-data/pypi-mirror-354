...
class Session(pdftools_sdk.crypto.providers.provider.Provider):
    ...
    When signing with this provider, these errors can occur:
        - `pdftools_sdk.permission_error.PermissionError`: The server did not accept the SSL client certificate or the Claimed Identity string.
        - `pdftools_sdk.permission_error.PermissionError`: The requested requested distinguished name of the on-demand certificate is not allowed (`pdftools_sdk.crypto.providers.swisscom_sig_srv.session.Session.create_signature_for_on_demand_identity`).
        - `pdftools_sdk.retry_error.RetryError`: The signing request could not be processed on time by the service.
    The service may be overloaded.
        - `ValueError`: The key identity of the Claimed Identity string is invalid or not allowed.
        - `pdftools_sdk.http_error.HttpError`: If a network error occurs or the service is not operational.
     
    When signing with step-up authorization, these errors can also occur.
        - `pdftools_sdk.permission_error.PermissionError`: The user canceled the authorization request or failed to enter correct authentication data (password, OTP).
    ...
    def __init__(self, url: str, http_client_handler: HttpClientHandler):
        ...
        Args:
            ...
            httpClientHandler (HttpClientHandler): 
                ...
                Use `pdftools_sdk.http_client_handler.HttpClientHandler.set_client_certificate` to set your SSL client certificate "clientcert.p12"
