...
class SignatureConfiguration(pdftools_sdk.sign.signature_configuration.SignatureConfiguration):
    ...
    def signature_format(self) -> SignatureFormat:
        ...
        Default: `pdftools_sdk.crypto.signature_format.SignatureFormat.ADBEPKCS7DETACHED`
    ...
    def embed_validation_information(self) -> bool:
        ...
        (see `pdftools_sdk.crypto.validation_information.ValidationInformation.EMBEDINSIGNATURE`).
        This value is only supported, if the `pdftools_sdk.crypto.providers.swisscom_sig_srv.signature_configuration.SignatureConfiguration.signature_format` is `pdftools_sdk.crypto.signature_format.SignatureFormat.ADBEPKCS7DETACHED`.
        LTV signatures for other formats can be created by adding validation information to the signed document (see `pdftools_sdk.sign.signer.Signer.process` and `pdftools_sdk.sign.output_options.OutputOptions.add_validation_information`).
            - `False`: Create a basic signature without validation information (see `pdftools_sdk.crypto.validation_information.ValidationInformation.NONE`).
