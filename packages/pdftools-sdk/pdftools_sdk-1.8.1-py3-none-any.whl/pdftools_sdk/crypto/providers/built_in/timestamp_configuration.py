...
class TimestampConfiguration(pdftools_sdk.sign.timestamp_configuration.TimestampConfiguration):
    ...
    def hash_algorithm(self) -> HashAlgorithm:
        ...
        *Note:* This algorithm must be supported by the time-stamp server; many support only SHA-256.
        ...
        ret_val = _lib.PdfToolsCryptoProvidersBuiltIn_TimestampConfiguration_GetHashAlgorithm(self._handle)
        if ret_val == 0:
    ...
    def hash_algorithm(self, val: HashAlgorithm) -> None:
        ...
        *Note:* This algorithm must be supported by the time-stamp server; many support only SHA-256.
        ...
        if not _lib.PdfToolsCryptoProvidersBuiltIn_TimestampConfiguration_SetHashAlgorithm(self._handle, c_int(val.value)):
