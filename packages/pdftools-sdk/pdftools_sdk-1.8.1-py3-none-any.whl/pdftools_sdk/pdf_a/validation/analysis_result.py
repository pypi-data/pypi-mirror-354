...
class AnalysisResult(_NativeObject):
    ...
    Result of the validator's method `pdftools_sdk.pdf_a.validation.validator.Validator.analyze` which is required for the conversion to PDF/A with `pdftools_sdk.pdf_a.conversion.converter.Converter.convert`.
     
    Note that `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult` objects remain valid as long as their `pdftools_sdk.pdf.document.Document` has not been closed and the analysis result has not been used in `pdftools_sdk.pdf_a.conversion.converter.Converter.convert`.
    ...
    def conformance(self) -> Conformance:
        ...
        The PDF/A level might differ from the `pdftools_sdk.pdf_a.validation.analysis_options.AnalysisOptions.conformance`.
        If the claimed PDF/A level of the input document is higher than  `pdftools_sdk.pdf_a.validation.analysis_options.AnalysisOptions.conformance`, the higher level is used for `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.conformance`.
         
        For example, if  `pdftools_sdk.pdf_a.validation.analysis_options.AnalysisOptions.conformance` is PDF/A-2b, but the document's claimed conformance is PDF/A-2u, the analysis checks if the document actually conforms to its claimed conformance PDF/A-2u.
    ...
    def recommended_conformance(self) -> Conformance:
        ...
        The optimal PDF/A conformance for the conversion (i.e. the `pdftools_sdk.pdf_a.conversion.conversion_options.ConversionOptions.conformance`).
    ...
    def is_conversion_recommended(self) -> bool:
        ...
        A conversion is generally recommended in the following cases:
            - If `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.is_conforming` is `False`, i.e. if the document does not conform to the `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.conformance`.
        ...
        Note that in certain processes it might also be beneficial to convert a document if its conformance does not match the `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.recommended_conformance`.
    ...
    def is_conforming(self) -> bool:
        ...
        Whether the document conforms to the `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.conformance`.
        Note that even if this property returns `True` a conversion might still be recommended as indicated by `pdftools_sdk.pdf_a.validation.analysis_result.AnalysisResult.is_conversion_recommended`.
