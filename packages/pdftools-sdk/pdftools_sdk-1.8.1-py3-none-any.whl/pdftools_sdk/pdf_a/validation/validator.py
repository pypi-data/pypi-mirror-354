...
class Validator(_NativeObject):
    ...
    def analyze(self, document: Document, options: Optional[AnalysisOptions] = None) -> AnalysisResult:
        ...
        This method validates the document's standards conformance like `pdftools_sdk.pdf_a.validation.validator.Validator.validate`.
        In addition to that, certain additional checks can be performed.
        However, the main difference is that the analysis result can be used in `pdftools_sdk.pdf_a.conversion.converter.Converter.convert` to convert the PDF document to PDF/A.
