...
def find_files(
    ...
    return FileScan.files(path, patterns, maximum)
...
def find_dirs(
    ...
    return FileScan.dirs(path, patterns, maximum)
...
def load_pickle(
    ...
    if not is_file(path, validate=default is None):
        return default
    with open(path, "rb", encoding=encoding, errors=errors) as f:
...
def save_pickle(
    ...
    with open(str(path), "wb", encoding=encoding, errors=errors) as f:
