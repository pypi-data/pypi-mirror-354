...
from pytx16s.models import DeviceReport, Device
...
class Joystick:
    """Joystick class to control the TX16s."""
    def __init__(
        self,
    ...
    def connect(
        ...
        if (device_model or self.device_model) and not self.connected:
            ...
            self.device = hid.Device(
                ...
                pid=device_model.product_id
                if device_model
                else self.device_model.product_id,
            )
            self.connected = True
            return self.device
        ...
        while not self.connected:
            with suppress(hid.HIDException):
                self.device = self.connect(
                    device_model=self.device_model, connect_mode="reconnect"
                )
        self.connected = True
