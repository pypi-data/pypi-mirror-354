...
class TMail:
    ...
    def __init__(self,
        ...
        """
        22.do临时邮箱
        :param token: 关注微信公众号：XiaoqiangClub，发送：用户认证。可获取token
        ...
        :param kwargs: 其它参数
        """
        if not user_authentication(token):
        ...
        if cache:
            ...
            if ac:
                ...
                self.ac = AutoChrome(addr_or_opts=addr_or_opts, headless=self.headless, win_size="max", proxy=proxy,
                                     auto_port=auto_port, auto_handle_alert=True, auto_download_browser=True,
