...
class TextService(BaseTextService, TextServiceInterface):
    ...
    def _chat(self, message: str, session_id: str, chat_history: List[Dict[str, str]] = None, 
             system_prompt: str = None, format: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
        """
        Chat with the model.
    ...
    def _chat_with_tools(self, message: str, session_id: str, tools: List[Dict[str, Any]] = None,
        ...
        if not current_tools:
            return self._chat(message, session_id, chat_history, system_prompt, format)
    ...
    def _chat_with_documents_and_tools(self, message: str, session_id: str, documents: List[Dict[str, str]],
        ...
        if not current_tools:
            return self._chat_with_documents(message, session_id, documents, chat_history, system_prompt, format)
    ...
    def chat(self, message: str, session_id: str, 
        ...
        if documents and current_tools:
            return self._chat_with_documents_and_tools(message, session_id, documents, current_tools, chat_history, system_prompt, format)
        elif documents:
            return self._chat_with_documents(message, session_id, documents, chat_history, system_prompt, format)
        elif current_tools:
            return self._chat_with_tools(message, session_id, current_tools, chat_history, system_prompt, format)
        else:
            return self._chat(message, session_id, chat_history, system_prompt, format)
