...
class OllamaChatRepository(BaseOllamaRepository, LlmChatRepository):
    ...
    def chat(self, user_message: str = None, session_id: str = None, chat_history: List[dict] = None, 
             ...
             messages: List[Any] = None, tools: List[Dict[str, Any]] = None) -> Optional[str]:
        """
        ...
        if messages is not None:
            ...
            if tools is not None:
                ...
                return self._chat_with_messages_and_tools(messages, tools, format, system_prompt)
            else:
                # Otherwise, use _chat_with_messages
                return self._chat_with_messages(messages, format, system_prompt)
        ...
        if tools is not None:
            logger.info(f"OllamaChatRepository: Using provided tools with {len(tools)} tools")
            result = self._chat_with_messages_and_tools(session.messages, tools, format, system_prompt)
        else:
            # Otherwise, use _chat_with_messages
            logger.info(f"OllamaChatRepository: Calling _chat_with_messages")
            result = self._chat_with_messages(session.messages, format, system_prompt)
    ...
    def _chat_with_messages(self, messages: List[Any], format: Dict[str, Any] = None, system_prompt: str = None) -> Optional[str]:
        """
        ...
        # Call the Ollama API
        return self._call_ollama_api(messages, format)


    def _chat_with_messages_and_tools(self, messages: List[Any], tools: List[Dict[str, Any]] = None, format: Dict[str, Any] = None, system_prompt: str = None) -> Optional[str]:
        """
    ...
    def _call_ollama_api(self, messages: List[Any], format: Dict[str, Any] = None) -> Optional[str]:
        """
