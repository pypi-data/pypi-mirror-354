        ...
        if self._otlp_config and "endpoint" in self._otlp_config:
            try:
                initialize_tracing(
                    ...
                    headers=self._otlp_config.get("headers", {}),
                    service_name=(
                        self.manifest.name if self.manifest else "thoughtful-run"
                    ),
                )
                tracer = trace.get_tracer(__name__)
                # Create a root span that will be the parent of all other spans
                self._root_span_cm = tracer.start_as_current_span(
                    name=self.manifest.name if self.manifest else "thoughtful-run",
