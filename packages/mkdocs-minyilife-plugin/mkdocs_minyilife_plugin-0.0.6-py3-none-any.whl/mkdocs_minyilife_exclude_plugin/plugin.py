...
class ExcludePlugin(mkdocs.plugins.BasePlugin):
    ...
    config_scheme = (
        ...
        ('log', mkdocs.config.config_options.Type(bool, default=False)),
    )
    ...
    def on_files(self, files, config):
        ...
        def include(name):
            for g in globs:
                if fnmatch.fnmatchcase(name, g):
                    return False
            for r in regexes:
                if re.match(r, name) or re.search(r, name):
                    return False
        ...
        out = []
        for f in files:
            if not include(f.src_path):
                if self.config['log']:
                    logger.info("exclude file: %s" % f.src_path)
                continue 
            out.append(f)
