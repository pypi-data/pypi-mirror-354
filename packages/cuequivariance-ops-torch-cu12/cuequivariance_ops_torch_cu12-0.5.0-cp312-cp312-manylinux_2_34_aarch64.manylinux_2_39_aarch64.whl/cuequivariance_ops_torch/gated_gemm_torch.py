...
from cuequivariance_ops.gated_gemm_triton import (
    Precision,
    fused_sigmoid_gated_dual_gemm_backward_pregemm_kernel,
    fused_sigmoid_gated_dual_gemm_forward_kernel,
)
from cuequivariance_ops.tuning_decorator import autotune_aot
...
def get_precision_mode():
    if torch.get_float32_matmul_precision() == "highest":
        return Precision.TF32x3
    elif torch.get_float32_matmul_precision() == "high":
...
def fused_sigmoid_gated_dual_gemm(
    ...
    Args:
        ...
        precision (Precision, optional): Precision mode for matrix multiplication.
            Can be DEFAULT, TF32, TF32x3, or IEEE. If None, automatically determined
            based on input dtype. Defaults to None.
    ...
    """
    if precision is None and x.dtype == torch.float32:
...
def fused_sigmoid_gated_dual_gemm_dual_x(
    ...
    """
    if precision is None and x1.dtype == torch.float32:
        precision = get_precision_mode()
    elif precision is None:
        precision = Precision.DEFAULT
