...
class Messages():
    ...
    @staticmethod
    def check_new_message(duration:str=None,wechat_path:str=None,close_wechat:bool=True)->str:
        '''
    ...
    def save_files(friend:str,folder_path:str=os.getcwd(),number:int=10,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->list[str]:
        ...
        def get_filename_and_date(ListItem):
            ...
            if re.match(r'\d+/\d+/\d+',texts[-2].window_text()):#先前的文件的时间戳是:2024/04/22这样的格式
                ...
                return filename,timestamp
        if folder_path and not Systemsettings.is_dirctory(folder_path):
            raise NotFolderError(f'所选路径不是文件夹!无法保存聊天记录,请重新选择!')
        #打开好友的对话框,返回值为编辑消息框和主界面
        chat_history_window,main_window=Tools.open_chat_history(friend=friend,TabItem='文件',wechat_path=wechat_path,is_maximize=is_maximize,search_pages=search_pages)
...
class AutoReply():
    ...
    @staticmethod
    def auto_reply_to_friend(friend:str,duration:str,content:str,save_chat_history:bool=False,capture_screen:bool=False,folder_path:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->None:
        '''
    ...
    def auto_reply_to_group(group_name:str,duration:str,content:str,at_only:bool=True,maxReply:int=3,at_others:bool=True,save_chat_history:bool=False,capture_screen:bool=False,folder_path:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->None:
        '''
...
def listen_on_chat(friend:str,duration:str,save_file:bool=True,save_photo:bool=True,save_method:int=0,file_folder:str=None,photo_folder:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->tuple[list[str],list[str],list[str]]:
    '''
    ...
    Examples:
        ...
        contents,senders,types=listen_on_chat(friend='测试群',duration='10min',file_folder=r"E:\Desktop\保存文件",photo_folder=r"E:\Desktop\保存图片")
        ```
    Returns:
    ...
    if not duration:#不按照指定的时间格式输入,需要提前中断退出
        raise TimeNotCorrectError
    if not file_folder and save_file:#filefolder没有输入默认为None且save_file为True那么就保存到本地
        file_folder=os.getcwd()
    if not photo_folder and save_photo:#photo_folder没有输入默认为None且save_meida为True那么就保存到本地
        photo_folder=os.getcwd()
    if file_folder and  not Systemsettings.is_dirctory(file_folder):#输入了file_folder但不是文件夹,报错
        raise NotFolderError
    if photo_folder and  not Systemsettings.is_dirctory(photo_folder):#输入了photo_folder但不是文件夹,报错
        raise NotFolderError
    ...
    if save_photo and photo_num:
        save_photos(friend=friend,number=photo_num,folder_path=photo_folder,save_method=save_method,is_maximize=is_maximize,search_pages=search_pages,close_wechat=close_wechat,wechat_path=wechat_path)
...
def save_files(friend:str,folder_path:str=os.getcwd(),number:int=10,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->list[str]:
    '''
    该函数用来保存与某个好友或群聊的聊天文件到指定文件夹中,如果有重复的文件,也会一并全部保存\n
    Args:
        friend:\t好友或群聊备注\n
        number:\t需要保存的文件数量,默认为10,如果聊天记录中没有那么多文件,则会保存所有的文件\n
...
def auto_reply_to_group(group_name:str,duration:str,content:str,at_only:bool=True,maxReplynum:int=3,at_others:bool=True,save_chat_history:bool=False,capture_screen:bool=False,folder_path:str=None,search_pages:int=5,wechat_path:str=None,is_maximize:bool=True,close_wechat:bool=True)->None:
    '''
...
def save_photos(friend:str,number:int,folder_path:str=os.getcwd(),save_method:int=0,is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None)->None:
    '''
    ...
    Examples:
        ...
        folder_path=r'E:\Desktop\新建文件夹'
        save_photos(friend='测试群',number=20,folder_path=folder_path,save_method=0)
        ```
    '''
    saved_num=0
    if save_method not in [0,1]:
    ...
    if not photoList.exists():
        chat_history_window.close()
        raise NoChatHistoryError(f'你与{friend}无任何聊天图片,无法保存!')
    ...
    if save_method==0:
        for  i in range(1,number+1):
            ...
            if video_player.exists():
                ...
                pic_path=os.path.join(folder_path,f'与{friend}的聊天图片{i}.png')
                chat_history_image.save(pic_path)
                pyautogui.press('left')
                #按下左键后可能会出现这是第一张图片的提示,那么直接退出循环
                if earliest_image.exists():
...
def save_videos(friend:str,number:int,folder_path:str=os.getcwd(),is_maximize:bool=True,close_wechat:bool=True,search_pages:int=5,wechat_path:str=None)->None:
    '''
    ...
    while saved_num<number: 
        #按下左键后可能会出现这是第一张图片的提示,那么直接退出循环
        if video_player.exists() and not video_expired.exists():
            save_as_button.click_input()
            Tools.NativeSaveFile(folder_path)
            saved_num+=1
            if earliest_image.exists():
