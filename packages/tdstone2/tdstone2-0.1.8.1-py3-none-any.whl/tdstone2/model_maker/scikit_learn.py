...
def extract_explicit_hyperparameters(steps):
    ...
    explicit_arguments = {}
    for step_name, step_instance in steps:
        # Instantiate a default object of the step's class for default parameters comparison
        default_instance = step_instance.__class__()
        # Get current parameters and default parameters
        current_params = step_instance.get_params()
        default_params = default_instance.get_params()

        # Compare and keep only explicitly set parameters (differing from defaults)
