    ...
    def _predict_cepo(
        ...
        while start_idx < X_query.shape[0]:
            ...
            res = self.icl_model.predict_cepo(
                ...
                X_context=torch.from_numpy(x_c).to(self.device).unsqueeze(0),
                # shape: (1, context_size)
                t_context=torch.from_numpy(t_c).to(self.device).unsqueeze(0),
                y_context=torch.from_numpy(y_c).to(self.device).unsqueeze(0),
                # shape: (1, query_size, num_features)
                X_query=torch.from_numpy(x_q).to(self.device).unsqueeze(0),
                # shape: (1, query_size)
                t_query=torch.from_numpy(t_q).to(self.device).unsqueeze(0),
                n_samples=n_samples,
    ...
    def _calculate_reg_ice(
        ...
        for val_idx in fold_indices:
            ...
            for b in range(b_size):
                ...
                res = self.icl_model.predict_cepo(
                    # shape: (1, context_size, num_features)
                    X_context=X_context[context_indices].unsqueeze(0),
                    # shape: (1, context_size)
                    t_context=t_context[context_indices].unsqueeze(0),
                    y_context=y_context[context_indices].unsqueeze(0),
                    # shape: (1, query_size, num_features)
...
class ATEEstimator(CausalEstimator):
    ...
    def ـestimate_ate_ra_learner(self) -> float:
    ...
    def ـestimate_ate_s_learner(self) -> float:
    ...
    def estimate_ate(self, algorithm: Literal["RA", "S"] = "S") -> float:
        ...
        if algorithm == "S":
            return self.ـestimate_ate_s_learner()
        elif algorithm == "RA":
            return self.ـestimate_ate_ra_learner()
        else:
