...
class WS2812:
    ...
    def __init__(self, pin:int, *, width:int, height:int, bright:float = 0.5, zigzag:bool = False):
        ...
        self.__np       = neopixel.NeoPixel(machine.Pin(self.__pin), self.width * self.height)

    def __map2d(self, x:int, y:int) -> int:
        """
        Map 2D coordinates (x, y) to a 1D index in the NeoPixel array.

        :param x: Horizontal coordinate (0 to width-1).
        :param y: Vertical coordinate (0 to height-1).
        :return: 1D index corresponding to the (x, y) coordinates.
        :raises IndexError: If (x, y) is out of bounds.
        """
        if not (0 <= x < self.__width and 0 <= y < self.__height):
            raise IndexError("Index out of range: (x,y) must be within the grid dimensions")
        if self.__zigzag and (y & 1):
            x = self.__width - 1 - x
        return y * self.__width + x

    def __fill(self, color:tuple[int,int,int]):
        """
        Fill the entire NeoPixel grid with a specified color.

        :param color: A tuple (r, g, b) representing the color to fill.
        :raises TypeError: If color is not a tuple of three integers.
        :raises ValueError: If any color value is not in the range 0-255.
        """
        c = tuple(round(v * self.bright) for v in color)
    ...
    def __setitem__(self, idx:int|tuple[int, int], color):
        ...
        if not isinstance(color, tuple) or len(color) != 3:
            raise TypeError("Color must be (r,g,b)")
        assert all(0<=c<=255 for c in color), "Color values must be between 0 and 255"

        self.__np[idx] = tuple(round(v * self.bright) for v in color)
    ...
    def deinit(self):
        ...
        self.off()
        del self.__np
        machine.Pin(self.__pin, machine.Pin.IN)

        
tLed = lambda bright=1.0: WS2812(pin=0, width=1, height=1, bright=bright)
...
class VL53L0X_old:
    ...
    def start_continuous(self, period_ms=0):
        ...
        :param period_ms: Period in milliseconds (default: 0 for default period)
        """
    ...
    def _set_register(self, config:tuple):
        """
        Configure the sensor with the given configuration.
        
        :param config: tuple containing register addresses and values
        """
