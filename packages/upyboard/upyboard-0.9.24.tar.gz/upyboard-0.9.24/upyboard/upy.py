...
def stdout_write_bytes(b):
    ...
    with _stdout_lock:
        if expected_bytes:
            ...
            hdr = b[0]   
            if   hdr & 0xF8 == 0xF0: expected_bytes = 3  
            elif hdr & 0xF0 == 0xE0: expected_bytes = 2  
            elif hdr & 0xE0 == 0xC0: expected_bytes = 1
            else:                       # Invalid UTF-8   
...
class UpyBoard:
    ...
    _DEIVCE_CHECK_SIZES = {
        'EFR32MG': 512,    # Conservative for slower device
        'ESP32': 1024,     # Medium speed
        'ESP32S3': 2048,   # Faster
        'ESP32C6': 2048,   # Faster
        'RP2350': 4096,    # Fastest
    }
    ...
    def __read_ex(self, min_num_bytes:int, ending:bytes, timeout:int = 5, data_consumer: Optional[Callable[[bytes], None]]=None) -> bytes:
        ...
        if not isinstance(ending, bytes) or len(ending) == 0:
            raise ValueError("ending must be non-empty bytes")
        
        if min_num_bytes < 0:
            raise ValueError("min_num_bytes must be non-negative")
        
        if data_consumer is not None and len(ending) != 1:
            raise ValueError("ending must be single byte when data_consumer is provided")
    ...
    def __enter_repl(self, soft_reset:bool=True):
        ...
        If soft_reset is True, it performs a soft reset before entering the raw REPL.
        
        :param soft_reset: If True, perform a soft reset before entering the raw REPL.
        """
        self.serial.write(b'\r' + self._CTRL_C)  # interrupt any running program
        time.sleep(0.1)
        ...
        self.serial.write(b'\r' + self._CTRL_A)  # enter raw REPL
        
        if soft_reset:
            data = self.__read_ex(1, self._RAW_REPL_PROMPT, timeout=1)
            if not data.endswith(self._RAW_REPL_PROMPT):
                print(data)
                raise UpyBoardError('could not enter raw repl')

            self.serial.write(self._CTRL_D) # ctrl-D: soft reset
            
            data = self.__read_ex(1, self._SOFT_REBOOT_MSG, timeout=1)
            if not data.endswith(self._SOFT_REBOOT_MSG):
                print(data)
                raise UpyBoardError('could not enter raw repl')

        data = self.__read_ex(1, self._RAW_REPL_PROMPT[:-1], timeout=1)
    ...
    def __follow_task(self, echo:bool):
        ...
        try:
            while True:
                ...
                if echo:
                    putch(ch)
                self.__write(CR if ch == LF else ch)
        finally:
            self._reset()
            sys.exit(0)

    def __raw_exec_old(self, command:bytes) -> bytes:
        """
        ...
        for i in range(0, len(command), self._REPL_BUFSIZE):
            self.serial.write(command[i:min(i + self._REPL_BUFSIZE, len(command))])
        self.serial.write(self._EOF_MARKER)
        
        data = self.__read_ex(1, self._OK_RESPONSE, timeout=1)
        if not data.endswith(self._OK_RESPONSE):
            raise UpyBoardError('could not command (response: %r)' % data)
    
    def __raw_exec(self, command: bytes) -> bytes:
        """
        Execute a command with adaptive buffer management based on device response.
        """
        data = self.__read_ex(1, b'>')
        if not data.endswith(b'>'):
            raise UpyBoardError('could not enter raw repl')
    ...
    def __exec(self, command:bytes, stream_output:bool=False, echo:bool=False) -> bytes:
        """
        ...
        if data_err:  # 2nd: Traceback or error message
    ...
    def run(self, *, filename:str=None, command:str=None, stream_output:bool=False, echo:bool=False):
        """
        Run a command or script on the device.
        
        :param filename: Path to the script file to execute.
        :param stream_output: If True, stream the output to stdout.
        :param echo: If True, echo the command to stdout.
        """
        out = None
        ...
        try:
            if filename:
                with open(filename, "rb") as f:
                    command = f.read()
                self.__exec(command, stream_output, echo)                                    
                self.__drain_eof(max_ms=200)
            elif command:
                    command = textwrap.dedent(command)
                    out, _ = self.__exec(command.encode('utf-8'))
            else:
                raise ValueError("Either 'filename' or 'command' must be provided.")
        finally:
            self.__leave_repl()
    ...
    def fs_get(self, remote, local, callback=None):
        """
        Download a file from the connected device using block-based transfer like fs_put.
        
        :param remote: The file to download.
        :param local: The local path to save the file. If a directory is provided, the file will be saved with the same name as the remote file.
        :param callback: Optional callback function to call with (filename, total_size) for progress display.
        :return: The contents of the file as bytes.
        """    
        file_size = self.fs_state(remote)
        if file_size == 0:
            raise UpyBoardError(f"File '{remote}' is empty.")
        ...
        if local and callback:
            th = threading.Thread(target=callback, args=(remote, file_size), daemon=True)
            th.start()

        optimal_chunk_size = self._DEIVCE_CHECK_SIZES.get(_core, 1024)
        ...
        try:
            init_command = f"""
                ...
                f = open('{remote}', 'rb')
                """
            self.__exec(textwrap.dedent(init_command).encode('utf-8'))
            ...
            while bytes_read < file_size:
                ...
                read_cmd = f"""
                    ...
                    if chunk:
                        sys.stdout.buffer.write(ubinascii.hexlify(chunk))
                        del chunk
                        gc.collect()
                    """
                out, _ = self.__exec(textwrap.dedent(read_cmd).encode('utf-8'))
                
                if out:
                    chunk_data = binascii.unhexlify(out)
                    if local_file:
                        local_file.write(chunk_data)
                    else:
                        print(chunk_data.decode('utf-8', errors='replace'), end='', flush=True)
                    bytes_read += len(chunk_data)
                else:
                    break
                            
            self.__exec(b"f.close(); del f; gc.collect()")
            if local_file:
                local_file.close()
            
            if bytes_read != file_size:
                raise UpyBoardError(f"File download incomplete: expected {file_size}, got {bytes_read} bytes")
                ...
                try:
                    ...
                    for item in os.listdir(path):
                        ...
                        try:
                            ...
                            size = stat_info[6] if not is_dir else 0
    ...
    def fs_put(self, local_data, remote, callback=None):
        """
        Upload a file to the connected device with optimized performance.
        
        :param local_data: The local data to upload.
        ...
        self.__enter_repl()
        try:
            self.__exec(f"f = open('{remote}', 'wb')".encode('utf-8'))
            
            for i in range(0, size, optimal_chunk_size):
                chunk = local_data[i:i + optimal_chunk_size]
                hex_chunk = binascii.hexlify(chunk).decode('ascii')
                
                write_cmd = f"f.write(__import__('ubinascii').unhexlify('{hex_chunk}'))"
                self.__exec(write_cmd.encode('utf-8'))
            
            self.__exec(b"f.close()")
            
        except UpyBoardError as e:
            if "EEXIST" in str(e) or "ENOENT" in str(e):
                self.__leave_repl()
                # Create directory if needed
                remote_dir = posixpath.dirname(remote)
                if remote_dir and remote_dir != '/':
                    self.fs_mkdir(remote_dir)
    ...
    def fs_format(self, core):
        """
        Format the filesystem of the connected device based on its core type.
        
        :param core: The core type of the device (e.g., "ESP32", "ESP32S3", "RP2350").
...
def cli(ctx, sport, baud, command):
    ...
    :param ctx: Click context object.
    :param sport: Serial port name for the connected device.
    :param baud: Baud rate for the serial connection (default 115200).
    :param command: Command to execute on the connected device.
    """
...
def __show_download_progress(remote_filename, total_size):
    """
    Show download progress bar with device-specific transfer rates.
    """
    if total_size == 0:
        return
    ...
    print(f"{ANSIEC.FG.BRIGHT_BLUE}{remote_filename.replace(_device_root_fs, '', 1)}{ANSIEC.OP.RESET}")
    
    # Device-specific download rates (bytes per second) - optimized values
    ...
    rate = download_rates.get(_core, 4800)

    while downloaded_size < total_size:
        elapsed_time = time.time() - start_time
        expected_size = min(int(rate * elapsed_time), total_size)
        ...
        print(f"{ANSIEC.OP.left()}[{bar}] {int(progress * 100)}%{speed_str}{eta_str}", 
              end="", flush=True)
...
def get(remote, local):
    ...
    :param progress: Show download progress bar for large files.
    """
    if not remote.startswith(_device_root_fs):
        remote = posixpath.join(_device_root_fs, remote)
    
    try:
        _upy.fs_get(remote, local, callback=__show_download_progress)
    except UpyBoardError:
...
def __get_icon_for_file(filename, is_dir):
    """
...
def ls(path):
    ...
    try:
        # Use the optimized detailed listing function
        ...
        if display_items:
            ...
            name_width = max(len(item[1]) for item in display_items)
...
def __show_waiting(remote_filename, total_size):
    """
    Show optimized progress bar with device-specific transfer rates.
    """
    if total_size == 0:
        return
    ...
    # Device-specific transfer rates (bytes per second) - optimized values
    transfer_rates = {
        'EFR32MG': 2400,
        'ESP32': 4800,  
        'ESP32S3': 9600,
    ...
    rate = transfer_rates.get(_core, 4800)
    ...
    while copied_size < total_size:
        ...
        # Enhanced progress display
        if elapsed_time > 0:
            speed = copied_size / elapsed_time
...
def put(local, remote):
    ...
    if os.path.isdir(local):
        _upy.fs_putdir(local, remote, __show_waiting)
    else:
        with open(local, "rb") as infile:        
            _upy.fs_put(infile.read(), remote, __show_waiting)
...
def run(local_file, not_waiting, input_echo):
    ...
    """
    try:
        _upy.run(filename=local_file, stream_output=not not_waiting, echo=input_echo)
    except IOError:
