...
class SummarizeDocumentTool(Tool):
    ...
    type: Literal["summarizeDocument"] = field(default="summarizeDocument", kw_only=True)
    configuration: Dict[str, Any] = field(default_factory=dict, kw_only=True)
...
class QueryKnowledgeGraphTool(Tool):
    ...
    type: Literal["queryKnowledgeGraph"] = field(default="queryKnowledgeGraph", kw_only=True)

    def to_dict(self) -> Dict[str, Any]:
        base_dict = super().to_dict()
        base_dict["configuration"] = self.configuration.to_dict()
        return base_dict
...
class QueryTimeSeriesDatapointsTool(Tool):
    ...
    """
    # Make fields with defaults keyword-only
    type: Literal["queryTimeSeriesDatapoints"] = field(default="queryTimeSeriesDatapoints", kw_only=True)
    configuration: Dict[str, Any] = field(default_factory=dict, kw_only=True)
...
class Reasoning:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Reasoning":
        ...
        return cls(
            content=[AgentContent.from_dict(c) for c in content_list]
        )
...
class AgentData:
    ...
    instances: Optional[Dict[str, Any]] = None # The structure of instances isn't fully defined in the spec
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentMessage":
        ...
        if "data" in data:
            for data_item_dict in data.get("data", []):
                data_items.append(AgentData.from_dict(data_item_dict)) # Use AgentData.from_dict
