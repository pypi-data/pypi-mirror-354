...
class APIBase:
    ...
    def _post(self, url_path: str, json_body: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        response = self._cognite_client.post(
            url, json=json_body, headers=self._default_headers
        )
    ...
    def _get(
        ...
        url = f"{self._get_base_url()}{url_path}"
        # Use the pre-constructed default headers
        response = self._cognite_client.get(
            url, params=params, headers=self._default_headers
        )
