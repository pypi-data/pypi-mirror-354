...
class AgentsAPI(APIBase):
    ...
    def create(self, agent: AgentDefinition) -> Agent:
        ...
        request = {"items": [agent_dict]}
        response = self._post("", json_body=request)
    ...
    def retrieve(
        self, external_id: str, ignore_unknown_ids: bool = False
    ) -> Optional[Agent]:
        """
        ...
        response = self._post("/byids", json_body=request)
    ...
    def delete(self, external_id: str, ignore_unknown_ids: bool = False) -> None:
        ...
        self._post("/delete", json_body=request)
