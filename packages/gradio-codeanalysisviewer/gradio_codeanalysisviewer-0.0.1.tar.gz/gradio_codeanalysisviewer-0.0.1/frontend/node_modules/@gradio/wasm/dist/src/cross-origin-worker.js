...
const worker_blob_url_cache = new Map();
function get_blob_url(url) {
...
function createWorker(url, shared, workerOptions) {
    if (shared) {
        if (typeof window.SharedWorker !== "undefined") {
            return new SharedWorker(url, workerOptions);
        }
        // e.g. Chrome for Android
        console.warn("SharedWorker is not supported in this browser. Use the regular Worker instead.");
...
export class CrossOriginWorkerMaker {
    ...
    constructor(url, options) {
        ...
        if (is_same_origin(url)) {
            console.debug(`Loading a worker script from the same origin: ${url.toString()}.`);
            ...
            console.debug(`Loading a worker script from a different origin: ${url.toString()}.`);
