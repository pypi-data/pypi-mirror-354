...
import { asgiHeadersToRecord, headersToASGI, logHttpReqRes } from "./http";
export class WorkerProxy extends EventTarget {
    worker;
    postMessageTarget;
    firstRunPromiseDelegate = new PromiseDelegate();
    constructor(options) {
        super();
        ...
        const workerMaker = new Worker(new URL("../dist/webworker/webworker.js", import.meta.url), {
        ...
        this.postMessageAsync({
            ...
            .then(() => {
            console.debug("WorkerProxy.constructor(): Environment initialization is done.");
        })
            .catch((error) => {
            console.error("WorkerProxy.constructor(): Initialization failed.", error);
        ...
        this.postMessageAsync({
            ...
            .then(() => {
            this.dispatchEvent(new CustomEvent("initialization-completed", {
                detail: null
            }));
        })
            .catch((error) => {
    ...
    _processWorkerMessage(msg) {
        switch (msg.type) {
            ...
            case "modules-auto-loaded": {
                this.dispatchEvent(new CustomEvent("modules-auto-loaded", {
            ...
            case "python-error": {
                this.dispatchEvent(new CustomEvent("python-error", {
...
function isSharedWorker(worker) {
    ...
    return (typeof window.SharedWorker !== "undefined" && worker instanceof SharedWorker);
}
function isDedicatedWorker(obj) {
    return "terminate" in obj;
}
function isMessagePort(obj) {
    return "close" in obj;
}
