...
function createLatexTokenizer(delimiters) {
    ...
    return {
        ...
        tokenizer(src, tokens) {
            for (const pattern of delimiterPatterns) {
                const match = new RegExp(`${pattern.start.source}([\\s\\S]+?)${pattern.end.source}`).exec(src);
                if (match) {
                    return {
                        type: "latex",
                        raw: match[0],
...
export function create_marked({ header_links, line_breaks, latex_delimiters }) {
