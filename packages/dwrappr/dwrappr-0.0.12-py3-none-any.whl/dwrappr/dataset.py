...
class DataSet:
    ...
    def as_df(self) -> pd.DataFrame:
        ...
        df = pd.concat([self.x_as_df, self.y_as_df], axis=1)

        # Apply the stored dtypes to the DataFrame
        df = self._update_df_dtypes(df)
        return df

    def _update_df_dtypes(self, df) -> None:
        """
    ...
    def from_dataframe(cls,
        ...
        # Combine feature columns with target columns if target columns are provided
        columns = meta.feature_names + (meta.target_names if meta.target_names else [])
        ...
        for _, row in df.iterrows():
            ...
            datapoint = DataPoint(x=np.array(x), y=np.array(y))
