...
import logging
import uuid
from typing import Optional, Dict, Any, List, Union, AsyncIterator, Callable, Awaitable
...
class LaxClient:
    ...
    async def subscribe(
        ...
        if callback:
            # Callback mode - run until stopped or error
            try:
                ...
                async for proto_msg in stream:
                    ...
                    try:
                        ...
                        if opts.manual_ack:
                            await msg.nack()
                        # Re-raise to stop subscription if needed
            ...
            # Iterator mode - return async generator
            return self._subscribe_generator(stub, request, opts)
    
    async def _subscribe_generator(self, stub, request, opts):
        """Internal async generator for subscription iterator mode."""
        try:
            # Create streaming call
            stream = stub.Subscribe(request, timeout=opts.timeout)
            
            # Process messages as generator
            async for proto_msg in stream:
                # Convert to Message object
                msg = Message.from_proto(
                    proto_msg,
