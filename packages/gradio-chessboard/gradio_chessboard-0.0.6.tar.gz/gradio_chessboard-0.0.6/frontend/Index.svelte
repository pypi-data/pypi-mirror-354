<script lang="ts">
	...
	const initBoard = () => {
		const boardContainer = document.getElementById(board_id);
		try {
			if (
				typeof Chessboard === "undefined" ||
				typeof jQuery === "undefined"
			) {
				throw new Error("Chessboard.js or jQuery not available");
			}
			...
			console.error("Chessboard init failed:", err);

			boardContainer.innerHTML = "";

			const errorMsg = document.createElement("div");
			errorMsg.className = "chessboard-error";
			errorMsg.textContent = "Oops! Failed to load the chessboard.";
			...
			boardContainer.appendChild(errorMsg);
		}
	};
