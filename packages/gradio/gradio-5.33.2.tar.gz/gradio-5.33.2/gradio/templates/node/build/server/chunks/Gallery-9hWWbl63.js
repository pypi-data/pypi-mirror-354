...
import { c as create_ssr_component, a as createEventDispatcher, v as validate_component, b as add_attribute, e as escape, f as each } from './ssr-Cwm06D-i.js';
import { j as BlockLabel, k as Image, n as Empty, o as IconButtonWrapper, q as IconButton, u as Download, F as FullscreenButton, v as ShareButton, O as Clear, G as Image$1, w as uploadToHuggingFace, a4 as Play } from './2-Ch-Upcn5.js';
import { M as ModifyUpload } from './ModifyUpload-t9RAbhbU.js';
import { V as Video } from './Video-BBWvfovT.js';
...
const Gallery = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` ${show_label ? `${validate_component(BlockLabel, "BlockLabel").$$render(
  ...
  })}` : `<div class="gallery-container"${add_attribute("this", image_container, 0)}>${selected_media && allow_preview ? `<button class="${["preview svelte-1atirkn", mode === "minimal" ? "minimal" : ""].join(" ").trim()}">${validate_component(IconButtonWrapper, "IconButtonWrapper").$$render(
    $$result,
    {
      display_top_corner: display_icon_button_wrapper_top_corner
    },
  ...
  )} <button class="media-button svelte-1atirkn" style="${"height: calc(100% - " + escape(selected_media.caption ? "80px" : "60px", true) + ")"}" aria-label="detailed view of selected image">${"image" in selected_media ? `${validate_component(Image$1, "Image").$$render(
    $$result,
    {
      "data-testid": "detailed-image",
      src: selected_media.image.url,
    ...
    {},
    {}
  )}`}</button> ${selected_media?.caption ? `<caption class="caption svelte-1atirkn">${escape(selected_media.caption)}</caption>` : ``} <div class="thumbnails scroll-hide svelte-1atirkn" data-testid="container_el" style="${"justify-content: " + escape("center", true) + ";"}"${add_attribute("this", container_element, 0)}>${each(resolved_value, (media, i) => {
    return `<button class="${[
      "thumbnail-item thumbnail-small svelte-1atirkn",
    ...
    "grid-wrap svelte-1atirkn",
    (mode === "minimal" ? "minimal" : "") + " " + (mode !== "minimal" && (!height || height == "auto") ? "fixed-height" : "") + " " + ("")
  ].join(" ").trim()}">${interactive && selected_index === null ? `${validate_component(ModifyUpload, "ModifyUpload").$$render($$result, { i18n }, {}, {})}` : ``} <div class="${["grid-container svelte-1atirkn", show_label ? "pt-6" : ""].join(" ").trim()}" style="${"--grid-cols:" + escape(columns, true) + "; --grid-rows:" + escape(rows, true) + "; --object-fit: " + escape(object_fit, true) + "; height: " + escape(height, true) + ";"}">${each(resolved_value, (entry, i) => {
    return `<button class="${[
      "thumbnail-item thumbnail-lg svelte-1atirkn",
      ...
      {},
      {}
    )}`} ${entry.caption ? `<div class="caption-label svelte-1atirkn">${escape(entry.caption)} </div>` : ``} </button>`;
  })}</div></div></div>`}`;
});

export { Gallery as default };
//# sourceMappingURL=Gallery-9hWWbl63.js.map
