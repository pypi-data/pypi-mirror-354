...
import { B as Block, z as BlockTitle } from "./2.ChNy08-U.js";
...
function create_default_slot(ctx) {
  ...
  return {
    ...
    h() {
      ...
      ctx[11] ? "YYYY-MM-DD HH:MM:SS" : "YYYY-MM-DD");
    ...
    p(ctx2, dirty) {
      ...
      2048 && input_placeholder_value !== (input_placeholder_value = /*include_time*/
      ctx2[11] ? "YYYY-MM-DD HH:MM:SS" : "YYYY-MM-DD")) {
  ...
  return {
    ...
    p(ctx2, dirty) {
      ...
      512)
        ...
        ctx2[9];
      if (dirty[0] & /*picker_ref, include_time, picker_position, selected_date, current_year, current_month, selected_hour, selected_minute, selected_second, is_pm, show_picker, disabled, calendar_button_ref, interactive, input_ref, entered_value, valid, gradio, root, show_label, info, label*/
      134216735 | dirty[1] & /*$$scope*/
      524288) {
...
function instance($$self, $$props, $$invalidate) {
  ...
  const submit_values = () => {
    ...
    if (!date_is_valid_format(entered_value, include_time))
      return;
    $$invalidate(33, old_value = $$invalidate(32, value = entered_value));
  ...
  const update_picker_from_value = () => {
    ...
    $$invalidate(20, selected_date = parsed.selected_date);
    $$invalidate(18, current_year = parsed.current_year);
    $$invalidate(19, current_month = parsed.current_month);
    $$invalidate(21, selected_hour = parsed.selected_hour);
    $$invalidate(22, selected_minute = parsed.selected_minute);
    $$invalidate(23, selected_second = parsed.selected_second);
    $$invalidate(24, is_pm = parsed.is_pm);
  };
  const calculate_picker_position = () => {
    if (calendar_button_ref) {
      const rect = calendar_button_ref.getBoundingClientRect();
      $$invalidate(17, picker_position = {
        top: rect.bottom + 4,
        left: rect.right - 280
      });
    }
  ...
  const toggle_picker = (event) => {
    if (!disabled) {
      ...
      $$invalidate(13, show_picker = !show_picker);
      if (show_picker) {
  ...
  const close_picker = () => {
    $$invalidate(13, show_picker = false);
    if (typeof window !== "undefined") {
      window.removeEventListener("click", handle_click_outside);
      window.removeEventListener("scroll", handle_scroll, true);
    }
  ...
  const handle_picker_update = (event) => {
    $$invalidate(12, entered_value = event.detail.formatted);
    submit_values();
  };
  const handle_picker_clear = () => {
    $$invalidate(12, entered_value = "");
    $$invalidate(32, value = "");
    close_picker();
    gradio.dispatch("change");
  };
  ...
  function input_binding($$value) {
    binding_callbacks[$$value ? "unshift" : "push"](() => {
      input_ref = $$value;
      $$invalidate(15, input_ref);
    });
  }
  function input_input_handler() {
    entered_value = this.value;
    $$invalidate(12, entered_value), $$invalidate(32, value), $$invalidate(33, old_value), $$invalidate(0, gradio);
  }
  ...
  $$self.$$set = ($$props2) => {
    ...
    if ("value" in $$props2)
      $$invalidate(32, value = $$props2.value);
    if ("scale" in $$props2)
  ...
  $$self.$$.update = () => {
    ...
    16) {
      $$invalidate(25, disabled = !interactive);
    }
    if ($$self.$$.dirty[0] & /*gradio*/
    1 | $$self.$$.dirty[1] & /*value, old_value*/
    6) {
      if (value !== old_value) {
        $$invalidate(33, old_value = value);
    ...
    6144) {
      $$invalidate(26, valid = date_is_valid_format(entered_value, include_time));
    }
  };
  return [
    gradio,
