...
def run_moe_ep_preproess(topk_ids: torch.Tensor, num_experts: int):
    ...
    compute_seg_indptr_triton_kernel[(num_experts,)](  # type: ignore
        reorder_topk_ids, seg_indptr, topk_ids.numel()
    )

    BLOCK_SIZE = 512
    grid = (triton.cdiv(topk_ids.numel(), BLOCK_SIZE),)
    compute_src2dst_triton_kernel[grid](  # type: ignore
        reorder_ids, src2dst, topk_ids.numel(), BLOCK_SIZE  # type: ignore
    )
...
def pre_reorder_triton_kernel(
    ...
    for idx in range(topk):
        ...
        if expert_id >= start_expert_id and expert_id <= end_expert_id:
            ...
            for start_offset in tl.range(0, hidden_size, BLOCK_SIZE):  # type: ignore
...
@triton.jit
def silu_and_mul_ops_triton_kernel(
    gateup_output,
    down_input,
    hidden_size,
    reorder_topk_ids,
...
def compute_m_range(
    ...
    m_range_end = min(tl.load(seg_indptr + idx + 1), m_range_start + BLOCK_SIZE_M)  # type: ignore
...
def grouped_gemm_triton(
    ...
    assert weight_column_major is True
    ...
    compute_m_num_tiles_indptr[(1,)](  # type: ignore
        m_num_tiles_indptr,
        seg_indptr,
        batch_size,  # type: ignore
        config["BLOCK_SIZE_M"],  # type: ignore
    )
    ...
    grouped_gemm_triton_kernel[grid](  # type: ignore
        a,
        b,
        c,
        batch_size,
        ...
        scale_a,
        scale_b,
        a.stride(0),  # type: ignore
        b.stride(0),  # type: ignore
        b.stride(1),  # type: ignore
        **config,  # type: ignore
    )
    return c
