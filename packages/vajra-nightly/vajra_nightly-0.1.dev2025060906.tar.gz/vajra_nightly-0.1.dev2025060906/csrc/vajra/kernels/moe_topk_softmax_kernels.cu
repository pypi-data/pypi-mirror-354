 ...
 * Copyright (c) 2024, The Vajra team.
...
#define WARP_SIZE 32
#define VAJRA_SHFL_XOR_SYNC_WIDTH(var, lane_mask, width) \
...
#pragma unroll
    ...
    {
        thread_max = max(thread_max, VAJRA_SHFL_XOR_SYNC_WIDTH(thread_max, mask, THREADS_PER_ROW));
    }
...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2)
    {
        row_sum += VAJRA_SHFL_XOR_SYNC_WIDTH(row_sum, mask, THREADS_PER_ROW);
    }
...
#pragma unroll
        for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2)
        {
            float other_max = VAJRA_SHFL_XOR_SYNC_WIDTH(max_val, mask, THREADS_PER_ROW);
            int other_expert = VAJRA_SHFL_XOR_SYNC_WIDTH(expert, mask, THREADS_PER_ROW);
    ...
    torch::Tensor& gating_output)               // [num_tokens, num_experts]
{
    ...
    vajra::moe::topkGatingSoftmaxKernelLauncher(
