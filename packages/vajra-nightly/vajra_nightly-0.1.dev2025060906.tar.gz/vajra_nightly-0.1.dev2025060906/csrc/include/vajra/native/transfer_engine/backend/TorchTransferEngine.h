...
class TorchTransferEngine : public BaseTransferEngine {
 ...
 private:
  Rank GetLocalPPRank(Rank local_rank,
                      const ReplicaResourceConfig& replica_config);
  Rank GetLocalTPRank(Rank local_rank,
                      const ReplicaResourceConfig& replica_config);
  std::vector<Rank> GetAllGlobalRanksForReplicaId(
      ReplicaId replica_id) noexcept;
  Rank global_rank_;
  Rank local_rank_;  // local rank within the replica
  std::vector<Rank>
