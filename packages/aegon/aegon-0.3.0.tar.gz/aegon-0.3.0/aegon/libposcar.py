...
def make_matrix(singlemoleculein, latsp):
    ...
    listx=[iatom.position[0] for iatom in mol0]
    listy=[iatom.position[1] for iatom in mol0]
    listz=[iatom.position[2] for iatom in mol0]
    ...
    diamx=abs(max(listx)-min(listx)) + latsp
    diamy=abs(max(listy)-min(listy)) + latsp
    diamz=abs(max(listz)-min(listz)) + latsp
    ...
    matrix=np.diag([diamx, diamy, diamz])
    return matrix
#------------------------------------------------------------------------------------------
def tag(poscarlist):
