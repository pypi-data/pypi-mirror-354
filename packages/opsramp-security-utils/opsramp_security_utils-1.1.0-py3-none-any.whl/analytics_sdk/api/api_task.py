...
class ApiTask:
    ...
    def start_process_on_parallel_task(self, query, latest_query_data, api_type, method):
        if query is None:
            query = QueryBuilder(self.form.tenant_id, param={})
    ...
    def fetch_api(self, type, api_client, task, method, session):
        try:
            #print(f"fetch_api data, type: {type}, method: {method}, task: {task.get_property('url','')}")
            ...
            if type == 'opsql':
                ...
                print(f"task.get_property('api_url') : {task.get_property('api_url','')}")
                return self.get_availability_data(api_client, task, session)
            else:
                if method == 'POST':
                    return self.get_post_request_results(task.get_property('api_url',''), task.get_property('data',[]), task.get_property('api_type',''))
                else:
                    return self.get_response(task.get_property('api_url',''), task.get_property('api_type',''))
        except Exception as e:
            logger.error(f"Error fetching API data, type: {type}, method: {method}, url: {task.get_property('url','')}, query: {task.get_query()}, error: {e}")
        return None
    ...
    def get_metric_data(self, api_client, task, session=None):
        ...
        if api_client:
            if task is not None and len(task) > 0:
                ...
                return {"tenant_id": task.tenant_id, "resp_type": task.get_property("resp_type",""), "api_url": task.get_property("api_url",""), "status_code": res.status_code, "data": res.json()}
        return resp
    
    def get_availability_data(self, task, session):
        res = get_response(task.get_property("api_url",""), task.get_property("api_type",""), session)
        if res and res is not None and res.json() is not None:
            return [{"tenant_id": task.tenant_id, "api_url": task.get_property("api_url", ""),
                    "availability_by_id": task.get_property("availability_by_id",""),
