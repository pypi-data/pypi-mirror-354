...
def save_results_2_txt(data, output_file_path, transpose=False):
    """
    Save results to a text file in a tabular format, with an option to transpose.
    ...
    transpose : bool, optional
        If True, the data will be saved in a transposed format (default is False).
    ...
    save_results_2_txt(data, "output/results.txt")
    """
    # Ensure the output directory exists
    os.makedirs(os.path.dirname(output_file_path), exist_ok=True)

    # Ensure all values in the data dictionary are strings
    data = {key: list(map(str, values)) for key, values in data.items()}

    with open(output_file_path, 'w') as file:
        # Standard logic: write column headers and rows
