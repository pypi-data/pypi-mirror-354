...
from idepy_next.dom import DOMEventHandler, ManipulationMode
from idepy_next.dom.classlist import ClassList
from idepy_next.dom import _dnd_state
from idepy_next.dom.propsdict import DOMPropType, PropsDict
from idepy_next.errors import JavascriptException
from idepy_next.event import EventContainer
...
class Element:
    def __init__(self, window, node_id) -> None:
        ...
        self._query_command = rf"""
            ...
            if ('{self._node_id}' === 'document') {{
                ...
                element = document.querySelector('[data-idepy-id="{self._node_id}"]');
            }}

            if (!element) {{
                throw new Error('Element with idepy-id {self._node_id} not found', {{ cause: 'ELEMENT_NOT_FOUND' }});
            }}
        """.replace('\n', '')
    ...
    def node(self) -> Dict[str, Any]:
        return self._window.evaluate_js(f"{self._query_command}; var r2 = idepy._processElements([element])[0]; r2")
    ...
    def children(self) -> List['Element']:
        children = self._window.evaluate_js(f"""
            ...
            for (var i = 0; i < children.length; i++) {{
                var nodeId = idepy._getNodeId(children[i]);
                nodeIds.push(nodeId);
            }}
    ...
    def parent(self) -> Union['Element', None]:
        node_id = self._window.evaluate_js(f"""
            ...
            parent ? idepy._getNodeId(parent) : null
        """)
    ...
    def next(self) -> Union['Element', None]:
        node_id = self._window.evaluate_js(f"""
            ...
            nextSibling ? idepy._getNodeId(nextSibling) : null
        """)
        return Element(self._window, node_id) if node_id else None
    ...
    def previous(self) -> Union['Element', None]:
        node_id = self._window.evaluate_js(f"""
            ...
            previousSibling ? idepy._getNodeId(previousSibling) : null
        """)
        return Element(self._window, node_id) if node_id else None
    ...
    def remove(self) -> None:
        ...
        self._window.run_js(f"""
            ...
            handlerIds.forEach(function(handlerId) {{
                delete idepy._eventHandlers[handler_id]
            }})
        """)
        self._event_handler_ids = {}
        self._event_handlers = defaultdict(list)
    ...
    def copy(self, target: Union[str, 'Element']=None, mode=ManipulationMode.LastChild, id: str=None) -> 'Element':
        ...
        node_id = self._window.evaluate_js(f"""
            {self._query_command};
            var target = document.querySelector('[data-idepy-id=\"{target._node_id}\"]');
            var newElement = element.cloneNode(true);
            newElement.removeAttribute('data-idepy-id');
            {id_command};

            var nodeId = idepy._getNodeId(newElement);
            idepy._insertNode(newElement, target, '{mode.value}')
            nodeId;
        """)
    ...
    def move(self, target: Union[str, 'Element'], mode=ManipulationMode.LastChild) -> 'Element':
        ...
        self._window.run_js(f"""
            {self._query_command};
            var target = document.querySelector('[data-idepy-id=\"{target._node_id}\"]');
            idepy._insertNode(element, target, '{mode.value}')
        """)
        return self
    ...
    def on(self, event: str, callback: Union[Callable, DOMEventHandler]) -> None:
        ...
        callback_func = f"window.idepy._jsApiCallback('idepyEventHandler', {{ event: e, nodeId: '{self._node_id}' }}, 'eventHandler')"
        debounced_func = (
            f'idepy._debounce(function() {{ {callback_func} }}, {debounce})'
            if debounce > 0
            else callback_func
        )
        ...
        handler_id = self._window.evaluate_js(f"""
            ...
            if (element) {{
                ...
                idepy._eventHandlers[handlerId] = function(e) {{
                    {prevent_default}
                    {stop_propagation}
                    {debounced_func};
                }}

                element.addEventListener('{event}', idepy._eventHandlers[handlerId]);
            }};
            handlerId;
        """)
    ...
    def off(self, event: str, callback: Callable) -> None:
        ...
        self._window.run_js(f"""
            {self._query_command};
            var callback = idepy._eventHandlers['{handler_id}'];
            if (element) {{
    ...
    def __generate_events(self):
        from idepy_next.dom.event import DOMEvent
