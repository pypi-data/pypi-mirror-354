...
from idepy_next.errors import WebViewException
import urllib.parse
...
def get_app_root() -> str:
    ...
    if 'pytest' in sys.modules and os.getenv('idepy_TEST'):
...
def inject_idepy(platform: str, window: Window) -> str:
    """"
    Generates and injects a global window.idepy object. The object contains exposed API functions
    as well as utility functions required by idepy. The function fires before_load event before
    ...
    def generate_js_object():
        ...
        window.events._idepyready.set()
        logger.debug('_idepyready event fired')
    ...
    logger.debug('before_load event fired. injecting idepy object')
...
def js_bridge_call(window: Window, func_name: str, param: Any, value_id: str) -> None:
    ...
    def _call():
        try:
            ...
            code = f'window.idepy._returnValues["{func_name}"]["{value_id}"] = {{value: \'{result}\'}}'
        except Exception as e:
            logger.error(traceback.format_exc())
            error = {'message': str(e), 'name': type(e).__name__, 'stack': traceback.format_exc()}
            result = json.dumps(error).replace('\\', '\\\\').replace("'", "\\'")
            code = f'window.idepy._returnValues["{func_name}"]["{value_id}"] = {{isError: true, value: \'{result}\'}}'
    ...
    if func_name == 'idepyEventHandler':
        ...
        if event['type'] == 'drop':
            ...
            for file in files:
                ...
                file['idepyFullPath'] = urllib.parse.unquote(path[0][1])
    ...
    if func_name == 'idepyAsyncCallback':
        ...
        with open(file, 'r') as f:
            ...
            if name == 'api':
                ...
                params = {
                    ...
                    'drag_selector': idepy_next.DRAG_REGION_SELECTOR,
...
def get_js_dir() -> str:
    ...
    # See https://github.com/r0x0r/idepy/issues/1565
    if '.zip' in path:
...
def sort_js_files(js_files: list[str]) -> list[str]:
    ...
    finally finish.js that fires a idepyready event.
    """
...
def android_jar_path() -> str:
    return os.path.join(os.path.dirname(os.path.realpath(__file__)), 'lib', 'idepy-android.jar')

def resolve_config(window_value, global_value):
    """解决None、False判断不准确异常"""
    return window_value if window_value is not None else global_value