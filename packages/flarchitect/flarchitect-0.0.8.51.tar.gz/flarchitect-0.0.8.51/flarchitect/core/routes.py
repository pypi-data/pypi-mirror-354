...
class RouteCreator(AttributeInitializerMixin):
    ...
    def _validate_authentication_setup(self):
        ...
        user_lookup = get_config_or_model_meta("API_USER_LOOKUP_FIELD", default=None)
        ...
        if authenticate and "api_key" in auth_method:
            ...
            if not hash_field:
                raise ValueError(
                    "If API_AUTHENTICATE_METHOD is set to 'api_key', API_CREDENTIAL_HASH_FIELD must be set to the name of the models field that holds the hashed API key.")
            if not user_lookup:
                raise ValueError(
                    "If API_AUTHENTICATE_METHOD is set to 'api_key', API_USER_LOOKUP_FIELD must be set to the name of the models field that holds the user lookup value (e.g. email, username).")
            if not check_method:
                raise ValueError(
                    "If API_AUTHENTICATE_METHOD is set to 'api_key', API_CREDENTIAL_CHECK_METHOD must be set to the name of the models method that checks the API key against the hashed value.")
