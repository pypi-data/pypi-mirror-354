...
from vicon.processing.kmer_analysis import find_best_kmers
from vicon.processing.sample_processing import process_samples
from vicon.processing.coverage_analysis import analyze_coverage
from vicon.visualization.plots import generate_plots
...
def main():
    parser = argparse.ArgumentParser(description="Run VICON pipeline")
    ...
    # Set up logging
    log_file = log_dir / f"vicon_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info("Starting VICON pipeline")
    logger.info(f"Using base path: {config['project_path']}")
    ...
    logger.info("Step 1: Dereplication using VSEARCH")
    run_vsearch(input_sample, derep_fasta, clusters_uc)

    logger.info("Step 2: Multiple sequence alignment using ViralMSA")
    run_viralmsa(
        email=email,
    ...
    # Process Samples
    logger.info("Step 3: Sample processing and coverage analysis")
    ...
    # Step 4: Find best pair of kmers
    logger.info("Step 4: Finding best pair of kmers")
    coverage_data = analyze_coverage(df_samples, config)
    best_kmers = find_best_kmers(coverage_data, config)

    # Step 5: Generate output files and plots
    logger.info("Step 5: Generating output files and plots")
    generate_plots(coverage_data, best_kmers, results_dir)
