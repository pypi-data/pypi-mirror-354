...
class RuleManager:
    ...
    def set_dynamic_context_values(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            context_df = self.get_dataframe(result)
    ...
    def set_custom_template(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        engine_type = rule.get('ENGINE_TYPE', '').lower()
        custom_template_path = rule.get('SQL_TEMPLATE_PATH', '')
        ...
        try:
            ...
            # Execute the SQL CONTENT, not the file path
            if engine_type == 'databricks':
                result = execute_run_databricks(template_sql_content, True)
            elif engine_type == 'postgres':
                db_connection = DbConnections()
                engine = db_connection.postgres_engine()
                result = execute_run_postgres(template_sql_content, engine, False)
            else:
                raise ValueError(f"Unsupported ENGINE_TYPE in rule: '{engine_type}'")

            print(f"DEBUG: SQL execution result: {result}")

            rule['CUSTOM_TEMPLATE_RESULT'] = self.format_sql_result(result)
    ...
    def extract_snippet_blocks(self, sql_text: str) -> Dict[str, str]:
        ...
        """
        pattern = r"#START <(?P<tag>[^>]+)>\s*(?P<code>.*?)#END <\1>"
        #pattern = r"#START\s+<(?P<tag>[^>]+)>\s*(?P<code>.*?)#END\s+<\1>"
        matches = re.finditer(pattern, sql_text, re.DOTALL | re.IGNORECASE)
        return {match.group("tag").strip(): match.group("code").strip() for match in matches}
    ...
    def inject_snippets_into_template(self, rule: Dict[str, Any], template_sql: str) -> str:
        ...
        for tag, code in snippet_blocks.items():
    ...
    def set_snippet_variables(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
        For each entry in rule["CODE_SNIPPETS"]:
          1. Load the file contents via get_file_text(...)
          2. extract all tagged blocks (using extract_snippet_blocks)
          3. replace all <PLACEHOLDER> in those blocks with rule values
          4. store the final blocks under rule["SNIPPET_SQL"]
        """
        code_snippets = rule.get("CODE_SNIPPETS", [])
        ...
        # Sort by 'order' so we process in declared order
        for snippet in sorted(code_snippets, key=lambda x: x.get('order', 0)):
            raw_path = snippet.get("path", "").lstrip(os.sep)  # remove leading slash if any
            try:
                raw_sql = self.get_file_text(raw_path, is_local=True)
            except FileNotFoundError as e:
                self.logger.error(f"Snippet file not found: {raw_path}. Error: {str(e)}")
                continue
            except Exception as e:
                self.logger.error(f"Error reading snippet file: {raw_path}. Error: {str(e)}")
                continue

            # Extract tagged blocks from this snippet
            blocks = self.extract_snippet_blocks(raw_sql)
            for tag, code_block in blocks.items():
                # Replace any <PLACEHOLDER> inside code_block
                rendered_block = self.replace_common_variables(code_block, rule)
                all_blocks[tag] = rendered_block
                self.logger.info(f"Loaded snippet block '{tag}' from {raw_path}")

        # Attach all snippet blocks to rule
        
        rule['SNIPPET_SQL'] = all_blocks
        return rule
    
    def execute_snippet_sql_blocks(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
        Executes each SQL block in rule["SNIPPET_SQL"].
        Stores results (or errors) in rule["SNIPPET_RESULTS"].
        """
        engine_type = rule.get("ENGINE_TYPE", "").lower()
        snippet_sql = rule.get("SNIPPET_SQL", {})
        if not snippet_sql:
            self.logger.info("No SNIPPET_SQL blocks to execute.")
            return rule

        results = {}
        rule["SNIPPET_RESULTS"] = {} 

        if engine_type == "databricks":
            self.logger.info("Executing snippets on Databricks...")
            for tag, sql in snippet_sql.items():
                try:
                    self.logger.debug(f"Running Databricks snippet '{tag}':\n{sql}")
                    result = execute_run_databricks(sql, True)  # Assuming True = fetch result
                    formatted = self.format_sql_result(result)
                    rule["SNIPPET_RESULTS"][tag] = formatted
                except Exception as e:
                    self.logger.error(f"Databricks execution failed for '{tag}': {str(e)}")
                    results[tag] = {"error": str(e)}

        else:  # Assume Postgres
            self.logger.info("Executing snippets on Postgres...")
            try:
                db_conn = DbConnections()
                engine = db_conn.postgres_engine()

                for tag, sql in snippet_sql.items():
                    try:
                        self.logger.debug(f"Running Postgres snippet '{tag}':\n{sql}")
                        result = execute_run_postgres(sql, engine, True)  
                        formatted = self.format_sql_result(result)
                        rule["SNIPPET_RESULTS"][tag] = formatted
                    except Exception as e:
                        self.logger.error(f"Postgres execution failed for '{tag}': {str(e)}")
                        results[tag] = {"error": str(e)}

            except Exception as e:
                self.logger.error(f"Failed to connect to Postgres: {str(e)}")

        #rule["SNIPPET_RESULTS"] = results
        #rule['SNIPPET_RESULTS'] = results

        for tag, result in results.items():
            try:
                if isinstance(result, list):
                    rule['SNIPPET_RESULTS'][tag] = self.format_sql_result(result)
                else:
                    rule['SNIPPET_RESULTS'][tag] = {"info": "No result (DDL/DML executed successfully)"}
            except Exception as e:
                rule['SNIPPET_RESULTS'][tag] = {"error": str(e)}
    ...
    def exec_custom_template_check_final_queries(self, rule):
        ...
        if engine_type == 'databricks':
            ...
            for tag, sql in rule.get("SNIPPET_SQL", {}).items():
                print(f"Executing snippet '{tag}' on Databricks...")
                execute_run_databricks(sql, False)
            ...
            try:
                execute_run_postgres(rule["CUSTOM_TEMPLATE_SQL"], engine, False)

                for tag, sql in rule.get("SNIPPET_SQL", {}).items():
                    print(f"Executing snippet '{tag}' on Postgres...")
                    execute_run_postgres(sql, engine, True)

            except Exception as e:
                print(f"Error executing CUSTOM_TEMPLATE_SQL: {str(e)}")
    ...
    def business_rule_check(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if self.template.get("RULE_CATEGORY_NAME") == 'BUSINESS_RULE_CHECK':
            for idx, rule in enumerate(self.rules.get("RULES", [])):
                ...
                if rule.get('STOP_ON_FAIL_STATUS'):  # for checking as per user input
            ...
            for idx, rule in enumerate(self.rules.get("RULES", [])):
                ...
                rule = self.set_system_variables(rule)
                rule = self.set_dynamic_context_values(rule)
                # rule = self.execute_snippet_sql_blocks(rule)
