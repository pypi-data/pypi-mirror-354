...
def main():
    ...
    print("nsys2prv v0.6.0-dev2506111")
    ...
    if context_info["deviceId"].unique().size == 1 and context_info.shape[0] > 1:
        ...
        if deviceId_matching_compat:
            ...
            print(f"\033[93m This will cause problems if translating GPU metrics. A solution is provided when using Nsight Systems 2025.1 or greater as backend for exporting. \033[00m")
        print(f"\033[93m However, if this is not intended, we recommend making sure that the GPU bindings are correctly done and that every process identifies its own GPU with a unique device [0 .. N-1].\n  \033[00m")
    ...
    streams = pd.merge(streams, context_info.reset_index().set_index(['hostname', 'processId'])[['gpuId']], right_index=True, left_on=['hostname', 'Pid'], how="left", sort=False)
