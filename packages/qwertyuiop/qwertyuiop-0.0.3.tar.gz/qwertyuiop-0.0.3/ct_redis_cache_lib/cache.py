...
class CtRedis:
    ...
    def cache(self, template_cache_key: str, field: str, expire_time: Optional[int] = None,
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    self.__check_and_add_connection_to_pipe()
    ...
    def update(self, template_cache_key: str, field: str, value: dict, is_global: Optional[bool] = False,
               expire_time: Optional[int] = None):
        """
        Update a cache entry by template key and field, without modifying expiry.
        Args:
            template_cache_key (str): Cache key template.
            ...
            *args: Arguments to generate the cache key and field.
        """
        ...
        try:
            self.__check_and_add_connection_to_pipe()
            cache_key = self.__add_env_to_the_key_if_needed(template_cache_key, is_global)
            hash_value_from_cache = g.redis_conn.hget(cache_key, field)
            g.pipe.hset(cache_key, field, json.dumps(value))
            if not hash_value_from_cache:
                # Add expiry time if key is not present in Redis
                g.pipe.expire(cache_key, expire_time if expire_time is not None else self.expire_time)

        except Exception as error:
            self.logger.info(f"Failed to update cache: {error}")

    def delete(self, template_cache_key: str, field: str, is_global: Optional[bool] = False):
        """
    ...
    def __check_and_add_connection_to_pipe(self):
