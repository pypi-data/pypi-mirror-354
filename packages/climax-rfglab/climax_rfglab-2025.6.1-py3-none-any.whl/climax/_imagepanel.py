...
from climax._renderer import HalfcellRenderer
...
class ImagePanel(Static, can_focus=True):    
    ...
    def set_pixels(self, theimage: np.ndarray, vmin: Optional[int] = 0, vmax: Optional[int] = 255, zoom_factor: Optional[float] = 1.):
        ...
        self.renderer = HalfcellRenderer(self.map_image(self.image, self.cmap, vmin, vmax))
    ...
    @classmethod
    def map_image(cls, theimage: np.ndarray, cmap: Optional[mcolors.LinearSegmentedColormap] = plt.get_cmap('gray'), vmin: Optional[int] = 0, vmax: Optional[int] = 255):
        """
        ...
        """
        mapped_image = cmap(cls.stretch(np.clip(theimage, vmin, vmax)).astype(np.uint8))
    ...
    @classmethod
    def stretch(cls, image_array: np.ndarray, minimum: int = 0, maximum: int = 255) \
            -> np.ndarray:
        """
