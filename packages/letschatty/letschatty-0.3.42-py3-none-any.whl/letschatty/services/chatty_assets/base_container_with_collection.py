...
class ChattyAssetContainerWithCollection(ChattyAssetBaseContainer[T], ABC):
    ...
    """
    def __init__(self, item_type: Type[T], collection: ChattyAssetCollectionInterface[T], cache_config: CacheConfig):
        """
    ...
    def get_preview_by_id(self, id: str, company_id: StrObjectId) -> ChattyAssetPreview:
        ...
        if not self.cache_config.keep_previews_always_in_memory and self.preview_last_accessed_at_cache < datetime.now(ZoneInfo("UTC")) - timedelta(seconds=self.cache_config.cache_expiration_time_previews):
    ...
    def get_all_previews(self, company_id: Optional[StrObjectId]) -> List[Dict[str, Any]]:
        ...
        previews= [json.loads(preview.model_dump_json()) for preview in previews]
    ...
    def clear_cache(self):
        ...
        if self.cache_config.keep_previews_always_in_memory:
    ...
    def update_preview(self, item: T):
        ...
        if not self.item_type.preview_class:
            raise ValueError(f"{self.__class__.__name__} has no preview class")
        preview = self.item_type.preview_class.from_asset(item)
    ...
    def update_previews_cache(self):
        ...
        if not self.item_type.preview_class:
            raise ValueError(f"{self.__class__.__name__} has no preview class")
        projection = self.item_type.preview_class.get_projection()
        collection_items = self.collection.get_preview_docs(projection=projection)
