...
class Cylinder(ControlVolume):
    ...
    def calculate_nodes_to_face(self):
        ...
        # Order counter-clockwise based on angle
        sorted_nodes = [[], []]
        sorted_coords = [[], []]
        sorted_angles = [[], []]
        for i in range(2):
            sorted_angles[i] = sorted(angles[i])
            for a in sorted_angles[i]:
                ind = angles[i].index(a)
                sorted_nodes[i].append(nodes[i][ind])
                sorted_coords[i].append(coords[i][ind])
            nodes[i] = sorted_nodes[i]
            coords[i] = sorted_coords[i]
            self.faces.append(nodes[i])

        self.nodes_to_cell = []
        self.coord_nodes_to_cell = []
        for i in range(2):
            self.nodes_to_cell.extend(nodes[i])
...
class Face:
    def __init__(self, cell_id1, face_id1, cell_id2, face_id2, pts_id, pts, id, type, f_aper = 0, n = 0):
        ...
        self.type = type
        self.pts_id = np.array(pts_id, dtype=np.int32)
        self.pts = pts
