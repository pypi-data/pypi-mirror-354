...
async def process_counter_metric(metric: MetricData, payload: MetricsPayload, metric_key: str):
    ...
    # Calculate average per-second rate if we have enough data points
    if len(counter_history[metric_key]) >= 2:
        data_points = counter_history[metric_key]
        
        # Get the oldest and newest data points for the time window
        oldest_point = min(data_points, key=lambda x: x["timestamp"])
        newest_point = max(data_points, key=lambda x: x["timestamp"])
        
        time_diff = (newest_point["timestamp"] - oldest_point["timestamp"]).total_seconds()
        
        if time_diff > 0:
            # Calculate total value difference over the time period
            value_diff = newest_point["value"] - oldest_point["value"]
            
            # Calculate average rate per second
            avg_rate_per_second = max(0, value_diff / time_diff)  # Ensure non-negative
            ...
            await backend.record(
                ...
                value=avg_rate_per_second,
                service=payload.service_name,
                pod=payload.pod_name,
                namespace=payload.pod_namespace
            )
            
            logger.debug(f"[metrix-server]: Calculated 60s average rate {per_second_name} = "
                        f"{avg_rate_per_second:.2f} (diff: {value_diff}, time: {time_diff}s, "
                        f"data_points: {len(data_points)})")  # fmt: skip
        else:
            logger.debug(f"[metrix-server]: No time difference for {metric_key}, skipping rate calculation")
    else:
        logger.debug(f"[metrix-server]: Not enough data points for {metric_key} "
                    f"({len(counter_history[metric_key])} points), skipping rate calculation")  # fmt: skip
