...
# Format: {metric_key: {"value": last_value, "timestamp": last_timestamp}}
counter_history: Dict[str, Dict[str, Any]] = {}
...
async def process_counter_metric(metric: MetricData, payload: MetricsPayload, metric_key: str):
    """
    Process counter metrics by calculating per-second rate.
    """
    ...
    if metric_key in counter_history:
        # Calculate rate based on previous value
        prev_data = counter_history[metric_key]
        ...
        if time_diff > 0:
            ...
            rate_per_second = max(0, value_diff / time_diff)  # Ensure non-negative
            
            # Store the rate with _per_second suffix
            per_second_name = f"{metric.name}_per_second"
            await backend.record(
                name=per_second_name,
                value=int(rate_per_second),
                service=payload.service_name,
                pod=payload.pod_name,
                namespace=payload.pod_namespace
            )
            
            logger.debug(f"[metrix-server]: Calculated rate {per_second_name} = "
                        f"{rate_per_second:.2f} (diff: {value_diff}, time: {time_diff}s)")  # fmt: skip
    ...
    logger.debug(f"[metrix-server]: Updated counter history for {metric_key}")
