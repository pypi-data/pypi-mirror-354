...
class external(nn.Module):
    """
    We use RNN module to calculate the external disturbance
    It can switch to any type of RNN (GRU, LSTM) or add look back window for MLP to consider solar radiation
    ...
    def __init__(self, input_size, hidden_size, output_size, model_type, window, num_layers=1):
        ...
        if model_type == "RNN":
            ...
            self.ext_mdl = nn.GRU(input_size=input_size, hidden_size=hidden_size,
                                           num_layers=num_layers, batch_first=True, bias=True)
        elif model_type == "LSTM":
            self.ext_mdl = nn.LSTM(input_size=input_size, hidden_size=hidden_size,
                                           num_layers=num_layers, batch_first=True, bias=True)

        self.fc1 = nn.Linear(hidden_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, output_size)
        self.solar1 = nn.Linear(1, hidden_size)
        self.solar2 = nn.Linear(hidden_size, hidden_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, x_input, hidden_state):
        ...
        solargain = self.solar1((x_input[:, :, [2]]))
        solargain = self.relu(solargain)
...
class ModNN(nn.Module):
    ...
    def forward(self, input_X):
        ...
        """
        Ext_X = input_X[:, :, [1, 2]]        # Ambient, Solar
        ...
        if self.ext_mdl == "RNN" or self.ext_mdl == "GRU":
        ...
        for i in range(window_size, self.encoLen):
            ...
            E_Zone_T =  input_X[:, i:i+1, [0]] + self.Zone(total_flux)
...
class ModNN_step(nn.Module):
    """
    We form it as a time-stepper model, in other words, we are predict the ΔTzone for each timestep
    """
    def __init__(self, args):
        super(ModNN_step, self).__init__()
        para = args["para"]
        self.encoLen = args["enLen"]
        self.device  = args['device']
        self.window  = para["window"]
    ...
    def forward(self, input_X):
        ...
        return E_Zone_T, None, (None, None, None)