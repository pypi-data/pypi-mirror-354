...
# 根据属性 name 返回该节点
def find_root_node_by_name(tree, name):
    root = tree.getroot()
    return root.find(name, namespace)
...
def get_compile_class_path(temp_path, tools_path):
    ...
    android_jar_path = os.path.join(os.getcwd(), tools_path, 'android_bak.jar')
...
def is_node_exists(parent, node_to_check, namespaces):
    """检查节点是否已存在"""
    for existing_node in parent:
        if nodes_equal(existing_node, node_to_check, namespaces):
            return True
    return False


def nodes_equal(node1, node2, namespaces):
    """比较两个节点是否相同"""
    if node1.tag != node2.tag:
        return False

    # 比较属性
    if not attributes_equal(node1.attrib, node2.attrib, namespaces):
        return False
    ...
    for child1, child2 in zip(node1, node2):
        if not nodes_equal(child1, child2, namespaces):
            return False

    return True


def attributes_equal(attrs1, attrs2, namespaces):
    """比较两个属性字典是否相同，处理android命名空间"""
    normalized_attrs1 = normalize_attributes(attrs1, namespaces)
    normalized_attrs2 = normalize_attributes(attrs2, namespaces)
    return normalized_attrs1 == normalized_attrs2


def normalize_attributes(attrs, namespaces):
    """规范化属性，处理android命名空间"""
    normalized = {}
    for k, v in attrs.items():
        if k.startswith('{') and 'android' in k:
            # 处理命名空间属性，如 {http://...}name -> android:name
            new_key = 'android:' + k.split('}')[-1]
