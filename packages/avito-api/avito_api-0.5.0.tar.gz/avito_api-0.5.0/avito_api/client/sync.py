"""
Синхронная реализация клиента API Авито.
"""
...
class SyncApiClient(BaseApiClient):
    ...
    def _request_sync(
        ...
        logger.debug(f"Выполнение {method} запроса к {full_url}{' с авторизацией' if auth_required else ''}")

        try:
            with httpx.Client(timeout=self.config.api.timeout) as client:
            ...
            raise NetworkError(f"Ошибка при выполнении запроса: {str(e)}", original_exception=e)
    ...
    async def _request_async(
        ...
        """
        Заглушка для асинхронного метода в синхронном клиенте.

        Raises:
            NotImplementedError: Всегда
        """
        raise NotImplementedError(
            "Асинхронные методы не реализованы в синхронном клиенте. "
            "Используйте AsyncApiClient для асинхронных запросов."
        )
    ...
    def get_token(self) -> dict[str, Any]:
        ...
        """
        logger.debug("Запрос токена доступа через client_credentials")
    ...
    def refresh_access_token(self) -> dict[str, Any]:
        ...
        logger.debug("Обновление токена доступа с использованием refresh_token")
