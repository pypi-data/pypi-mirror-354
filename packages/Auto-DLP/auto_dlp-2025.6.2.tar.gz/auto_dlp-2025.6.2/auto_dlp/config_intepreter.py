...
class Config:
    ...
    def from_json(cls, json):
        ...
        if config.invalid_item_names > 0:
            print(f"{config.invalid_item_names} name(s) over {config.invalid_item_name_artist_count} artist(s) contain an illegal character", file=sys.stderr)
            sys.exit(-1)
