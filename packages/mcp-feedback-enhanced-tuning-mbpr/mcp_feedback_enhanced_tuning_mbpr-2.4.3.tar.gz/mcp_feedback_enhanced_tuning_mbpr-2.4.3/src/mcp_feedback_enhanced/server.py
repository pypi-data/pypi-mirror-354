...
# ===== AI 對話檢測功能 =====
def detect_ai_conversation_end(text: str) -> bool:
    """
    檢測 AI 對話結束的標誌性語句
    ...
    for pattern in end_patterns:
        if pattern.lower() in text_lower:
            debug_log(f"檢測到 AI 對話結束標誌: '{pattern}'")
            return True

    return False


def should_auto_trigger_feedback(summary: str) -> bool:
    """
    判斷是否應該自動觸發 MCP feedback

    Args:
        summary: AI 提供的摘要文字

    Returns:
        bool: 如果應該自動觸發則返回 True
    """
    # 檢查環境變數是否啟用自動觸發
    auto_trigger = os.getenv("MCP_AUTO_TRIGGER", "true").lower() in ("true", "1", "yes", "on")
    if not auto_trigger:
        debug_log("MCP 自動觸發已停用")
        return False

    # 檢測 AI 對話結束標誌
    if detect_ai_conversation_end(summary):
        debug_log("檢測到 AI 對話結束，將自動觸發 MCP feedback")
        return True
...
async def interactive_feedback(
    ...
    timeout: Annotated[int, Field(description="等待用戶回饋的超時時間（秒）")] = 840
) -> List:
    """
    收集用戶的互動回饋，支援文字和圖片
    ...
    Args:
        ...
        timeout: 等待用戶回饋的超時時間（秒），預設為 840 秒（14 分鐘）
    ...
    # 檢測是否為 AI 對話結束並自動觸發
    if should_auto_trigger_feedback(summary):
        debug_log("自動觸發 MCP feedback 由於檢測到 AI 對話結束標誌")
