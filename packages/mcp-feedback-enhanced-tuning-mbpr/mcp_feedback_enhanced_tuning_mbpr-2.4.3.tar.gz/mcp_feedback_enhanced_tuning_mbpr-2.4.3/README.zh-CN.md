# MCP Feedback Enhanced Tuning MBPR（交互反馈 MCP 调优版）
...
**调优 MBPR：** [MBPR](https://mbpr.cc) | [GitHub](https://github.com/mbprcc) | [网站](https://mbpr.cc)
...
这是原版 MCP Feedback Enhanced 的**调优版本**，由 **MBPR** 优化，专门针对 AI 对话流程和用户体验进行了改进。

### 🔧 **调优团队**
- **调优方**: [MBPR](https://mbpr.cc) - AI 开发与优化专家
- **GitHub**: [https://github.com/mbprcc](https://github.com/mbprcc)
- **官网**: [https://mbpr.cc](https://mbpr.cc)
- **原项目**: 基于 Fábio Ferreira 的 MCP Feedback Enhanced

### ✨ **调优版新特性**
- 🤖 **AI 对话结束检测** - 自动检测 AI 询问"您希望我继续吗？"等结束语句
- ⏱️ **延长超时时间** - 超时时间增加 40%（600秒 → 840秒），提升用户体验
- 🎯 **智能自动触发** - 可配置的自动 MCP 反馈触发功能
- 🔧 **增强包管理** - 独立包名（`mcp-feedback-enhanced-tuning-mbpr`）避免冲突
- 📋 **多语言文档** - 完整的英文、简体中文、繁体中文文档

### 🎨 **MBPR 优化理念**
MBPR 专注于创建无缝的 AI-人类交互工作流程。此调优版本专门解决 AI 辅助开发中的常见痛点：
- **对话流程**: 当 AI 到达决策点时，无需手动调用 MCP
- **用户体验**: 延长的超时时间为用户提供更多思考和回应的时间
- **可靠性**: 独立包确保稳定性和兼容性
...
### 🤖 AI 对话结束检测（v2.4.0 新功能）
- **智能检测**：自动检测 AI 对话结束短语，如"您希望我继续吗？"
- **自动触发**：检测到对话结束时自动打开 MCP 反馈
- **可配置**：可通过 `MCP_AUTO_TRIGGER` 环境变量控制
- **延长超时**：默认超时时间增加 40%（从 600 秒增至 840 秒），提升用户体验
...
| `MCP_AUTO_TRIGGER` | AI 对话结束自动触发 | `true`/`false` | `true` |

### 测试选项
```bash
# 版本查询
uvx mcp-feedback-enhanced-tuning-mbpr@latest version       # 检查版本

# 指定界面测试
uvx mcp-feedback-enhanced-tuning-mbpr@latest test --gui    # 快速测试 Qt GUI
uvx mcp-feedback-enhanced-tuning-mbpr@latest test --web    # 测试 Web UI (自动持续运行)
...
# 方式三：指定界面测试
uvx --with-editable . mcp-feedback-enhanced-tuning-mbpr test --gui    # 快速测试 Qt GUI
uvx --with-editable . mcp-feedback-enhanced-tuning-mbpr test --web    # 测试 Web UI (自动持续运行)
```
...
### 最新版本亮点（v2.4.0）
- 🤖 **AI 对话结束检测**: 自动检测 AI 对话结束短语并触发 MCP 反馈
- ⏱️ **延长超时时间**: 默认超时时间增加 40%（从 600 秒增至 840 秒），提升用户体验
- 🎯 **自动触发控制**: 新增 `MCP_AUTO_TRIGGER` 环境变量控制自动触发功能
- 🔧 **增强包名**: 发布为 `mcp-feedback-enhanced-tuning-mbpr` 以避免冲突
- 📋 **改进文档**: 更新配置示例和使用指南
...
**Q: 中文字符乱码**
A: 已在 v2.0.3 修复。更新到最新版本：`uvx mcp-feedback-enhanced-tuning-mbpr@latest`
