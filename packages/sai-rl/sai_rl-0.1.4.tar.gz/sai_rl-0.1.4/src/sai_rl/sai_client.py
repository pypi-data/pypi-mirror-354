    ...
    def _create_env_factory(self):
        ...
        def env_factory(**kwargs):
            ...
            if env_type == "gymnasium":
                return gym.make(env_id, **env_vars)
            elif env_type == "gym-v26":
                return gym.make("GymV26Environment-v0", env_id=env_id, **env_vars)
            elif env_type == "gym-v21":
                return gym.make("GymV21Environment-v0", env_id=env_id, **env_vars)
            ...
            try:
                ...
                if seeds is not None:
                    ...
                    if len(unique_scores) == 1:
                        ...
                        remaining_scores = [(seed, score) for seed, score in scores.items() if seed not in used_seeds]
    ...
    def submit(
        ...
        *,
        ...
        if self._competition["opensource"]:
            raise SubmissionError("Cannot use python to submit to an open source competition. Please submit through the SAI website.")
        ...
        with self._console.status("Submitting model to the competition...") as status:
            ...
            self._print_submission_details(
                name,
                model_manager,  # use_onnx, action_function_id
            )
            ...
            if action_function:
                temp_action_fn_path = f"{config.temp_path}/{random_id}.py"
                model_manager.save_action_function(temp_action_fn_path)

            status.update("Creating submission...")

            adj_temp_model_path = f"{temp_model_path}{'/saved_model.pb' if isTensorflowV2 else ''}"
            files = {
                "model": (
            ...
            isSb3 = model_manager.model_type == "stable_baselines3"
            submission = None
            try:
                submission = self._api.submission.create(
                    {
                        "name": name,
                        "type": model_manager.model_type,
                        "competitionId": self._competition.get("id"),
                        "algorithm": model.__class__.__name__.lower() if isSb3 else None,
                        "method": "python",
                        "tag": tag
                    },
                    files=files,
