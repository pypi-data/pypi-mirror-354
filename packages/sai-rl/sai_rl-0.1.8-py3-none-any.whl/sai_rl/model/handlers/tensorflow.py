...
class TensorFlowModelHandler(BaseModelHandler):
    ...
    def _load_model_v1(self, model_path):
        ...
        return FrozenTF1Model(model_path, input_name="states:0", output_name="policy:0", tf=self._tf_v1)
    ...
    def save_model(self, model, model_path):
        ...
        if self.is_tf1_model(model):
            ...
            self._tf.saved_model.save(model, folder_name, signatures={"serving_default": concrete_fn})
            self._save_as_pbx(folder_name, model_path + ".pbx")
    ...
    def _check_compliance_v1(self, model):
        ...
        if not hasattr(model, "states"):
            ...
            states = model.states
            if not isinstance(states, self._tf_v1.Tensor) or not states.op.type.startswith("Placeholder"):
        ...
        if not hasattr(model, "get_concrete_function"):
            ...
            try:
                ...
                if not isinstance(fn, self._tf.types.experimental.ConcreteFunction):
                    errors.append("`get_concrete_function()` must return a tf.ConcreteFunction")
            except Exception as e:
    ...
    def hash(self, model):
        return {
            "architecture_hash": self._hash(self._infer_architecture(model)),
            "parameters_hash": self._hash(self._get_parameters_array(model)),
        }
