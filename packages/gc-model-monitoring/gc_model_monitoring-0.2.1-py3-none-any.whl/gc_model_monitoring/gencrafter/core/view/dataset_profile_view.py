...
from gencrafter.api.writer.writer import _Writable
from gencrafter.core.configs import SummaryConfig
from gencrafter.core.errors import DeserializationError
from gencrafter.core.proto import (
    ChunkHeader,
    ChunkMessage,
    ChunkOffsets,
    ColumnMessage,
...
from gencrafter.core.stubs import is_not_stub
import pandas as pd
from gencrafter.core.utils import (
    ensure_timezone,
...
from gencrafter.core.utils.utils import deprecated
from gencrafter.core.view.column_profile_view import ColumnProfileView

# Magic header for gencrafter using the first 4 bytes
gencrafter_MAGIC_HEADER = "WHY1"
gencrafter_MAGIC_HEADER_LEN = 4

gencrafter_MAGIC_HEADER_BYTES = gencrafter_MAGIC_HEADER.encode("utf-8")
_MODEL_PERFORMANCE = "model_performance_metrics"
_TAG_PREFIX = "gencrafter.tag."
...
class DatasetProfileView(_Writable):
    ...
    def _write_it(self, out_f: BinaryIO) -> Tuple[bool, str]:
        if self._metrics and _MODEL_PERFORMANCE in self._metrics:
            from gencrafter.migration.converters import v1_to_dataset_profile_message_v0
    ...
    def _do_write(self, out_f: BinaryIO) -> Tuple[bool, str]:
        ...
        with tempfile.TemporaryFile("w+b") as f:
            ...
            out_f.write(gencrafter_MAGIC_HEADER_BYTES)
    ...
    def _do_read(cls, f: BinaryIO) -> "DatasetProfileView":
        buf = f.read(gencrafter_MAGIC_HEADER_LEN)
        try:
            decoded_header = buf.decode("utf-8")
        except UnicodeDecodeError as e:
            raise DeserializationError("Invalid magic header. Decoder error: %s", e)

        if gencrafter_MAGIC_HEADER != decoded_header:
            raise DeserializationError(
                f"Invalid magic header. Got: {decoded_header} but expecting: {gencrafter_MAGIC_HEADER}"
            )
