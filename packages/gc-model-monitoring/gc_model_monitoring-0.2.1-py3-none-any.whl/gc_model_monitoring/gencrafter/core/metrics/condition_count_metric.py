...
from gencrafter.core.metrics.metric_components import IntegralComponent, MetricComponent
...
from gencrafter.core.relations import Relation as Rel
...
class Condition:
    ...
    throw_on_failure: bool
        If throw_on_failure is true, gencrafter will immediately raise a ValueError if
        data that does not satisfy the condition is logged.
    log_on_failure: bool
        If log_on_failure is true, gencrafter will log a warning message if data that does not
...
class ConditionCountMetric(Metric):
    """
    A gencrafter metric that counts how many column entries satisfy a condition.
