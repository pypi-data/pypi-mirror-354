...
from gencrafter.core.metrics.metrics import conf
import pandas as pd
...
def collect_column_profile_views(
    ...
    gencrafter_pandas_map_profiler_with_schema = partial(gencrafter_pandas_map_profiler, schema=schema)

    profile_bytes_df = input_df_arrays.mapInPandas(gencrafter_pandas_map_profiler_with_schema, schema=cp)  # type: ignore
...
def _collect_column_profile_views_batched(
    batch_size: int, input_df: SparkDataFrame, schema: Optional[DatasetSchema] = None # type: ignore
) -> Dict[str, ColumnProfileView]:
    if batch_size <= 0:
        logger.warning(
