...
class _FugueProfiler:
    ...
    def to_col_profiles(self, df: pd.DataFrame) -> Iterable[Dict[str, Any]]:
        res = gen.log(df[self._cols] if self._cols is not None else df, schema=self._schema)
    ...
    def profile_partition(self, df: pd.DataFrame) -> pd.DataFrame:
        res = gen.log(df[self._cols] if self._cols is not None else df, schema=self._schema).view().serialize()
