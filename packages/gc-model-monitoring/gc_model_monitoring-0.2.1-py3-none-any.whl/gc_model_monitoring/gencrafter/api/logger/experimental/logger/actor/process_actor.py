...
from gencrafter.api.logger.experimental.logger.actor.actor import (
    Actor,
    QueueConfig,
    QueueWrapper,
)
from gencrafter.api.logger.experimental.logger.actor.future_util import wait_result_while
from gencrafter.api.logger.experimental.logger.actor.pipe_signaler import PipeSignaler
from gencrafter.api.logger.experimental.logger.actor.process_rolling_logger_messages import (
    ProcessStatusMessage,
    SyncMessage,
)
from gencrafter.api.logger.experimental.logger.actor.signal_util import suspended_signals
...
class ProcessActor(Actor[Union[ProcessMessageType, ProcessStatusMessage]], mp.Process, Generic[ProcessMessageType, StatusType]):
    ...
    def __init__(
        ...
        if queue_type == QueueType.MP:
            from gencrafter.api.logger.experimental.logger.actor.mp_queue_wrapper import (
                MPQueueWrapper,
            )

            self._wrapper = MPQueueWrapper()
        elif queue_type == QueueType.FASTER_FIFO:
            from gencrafter.api.logger.experimental.logger.actor.faster_fifo_queue_wrapper import (
                FasterQueueWrapper,
            )
