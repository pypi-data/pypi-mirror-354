...
from gencrafter.api.whylabs.session.config import INIT_DOCS
from gencrafter.api.whylabs.session.session_manager import default_init
from gencrafter.api.writer.whylabs import WhyLabsWriter

try:
    import orjson  # type: ignore
except ImportError:
    from gencrafter.api.logger.experimental.logger.actor.proc_error_message import (
        proc_error_message,
    )
...
from gencrafter.api.logger.experimental.logger.actor.actor import CloseMessage, QueueConfig
from gencrafter.api.logger.experimental.logger.actor.data_logger import (
    DataLogger,
    TrackData,
)
from gencrafter.api.logger.experimental.logger.actor.future_util import wait_result_while
from gencrafter.api.logger.experimental.logger.actor.process_actor import (
    ProcessActor,
    QueueType,
)
from gencrafter.api.logger.experimental.logger.actor.process_rolling_logger_messages import (
    DataDict,
    FlushMessage,
...
from gencrafter.api.logger.experimental.logger.actor.string_util import encode_strings
from gencrafter.api.logger.experimental.logger.actor.thread_rolling_logger import (
    LoggerStatus,
    StatusMessage,
    ThreadRollingLogger,
)
from gencrafter.api.logger.experimental.logger.actor.time_util import (
    Schedule,
    TimeGranularity,
    current_time_ms,
)
from gencrafter.api.writer import Writer
...
class BaseProcessRollingLogger(
    ...
    This class mostly wraps and manages several ThreadRollingLoggers that do the real logging with gencrafter.
    ...
    Args:
        ...
        schema: The DatasetSchema to use for gencrafter under the hood.
    ...
    def process_close_message(self, messages: List[CloseMessage]) -> None:
        ...
        for datasetId, logger in self.loggers.items():
            self._logger.info(f"Closing gencrafter logger for {datasetId}")
            logger.close()
