...
import gencrafter.viz.drift.column_drift_algorithms as column_drift_algorithms
...
from gencrafter.migration.uncompound import _uncompound_dataset_profile
from gencrafter.viz.enums.enums import PageSpec, PageSpecEnum
from gencrafter.viz.utils.frequent_items_calculations import zero_padding_frequent_items
from gencrafter.viz.utils.html_template_utils import _get_compiled_template
from gencrafter.viz.utils.profile_viz_calculations import (
...
class NotebookProfileVisualizer:
    ...
    def _display_distribution_chart(
        ...
        if self._target_view:
            ...
            distribution_chart = template(
                {
                    "profile_from_gencrafter": json.dumps(target_profile_features),
                    "reference_profile_from_gencrafter": json.dumps(reference_profile_features),
                }
            )
    ...
    def _display_histogram_chart(self, feature_name: str, cell_height: Optional[str] = None) -> Optional[HTML]:
        ...
        if self._target_view:
            ...
            histogram_chart = template(
                {
                    "profile_from_gencrafter": json.dumps(target_features),
                    "reference_profile_from_gencrafter": json.dumps(ref_features),
                }
            )
    ...
    def summary_drift_report(self, height: Optional[str] = None) -> HTML:
        ...
        .. code-block:: python
            from gencrafter.viz.drift.column_drift_algorithms import Hellinger, ChiSquare
            from gencrafter.viz import NotebookProfileVisualizer
    ...
    def feature_statistics(
        ...
        for feature in feature_name:
            ...
            rendered_template = template(
                {
                    "profile_feature_statistics_from_gencrafter": json.dumps(
