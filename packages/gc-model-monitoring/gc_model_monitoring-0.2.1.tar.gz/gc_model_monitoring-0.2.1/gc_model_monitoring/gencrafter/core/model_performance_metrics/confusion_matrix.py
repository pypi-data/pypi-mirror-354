...
from gencrafter.core.metrics.metric_components import FractionalComponent, KllComponent
from gencrafter.core.metrics.metrics import DistributionMetric, MetricConfig
        ...
        if labels:
            ...
            if labels_size > MODEL_METRICS_LABEL_SIZE_WARNING_THRESHOLD:
                _logger.warning(
                    ...
                    " confusion matrix will be larger than is recommended with gencrafter current"
                    " representation of the model metric for a confusion matrix of this size."
                )
            if labels_size > MODEL_METRICS_MAX_LABELS:
                raise ValueError(
                    f"The initialized confusion matrix has {labels_size} labels and the resulting"
                    " confusion matrix will be larger than is supported by gencrafter current"
                    " representation of the model metric for a confusion matrix of this size,"
