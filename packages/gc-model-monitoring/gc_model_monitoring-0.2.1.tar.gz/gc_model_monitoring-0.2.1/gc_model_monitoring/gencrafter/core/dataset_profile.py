...
from gencrafter.core.preprocessing import ColumnProperties
from gencrafter.core.utils.utils import deprecated, deprecated_alias, ensure_timezone
    ...
    def _do_track(
        ...
        *,
        ...
        if row is not None:
            ...
            if pandas.empty:
                logger.warning("gencrafter was passed an empty pandas DataFrame so nothing to profile in this call.")
                return
            for k in pandas.keys():
                column_values = pandas.get(k)
                if column_values is None:
                    logger.error(
                        f"gencrafter was passed a pandas DataFrame with key [{k}] but DataFrame.get({k}) returned nothing!"
                    )
                    return
