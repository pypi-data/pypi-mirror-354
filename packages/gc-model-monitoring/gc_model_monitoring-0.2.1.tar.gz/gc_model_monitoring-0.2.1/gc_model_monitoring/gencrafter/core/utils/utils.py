    ...
    def deprecated_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if arg_name in kwargs:
                warnings.warn(
                    f"Argument {arg_name} is deprecated, {message if message else 'it will be removed in the next major version of gencrafter'}."
                )
            return func(*args, **kwargs)
...
def ensure_timezone(dt: datetime.datetime) -> None:
    if dt.tzinfo is None:
        raise ValueError("gencrafter requires timezone-aware datetime. Please use datetime.datetime(..., tzinfo=..) API")
