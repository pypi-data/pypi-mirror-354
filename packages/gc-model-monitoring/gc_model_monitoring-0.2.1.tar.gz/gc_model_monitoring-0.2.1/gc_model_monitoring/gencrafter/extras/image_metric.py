...
def get_pil_image_statistics(
    img: ImageType, channels: List[str] = _IMAGE_HSV_CHANNELS, image_stats: List[str] = _STATS_PROPERTIES # type: ignore
) -> Dict:
    """
...
def get_pil_exif_metadata(img: ImageType) -> Dict: # type: ignore
    """
...
def image_based_metadata(img: ImageType) -> Dict[str, int]: # type: ignore
    return {
...
def get_pil_image_metadata(img: ImageType) -> Dict: # type: ignore
    """
...
def log_image(
    images: Union[ImageType, List[ImageType], Dict[str, ImageType]], # type: ignore
