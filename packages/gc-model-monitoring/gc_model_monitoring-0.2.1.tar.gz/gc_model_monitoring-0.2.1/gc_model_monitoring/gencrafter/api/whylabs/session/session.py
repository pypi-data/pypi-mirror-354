...
from gencrafter.api.logger.result_set import ResultSet
from gencrafter.api.whylabs.session.config import INIT_DOCS, SessionConfig
from gencrafter.api.whylabs.session.lazy import Lazy
from gencrafter.api.whylabs.session.session_types import InteractiveLogger as il
from gencrafter.api.whylabs.session.session_types import NotSupported, SessionType
from gencrafter.api.whylabs.session.whylabs_client_cache import WhylabsClientCache
from gencrafter.core.view.dataset_profile_view import DatasetProfileView
from gencrafter.migration.uncompound import _uncompound_dataset_profile  # type: ignore
...
class GuestSession(Session):
    ...
    def __create_session_api(self) -> SessionsApi:
        from gencrafter.api.whylabs.session.whylabs_client_cache import ClientCacheConfig
    ...
    def _validate_config_session(self) -> Optional[str]:
        ...
        if not self._validate_session_id(session_id):
            ...
            il.warning(
                ...
                "account then authenticate with an api token using `python -m gencrafter.api.whylabs.session.why_init`."
                f"See {INIT_DOCS}"
            )
            return None
        else:
