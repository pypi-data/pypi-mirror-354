...
from gencrafter.api.logger.result_set import ResultSet, SegmentedResultSet
from gencrafter.api.pyspark.experimental.profiler import COL_NAME_FIELD, COL_PROFILE_FIELD
...
def gencrafter_pandas_segmented_profiler(
    ...
    for input_df in pdf_iterator:
        ...
        segmented_results = gen.log(input_df, schema=schema)
...
def collect_segmented_column_profile_views(
    input_df: SparkDataFrame, schema: Optional[DatasetSchema] = None # type: ignore
) -> Dict[Segment, Dict[str, ColumnProfileView]]:
    if SparkDataFrame is None:
        logger.warning("Unable to load pyspark; install pyspark to get gencrafter profiling support in spark environment.")
    vector_columns = [
    ...
    gencrafter_pandas_map_profiler_with_schema = partial(gencrafter_pandas_segmented_profiler, schema=schema)
    segmented_profile_bytes_df = input_df_arrays.mapInPandas(gencrafter_pandas_map_profiler_with_schema, schema=cp)  # type: ignore
...
def collect_segmented_results(
    input_df: SparkDataFrame, # type: ignore
