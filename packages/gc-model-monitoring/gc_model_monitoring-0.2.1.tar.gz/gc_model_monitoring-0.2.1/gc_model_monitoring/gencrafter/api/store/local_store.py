...
from gencrafter.api.store.profile_store import ProfileStore
from gencrafter.api.store.query import DatasetIdQuery, DateQuery
from gencrafter.api.writer.local import LocalWriter
...
class LocalStore(ProfileStore):
    ...
    ```python
    import gencrafter as gen
    from gencrafter.api.store.local import LocalStore

    logger = gen.logger(mode="rolling", interval=10, when="S", base_name="my_model")
    ...
    from gencrafter.api.store.date_config import DateConfig
    ...
    def get(self, query: Union[DateQuery, DatasetIdQuery]) -> Optional[DatasetProfileView]:
        ...
        profiles_list = (gen.read(file).view() for file in files_list)
