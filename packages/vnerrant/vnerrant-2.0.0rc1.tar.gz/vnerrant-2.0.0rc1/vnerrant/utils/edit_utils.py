...
def update_negative_contraction_edit(collection: EditCollection) -> EditCollection:
    """
    Update the negative contraction edit.
    Args:
        collection (EditCollection): An EditCollection object to be updated.
    Returns: An EditCollection object after updating.

    Example:
        >>> orig_text = "Ain't a cat"
        >>> corr_text = "Am not a cat"
        >>> edits = annotator.annotate_raw(orig_text, corr_text)
        >>> edits = update_negative_contraction_edit(edits)
    """
    ...
    for idx, edit in enumerate(collection.edits):
        ...
        if edit_type == "CONTR" and orig_text[orig_start_char - 1 : orig_start_char] and orig_start_char != 0:
            while orig_text[orig_start_char - 1 : orig_start_char] != " " and orig_start_char != 0:
                orig_start_char -= 1
            while cor_text[corr_start_char - 1 : corr_start_char] != " " and corr_start_char != 0:
                corr_start_char -= 1
        ...
        if idx != 0 and (collection.edits[idx - 1].original.end_char >= orig_start_char):
            if collection.edits[idx - 1].edit_type[2:] in ["CONTR", "ORTH", "MORPH"]:
                edits_[-1] = edit_
            else:
                edits_.append(edit)
        else:
