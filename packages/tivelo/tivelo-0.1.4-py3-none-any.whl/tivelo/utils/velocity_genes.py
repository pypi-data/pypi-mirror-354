...
def compute_velocity_genes(adata_raw, n_top_genes, r2_adjust=True, inplace=True, min_ratio=0.01, min_r2=0.01,
                           max_r2=0.95, perc=[5, 95]):
    ...
    scaling = np.std(Mu, axis=0) / np.std(Ms, axis=0) if np.std(Ms, axis=0).any() != 0 else 1
