...
from evtpooling.etl.transform import (ExtractError, validate_data, get_valid_date_coverage,
                                      filter_by_price, apply_curcdd_preference, handle_missing_data,
                                      clean_categorical_data, calculate_daily_loss_returns,
                                      calculate_weekly_loss_returns, pivot_data)
...
def test_valid_data(valid_df):
    ...
    df, _ = get_valid_date_coverage(
        valid_df, expected_range=(pd.Timestamp('2008-01-01'), pd.Timestamp('2008-01-16')))
...
def test_incomplete_date_coverage(incomplete_date_df):
    with pytest.raises(ExtractError,
                       match="No stocks have complete date coverage"):  # schema still valid
