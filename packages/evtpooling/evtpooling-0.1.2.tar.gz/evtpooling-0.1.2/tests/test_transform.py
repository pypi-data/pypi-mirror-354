...
from evtpooling import transform_data
from evtpooling.etl.transform import (ExtractError, validate_data, get_valid_date_coverage, filter_by_price_completeness, 
                                      apply_curcdd_preference, handle_missing_data, clean_categorical_data, 
                                      calculate_daily_loss_returns, calculate_weekly_loss_returns, pivot_data)

import pytest
import pandas as pd
import numpy as np
# Now, since your transform pipeline operates on a pandas DataFrame (not on reading CSV files directly), 
...
def valid_df():
    ...
    dates = ['2008-01-01', '2008-01-02', '2008-01-03', '2008-01-04', 
             ...
             '2008-01-11', '2008-01-14', '2008-01-15', '2008-01-16'] * 2
...
def test_valid_data(valid_df):
    ...
    df, _ = get_valid_date_coverage(valid_df, expected_range= (pd.Timestamp('2008-01-01'), pd.Timestamp('2008-01-16')))
    df, _ = filter_by_price_completeness(df)
...
def test_incomplete_date_coverage(incomplete_date_df):
    with pytest.raises(ExtractError, match="No stocks have complete date coverage"):  # schema still valid
        get_valid_date_coverage(incomplete_date_df)

def test_low_completeness(low_completeness_df):
    validate_data(low_completeness_df)
    df, dropped = filter_by_price_completeness(low_completeness_df)
