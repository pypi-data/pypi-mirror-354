...
# Define a custom exception for specific errors if certain conditions are not met defined below
class ExtractError(Exception):
    """Custom exception for errors during data extraction."""
    pass

def validate_data(df: pd.DataFrame, required_columns: List[str] = None, expected_col_types: Dict[str, Type] = None) -> None:
    """
    ...
    for col, expected_type in expected_col_types.items():
        if not np.issubdtype(df[col].dtype, expected_type):
            raise TypeError(f'Column {col} has wrong dtype: {df[col].dtype}, expected {expected_type}')
            
    print("✅ Data validation passed.")

def get_valid_date_coverage(df: pd.DataFrame, date_column: str = None, id_columns: List[str] = None, expected_range: Tuple[str, str] = None) -> Tuple[pd.DataFrame, List[Tuple]]:
    """
    ...
    expected_range : Tuple[str, str], optional
        Tuple containing start and end dates as strings. Defaults to (START_DATE, END_DATE).
    ...
    if not valid_groups:
        ...
        raise ExtractError(f'No stocks have complete date coverage between {start_expected} and {end_expected}.')
    else:
...
def filter_by_price_completeness(df: pd.DataFrame, id_columns: List[str] = None, price_columns: List[str] = None, threshold_pct: float = 80.0) -> Tuple[pd.DataFrame, List[Tuple]]:
    """
    ...
    price_columns : List[str], optional
        List of price columns to check for completeness. Defaults to [PRICE_CLOSE_DAY, PRICE_CLOSE_HIGH, PRICE_CLOSE_LOW].
    threshold_pct : float, optional
        Percentage threshold for completeness. Defaults to 80.0.
    ...
    for name, group in df.groupby(id_columns):
        ...
        for price_col in price_columns:
            ...
            if completeness < threshold_pct:
                dropped_stocks.append((name, f"Insufficient data in '{price_col}' ({completeness:.1f}% valid)"))
                sufficient_data = False
                break
...
def apply_curcdd_preference(df: pd.DataFrame, id_columns: List[str] = None, curcdd_column: str = 'curcdd', preference: str = 'EUR', price_columns: List[str] = None) -> pd.DataFrame:
    """
    Apply preference for 'curcdd' column to select the best stock data based on completeness.
    ...
    -------
    pd.DataFrame
        DataFrame with the best stock data based on completeness and currency preference.
    """
    if id_columns is None:
        id_columns = IDENTIFIER_COLUMNS
    if price_columns is None:
    ...
    for name, group in df.groupby(id_columns):
        ...
        completeness_records.append({'gvkey': name[0], 'curcdd': name[1], 'iid': name[2], 'completeness': avg_completeness})
    ...
    completeness_df.sort_values(by=['gvkey', 'iid', 'curcdd_preference', 'completeness'], 
                                ascending=[True, True, True, False], inplace=True)

    # Select one per (gvkey, iid)
    best_per_gvkey_iid = completeness_df.groupby(['gvkey', 'iid']).first().reset_index()

    if best_per_gvkey_iid.empty:
        print("⚠️ No valid groups remaining after curcdd preference applied.")
    else:
        print("✅ Best groups selected based on curcdd preference and completeness:", len(best_per_gvkey_iid))
...
def handle_missing_data(df: pd.DataFrame, price_list: List[str] = None, id_columns: List[str] = None) -> pd.DataFrame:
    """
    ...
    ------
    ValueError
        If any required price column is missing from the DataFrame or if any identifier column is missing.
    TypeError
    ...
    for price_col in price_list:
        ...
        if not np.issubdtype(df[price_col].dtype, np.floating):
            raise TypeError(f'Column {price_col} has wrong dtype: {df[price_col].dtype}, expected float type')
        
        df[price_col] = df.groupby(id_columns)[price_col].transform(lambda x: x.fillna(x.mean()))

    return df

def extract_clean_categories(df: pd.DataFrame, column: str, threshold_pct: float = 1.0) -> List[str]:
    """
    Extracts frequent categories from a column by removing categories below the threshold percentage.
    ...
    threshold_pct : float, optional
        Threshold percentage (0-100) below which categories are considered rare.
    ...
    frequent_categories = percentage[percentage >= threshold_pct].index.tolist()
    frequent_categories = sorted(frequent_categories)
    
    return frequent_categories

def comparing_categories(dirty_cat_list: pd.Series, clean_cat_list: List[str], threshold: int = 80) -> None:
    """
...
def clean_categorical_data(df: pd.DataFrame, cat_columns: List[str] = None) -> pd.DataFrame:
    ...
    for col in cat_columns:
        df[col] = df[col].astype(str).str.strip().str.upper().str.replace(r'[^A-Z0-9]', '', regex=True)
        clean_cat_list = extract_clean_categories(df, col, threshold_pct=1.0)
        col_series = df[col].copy() # Safer to work on a copy to avoid modifying the original data directly
        comparing_categories(col_series, pd.Series(clean_cat_list), threshold=80)
        df[col] = col_series.astype('category')

    return df
    
def calculate_daily_loss_returns(df: pd.DataFrame, returns: str = None, id_columns: List[str] = None) -> pd.DataFrame:
    """
    ...
    id_columns : List[str], optional
        List of identifier columns to group by when calculating returns. Defaults to IDENTIFIER_COLUMNS.
...
def calculate_weekly_loss_returns(df: pd.DataFrame, date_column: str = None, returns: str = None, id_columns: List[str] = None, lag_days: int = 5) -> pd.DataFrame:
    """
    ...
    df_copy['anchor_close'] = df_copy['prccd'].where(df_copy['is_anchor_day'])
    df_copy['lagged_anchor_close'] = (
        df_copy.groupby(id_columns)['anchor_close'].transform(lambda x: x.shift(lag_days))
    )
    df['weekly_loss_returns'] = -((df_copy[returns] - df_copy['lagged_anchor_close']) / df_copy['lagged_anchor_close']) * 100

    return df

def pivot_data(df: pd.DataFrame, date_column = None, id_columns = IDENTIFIER_COLUMNS, pivot_column = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    if pivot_column is None:
        pivot_column = id_columns[0]

    # Safety: Check columns exist
    required_columns = [date_column, pivot_column, 'daily_loss_returns', 'weekly_loss_returns']
    for col in required_columns:
        if col not in df.columns:
            raise ValueError(f"Missing required column: {col}")
    ...
    losses_week = df.pivot_table(values = 'weekly_loss_returns', index = date_column, columns = pivot_column)
    losses_day = df.pivot_table(values = 'daily_loss_returns', index = date_column, columns = pivot_column)

    # OPTIONAL: Sort the values in each column for better readability 
    # losses_week_dict_sorted = {col: losses_week[col].dropna().sort_values() for col in losses_week.columns}
    # losses_day_dict_sorted = {col: losses_day[col].dropna().sort_values() for col in losses_day.columns}
...
def transform_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 1️⃣ Validate schema & dtypes
    validate_data(df)
    
    # 2️⃣ Date coverage filtering
    df, dropped_dates = get_valid_date_coverage(df)
    print(f"📅 Dropped {len(dropped_dates)} stocks due to date coverage issues: {dropped_dates}")
    
    # 3️⃣ Price completeness filtering
    df, dropped_completeness = filter_by_price_completeness(df)
    print(f"💰 Dropped {len(dropped_completeness)} stocks due to price completeness issues: {dropped_completeness}")
    
    # 4️⃣ Apply curcdd preference
    df = apply_curcdd_preference(df)
    
    # 5️⃣ Handle missing data filling
    df = handle_missing_data(df)
    
    # 6️⃣ Clean categorical fields
    df = clean_categorical_data(df)
    
    # 7️⃣ Calculate daily returns
    df = calculate_daily_loss_returns(df)
    
    # 8️⃣ Calculate weekly returns
    df = calculate_weekly_loss_returns(df)
    
    # 9️⃣ Pivot final outputs
    losses_week, losses_day = pivot_data(df)
