...
try:
    ...
    print("Warning: Could not robustly add project directories to sys.path. Imports might fail if not run from project root or with PYTHONPATH set.")

# Mock boto3 to prevent import errors since we're mocking resolve_dataset anyway
sys.modules['boto3'] = MagicMock()

# Use absolute imports from the project root
from qcog_exp.intech.pytorch_models.model import train, TrainContext  # type: ignore
...
class TestModelTrainingRuns:
    ...
    def _create_mock_dataset(self) -> pd.DataFrame:
        ...
        feature_data = {
            ...
            'scaled_demedian_forward_return_22d': np.random.randn(n_samples),  # Target variable
        }
    ...
    def _run_training_test(self, hyperparameters_dict: dict):
        ...
        with patch('qcog_exp.intech.pytorch_models.model.resolve_dataset') as mock_resolve:
            ...
            mock_resolve.assert_called_once_with(
                "mock://test_dataset.csv",
                context["dataset_bucket"],
                context["dataset_format"]
            )
        ...
        assert isinstance(results["metrics"]["test_loss"], float), "test_loss is not a float"
        print(f"Successfully ran training for {hyperparameters_dict['hsm_model']}. Test loss: {results['metrics']['test_loss']:.4f}")
