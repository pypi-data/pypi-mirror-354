    ...
    *,
    ...
    if checkpoint:
        ...
        if not model_state_dict:
            raise ValueError("Model state dictionary must be provided when saving a checkpoint")
        ...
        if not optimizer_state_dict:
            raise ValueError("Optimizer state dictionary must be provided when saving a checkpoint")
...
def predict(
    ...
    *,
    ...
    model = WeightedLayer(
        hsm_layer=PytorchGeneralHSM(
            input_operator_count=hyperparameters["hilbert_space_dims"],  # Wrong but we are gonna live with it
            output_operator_count=1,
