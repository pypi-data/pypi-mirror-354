...
def install_or_upgrade_package(pkg):
    try:
        # Kiểm tra xem đã cài chưa
        importlib.import_module(pkg)
        # Nếu đã cài, nâng cấp lên bản mới nhất
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", pkg])
    except ImportError:
        # Nếu chưa cài, tiến hành cài đặt
        subprocess.check_call([sys.executable, "-m", "pip", "install", pkg])

# Danh sách các thư viện cần
required_packages = ["groq", "requests","pydantic","pydantic-core"]
