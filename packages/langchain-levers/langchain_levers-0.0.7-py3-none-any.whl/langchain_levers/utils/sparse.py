...
class BM25SparseEmbedding(BaseSparseEmbedding):
    ...
    def _sparse_to_dict(self, sparse_array: Any) -> Dict[int, float]:
        print(f"sparse_array: {type(sparse_array)}, {sparse_array.shape}, {sparse_array.ndim}")
        # print(f"{type(sparse_array.nonzero())}, {sparse_array.nonzero()}")
        # print(f"{type(sparse_array.data)}, {sparse_array.data}")
        ...
        if sparse_array.ndim == 1:
            sparse_array = sparse_array.reshape(1, -1)
        print(f"sparse_array: {type(sparse_array)}, {sparse_array.shape}, {sparse_array.ndim}")
        coo = sparse_array.tocoo()
        result_dict = {int(c): float(v) for c, v in zip(coo.col, coo.data)}
