"""Implementation of the slope-normalized variant of SoftAdaptX."""
...
class NormalizedSoftAdapt(SoftAdaptBase):
    ...
    def get_component_weights(
        ...
        *loss_component_values: tuple[np.ndarray | list],
        ...
        rates_of_change_array = np.array(rates_of_change)
        rates_of_change_normalized = rates_of_change_array / np.sum(rates_of_change_array)

        # Calculate the weight and return the values.
        return self._softmax(input_tensor=rates_of_change_normalized, beta=self.beta)
