...
class GmailService():
    ...
    def create_draft(self, to: str, subject: str, body: str, cc: list[str] | None = None, 
                     html_body: str | None = None, attachments: list[str] | None = None) -> dict | None:
        """
        Create a draft email message with support for HTML content and attachments.
        ...
        try:
            ...
            if html_body or attachments:
                # Use multipart message for HTML or attachments
    ...
    def create_reply(self, original_message: dict, reply_body: str, send: bool = False, cc: list[str] | None = None,
                    html_body: str | None = None, attachments: list[str] | None = None) -> dict | None:
        """
        Create a reply to an email message and either send it or save as draft with support for HTML and attachments.
        
        Args:
            original_message (dict): The original message data (as returned by get_email_by_id)
            reply_body (str): Plain text body content of the reply
            send (bool): If True, sends the reply immediately. If False, saves as draft.
            cc (list[str], optional): List of email addresses to CC
            html_body (str, optional): HTML version of the reply body
            attachments (list[str], optional): List of file paths to attach
        ...
        try:
            ...
            if html_body or attachments:
                ...
                if html_body:
                    ...
                    # Add plain text part
                    text_part = MIMEText(full_reply_body, 'plain')
                    msg_alternative.attach(text_part)
                    
                    # Create HTML version with quoted original
                    html_quoted_body = original_body.replace('\n', '<br>\n') if original_body else '[No message body]'
                    full_html_reply = (
                        f"{html_body}<br><br>"
                        f"On {original_date}, {original_from} wrote:<br>"
                        f"<blockquote>{html_quoted_body}</blockquote>"
                    )
                    html_part = MIMEText(full_html_reply, 'html')
                    msg_alternative.attach(html_part)
    ...
    def send_email(self, to: str, subject: str, body: str, cc: list[str] | None = None, 
                   html_body: str | None = None, attachments: list[str] | None = None) -> dict | None:
        """
        Send an email message immediately with support for HTML content and attachments.
