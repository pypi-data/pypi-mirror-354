...
class CreateDraftToolHandler(toolhandler.ToolHandler):
    ...
    def get_tool_description(self) -> Tool:
        return Tool(
            name=self.name,
            description="""Creates a draft email message from scratch in Gmail with support for plain text, HTML formatting, and file attachments.
...
class ReplyEmailToolHandler(toolhandler.ToolHandler):
    ...
    def get_tool_description(self) -> Tool:
        return Tool(
            name=self.name,
            description="""Creates a reply to an existing Gmail email message with support for HTML formatting and attachments, and either sends it or saves as draft.

            The tool supports:
            - Plain text replies (reply_body parameter only)
            - HTML formatted replies (both reply_body and html_body parameters)
            - File attachments (list of file paths)
            - CC recipients (use this for "reply all")
            
            Use this tool if you want to draft or send a reply to an existing email.
            """,
            inputSchema={
                "$schema": "https://json-schema.org/draft/2020-12/schema",
                "type": "object",
                "properties": {
                    ...
                    "reply_body": {
                        "type": "string",
                        "description": "The plain text body content of your reply message"
                    },
                    "send": {
                        "type": "boolean",
                    ...
                    "html_body": {
                        "type": "string",
                        "description": "Optional HTML version of the reply body. If provided, the email will be sent as multipart/alternative with both plain text and HTML versions."
                    },
                    "attachments": {
                        "type": "array",
                        "items": {
                        ...
                        "description": "Optional list of file paths to attach to the reply. Each path should be an absolute path to a file on the local filesystem."
                    }
                },
