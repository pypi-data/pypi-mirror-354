...
class NetworkProbe:
    ...
    def _tls_probe(self, host: str) -> str:
        ...
        # Get the public key object from the certificate
        pubkey = cert.public_key()
        # Serialize the public key to the standard SPKI format
    ...
    def _random_host(self):
        ...
        # Get the first hostname from the pinned list
        base_host = next(iter(self.PINNED))
        # Extract the parent domain (e.g., 'probecheck.fyi' from 'c1.probecheck.fyi')
        parent_domain = ".".join(base_host.split('.')[1:])
