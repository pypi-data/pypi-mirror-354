...
class Components:
    def __init__(self,json):
        try:
            ...
            self.plugins=json.get("plugins",None)
...
class FingerprintSwapper:
    ...
    def _apply_selenium(self,fp:Fingerprint):
        ...
        plugins=fp.components.plugins or []
        langs_raw=fp.components.languages or []
        plugins_js="["+",".join(f'"{p}"' for p in plugins)+"]"
        self.driver.header_overrides={"User-Agent":fp.userAgent}
        ...
        langs_flat=list(dict.fromkeys(i for sub in langs_raw for i in (sub if isinstance(sub,list) else [sub])))
        langs_js="[" + ",".join(f'"{lang}"' for lang in langs_flat) + "]"
        ...
        Object.defineProperties(navigator,{{
            ...
            deviceMemory:{{get:()=>{fp.components.deviceMemory or 2}}}
        }});
    ...
    def _apply_playwright(self,fp:Fingerprint):
        ...
        context_args={
            ...
            "device_scale_factor":fp.components.deviceMemory or 2,
