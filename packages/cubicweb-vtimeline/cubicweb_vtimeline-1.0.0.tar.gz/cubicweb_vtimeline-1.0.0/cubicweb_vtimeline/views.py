...
class VeriteCoTimelineJsonDataView(EntityView):
    ...
    def call(self):
        ...
        for entity in self.cw_rset.entities():
            ...
            if calendarable.start or calendarable.stop:
                date = {
...
class VTimelineItemView(EntityView):
    ...
    def entity_call(self, entity):
        self.w(f"<p>{entity.dc_description()}</p>")
...
class VeriteCoTimelineView(EntityView):
    ...
    default_settings = {
        # could be an expression such as '$("#pageContent").width()'
        "width": "100%",
        "height": "400",
        "start_at_slide": 0,
    }
    ...
    def call(self, custom_settings={}):
        ...
        self.w(f"<div id=\"{settings['embed_id']}\"></div>")
    ...
    def js_story_factory(self, settings):
        return f"createStoryJS({json_dumps(settings)})"

    def build_settings(self, custom_settings):
        divid = make_uid("t")
        json_url = self._cw.build_url(
            "view", rql=self.cw_rset.printable_rql(), vid="vtimeline.json"
        )
        return mergedicts(
            {
                "lang": self._cw.lang,
