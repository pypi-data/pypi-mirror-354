...
inline double ODE_Solver_error_check(double z, double rtol, double atol, double y,
                                     double Dy, int mflag, std::string mess="")
{
    Dy=fabs(Dy);
    ...
    {
        if(mflag==1) {
            cout << " Time-stepper: absolute error control for ( ";
            cout << mess.c_str()
                 << " ) at z= " << z << " and y= " << y << " |Dy|= "
    ...
    if(Dz_z*0.9>Dz_z_new)
    {
        ...
        if(ODE_Solver_info.n_down>=3)
        {
            ...
            else
            {
                ...
                // cout << "\n Checking to decreasing the step-size by additional factor of " << pow(2.0, ODE_Solver_info.n_down-1) << endl;
            }
        }
