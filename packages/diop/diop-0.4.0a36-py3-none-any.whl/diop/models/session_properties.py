...
class SessionProperties(models.Model):
    """Class representing properties of a given session."""
    ...
    def get_or_new(
        ...
        t_change : datetime
            The timestamp at which the session state change happened.
        state : str
            The label of the session state.
        defaults : dict
            A `defaults` dict, passed on directly to the underlying
            `get_or_create` call.
