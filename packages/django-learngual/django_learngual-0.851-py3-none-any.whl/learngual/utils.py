...
def decode_jwt_access_token(token):
    ...
    if "exp" in decoded_token:
        decoded_token["exp"] = (
            datetime.fromtimestamp(decoded_token["exp"]) - datetime.now()
        )
    if "iat" in decoded_token:
        decoded_token["iat"] = (
            datetime.fromtimestamp(decoded_token["iat"]) - datetime.now()
        )
    if "nbf" in decoded_token:
        decoded_token["nbf"] = (
            datetime.fromtimestamp(decoded_token["nbf"]) - datetime.now()
        )
    if "orig_iat" in decoded_token:
        decoded_token["orig_iat"] = (
            datetime.fromtimestamp(decoded_token["orig_iat"]) - datetime.now()
        )
