...
class OpenAIEmbedder(Embedder):
    ...
    def _embed_batch(self, texts: List[str], **kwargs) -> List[EmbeddingResult]:
        """Embed a batch of texts efficiently using OpenAI batch API"""
        start_time = time.time()
        results = []
            ...
            try:
                ...
                for i, (text, embedding_data) in enumerate(zip(uncached_texts, response.data)):
                    ...
                    result = EmbeddingResult(
                        ...
                        token_count=response.usage.total_tokens // len(uncached_texts),
                        success=True
                    )
                ...
                for i, text in enumerate(uncached_texts):
                    ...
                    error_result = EmbeddingResult(
                        ...
                        dimension=0,
                        model_name=self.config.model_name,
                        processing_time=processing_time / len(uncached_texts),
                        error=error_msg,
                        success=False
                    )
                    results.append((uncached_indices[i], error_result))
