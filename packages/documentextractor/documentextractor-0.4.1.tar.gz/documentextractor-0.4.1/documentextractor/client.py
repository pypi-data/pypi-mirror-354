...
class FilesCollection:
    ...
    def upload(
        ...
        """
        # Validate that exactly one of the file sources is provided.
        ...
        # Determine the filename that will be sent to the server.
        _filename = None
        if filename:
            _filename = filename
        elif file_path:
            ...
            # Safely get filename from stream, e.g. from an object created with open()
            _filename = file_stream.name.split('/')[-1]

        # If no filename could be determined, and it's required, raise an error.
        ...
        # Guess the MIME type from the filename, falling back to a default.
        mime_type, _ = mimetypes.guess_type(_filename)
        if mime_type is None:
            mime_type = 'application/octet-stream'

        # This inner function handles the actual request, avoiding code repetition.
        ...
        # Execute the request based on the provided input source.
        if file_path:
