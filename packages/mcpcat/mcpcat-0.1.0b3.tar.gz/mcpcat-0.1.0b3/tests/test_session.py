...
class TestGetSessionInfo:
    ...
    def test_without_mcpcat_data(self):
        ...
        assert session_info.sdk_language == f"Python {sys.version_info.major}.{sys.version_info.minor}"
        assert session_info.mcpcat_version == get_mcpcat_version()
        assert session_info.server_name == "todo-server"
        assert session_info.server_version is None  # FastMCP doesn't have version attribute
    ...
    def test_with_identified_actor(self):
        ...
        user_identity = UserIdentity(
            userId="user123",
            userName="John Doe",
            userData={"role": "admin", "department": "engineering"}
        )
        ...
        assert session_info.identify_data == {"role": "admin", "department": "engineering"}
...
class TestGetServerSessionId:
    ...
    def test_creates_new_session_when_timed_out(self):
        ...
        timeout_time = self.initial_time + timedelta(minutes=INACTIVITY_TIMEOUT_IN_MINUTES + 1)
    ...
    def test_exactly_at_timeout_boundary(self):
        ...
        boundary_time = self.initial_time + timedelta(minutes=INACTIVITY_TIMEOUT_IN_MINUTES)
    ...
    def test_timeout_calculation_edge_cases(self):
        ...
        with freeze_time(self.initial_time + timedelta(minutes=INACTIVITY_TIMEOUT_IN_MINUTES - 1)):
            session_id = get_server_session_id(self.server)
            assert session_id == self.initial_session_id
