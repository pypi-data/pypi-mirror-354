...
class TestToolContext:
    ...
    async def test_context_parameter_injection_enabled(self):
        ...
        async with create_test_client(server) as client:
            ...
            # Check each tool (except report_missing if present)
    ...
    async def test_schema_with_no_input_schema(self):
        ...
        async with create_test_client(server) as client:
            tools_result = await client.list_tools()
            simple_tool_def = next(t for t in tools_result.tools if t.name == "simple_tool")
    ...
    async def test_schema_with_no_required_fields(self):
        ...
        async with create_test_client(server) as client:
            tools_result = await client.list_tools()
            tool = next(t for t in tools_result.tools if t.name == "optional_params_tool")
    ...
    async def test_server_with_no_tools(self):
        ...
        async with create_test_client(server) as client:
            ...
            # Should have no tools (or only report_missing if enabled)
            non_report_tools = [t for t in tools_result.tools if t.name != "report_missing"]
            assert len(non_report_tools) == 0

    @pytest.mark.asyncio
    async def test_report_missing_exclusion_with_context(self):
        """Test that report_missing doesn't get context when both features are enabled."""
        server = create_todo_server()
        ...
        async with create_test_client(server) as client:
            ...
            report_missing_tool = next(t for t in tools_result.tools if t.name == "report_missing")
            ...
            other_tools = [t for t in tools_result.tools if t.name != "report_missing"]
    ...
    async def test_schema_with_validation_rules(self):
        ...
        async with create_test_client(server) as client:
            ...
            assert age_schema.get("minimum") == 0 or age_schema.get("exclusiveMinimum") == -1
            assert age_schema.get("maximum") == 150 or age_schema.get("exclusiveMaximum") == 151
    ...
    async def test_tool_with_existing_context_parameter(self):
        ...
        async with create_test_client(server) as client:
            ...
            if desc == "Describe why you are calling this tool and how it fits into your overall task":
    ...
    async def test_schema_with_allof_anyof_oneof(self):
        ...
        async with create_test_client(server) as client:
            ...
            assert "anyOf" in data_schema or "oneOf" in data_schema or data_schema.get("type") == ["string", "integer"]
    ...
    async def test_original_functionality_preserved(self):
        ...
        async with create_test_client(server) as client:
            ...
            await client.call_tool(
                "add_todo",
                {"text": "First todo", "context": "Adding first item"}
            )
            await client.call_tool(
                "add_todo",
                {"text": "Second todo", "context": "Adding second item"}
            )
            
            # List todos
            list_result = await client.call_tool(
                "list_todos",
                {"context": "Listing all todos to verify they were added"}
            )
    ...
    async def test_context_not_passed_to_original_handler(self):
        ...
        async with create_test_client(server) as client:
            ...
            result = await client.call_tool(
                ...
                {
                    "text": "test data",
                    "context": "This context should be stripped"
                }
            )
