...
from aas_thing.s3i.message.reference import I40MessageKeys, I40MessageType, I40MessageSemanticProtocols
from aas_thing.s3i.message.message import I40Message
from aas_thing.s3i.message.event.event import I40EventMessage_ #TODO
...
class S3IConnector:
    ...
    def connect(self) -> None:
        ...
        if self._idp:
            ...
            self._callbacks.process(self.idp_ready)
        ...
        if self._msg_broker:
            ...
            self._msg_broker.add_on_channel_open_callback(
                lambda *_: self._callbacks.process(self.message_broker_ready),
                one_shot=True,
                is_async=False,
            )

        # Connect the broker for exchainging events
        if self._evt_broker:
            self._evt_broker.connect(token=token)
            # Trigger the lifecycle callback after the event broker is ready
            self._callbacks.process(self.event_broker_ready)
    ...
    def _refresh_token(self) -> None:
        ...
        try:
            ...
            self._callbacks.process(self.token_refreshed)
    ...
    def _on_message(self, ch, method, props, body: bytes) -> None:
        ...
        try:
            ...
            if frame.type == I40MessageType.request:
                task = self._handle_request(frame, msg)
            elif frame.type == I40MessageType.reply:
                task = self._handle_reply(frame, msg)
            else:
    ...
    async def _handle_request(self, frame: Frame, msg: Dict[str, Any]) -> None:
        """
        ...
        i40_message_req = I40Message.from_json(msg)
    ...
    async def _handle_reply(self, frame: Frame, msg: Dict[str, Any]) -> None:
        """
        ...
        """
        self.__check_protocol_validity(frame)
        i40_message_reply = I40Message.from_json(msg)
        await self._handler.handle_reply(i40_message_reply)

    def send_request(self, request: Any) -> asyncio.Future:
        """
        ...
        self._pending[request.frame.message_id] = fut
    ...
    def _on_event(self, ch, method, props, body: bytes) -> None:
        ...
        try:
            ...
            if frame.type == I40MessageType.event:
                evt = I40EventMessage_.from_json(msg)
                asyncio.create_task(self._handler.handle_event(evt))
        except Exception as e:
