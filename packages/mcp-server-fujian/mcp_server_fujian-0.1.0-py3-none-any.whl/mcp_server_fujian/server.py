...
async def serve() -> None:
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(
                ...
                description="对附件内容进行解析，包括表格拆分和压缩。",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "zip_file_path": {
                            "type": "string",
                            "description": "附件压缩包存储的地址",
                        },
                        "output_dir": {
                            "type": "string",
                            "description": "附件经过拆分后存放的地址",
                        }
                    },
                    "required": ["zip_file_path", "output_dir"]
                },
            ),
        ]
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        try:
            ...
            if not all(k in arguments for k in ["zip_file_path", "output_dir"]):
                raise ErrorData(
                    code=INVALID_PARAMS,
                    message="缺少必要的参数：zip_file_path 或 output_dir"
                )

            result = fujian_server.process_files(arguments["zip_file_path"], arguments["output_dir"])
