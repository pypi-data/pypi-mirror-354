...
# 审计系统异常。
FAILEDOPERATION_AUDITSYSTEMERROR = 'FailedOperation.AuditSystemError'
...
# 无效的审计事件类型。
INVALIDPARAMETER_INVALIDAUDITEVENTTYPE = 'InvalidParameter.InvalidAuditEventType'

# 自动拉起参数无效。
INVALIDPARAMETER_INVALIDAUTORESUME = 'InvalidParameter.InvalidAutoResume'

# 无效的集群类型。
INVALIDPARAMETER_INVALIDCLUSTERTYPE = 'InvalidParameter.InvalidClusterType'
...
# 子用户无权创建UDF。
UNAUTHORIZEDOPERATION_CREATEUDF = 'UnauthorizedOperation.CreateUDF'
...
# 子用户无权删除UDF。
UNAUTHORIZEDOPERATION_DELETEUDF = 'UnauthorizedOperation.DeleteUDF'
...
# 子用户无权修改UDF描述信息。
UNAUTHORIZEDOPERATION_MODIFYUDF = 'UnauthorizedOperation.ModifyUDF'
