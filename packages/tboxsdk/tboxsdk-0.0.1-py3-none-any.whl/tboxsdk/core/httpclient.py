...
class HttpClientConfig(object):
    ...
    def __init__(self,
        ...
        self.host = host if host is not None else "tdoraemonopen.cloud.alipay.com"
        return
...
class HttpClient(object):
    ...
    def post(self, path, data, headers=None):
        ...
        if response.status_code != 200:
            logger.error(f"http request failed, status_code: {response.status_code}, text: {response.text}")
            raise TboxHttpResponseException(
                f"http request failed, status_code: {response.status_code}, text: {response.text}")
    ...
    def post_stream(self, path, data, headers=None, timeout=110):
        ...
        with httpx.stream(method="POST", url=url, json=data, headers=self.generate_headers(headers),
            ...
            if not response.status_code == 200:
                raise TboxHttpResponseException(response.status_code, response.reason_phrase)
