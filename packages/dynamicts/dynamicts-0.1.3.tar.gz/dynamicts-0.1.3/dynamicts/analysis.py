...
This module provides the UnivariateAnalysis class for exploratory and statistical analysis of time series data,
...
class UnivariateAnalysis:
    ...
    def __init__(self, filepath: str, target_col: str, index_col: str = "date"):
        self.target_col = target_col
        self.loader = DataLoader(filepath=filepath, index_col=index_col)
        self.df = self.loader.run_pipeline()

        if self.df is None:
            raise ValueError("Loaded data is not regular. Analysis cannot proceed.")
    ...
    @staticmethod
    def run_univariate_analysis(filepath: str, target_col: str, index_col: str = "date"):
        try:
            print(f"\nRunning Univariate Time Series Analysis on '{target_col}' from file: {filepath}")
            analysis = UnivariateAnalysis(filepath=filepath, target_col=target_col, index_col=index_col)
            results = {}
            ...
            display(Markdown("### Rolling Statistics Overview"))
            display(rolling["fig"])
            results["rolling_stats"] = rolling
...
if __name__ == "__main__":
    UnivariateAnalysis.run_univariate_analysis(
