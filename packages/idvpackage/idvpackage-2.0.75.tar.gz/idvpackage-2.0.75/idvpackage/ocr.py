...
class IdentityVerification:
    ...
    def extract_document_info(self, image, side, document_type, country, nationality, step_data=None):
        ...
        if country == 'IRQ':
            document_data = self.agent_extraction(image, country, side, step_data)
            logging.info(
                f"--------------Time taken for Front ID Extraction in IDV package: {time.time() - st} seconds\n")
            return document_data
    ...
    def agent_extraction(self, front_id, country, side, step_data):
        ...
        try:
            ...
            if side=='front' or side=='passport':
                st = time.time()
