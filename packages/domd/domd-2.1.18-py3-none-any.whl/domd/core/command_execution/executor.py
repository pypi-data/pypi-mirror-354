...
class CommandExecutor:
    ...
    def execute(
        ...
        if use_docker:
            ...
            return self._execute_in_docker(
                command=command, cwd=cwd, env=env, image=image, docker_config=docker_config
            )
        return self._execute_locally(command=command, cwd=cwd, env=env)

    def _execute_locally(self, command: str, cwd: Union[str, Path], env: Dict[str, str]) -> int:
    ...
    def _execute_in_docker(
        ...
        cwd_str = str(cwd)
        volumes = {cwd_str: {"bind": "/app", "mode": "rw"}}
        workdir = "/app"
        ...
        if docker_config:
            ...
            if 'volumes' in docker_config:
                volumes.update({
                    src: dst for src, dst in docker_config['volumes'].items()
                })
