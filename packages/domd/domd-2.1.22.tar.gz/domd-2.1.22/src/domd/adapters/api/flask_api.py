...
class DomdFlaskApi:
    ...
    def _register_routes(self) -> None:
        ...
        self.app.route("/api/commands", methods=["GET"])(self.require_auth()(self.get_commands))
        self.app.route("/api/commands/scan", methods=["POST"])(self.require_auth(["admin"])(self.scan_commands))
        self.app.route("/api/commands/test", methods=["POST"])(self.require_auth(["admin"])(self.test_commands))
        self.app.route("/api/commands/<string:command_id>", methods=["GET"])(self.require_auth()(self.get_command))
        self.app.route("/api/commands/<string:command_id>/run", methods=["POST"])(self.require_auth(["admin"])(self.run_command))

        # Reports API (protected)
        self.app.route("/api/reports", methods=["GET"])(self.require_auth()(self.get_reports))
        self.app.route("/api/reports/generate", methods=["POST"])(self.require_auth(["admin"])(self.generate_reports))

        # Stats API (protected)
        self.app.route("/api/stats", methods=["GET"])(self.require_auth()(self.get_stats))

        # Project API (protected)
        self.app.route("/api/project", methods=["GET"])(self.require_auth()(self.get_project_info))
        ...
        try:
            ...
            if search_term:
                commands = [
                    ...
                    if (search_term in cmd.command.lower() or
                         (cmd.description and search_term in cmd.description.lower()))
                ]
            ...
            return self._create_response(
                data={
                    "commands": [self._command_to_dict(cmd) for cmd in paginated_commands],
                    "pagination": {
                        "page": page,
                        "per_page": per_page,
            ...
            return self._create_response(
                data={
                    ...
                    "stats": {
                        ...
                        "failed_commands": len(failed_commands),
                        "ignored_commands": len(ignored_commands),
                        "success_rate": len(successful_commands) / len(all_commands) if all_commands else 0,
                    },
                    "disk_usage": {
