...
class CommandService:
    ...
    def should_ignore_command(self, command: Command) -> bool:
        ...
        # Check if any ignore pattern matches the command
        for pattern in self.ignore_patterns:
            # Use fnmatch for pattern matching (supports * and ? wildcards)
            if fnmatch.fnmatch(command.command, pattern):
                return True
            # Also check if the pattern matches the command type or description
            if hasattr(command, 'type') and fnmatch.fnmatch(command.type or '', pattern):
                return True
            if hasattr(command, 'description') and fnmatch.fnmatch(command.description or '', pattern):
                return True
        return False
