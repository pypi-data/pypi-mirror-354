...
DOCKER_COMMANDS = {
    'docker', 'docker-compose', 'kubectl', 'helm', 'ansible-playbook',
    'terraform', 'packer', 'vagrant', 'minikube', 'kind', 'k3s', 'k3d',
    'docker-machine', 'docker-swarm', 'docker-stack', 'docker-service'
}

# Commands that indicate a specific environment is required
ENV_SPECIFIC_COMMANDS = {
    'npm', 'yarn', 'pnpm',  # Node.js
    'python', 'python3', 'pip', 'pip3', 'poetry',  # Python
...
UNIVERSAL_COMMANDS = {
    'ls', 'cd', 'pwd', 'echo', 'cat', 'grep', 'find', 'mkdir', 'rm',
    'cp', 'mv', 'chmod', 'chown', 'touch', 'head', 'tail', 'less', 'more'
}
    ...
    def should_run_in_docker(command: str, args: List[str] = None) -> Tuple[bool, str]:
        ...
        if not shutil.which(command):
            if command in ENV_SPECIFIC_COMMANDS:
                return True, f"Command '{command}' not found locally but is environment-specific"
