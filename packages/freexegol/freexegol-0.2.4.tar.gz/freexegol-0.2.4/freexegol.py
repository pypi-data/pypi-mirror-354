...
# Downloads the Docker image 'dockerprocessus/freexegol:latest' from Docker Hub and displays the progress.
def install():
    image_name="dockerprocessus/freexegol:latest"
        ...
        for line in client.api.pull(image_name, stream=True, decode=True):
            ...
            if status:
                if progress:
                    ...
                    print(status)
        print(f"\n[+] Image '{image_name}' sucessfully downloaded !")
    except docker.errors.APIError as e:
        print(f"[!] Error while trying to download : {e.explanation}")

# Lists Docker containers and images available on the current Docker host.
def info():
    client = docker.from_env()
...
# Returns the name of the currently running WSL2 distribution used by Docker Desktop.
def get_docker_wsl_distribution():
    try:
        ...
        print(f"[!] Error while trying to retrieve WSL2 : {e}")
        return None

# Records an asciinema session from within a running Docker container and saves the session file.
...
# Displays detailed attributes and configuration of a Docker container.
...
# Stops and removes a Docker container and optionally deletes the associated local folder.
...
# Stops a running Docker container based on its name.
...
# Checks if the 'dockerprocessus/freexegol' Docker image already exists locally.
def check_image():
    image_name="dockerprocessus/freexegol"
    client = docker.from_env()
    images = client.images.list(name=image_name)
    return bool(images)

# Initializes (or runs) a Docker container from the 'dockerprocessus/freexegol' image, setting up required folders and volumes.
def init(container_name):
    client = docker.from_env()
    image_tag = f"dockerprocessus/freexegol"
    try:
        container = client.containers.get(container_name)
        if container.status != "running":
    ...
    image_exists = check_image()
    if not image_exists:
        print(f"[*] Initial image does not exists, trying to install it first...")
        install()
...
# Starts a Docker container based on its name (if not running) or creates it if necessary.
def start(container_name):
    ...
    try:
        ...
        if container.status != "running":
            ...
            print(f"[!] Container '{container_name}' already running (id: {container.short_id}).")
        print(f"[~] Opening interactive shell in '{container_name}'...\n")
        subprocess.call(["docker", "exec", "-it", "-w", "/workspace", container_name, "/bin/zsh"])
        return
    except docker.errors.NotFound:
        pass
    home = Path.home()
...
# Builds the local custom 'freexegol' Docker image from its Dockerfile.
def build():
    home = Path.home()
    ...
    print(f"[+] Don't forget to manually tag your image with \"dockerprocessus/freexegol\"")

# Main entry point: handles CLI arguments and orchestrates commands and logic.
def main():
    home = Path.home()
