...
def slice_imgs(video_path: Union[str, os.PathLike],
    ...
    timer, save_temp_dir, results = SimbaTimer(start=True), None, None
    if save_dir is None:
    ...
    for batch_cnt, start_img_idx in enumerate(range(0, n, batch_size)):
        ...
        if save_dir is not None:
            batch_save_path = os.path.join(save_temp_dir, f'{video_meta_data["video_name"]}.mp4')
            results = np.zeros((batch_n, h, w, 3), dtype=np.uint8) if is_color else np.zeros((batch_n, h, w), dtype=np.uint8)
        ...
        if save_dir is None:
            ...
            results[0: batch_n] = batch_img_dev.copy_to_host()
            results = {k: v for k, v in enumerate(results)}
            ImageMixin().img_stack_to_video(imgs=results, fps=video_meta_data['fps'], save_path=batch_save_path, verbose=verbose)
    timer.stop_timer()
    if verbose:
