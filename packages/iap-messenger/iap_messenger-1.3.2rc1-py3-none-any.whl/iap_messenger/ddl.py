...
class DDL:
            ...
            try:
                ...
                if self.server_socket in socks and socks[self.server_socket] == zmq.POLLIN:
                    ...
                    if uid_request in self.datastore:
                        data_to_send = self.datastore[uid_request]["data"]
    ...
    def _cleanup_loop(self) -> None:
        ...
        SENT_DATA_EXPIRY_SECONDS = 2 * 60  # 2 minutes
        STORED_DATA_EXPIRY_SECONDS = 24 * 60 * 60  # 1 day

        LOGGER.info("DDL Cleanup loop started.")
        while not self._stop_event.is_set():
            try:
                ...
                if keys_to_delete:
                    LOGGER.info(f"Cleanup: Attempting to delete {len(keys_to_delete)} expired entries.")
                    deleted_count = 0
                    for uid_del in keys_to_delete:
                        if uid_del in self.datastore:  # Final check before deletion
                            try:
                                del self.datastore[uid_del]
                                LOGGER.info(f"Cleanup: Successfully deleted UID {uid_del}.")
                                deleted_count +=1
                            except KeyError:
                    ...
                    if deleted_count > 0:
                        ...
                        LOGGER.info(f"Cleanup: Deleted {deleted_count} entries. Suggested GC.")
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if client_socket in socks and socks[client_socket] == zmq.POLLIN:
                    ...
                    LOGGER.debug(f"Client (attempt {attempt+1}) received response in {elapsed_time:.2f} ms for UID: '{uid}'")
                    
                    try:
                        # Check if the response is a server-side error message
                        decoded_response = response.decode('utf-8')
                        if decoded_response.startswith("ERROR:"):
                            LOGGER.warning(f"Client (attempt {attempt+1}) received server error: {decoded_response}")
                            return b"", decoded_response # Server error, no retry
                    except UnicodeDecodeError:
                        # Not a UTF-8 string, assume it's binary data (success)
                        pass

                    LOGGER.debug(f"Client (attempt {attempt+1}) successfully received data for UID: '{uid}' (size: {len(response)} bytes)")
