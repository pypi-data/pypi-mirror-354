...
\___ \| '_ \ / _ \| __| \___ \| __/ _` | '__/ __|
 ___) | | | | (_) | |_   ___) | || (_| | |  \__ \
|____/|_| |_|\___/ \__| |____/ \__\__,_|_|  |___/[/yellow]  v3.4, author: https://github.com/snooppr
""")
...
def parsing(diff=False):
    ...
    if token != "None":
        head = {'User-Agent': f'Shotstars v3.4', 'Authorization': f'Bearer {token}'}
    elif token == "None":
...
##
    ...
    console.print(f"[cyan]Average-change-in-fact (adding stars in the previous month compared to the month before last)" + \
                  f"::[/cyan] {average_change}", highlight=False)
...
transition: transform 0.15s}
                ...
                file_html.write(f"\n<a class='but' href='file://{path}/all_gone_stars.html' " + \
                                ...
                                f"📊 Average-change-in-fact (adding stars in the previous month compared " + \
                                f"to the month before last):: {average_change}<br>\n" + \
