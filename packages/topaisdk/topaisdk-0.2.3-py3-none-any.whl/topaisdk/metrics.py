...
class MetricsCollector:
    ...
    async def _report_metrics_async(self):
        ...
        if not metrics_list:
            logger.info(f"TopaiSdk: No metrics to report for {self.service_id} (async mode)")
            return
        ...
        logger.info(f"TopaiSdk: Reported {self.service_id} {len(metrics_list)} latency metrics to {self.metric_server_url} done, used {(time.time() - start_time) * 1000:.2f} ms (async mode)")
    ...
    def _report_metrics_sync(self):
        ...
        metrics_list, payload = self._prepare_metrics_payload()
        if not metrics_list:
            logger.info(f"TopaiSdk: No metrics to report for {self.service_id} (threaded mode)")
            return
        ...
        logger.info(f"TopaiSdk: Reported {self.service_id} {len(metrics_list)} latency metrics to {self.metric_server_url} done, used {(time.time() - start_time) * 1000:.2f} ms (threaded mode)")
