...
import gammalearn.criterion.DANN_loss as criterions
import gammalearn.optimizers as optimizers
import gammalearn.training_steps as training_steps
from gammalearn.configuration.constants import GAMMA_ID, PROTON_ID, REAL_DATA_ID, SOURCE, TARGET
from gammalearn.criterion.loss_balancing.loss_weight_scheduler import ExponentialW
...
from gammalearn.data.save_results.write_dl2_files import WriteDL2Files
from gammalearn.data.transforms import GLearnCompose
from gammalearn.gl_logging.loss_balancing import (
    LogLambda,
    LogLossWeighting,
    LogUncertaintyTracker,
)
from gammalearn.gl_logging.net import LogGradientCosineSimilarity, LogGradientNorm, LogGradientNormPerTask
...
targets = collections.OrderedDict(
    {
        ...
        "domain_class": {
            ...
            "metrics": {
                "Accuracy_domain": Accuracy(threshold=0.5, task="multiclass", num_classes=len(particle_dict)),
                "AUC_domain": AUROC(task="multiclass", num_classes=2),
            },
            "mt_balancing": True,
        },
    }
...
dataset_size = {"target": {GAMMA_ID: 10, PROTON_ID: 10}}
...
optimizer_dic = {"network": optimizers.load_adam, "loss_balancing": optimizers.load_sgd}
