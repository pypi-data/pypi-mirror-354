...
class Session:
    ...
    def __init__(
        ...
        allow_redirects: bool = True, verify: bool = False, http_version: Literal["http1", "http2"] = "http2", sync: bool = True
    ):
        ...
        self._session = (rnet.BlockingClient if sync else rnet.Client)(
            ...
            **(
                { "http1_only": True, "http2_only": False } if http_version == "http1" 
                else { "http1_only": False, "http2_only": True }
            ),
            proxies=proxies
        )
    ...
    def complete_headers(self, headers: Union[Dict[str, str], List[str], str]):
            ...
            for header in headers:
                ...
                if key.lower() == "cookie":
                    ...
                    for set_cookie in set_cookies:
                        ...
                        cookie[set_cookie_key] = set_cookie_value
                    continue                        

                processed_headers.append(key, value)
            
            if len(cookie) > 0:
                processed_headers.append('Cookie', cookie.output(header='', sep=';').strip().replace('="', '=').replace('"; ', '; ').strip('"'))
        ...
        if len(cookie) > 0:
            if 'Cookie' in headers:           
                headers['Cookie'] = cookie.output(header='', sep=';').strip().replace('="', '=').replace('"; ', '; ').strip('"')
            else:
                headers['cookie'] = cookie.output(header='', sep=';').strip().replace('="', '=').replace('"; ', '; ').strip('"')
        ...
        if accept_language:
            if not set_accept_language:
                ...
                if set_accept_language != accept_language:
                    headers['Accept-Language'] = accept_language
    ...
    def set_headers_order(self, headers: Union[Dict[str, str], rnet.HeaderMap]):
        ...
        :is_fetch: 是否是 fetch 发送的请求, 默认 xhr
        :return
        """
...
class AsyncSession(Session):
    ...
    def __init__(
        ...
        user_agent: Union[int, Tuple[int, Literal["windows", "macos"], Literal["chrome", "edge"]]] = None,
        cookies: Optional[Dict[str, str]] = None, proxy: Optional[str] = None, ipinfo: dict = {}, timeout: int = 15,
        allow_redirects: bool = True, verify: bool = False, http_version: Literal["http1", "http2"] = "http2"
    ):
        super().__init__(
    ...
    async def __aenter__(self):
        if not self.ipinfo.get("country"):
            await self.update_ipinfo()        
        return self
