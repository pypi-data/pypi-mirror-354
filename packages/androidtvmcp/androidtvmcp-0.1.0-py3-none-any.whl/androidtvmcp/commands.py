...
class CommandProcessor:
    ...
    async def execute_navigation(self, command: NavigationCommand) -> CommandResult:
        ...
        """
        try:
            remote = await self.device_manager.ensure_connection(command.device_id)
            if not remote:
            ...
            navigation_map = {
                "up": remote.send_key_up,
                "down": remote.send_key_down,
                "left": remote.send_key_left,
                "right": remote.send_key_right,
                "select": remote.send_key_enter,
                "menu": remote.send_key_menu,
                "back": remote.send_key_back,
                "home": remote.send_key_home,
            }
            ...
            await navigation_map[command.direction]()
    ...
    async def input_text(self, device_id: Optional[str], text: str) -> CommandResult:
        ...
        """
        try:
            remote = await self.device_manager.ensure_connection(device_id)
            if not remote:
                return CommandResult(
                    success=False,
                    message="No Android TV device available",
    ...
    async def execute_playback(self, command: PlaybackCommand) -> CommandResult:
        ...
        try:
            ...
            playback_map = {
                "play": remote.send_key_play,
                "pause": remote.send_key_pause,
                "stop": remote.send_key_stop,
                "fast_forward": remote.send_key_fast_forward,
                "rewind": remote.send_key_rewind,
                "next": remote.send_key_next,
                "previous": remote.send_key_previous,
            }
            ...
            await playback_map[command.action]()
    ...
    async def execute_volume(self, command: VolumeCommand) -> CommandResult:
        ...
        try:
            ...
            if command.level is not None:
                ...
                await remote.set_volume(command.level)
                message = f"Volume set to {command.level}"
            else:
                # Map volume actions to remote methods
                volume_map = {
                    "up": remote.send_key_volume_up,
                    "down": remote.send_key_volume_down,
                    "mute": remote.send_key_mute,
                    "unmute": remote.send_key_mute,  # Toggle mute
                }
                ...
                await volume_map[command.action]()
    ...
    async def execute_device_command(self, command: DeviceCommand) -> CommandResult:
        ...
        try:
            ...
            # Map power actions to remote methods
            power_map = {
                "on": remote.send_key_power,
                "off": remote.send_key_power,
                "sleep": remote.send_key_sleep,
                "wake": remote.send_key_wake,
            }

            if command.action not in power_map:
                return CommandResult(
                    success=False,
                    message=f"Unknown power action: {command.action}",
                    error_code="INVALID_ACTION",
                    device_id=command.device_id
                )

            # Execute the power command
            await power_map[command.action]()

            return CommandResult(
                success=True,
                message=f"Power command '{command.action}' executed successfully",
                device_id=command.device_id
            )
