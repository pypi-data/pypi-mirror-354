...
class Slab:
    ...
    def draw_transverse(self, document: Drawing,
        ...
        if bars:
            ...
            for sp_bar in (sp_bars_tr_sup + sp_bars_tr_inf):
            ...
            for sp_bar in sp_bars_lg_sup:
                ...
                y_coord = y + self.thickness - self.cover - sp_bar.diameter / 2 - max(sp_bar.left_anchor,
                                                                                      sp_bar.right_anchor,
                                                                                      sp_bar.bend_longitud)
    ...
    def __gen_bars(self,
        ...
        def get_x_y_by_position(n_position: int,
            ...
            if as_position == Position.SUPERIOR:
                if as_direction == Direction.HORIZONTAL:  # Superior X.
                    ...
                    y_transverse = self.thickness - self.cover
        ...
        length, reincorcement_length = get_util_witdh_length_by_direction(direction=as_direction)
        ...
        for i in range(total_bars):
            ...
            if as_direction == Direction.HORIZONTAL:
                ...
                mandrel_radius = self.max_db_inf_y
                if as_position == Position.SUPERIOR:
                    mandrel_radius = self.max_db_sup_y

            if as_direction == Direction.VERTICAL:
                x = x - as_db[i] / 2
                x_tr = x_tr - as_db[i] / 2

                mandrel_radius = self.max_db_inf_x
                if as_position == Position.SUPERIOR:
                    y_tr = y_tr - as_db[i]
                    mandrel_radius = self.max_db_sup_x
            ...
            spaced_bars.append(SpacedBars(reinforcement_length=reincorcement_length - (as_sp[i] / total_bars) * i,
                                          length=length,
                                          diameter=as_db[i],
                                          spacing=as_sp[i],
                                          x=x,
