...
import requests
import queue
from agentcp.base.log import log_debug, log_info, log_error, set_log_enabled,log_exception
...
from agentcp.message import AssistantMessageBlock,AgentInstructionBlock
from agentcp.msg.session_manager import SessionManager, Session
...
from .llm_server import run_server, get_base_url, get_llm_api_key,add_llm_aid,llm_server_is_running,add_llm_api_key
...
class AgentID(abc.ABC): 
    ...
    def online(self):
        ...
        try:
            ...
            self.session_manager = SessionManager(
                self.id, self.ap_client.get_message_server(),self.ca_client.get_aid_certs_path(self.id),self.seed_password,self.db_manager
            )
    ...
    def add_message_handler(
        self, handler: typing.Callable[[dict], typing.Awaitable[None]],session_id:str="",router:str="",from_acp:bool=False
    ):
        """消息监听器装饰器"""
    ...
    def quick_send_message(self,to_aid:str, message: Union[AssistantMessageBlock, list[AssistantMessageBlock], dict],asnyc_message_result,insert_message: bool = True):
        session_id = self.__quick_send_message_base(to_aid,asnyc_message_result)
    ...
    def insert_message(self,role,aid,session_id, to_aids, message: Union[AssistantMessageBlock, list[AssistantMessageBlock], dict,str],parent_message_id="",message_id:str=""):
        # 处理对象转换为字典
        ...
        self.db_manager.insert_message(
            role, aid, session_id, aid, parent_message_id, ",".join(to_aids), "",json.dumps(message_data), "sent", message_id
        )
        
    # 发送自定义指令消息
    def send_instruction_message(self,session_id: str,to_aid: str,agent_cmd_block:AgentInstructionBlock=None,ref_msg_id: str=""):
        # 处理对象转换为字典
        if session_id == "" or session_id is None:
            return
        self.send_message(session_id,[to_aid],None,agent_cmd_block=agent_cmd_block,ref_msg_id = ref_msg_id)
    ...
    def send_form_message(self,session_id: str,to_aid_list: [],result: [],ref_msg_id: str):
        try:
            ...
            self.session_manager.send_msg(session_id ,msg_array, ";".join(to_aid_list), ref_msg_id,"",None)
        except Exception as e:
            log_exception(f"send_form_message failed: {e}")
            return
        
    def send_message(self, sessionId: str,to_aid_list: list,  message: Union[AssistantMessageBlock,list[Union[AssistantMessageBlock]], dict,str],ref_msg_id: str="",message_id:str="",agent_cmd_block:AgentInstructionBlock=None,insert_message:bool=True):
        # 处理对象转换为字典
        ...
        self.session_manager.send_msg(sessionId ,message_data, ";".join(to_aid_list), ref_msg_id,message_id,agent_cmd_block)
                ...
                for line in response.iter_lines():
                    ...
                    if not decoded_line.startswith("data:") and not decoded_line.startswith("event:"):
                        ...
                        if chunk is None:
                            ...
                            try:
                                if is_continue:
                                    content_text = content_text + chunk.get('choices', [])[0].get('delta', {}).get('content', '')
                            except Exception as e:
    ...
    def __agentid_message_listener(self, data):
        ...
        if save_message_list is None or len(save_message_list) == 0:
            self.db_manager.insert_message("assistant",self.id,session_id,sender, ref_msg_id, receiver, instruction_str,json.dumps(message_list), "text", "success",message_id)
        else:
...
class AgentCP(_AgentCP):
    ...
    def __init__(self, agent_data_path, certificate_path:str = "", seed_password:str = "",debug = False,log_level:int=logging.INFO,port:int = 0,run_proxy:bool = True) -> None:
        super().__init__()
    ...
    def save_aid_info(self, agent_id: str,seed_password:str,private_key:str,cert:str) -> AgentID:
        private_key_ = serialization.load_pem_private_key(
            ...
            password = self.__get_sha256(seed_password).encode('utf-8')
        )
    ...
    def add_message_handler(
        self, handler: typing.Callable[[dict], typing.Awaitable[None]], aid_str: str
    ):
        """消息监听器装饰器"""
        log_debug("add message handler")
        if not aid_str:
