...
class SharedData:    
    ...
    """
    '''
    Class for managing shared data resources, including tables, timeseries, and collections, with support for local disk storage, in-memory storage, MongoDB collections, and AWS S3 integration.
    
    Features:
    - Initialization with user credentials, AWS keys, and environment setup.
    - Methods to create, access, and manage data containers: tables, timeseries, dataframes (stub), and collections.
    ...
    - Loading and deleting of tables, timeseries, and collections with error handling and logging.
    ...
    - secret_access_key (str, optional): AWS secret access key; required if access_key_id is provided.
    - quiet (bool, optional): Supp
    """
    def __init__(self, source, user=None,
                 ...
                 quiet=False):
        """
        Initializes a SharedData instance by configuring environment variables, setting up database folders,
        connecting to the logger, and preparing the schema and authentication details.
    ...
    def collection(self, database, period, source, tablename,
        ...
        Parameters:
            ...
            names (list, optional): List of field names for the collection.
            formats (list, optional): List of formats corresponding to the field names.
            size (int, optional): Size parameter for the collection.
            hasindex (bool, optional): Whether the collection has an index. Defaults to True.
            value (optional): Initial records or data to populate the collection.
            user (str, optional): User identifier. Defaults to 'master'.
            overwrite (bool, optional): Whether to overwrite existing data. Defaults to False.
            type (str, optional): Type of the collection backend. Currently supports 'MONGODB' only. Defaults to 'MONGODB'.
            partitioning (optional): Partitioning information, inferred from tablename if not provided.
    ...
    def init_schema(self):        
        ...
        This method sets up the schema by defining the column names and their corresponding data types,
        including fields related to local and remote file metadata, mutex information, and additional schema metadata.
        The schema is associated with the current computer name obtained from the environment variable 'COMPUTERNAME'.
        
        Attributes set:
        - self.schema: A table schema object created with the specified names, formats, and size.
        """
        names = [
            'symbol', 'mtime', 
    ...
    def acquire(mutex, pid, relpath):
        ...
        If the mutex is already held, the method periodically checks if the locking process is still active.
        If the locking process has terminated, it forcibly acquires the mutex.
        Logs a warning if waiting for the semaphore exceeds certain time thresholds.
        ...
        Behavior:
            - Uses a compare-and-swap atomic operation to acquire the mutex.
            - Waits in a loop, checking every microsecond.
            - If the mutex is held for more than 1 second on the first check or 15 seconds thereafter, verifies if the locking process is alive.
            - If the locking process is dead, forcibly acquires the mutex.
            - Logs a warning if waiting continues beyond the initial timeout.
    ...
    def release(mutex, pid, relpath):
        """
        Release a semaphore held by a process.
        
        Attempts to release the semaphore identified by the given mutex and process ID (pid).
        If the semaphore is not currently held by the specified pid, an error is logged and an exception is raised.
        
        Parameters:
            mutex (object): The semaphore object with an underlying memory interface.
            pid (int): The process ID attempting to release the semaphore.
            relpath (str): A string representing the relative path or identifier used for logging purposes.
    ...
    def freeall():
        """
        Frees all shared memory segments listed by SharedData.
        
        Retrieves the list of shared memory names using SharedData.list_memory()
        and iterates through each name to free the corresponding shared memory
        segment using SharedData.free().
        """
    ...
    def list_all(self, keyword='', user='master'):
        ...
        """
        Retrieve and merge data from remote, local, and collection sources into a single DataFrame.
        ...
        Returns:
            pandas.DataFrame: A DataFrame containing merged and cleaned metadata from remote, local, and collection sources,
            with standardized columns and filled missing values.
        """
    ...
    def list_local(self, keyword='', user='master'):
        """
        Scan local database folders for binary files matching the specified user and keyword, and return a summarized pandas DataFrame with metadata.
    ...
    def list_collections(self, keyword='', user='master'):        
        ...
        Returns:
            pd.DataFrame: A DataFrame indexed by collection path with columns:
                ...
                - container: The container type, set as 'collection' or overridden by path parsing.
                ...
                - container: Extracted container segment from the collection path if present, else 'collection'.
        ...
        If no collections match the keyword, returns an empty Data
        """
