...
class SyncTable():
    ...
    async def websocket_publish_loop(client):
        ...
        """
        Asynchronously publishes updates from a client's data records over a websocket connection in a continuous loop.
        
        This method retrieves the IDs of records to send, compresses the corresponding data using LZ4 compression, and sends the compressed data in chunks to the client while respecting bandwidth limits. It also tracks and updates the client's transfer rate and upload statistics. The loop runs indefinitely until an exception occurs (e.g., client disconnection), at which point it logs the error and terminates after a delay.
        
        Args:
            client (dict): A dictionary containing client-specific information including:
                - 'database', 'period', 'source', 'tablename': identifiers for the data source.
                - 'addr': client address tuple.
                - 'conn': websocket connection object supporting async send().
                - 'records': data records accessible by index.
                - 'maxrows': maximum number of rows to send per message.
                - 'bandwidth': bandwidth limit in bytes per second.
                - 'transfer_rate': updated with the current transfer rate in MB/s.
                - 'upload': cumulative bytes uploaded.
                - 'watchdog': timestamp to monitor client activity.
        
        Raises:
            None explicitly; exceptions are caught internally to handle client disconnections.
        """
        Logger.log.info('Publishing updates of %s/%s/%s/%s -> %s' %
                        (client['database'], client['period'],
                         client['source'], client['tablename'], client['addr'][0]))
    ...
    async def websocket_subscription_loop(client):
        ...
        Behavior:
            - Continuously receives binary data from the WebSocket connection.
            - If an empty message is received, logs a warning, closes the WebSocket, and stops receiving.
            - Otherwise, decompresses the received data using lz4f, appends it to a bytes buffer,
              and processes the buffer using the table's record reader.
            - Updates the 'download' counter with the size of each received data chunk.
            - Handles exceptions by logging an error message, closing the WebSocket, and terminating the loop.
        """
        table = client['table']
