...
class SessionManager:
        ...
        try:
            ...
            match ext:
                ...
                case "csv":
                    ...
                    msg = SessionMessages.session_as_csv_loaded_message(filename)[0]
                case "sqlite":
                    self.storer.store_sessions_sqlite(filename, self.sessions)
                    msg = SessionMessages.session_as_sqlite_loaded_message(filename)[0]
                case "psql" | "postgresql":
                    self.storer.store_sessions_postgresql(self.sessions, filename)
                    msg = SessionMessages.session_as_postgresql_loaded_message(filename)[0]
                case _:
                    if self.debug:
                        self.logs["errors"].append(SessionMessages.UNSUPPORTED_FILE_EXTENTION)
                    return False
    ...
    def load(self, filename: str = None):
        ...
        try:
            match ext:
                ...
                case "csv":
                    data = self.storer.load_sessions_csv(filename)
                    self.sessions = self._deserialize_session_data(data)
                    msg = SessionMessages.session_as_csv_loaded_message(filename)
                case "sqlite":
                    data = self.storer.load_sessions_sqlite(filename)
                    self.sessions = self._deserialize_session_data(data)
                    msg = SessionMessages.session_as_sqlite_loaded_message(filename)
                case "psql" | "postgresql":
                    data = self.storer.load_sessions_postgresql(filename)
                    self.sessions = self._deserialize_session_data(data)
                    msg = SessionMessages.session_as_postgresql_loaded_message(filename)
                case _:
    ...
    def get_session_with_unick_name(self, unick_name: str, logging:bool=False) -> Optional[str]:
        """get_session_id_by_unick_name
