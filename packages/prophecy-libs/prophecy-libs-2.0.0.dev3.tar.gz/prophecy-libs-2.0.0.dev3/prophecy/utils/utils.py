...
from prophecy.executionmetrics.models import RunType
...
class MetricsCollector:
    ...
    def _handle_end_for_serverless(
        cls, spark: SparkSession, uuid_str: Optional[str] = None
    ):
        if not is_serverless:
            # Extra check so that this does not run on non-serverless
            return
...
class SerializableException:
    ...
    def from_python_exception(exception: BaseException):
        ...
        return SerializableException(
            exception_type, exception_message, exception_cause, exception_stack_trace
        )
