...
def get_spark_property(key: str, spark) -> Optional[str]:
    """
    Databricks Spark Connect hides SparkContext, but `spark.conf.get` is still
    available.  It raises if the key is missing, so wrap in a try/except.
    """
    try:
...
def get_running_code(spark, session: str) -> RecursiveDirectoryContent:
    ...
    parts_key = f"{get_session_appended_key(SPARK_CONF_PIPELINE_CODE_KEY, session)}_parts"
    ...
    if parts is not None:  # ⇐ Some(parts) in Scala
        ...
        try:
            ...
            logger.error("Failed to parse JSON with stdlib json; trying fallback", exc_info=exc1)
            try:
...
class SerializableException:
    def __init__(self, exceptionType: str, msg: str, causeMsg: str, stackTrace: str, time: int):
        self.exceptionType = exceptionType
        self.msg = msg
        self.causeMsg = causeMsg
...
class TimestampedOutput:
    def __init__(self, content: str, time: int):
    ...
    def from_content(cls, content: str):
        import time as time_module
        current_time = int(time_module.time() * 1000)
        return cls(content, current_time)

    @classmethod
    def from_dict(cls, d: dict):
        return cls(d.get("content", ""), int(d.get("time", 0)))
...
class ProphecyGemProgressEvent:
    ...
    def to_dict(self):
        ...
        def serialize_list(lst):
            if lst is None:
                return None
            return [item.to_dict() if hasattr(item, "to_dict") else item for item in lst]
        ...
        return {
            ...
            "exception": self.exception.to_dict() if self.exception and hasattr(self.exception, "to_dict") else None
        }

    def to_json(self):
        return json.dumps({"Event": "ProphecyGemProgressEvent", "ProphecyGemProgressEvent": self.to_dict()})
...
def sendGemProgressEvent3(spark,
                          userSession,
                          process_id,
                          state,
                          startTime,
    ...
    import time as time_module
    current_time = int(time_module.time() * 1000)
    serializableException = SerializableException(exception_type, msg, cause_msg, stack_trace, current_time)
    sendGemProgressEvent2(spark,
                          userSession,
                          process_id,
                          state,
                          startTime,
...
def sendGemProgressEvent2(spark,
                          userSession,
                          process_id,
                          state,
                          startTime,
    ...
    gem_event = ProphecyGemProgressEvent(session=get_session_hack(spark),
...
def sendPipelineProgressEvent3(spark, userSession: str, pipelineId: str, state: str,
    ...
    current_time = int(time_module.time() * 1000)
    serializableException = SerializableException(exception_type, msg, cause_msg, stack_trace, current_time)
    sendPipelineProgressEvent2(spark, userSession, pipelineId, state, startTime, endTime, serializableException)



def sendPipelineProgressEvent2(spark, userSession: str, pipelineId: str, state: str,
                              startTime: str, endTime: str = "", exception: Optional[Any] = None):

    submission_time = spark.conf.get(f"spark.prophecy.pipeline.submission-time.{userSession}")
    ...
    pipeline_progress_event = ProphecyPipelineProgressEvent(session=userSession,
