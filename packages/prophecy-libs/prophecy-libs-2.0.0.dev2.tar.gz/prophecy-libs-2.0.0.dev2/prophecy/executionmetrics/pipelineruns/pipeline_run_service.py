...
def create_entity(
    ...
    for details in component_details:
        component_run = ComponentRuns(
            ...
            input_gems=(
                details.gem_progress.input_gems if details.gem_progress else None
            ),
            output_gems=(
                details.gem_progress.output_gems if details.gem_progress else None
            ),
            ...
            exception=details.gem_progress.exception if details.gem_progress else None,
        )
...
class PipelineRunsService:
    ...
    def _get_component_details(
        ...
        for connection in p_graph.get("connections", []):
            target_port_connection_map[connection["targetPort"]] = connection

        component_details = []

        for process_id, node in p_graph.get("processes", {}).items():
            if node.get("component") == "Subgraph":
                # Handle subgraph
                node_subgraph_name = node.get("metadata", {}).get("slug", "")
                component_name = node_subgraph_name
                component_type = node["component"]
                component_uri = f"1/components/{component_name}"
                interim_ports = [
                    p["id"] for p in node.get("ports", {}).get("outputs", [])
                ]
                gem_progress = (
                ...
                component_name = node.get("metadata", {}).get("slug", "")
                component_type = node["component"]

                # Extract component URI
                properties = node.get("properties", {}).get("value", {})
                ...
                if component_type == "Target" and node.get("ports", {}).get("inputs"):
                    ...
                    if connection:
                        interim_node = graph["processes"][connection["source"]]
                        interim_subgraph_name = target_subgraph_name
                        interim_ports = [connection["sourcePort"]]

                interim_process_id = interim_node["id"]
                interim_component_name = interim_node.get("metadata", {}).get(
                    "slug", ""
                )
                gem_progress = (
                    gem_progress_map.get(process_id) if gem_progress_map else None
    ...
    def _fetch_target_component_source_port(
        self,
        target_port_connection_map: Dict[str, Any],
        p_graph: Dict[str, Any],
        subgraph_name: str,
        port: str,
        ...
        source_node = p_graph.get("processes", {}).get(connection["source"])
        if not source_node:
            return connection, p_graph, subgraph_name

        if source_node.get("component") == "Subgraph":
            # Recurse into subgraph
            source_node_subgraph_name = source_node.get("metadata", {}).get("slug", "")
            subgraph_target_port_map = {}
            for conn in source_node.get("connections", []):
                subgraph_target_port_map[conn["targetPort"]] = conn
