    ...
    def initializeMetrics(cls, spark: SparkSession):
        try:
            ...
            if is_serverless:
                ...
                cls.jvm_accessible = True
                print(f"Failed to initialize MetricsCollector, Spark context is not available, but still making cls.jvm_accessible = {cls.jvm_accessible}")
            else:
            ...
            if cls.jvm_accessible:
                ...
                try:
                    ...
                    try:
                        ...
                        revert_monkey_patching()
                        if not is_serverless:
                            # Not calling this .end() for serverless case
...
def sendPipelineProgressEvent(spark: SparkSession, userSession: str, pipelineId: str, state: str,
     ...
     if is_serverless:
        ...
        sendPipelineProgressEvent2(spark, userSession, pipelineId, state, startTime, endTime, exception)
     else:
         if exception:
...
def send_pipeline_progress_event_on_python_exception(spark: SparkSession, userSession: str,
    ...
    if is_serverless:
        sendPipelineProgressEvent3(spark, userSession, pipelineId, state, startTime, endTime,
...
def send_gem_progress_event_on_python_exception(spark: SparkSession, userSession: str, process_id: str,
    ...
    if is_serverless:
        sendGemProgressEvent3(
            spark, userSession, process_id, state, startTime, endTime, stdout,
