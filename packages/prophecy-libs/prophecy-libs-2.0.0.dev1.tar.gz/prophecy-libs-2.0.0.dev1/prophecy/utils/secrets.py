...
# ---------------------------------------------------------------- connection
class SecretsProviderConnectionDetails(_Sealed, ABC):
    ...
    def from_dict(raw: Dict[str, Any]) -> "SecretsProviderConnectionDetails":
        ...
        if cls is None:
            raise ValueError(f"Unknown SecretsProviderConnectionDetails: {simple_type!r}")
...
def conn_detail_type(cls: Type[SecretsProviderConnectionDetails]) -> Type[SecretsProviderConnectionDetails]:
...
class SecretCrudRequest(RequestMethod):
    ...
    def __post_init__(self):
        if isinstance(self.providerConnectionDetails, dict):
            self.providerConnectionDetails = (
                SecretsProviderConnectionDetails.from_dict(
                    self.providerConnectionDetails
                )
            )
...
def secrets_op_type(cls: Type["SecretsOperationResponse"]) -> Type["SecretsOperationResponse"]:
    cls._op_type = cls.__name__                       # type: ignore[attr-defined]
    _op_registry[cls.__name__] = cls
    return cls


class SecretsOperationResponse(_Sealed, ABC):
...
class HashiCorpHealthResponse(SecretsOperationResponse):
    ...
    def from_rest_response(health_resp: dict):  # hvac returns a dict
        ...
        * message: human-friendly guidance if not connectable
        ...
        sealed       = bool(health_resp.get("sealed")) if isinstance(health_resp, dict) else bool(health_resp.sealed)
        initialized  = bool(health_resp.get("initialized")) if isinstance(health_resp, dict) else bool(health_resp.initialized)
        standby      = bool(health_resp.get("standby")) if isinstance(health_resp, dict) else bool(health_resp.standby)

        can_connect  = (not sealed) and initialized
        msg = (
            "Vault isn't initialized. Please contact your administrator"
            if not initialized
            else "Vault is sealed. Please contact your administrator"
            if sealed
            else None
        )
...
class SecretManager:
    ...
    @staticmethod
    def deleteHashiCorpSecret(
            path: str, key: str, conn: HashiCorpConnectionDetails
    ):
...
async def handle_secrets_crud(req: SecretCrudRequest) -> LSecretsResponse:
    ...
    if provider == SecretsProvider.HASHICORP:
        ...
        if op == CrudOperation.HEALTH:
            try:
                ...
                msg  = ("Couldn't connect to HashiCorp endpoint. "
        ...
        # ---------- LIST SECRETS
            ...
            try:
                secrets = await asyncio.to_thread(SecretManager.listHashiCorpSecrets, conn)
    ...
    # ------------------------------------------------------------------ Environment
        ...
        if op == CrudOperation.READ:
            try:
                val = await asyncio.to_thread(SecretManager.get_env, req.secretKey or "")
                out = ReadResponse(req.secretScope, req.secretKey or "", val, None)
                return LSecretsResponse.from_request(req, out, True)
            except Exception as exc:
                out = ReadResponse(req.secretScope, req.secretKey or "", None, None)
...
class VaultUtils:
    ...
    @staticmethod
    def _client_from_conn(conn: "HashiCorpConnectionDetails", namespace_agnostic = False) -> hvac.Client:
        cfg = VaultUtils.get_vault_config(
            conn.address, conn.token, conn.namespace, namespace_agnostic=namespace_agnostic
        )
    ...
    def vault_health(conn: "HashiCorpConnectionDetails") -> Response:
        ...
        cli = VaultUtils._client_from_conn(conn, namespace_agnostic = True)        # namespace-agnostic OK
    ...
    def _read_kv(cli, mount, inner, kv_ver):
        if kv_ver == 2:
            ...
            return cli.secrets.kv.v1.read_secret(
                path=inner or "", mount_point=mount
            )["data"]
