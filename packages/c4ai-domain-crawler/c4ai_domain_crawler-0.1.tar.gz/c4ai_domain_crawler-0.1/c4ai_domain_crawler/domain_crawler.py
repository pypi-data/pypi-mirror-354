...
visited = set()
crawled = 0
global DOMAIN, VISITED_FILE, QUEUE_FILE, CONTENT_DIR
...
async def crawl_and_scrape(url, crawler, session):
    global crawled
    global visited, DOMAIN, VISITED_FILE, QUEUE_FILE, CONTENT_DIR
    try:
        # if "www.superprocure.com/freight-sourcing" in url:
       # import pdb; pdb.set_trace()  # Debugging breakpoint
...
# --- Entry Point ---
async def main():
    global DOMAIN, VISITED_FILE, QUEUE_FILE, CONTENT_DIR, DATA_DIR
    seed_url = DOMAIN = sys.argv[1]
    seed_url = seed_url if seed_url.startswith('http') else f'https://{seed_url}'
    VISITED_FILE = DATA_DIR / "visited.json"
    QUEUE_FILE = DATA_DIR / "queue.json"
    CONTENT_DIR = DATA_DIR / "content"
