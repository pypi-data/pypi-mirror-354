...
class ToscaTestcases():
    ...
    def getTestcasesInfo(self,tql_output:str)->list:
        try:
            ...
            return [{'S.No':i+1,'testcase':tc_names[i],'duartion':tc_durations[i],'result':tc_results[i],'date of execution':tc_exec_dates[i],'paths':tc_req_res_paths[i]} for i in range(len(tc_names))]
