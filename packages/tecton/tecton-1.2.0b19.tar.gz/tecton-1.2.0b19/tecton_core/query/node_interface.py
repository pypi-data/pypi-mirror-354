...
class JoinKeyRangePartitioning(Partitioning):
    def __init__(self, join_keys: List[str], boundaries: List[List[Any]]) -> None:
        self._join_keys = join_keys
    ...
    def partition_expression(self, partition_by: pypika.Field) -> pypika.terms.Term:
        ...
        """
        return pypika.functions.Coalesce(
            pypika.terms.Function(
                "LIST_ANY_VALUE",  # returns first non-null value
                pypika.terms.Function(
                    "LIST_TRANSFORM",
                    self._boundaries,
                    pypika.terms.LiteralValue(
                        # DuckDB indexing is 1-based, that's why "i - 1"
                        f"(x, i) -> if(x > {partition_by.get_sql()}, i - 1, NULL)"
                    ),
                ),
            ),
            len(self._boundaries),  # if there's no boundaries greater than current value - it's the last partition
        )
