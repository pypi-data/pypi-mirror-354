...
def update_repo_on_remote_machine(hostname, username, password, repo, install = False):
    client = SshClient(host=hostname, port=22, username=username, password=password)
    branch = 'main'
    if repo == 'finance':
        branch = 'master'
    try:
        cmd = f"""cd /home/jay/PycharmProjects/{repo}/; git reset --hard HEAD~1; git pull {repo} {branch}"""
        if install:
...
def update_repo_on_remote_machine_with_jragbeer_common(hostname, username, password, repo, ):
    client = SshClient(host=hostname, port=22, username=username, password=password)
    try:
        ret = client.execute(command=f"""cd /home/jay/PycharmProjects/{repo}/;
        source venv/bin/activate;
        pip install --upgrade /home/jay/PycharmProjects/jragbeer_common/dist/jragbeer_common-0.2.0-py3-none-any.whl --force-reinstall""" , )
        print("  ".join(ret["out"]), "  E ".join(ret["err"]), ret["retval"])

    finally:
        client.close()

def create_jragbeer_common_wheel(hostname, username, password,):
    client = SshClient(host=hostname, port=22, username=username, password=password)
    try:
        ret = client.execute(command="""cd /home/jay/PycharmProjects/jragbeer_common/; python3 setup.py bdist_wheel""" , )
        print("  ".join(ret["out"]), "  E ".join(ret["err"]), ret["retval"])
