...
from ..ubuntu.jragbeer_common_ubuntu import execute_cmd_ubuntu_sudo
...
def pg_backup_database(database_name: str, host:str, password:str, user:str, port:str) -> None:
    """
    ...
    t = [rf"PGPASSWORD={password}",
        ...
        rf"-d{database_name}",
        rf"-p{port}",
        rf"-f/home/jay/PycharmProjects/jragbeer_home/data/pg_{database_name}_dump_latest.sql"
    ]
    print(t)
    print()
    execute_cmd_ubuntu_sudo(t)
...
def sql_table_drop_duplicates_single(new_engine: sqlalchemy.Engine, table_name:str) -> None:
    """
...
def sql_table_drop_duplicates(list_of_table_names: list[str], connection_string: str | None =None, database:str | None =None) -> None:
    ...
    if connection_string:
        ...
        db_url = f"postgres://{os.getenv('local_db_username')}:{os.getenv('local_db_password')}@{os.getenv('local_db_address')}:{os.getenv('local_db_port')}/{database}"
...
def copy_backup_to_cloud(database_name: str) -> None:
    """
    ...
    adls_upload_file(data_path + f"{database_name}_dump_latest.sql", f"database_backups/{database_name}_dump_latest.sql")
    dagster_logger.info(f"{database_name} migration to BLOB complete.")
def backup_databases(db: str ='postgres') -> None:
    """
    ...
    for db_name in [
        ...
        if db == 'mysql':
            ...
            pg_backup_database(db_name, host=host, password=password, user=user, port = port)
        copy_backup_to_cloud(db_name)
