...
def test_convert_xml_auto(test_env):
    ...
    # Verify that the output directory exists and contains JSON files
    assert output_dir.exists(), f"Output directory {output_dir} was not created"
    json_files = list(output_dir.glob('*.json'))
    assert len(json_files) > 0, f"No JSON files were created in {output_dir}"

    # Verify the structure of the first JSON file
