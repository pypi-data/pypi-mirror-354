...
class _AbstractDescriptorProperties(AbstractObject):
  """
  AbstractField provides an implementation of the descriptor protocol
  that allow the owning class to explicitly define the accessor methods.
  """
  ...
  def _getFixKeys(self, **kwargs) -> Keys:
    """
    Getter-function for the names of the methods called to fix the value
    of the field. The recommended design pattern is to decorate methods
    in the class body with 'FIX'. The decorated methods should simply be
    instance methods.
    """
    if self.__fixer_keys__ is None:
      return ()
    if isinstance(self.__fixer_keys__, list):
      if kwargs.get('_recursion', False):
        raise RecursionError
      self.__fixer_keys__ = (*self.__fixer_keys__,)
      return self._getFixKeys(_recursion=True, )
    if isinstance(self.__fixer_keys__, tuple):
      for key in self.__fixer_keys__:
        if not isinstance(key, str):
          raise TypeError(typeMsg('fixerKey', key, str))
      else:
        return self.__fixer_keys__
    name, value = '__fixer_keys__', self.__fixer_keys__
  ...
  def _getPreDeleteKeys(self) -> Keys:
    """
  ...
  def _getPostDeleteKeys(self) -> Keys:
    """
  ...
  def _getFix(self, **kwargs) -> Funcs:
    """
    Getter-function for the fix functions.

    These functions receive:
    func(self.instance, value: Any) -> None
    """
    if self.owner is self.__field_owner__:  # return function objects
      if self.__fixer_funcs__ is not None:
        return self._getFixFuncObjects(**kwargs)
    fixerKeys = self._getFixKeys(**kwargs)
    fixerFuncs = []
    for fixerKey in fixerKeys:
      fixerFunc = getattr(self.owner, fixerKey, None)
      if fixerFunc is None:
        raise MissingVariable(fixerKey, Func)
      if isinstance(fixerFunc, Func):
        fixerFuncs.append(fixerFunc)
        continue
      name, value = fixerKey, fixerFunc
      raise TypeException(name, value, Func)
    return (*fixerFuncs,)

  def _getFixFuncObjects(self, **kwargs) -> Funcs:
    """
    Getter-function for the fix functions.
  ...
  def _getPreGet(self, **kwargs) -> Funcs:
    """
  ...
  def _getPreGetFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _getPostGet(self, **kwargs) -> Funcs:
    """
  ...
  def _getPostGetFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _getPreSet(self, **kwargs) -> Funcs:
    """
  ...
  def _getPreSetFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _getPostSet(self, **kwargs) -> Funcs:
    """
  ...
  def _getPostSetFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _getPreDelete(self, **kwargs, ) -> Funcs:
    """
  ...
  def _getPreDeleteFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _getPostDelete(self, **kwargs, ) -> Funcs:
    """
  ...
  def _getPostDeleteFuncObjects(self, **kwargs) -> Funcs:
    """
  ...
  def _addFixer(self, callMeMaybe: Func) -> Func:
    """
    Add a fixer function to the field.

    This function is used to add a fixer function to the field. The
    function should be an instance method of the owner class.
    """
    if not isinstance(callMeMaybe, Func):
      name, value = '_addFixer', callMeMaybe
      raise TypeException(name, value, Func)
    existingFuncs = self._getFix()
    existingKeys = self._getFixKeys()
    self.__fixer_funcs__ = (*existingFuncs, callMeMaybe)
    self.__fixer_keys__ = (*existingKeys, callMeMaybe.__name__,)
    return callMeMaybe
  ...
  def _addPreGet(self, callMeMaybe: Func) -> Func:
    ...
    existingFuncs = self._getPreGetFuncObjects()
