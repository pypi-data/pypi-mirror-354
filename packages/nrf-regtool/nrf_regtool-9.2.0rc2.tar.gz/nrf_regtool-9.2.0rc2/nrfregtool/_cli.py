...
def make_arg_parser(prog: Optional[str] = None) -> argparse.ArgumentParser:
    ...
    migrate_opt.add_argument(
        "--spu-ownerid-configure",
        default=False,
        action="store_true",
        help=(
            "Set the OWNERID in the SPU PERIPH[n].PERM and FEATURE.* registers to the owner of "
            "the domain the UICR is for. If not provided, the OWNERID is kept at NRF_OWNER_NONE."
        ),
    )
...
def cmd_uicr_migrate(
    ...
    **kwargs: Any,
    ...
    if spu_ownerid_configure:
        flags = flags | MigrateFlag.SPU_OWNERID
