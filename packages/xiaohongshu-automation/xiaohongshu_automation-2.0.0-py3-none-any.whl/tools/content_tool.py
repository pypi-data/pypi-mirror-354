...
class GetNoteContentTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # 执行内容获取
            self.logger.info(f"开始获取笔记内容: {url}")
            
            result = await self.adapter.get_note_content(url.strip())
            ...
            if result.get("success"):
                ...
                if note_data:
                    ...
                    return ToolResult(
                        ...
                        message=f"成功获取笔记内容: {formatted_content['标题']}"
                    )
                else:
                    return ToolResult(
                        success=False,
                        error="获取到的笔记数据为空",
                        message="请检查笔记URL是否正确或笔记是否可访问"
                    )
            else:
                ...
                return ToolResult(
                    success=False,
                    error=error_message,
                    message=f"获取笔记内容时出现问题"
                )
        
        except Exception as e:
            self.logger.error(f"获取笔记内容时出错: {str(e)}")
            return ToolResult(
                success=False,
                error=str(e),
                message="获取笔记内容操作执行失败"
            )
