...
class GetCommentsTool(BaseTool):
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "url": {
                    "type": "string",
                    "description": "小红书帖子URL",
                    "pattern": r"^https?://.*xiaohongshu\.com.*"
                },
                "analyze": {
                    "type": "boolean",
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # 获取评论数据
            comments_data = await self.adapter.get_comments(validated_url)
...
class ReplyCommentsTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # 调用适配器回复评论
            result = await self.adapter.reply_comments(validated_responses, url)
            ...
            message = f"成功回复 {total_comments} 条评论，共 {total_replies} 条回复"
            if auto_optimize:
                message += "（已自动优化）"
