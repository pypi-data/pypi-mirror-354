...
class ClusterCommand:
    ...
    def _validate_shape_architecture(self, instance_shape: Shape, instance_type: str) -> None:
        ...
        if has_processor_desc:
            if any(arch in instance_shape.processor_description.lower() for arch in ["amd", "intel"]):
                shape_arch = "x86_64"
            elif any(arch in instance_shape.processor_description.lower() for arch in ["arma", "amperea"]):
                shape_arch = "aarch64"

        if not shape_arch and not config["arch"]:
            raise CODException(
                f"Cannot determine architecture of {instance_type}'s shape {instance_shape.shape}, and "
                "no architecture was specified in the config. Please specify the architecture with "
                "`--arch x86_64|aarch64`"
            )
        if (shape_arch and config["arch"]) and (shape_arch != config["arch"]):
            raise CODException(
                f"Requested {instance_type}'s shape {instance_shape.shape} supports {shape_arch} architecture, instead "
                f"of requested {config['arch']}. Aborting cluster creation. Please choose a different shape or "
                "architecture."
            )
        if not shape_arch and config["arch"]:
            log.warning(
                f"Cannot determine architecture of {instance_type}'s shape {instance_shape.shape}. Please ensure that "
                f"arch={config['arch']} is the correct architecture for this shape, otherwise cluster creation will "
                "fail."
            )
        elif shape_arch and not config["arch"]:
            log.info(
                f"No architecture specified in config, setting `arch` {shape_arch} based on {instance_type}'s "
                "shape {instance_shape.shape}"
            )
            config["arch"] = shape_arch
    ...
    def _validate_shape(self):
        ...
        node_shape = self.client.compute.get_shape(config["node_shape"],
                                                   self.availability_domain_name, self.compute_cid)
        self._validate_shape_architecture(head_shape, "head node")
