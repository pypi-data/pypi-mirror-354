...
def bdc_power_det():
    columns = ['pol', 'sgt_attn_cmd', 'p_out', 'TMP_BDC_COMBINED_SCALED']
    ...
    TMP_BDC_COMBINED_SCALED = -40


    temporary_p_out = 80
    for i in range(13):
        #sgt_attn_cmd - diff by temperature, check bdc_gain 40 df
        temporary_p_out = temporary_p_out - 4
        sgt_attn_cmd = 0
        df.loc[i] = [pol, sgt_attn_cmd, temporary_p_out, TMP_BDC_COMBINED_SCALED]
    ...
    TMP_BDC_COMBINED_SCALED = 55

    temporary_p_out = 80
    for i in range(13):
        #sgt_attn_cmd - diff by temperature, check bdc_gain 55 df
        temporary_p_out = temporary_p_out - 4
        sgt_attn_cmd = 0
        df.loc[i] = [pol, sgt_attn_cmd, temporary_p_out, TMP_BDC_COMBINED_SCALED]
