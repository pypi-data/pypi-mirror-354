...
def predict(path: Annotated[str, typer.Option()] = None):
    ...
    for filename in os.listdir(path)[:100]:
        ...
        if full_name[-20:] == "gain_bdc_lhcp_25.csv" or full_name[-20:] == "gain_bdc_rhcp_25.csv":
            gain_room_temp_file = full_name
    
    # print(autobias_room_temp_file)
    # bdc_autobias_output = bdc_autobias(autobias_room_temp_file)
    print(gain_room_temp_file)
    bdc_gain_output = bdc_gain(gain_room_temp_file)
    print("Varun")
    print(bdc_gain_output.to_string())
