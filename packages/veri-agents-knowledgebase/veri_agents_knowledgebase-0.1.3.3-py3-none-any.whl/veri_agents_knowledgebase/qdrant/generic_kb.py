...
from veri_agents_knowledgebase.knowledgebase import DocumentLoader, DataSource, RWKnowledgebase
...
class GenericDocumentLoader(DocumentLoader):
    ...
    def load_documents(self, **kwargs) -> Iterator[tuple[Document, list[Document] | None]]:
        ...
        if data_location_path.is_file():
            ...
            files = [
                str(file)
                for file in data_location_path.rglob("*")
                if file.is_file()
            ]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.doc_summarize:
            if self.llm is None:
                raise ValueError(
                    f"llm required for summarization"
                )
