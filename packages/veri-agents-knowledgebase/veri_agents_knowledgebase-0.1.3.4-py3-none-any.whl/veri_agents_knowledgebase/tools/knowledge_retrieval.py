...
class FixedKnowledgebaseWithTagsQuery(BaseTool):
        ...
        if config:
            ...
            if user_filter != filter:
                ...
                if out_llm_prompt is not None:
                    out_llm_prompt.append(f"User applied the following filters: {user_filter}.\n")
        return filter
    ...
    def _run(
        ...
        ret_text, ret_docs = self.knowledgebase.retrieve(query, limit=self.num_results, filter=filter)
    ...
    async def _arun(
        ...
        try:
            ret_text, ret_docs = await self.knowledgebase.aretrieve(query, limit=self.num_results, filter=filter)
...
class FixedKnowledgebaseListDocuments(BaseTool):
    ...
    def _run(
        ...
        for d in docs:
            ...
            metadata_str = "\n".join([f"{key}: {value}" for key,value in d.metadata.items()])

            return_texts.append(f"ID: \"{id}\"\n\nMetadata: {metadata_str}")
