...
class LLMClient:
    ...
    async def llm_stream(
        ...
        request = LLMRequest(
            ...
            back_service_urls=back_service_urls,
            auto_use_system_back_service=auto_use_system_back_service,
            rid=rid,
            extra_params=params
        )
    ...
    async def llm_stream_with_request(
            self,
            request: LLMRequest
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """流式调用LLM服务 - 使用请求对象方式"""
    ...
    async def llm_invoke_with_request(
            self,
            request: LLMRequest
    ) -> Dict[str, Any]:
        """同步调用LLM服务 - 使用请求对象方式"""
...
llm_stream_with_request = _client.llm_stream_with_request
llm_invoke_with_request = _client.llm_invoke_with_request
