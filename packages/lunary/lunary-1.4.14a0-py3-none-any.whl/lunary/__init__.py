...
try:
    ...
    class LunaryCallbackHandler(BaseCallbackHandler):
        ...
        __app_id: str
        __api_url: str
        __ignore_rules: List[Dict[str, Any]]

        def __init__(
            self,
            app_id: Union[str, None] = None,
            api_url: Union[str, None] = None,
            ignore: Union[List[Dict[str, Any]], None] = None,
        ) -> None:
            super().__init__()
            config = get_config()
            try:
        ...
        def _should_ignore_run(self, run_id: str, run_type: str = None, name: str = None) -> bool:
            ...
            # Check if current run matches any ignore rule
            for rule in self.__ignore_rules:
                # Check type match (if specified in rule)
                if "type" in rule and run_type and rule["type"] != run_type:
                    continue
                
                # Check name match (if specified in rule)
                if "name" in rule and isinstance(rule["name"], list):
                    if name and self._matches_any_pattern(name, rule["name"]):
                        return True
            
            # Check if any ancestor is ignored by traversing the parent hierarchy
            run = run_manager.runs.get(str(run_id))
            while run:
                # Check if this run is marked as ignored (we'll set this in on_chain_start)
        ...
        def on_llm_start(
            ...
            **kwargs: Any,
            ...
            try:
                ...
                # Check if this run should be ignored
        ...
        def on_llm_end(
            ...
            **kwargs: Any,
            ...
            try:
                # Check if this run should be ignored before ending it
            ...
            **kwargs: Any,
            ...
            try:
                ...
                if self._should_ignore_run(run.id, run_type=type, name=name):
                    # Mark the run as ignored so children can inherit this state
                    run._ignored = True
                    return
