...
def assume_role_with_azure_identity(role_arn):
    ...
    azure_token = azure_identity.get_token("api://AzureADTokenExchange")
    logging.info('Assuming AWS role')
    sts_client = boto3.client('sts')
    assumed_role = sts_client.assume_role_with_web_identity(
        RoleArn=role_arn,
        RoleSessionName='AzFuncSession',
...
def load_env_from_aws_secret():
    ...
    if use_azure:
        creds = assume_role_with_azure_identity(role_arn)
    else:
